...
struct sk_buff *dbg_rtw_cfg80211_vendor_event_alloc_22b(struct wiphy *wiphy, struct wireless_dev *wdev, int len, int event_id, gfp_t gfp
...
#define rtw_cfg80211_vendor_event_alloc_22b(wiphy, wdev, len, event_id, gfp) \
	dbg_rtw_cfg80211_vendor_event_alloc_22b(wiphy, wdev, len, event_id, gfp, MSTAT_FUNC_CFG_VENDOR | MSTAT_TYPE_SKB, __FUNCTION__, __LINE__)
...
struct sk_buff *rtw_cfg80211_vendor_event_alloc_22b(
 ...
 * be used).
 */
int rtw_cfgvendor_send_async_event_22b(struct wiphy *wiphy,
	   struct net_device *dev, int event_id, const void  *data, int len)
{
	...
	skb = rtw_cfg80211_vendor_event_alloc_22b(wiphy, ndev_to_wdev(dev), len, event_id, kflags);
	if (!skb) {
...
#include <hal_data.h>
int rtw_dev_get_feature_set_22b(struct net_device *dev)
{
	...
	if (is_supported_5g(adapter_to_regsty(adapter)->wireless_mode)
		&& hal_chk_band_cap_22b(adapter, BAND_CAP_5G)) /* v5.3 has no rtw_init_wireless_mode(), need checking hal spec here */
...
int *rtw_dev_get_feature_set_22b_matrix(struct net_device *dev, int *num)
{
	...
	*num = 0;
	...
	feature_set_full = rtw_dev_get_feature_set_22b(dev);
		...
		struct wireless_dev *wdev, const void  *data, int len)
{
	...
	reply = rtw_dev_get_feature_set_22b(wdev_to_ndev(wdev));
		...
		struct wireless_dev *wdev, const void  *data, int len)
{
	...
	reply = rtw_dev_get_feature_set_22b_matrix(wdev_to_ndev(wdev), &num);
	...
	struct net_device *dev, void  *data, int len, rtw_vendor_event_t event)
{
	...
	while (total > 0) {
		...
		/* Alloc the SKB for vendor_event */
		skb = rtw_cfg80211_vendor_event_alloc_22b(wiphy, ndev_to_wdev(dev), malloc_len, event, kflags);
		if (!skb) {
...
void rtw_cfgvendor_rtt_evt(void *ctx, void *rtt_data)
{
	...
	kflags = in_atomic() ? GFP_ATOMIC : GFP_KERNEL;
	/* Alloc the SKB for vendor_event */
	skb = rtw_cfg80211_vendor_event_alloc_22b(wiphy, wdev, tot_len, GOOGLE_RTT_COMPLETE_EVENT, kflags);
	if (!skb) {
		WL_ERR(("skb alloc failed"));
		goto exit;
	}
...
static void LinkLayerStats(_adapter *padapter)
{
	...
	if ( padapter->netif_up == _TRUE ) {
		...
		if (rtw_mi_check_fwstate_22b(padapter, _FW_LINKED)) {
		...
		if ( trx_total_bytes == 0) {
			...
			tmp = rtw_division64_22b(tmp, trx_total_bytes);
			pwrpriv->tx_time = tmp;
...
int rtw_cfgvendor_attach_22b(struct wiphy *wiphy)
{
...
int rtw_cfgvendor_detach_22b(struct wiphy *wiphy)
{
