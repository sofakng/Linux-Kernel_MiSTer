		...
		char *start, char *stop)
{
	...
	if (SCAN_RESULT_WFD_TYPE == pwdinfo->wfd_info->scan_result_type) {
		...
		if (wfd_ie) {
			...
			wfd_devinfo = rtw_get_wfd_attr_content_22b(wfd_ie, wfd_ielen, WFD_ATTR_DEVICE_INFO, NULL, &wfd_devlen);
			if (wfd_devinfo) {
	...
	if (strcmp(tmp[0], "wmap") == 0) {
		...
		*extra = 0;
		...
		if (wifimaplen > EFUSE_MAX_MAP_LEN)
			...
			cnts = wifimaplen;
		if (rtw_efuse_mask_map_read(padapter, 0, cnts, pEfuseHal->fakeEfuseModifiedMap_22b) == _SUCCESS)
