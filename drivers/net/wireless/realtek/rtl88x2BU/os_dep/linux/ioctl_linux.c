...
extern u8 key_2char2num_22b(u8 hch, u8 lch);
extern u8 str_2char2num_22b(u8 hch, u8 lch);
extern void macstr2num_22b(u8 *dst, u8 *src);
extern u8 convert_ip_addr_22b(u8 hch, u8 mch, u8 lch);

u32 rtw_rates_22b[] = {1000000, 2000000, 5500000, 11000000,
...
static void indicate_wx_custom_event(_adapter *padapter, char *msg)
{
	...
	_rtw_memcpy_22b(buff, msg, strlen(msg));

	_rtw_memset_22b(&wrqu, 0, sizeof(wrqu));
...
static void request_wps_pbc_event(_adapter *padapter)
{
	...
	_rtw_memset_22b(buff, 0, IW_CUSTOM_MAX);
...
void rtw_request_wps_pbc_event(_adapter *padapter)
{
	...
	rtw_led_control_22b(padapter, LED_CTL_START_WPS_BOTTON);
}
#endif/* #ifdef CONFIG_SUPPORT_HW_WPS_PBC */

void indicate_wx_scan_complete_event_22b(_adapter *padapter)
{
	union iwreq_data wrqu;
	struct	mlme_priv *pmlmepriv = &padapter->mlmepriv;

	_rtw_memset_22b(&wrqu, 0, sizeof(union iwreq_data));

	/* RTW_INFO("+rtw_indicate_wx_scan_complete_event_22b\n"); */
#ifndef CONFIG_IOCTL_CFG80211
...
void rtw_indicate_wx_assoc_event_22b(_adapter *padapter)
{
	union iwreq_data wrqu;
	struct	mlme_priv *pmlmepriv = &padapter->mlmepriv;
	...
	if (check_fwstate(pmlmepriv, WIFI_ADHOC_MASTER_STATE) == _TRUE)
		_rtw_memcpy_22b(wrqu.ap_addr.sa_data, pnetwork->MacAddress, ETH_ALEN);
	else
		_rtw_memcpy_22b(wrqu.ap_addr.sa_data, pmlmepriv->cur_network.network.MacAddress, ETH_ALEN);
...
void rtw_indicate_wx_disassoc_event_22b(_adapter *padapter)
{
	union iwreq_data wrqu;
	...
	wrqu.ap_addr.sa_family = ARPHRD_ETHER;
	_rtw_memset_22b(wrqu.ap_addr.sa_data, 0, ETH_ALEN);
...
/*
uint	rtw_is_cckrates_included_22b_22b(u8 *rate)
{
		u32	i = 0;
...
uint	rtw_is_cckratesonly_included_22b_22b(u8 *rate)
{
	u32 i = 0;
		...
		char *start, char *stop)
{
...
#endif /* CONFIG_WFD */
	{
		if (!rtw_p2p_chk_state(pwdinfo, P2P_STATE_NONE)) {
			...
			/*	Verifying the SSID */
			if (_rtw_memcmp_22b(pnetwork->network.Ssid.Ssid, pwdinfo->p2p_wildcard_ssid, P2P_WILDCARD_SSID_LEN)) {
				u32	p2pielen = 0;

				/*	Verifying the P2P IE */
				if (rtw_bss_ex_get_p2p_ie_22b(&pnetwork->network, NULL, &p2pielen))
					blnGotP2PIE = _TRUE;
			}
	...
	if (SCAN_RESULT_WFD_TYPE == pwdinfo->wfd_info->scan_result_type) {
		...
		wfd_ie = rtw_bss_ex_get_wfd_ie_22b(&pnetwork->network, NULL, &wfd_ielen);
		if (wfd_ie) {
			u8 *wfd_devinfo;
			uint wfd_devlen;

			wfd_devinfo = rtw_get_wfd_attr_22b_content_22b(wfd_ie, wfd_ielen, WFD_ATTR_DEVICE_INFO, NULL, &wfd_devlen);
			if (wfd_devinfo) {
		...
		char *start, char *stop, struct iw_event *iwe)
{
	...
	_rtw_memcpy_22b(iwe->u.ap_addr.sa_data, pnetwork->network.MacAddress, ETH_ALEN);
		...
		char *start, char *stop, struct iw_event *iwe)
{
	...
	iwe->cmd = SIOCGIWFREQ;
	iwe->u.freq.m = rtw_ch2freq_22b(pnetwork->network.Configuration.DSConfig) * 100000;
	iwe->u.freq.e = 1;
		...
		char *start, char *stop, struct iw_event *iwe)
{
	...
	/* parsing HT_CAP_IE	 */
	p = rtw_get_ie_22b(&pnetwork->network.IEs[ie_offset], _HT_CAPABILITY_IE_, &ht_ielen, pnetwork->network.IELength - ie_offset);
	if (p && ht_ielen > 0)
		ht_cap = _TRUE;

#ifdef CONFIG_80211AC_VHT
	/* parsing VHT_CAP_IE */
	p = rtw_get_ie_22b(&pnetwork->network.IEs[ie_offset], EID_VHTCapability, &vht_ielen, pnetwork->network.IELength - ie_offset);
	if (p && vht_ielen > 0)
		vht_cap = _TRUE;
#endif
	/* Add the protocol name */
	iwe->cmd = SIOCGIWNAME;
	if ((rtw_is_cckratesonly_included_22b_22b((u8 *)&pnetwork->network.SupportedRates)) == _TRUE) {
		if (ht_cap == _TRUE)
			snprintf(iwe->u.name, IFNAMSIZ, "IEEE 802.11bn");
		else
			snprintf(iwe->u.name, IFNAMSIZ, "IEEE 802.11b");
	} else if ((rtw_is_cckrates_included_22b_22b((u8 *)&pnetwork->network.SupportedRates)) == _TRUE) {
		if (ht_cap == _TRUE)
		...
		char *start, char *stop, struct iw_event *iwe)
{
	...
	if (p && ht_ielen > 0) {
		...
		_rtw_memcpy_22b(&mcs_rate , pht_capie->supp_mcs_set, 2);
	...
	if (p && vht_ielen > 0) {
		...
		_rtw_memcpy_22b(mcs_map, GET_VHT_CAPABILITY_ELE_TX_MCS(p + 2), 2);

		vht_highest_rate = rtw_get_vht_highest_rate_22b(mcs_map);
		vht_data_rate = rtw_vht_mcs_to_data_rate_22b(CHANNEL_WIDTH_80, short_GI, vht_highest_rate);
	}
#endif
		...
		char *start, char *stop, struct iw_event *iwe)
{
	...
	if (pbuf) {
		...
		if (pnetwork->network.Reserved[0] != BSS_TYPE_PROB_REQ) { /* Probe Request */
			out_len = rtw_get_sec_ie_22b(pnetwork->network.IEs , pnetwork->network.IELength, rsn_ie, &rsn_len, wpa_ie, &wpa_len);

			if (wpa_len > 0) {

				_rtw_memset_22b(pbuf, 0, buf_size);
				p += sprintf(p, "wpa_ie=");
				for (i = 0; i < wpa_len; i++)
				...
				_rtw_memset_22b(iwe, 0, sizeof(*iwe));
				iwe->cmd = IWEVCUSTOM;
		...
		char *start, char *stop, struct iw_event *iwe)
{
	...
	while (cnt < total_ielen) {
		if (rtw_is_wps_ie_22b(&ie_ptr[cnt], &wps_ielen) && (wps_ielen > 2)) {
		...
		char *start, char *stop, struct iw_event *iwe)
{
	...
	if (pnetwork->network.Reserved[0] != BSS_TYPE_PROB_REQ) { /* Probe Request */
		...
		out_len_wapi = rtw_get_wapi_ie_22b(pnetwork->network.IEs , pnetwork->network.IELength, wapi_ie, &wapi_len);
		...
		if (wapi_len > 0) {
			p = buf_wapi;
			/* _rtw_memset_22b(buf_wapi, 0, MAX_WAPI_IE_LEN*2); */
			p += sprintf(p, "wapi_ie=");
		...
		char *start, char *stop, struct iw_event *iwe)
{
	...
	if (check_fwstate(pmlmepriv, _FW_LINKED) == _TRUE &&
	    is_same_network_22b(&pmlmepriv->cur_network.network, &pnetwork->network, 0)) {
...
#else
	{
		...
		iwe->u.qual.level = (u8)phydm_signal_scale_mapping_22b(&pHal->odmpriv, ss);
	}
#endif
#endif
	...
	if (IS_NM_ENABLE(padapter)) {
		...
		noise = translate_dbm_to_percentage_22b(noise);/*percentage*/
		#endif
		...
		char *start, char *stop)
{
	...
	if (pnetwork->network.Reserved[0] == BSS_TYPE_PROB_REQ) /* Probe Request */
		...
		_rtw_memcpy_22b((u8 *)&cap, rtw_get_capability_22b_from_ie_22b(pnetwork->network.IEs), 2);
		cap = le16_to_cpu(cap);
	}
...
static int wpa_set_encryption(struct net_device *dev, struct ieee_param *param, u32 param_len)
{
	...
	if (strcmp(param->u.crypt.alg, "WEP") == 0) {
		...
		_rtw_memcpy_22b(&(psecuritypriv->dot11DefKey[wep_key_idx].skey[0]), param->u.crypt.key, wep_key_len);
		...
		if (check_fwstate(pmlmepriv, WIFI_STATION_STATE | WIFI_MP_STATE) == _TRUE) { /* sta mode */
			...
			if (psta == NULL) {
				...
				if (param->u.crypt.set_tx == 1) { /* pairwise key */
					...
					_rtw_memcpy_22b(psta->dot118021x_UncstKey.skey,  param->u.crypt.key, (param->u.crypt.key_len > 16 ? 16 : param->u.crypt.key_len));
					if (strcmp(param->u.crypt.alg, "TKIP") == 0) { /* set mic key */
						_rtw_memcpy_22b(psta->dot11tkiptxmickey.skey, &(param->u.crypt.key[16]), 8);
						_rtw_memcpy_22b(psta->dot11tkiprxmickey.skey, &(param->u.crypt.key[24]), 8);
						padapter->securitypriv.busetkipkey = _FALSE;
					}
					psta->dot11txpn.val = RTW_GET_LE64(param->u.crypt.seq);
					psta->dot11rxpn.val = RTW_GET_LE64(param->u.crypt.seq);
					psta->bpairwise_key_installed = _TRUE;
					rtw_setstakey_cmd_22b(padapter, psta, UNICAST_KEY, _TRUE);

				} else { /* group key */
					if (strcmp(param->u.crypt.alg, "TKIP") == 0 || strcmp(param->u.crypt.alg, "CCMP") == 0) {
						RTW_INFO(FUNC_ADPT_FMT" set %s GTK idx:%u, len:%u\n"
							, FUNC_ADPT_ARG(padapter), param->u.crypt.alg, param->u.crypt.idx, param->u.crypt.key_len);
						_rtw_memcpy_22b(padapter->securitypriv.dot118021XGrpKey[param->u.crypt.idx].skey,  param->u.crypt.key,
							(param->u.crypt.key_len > 16 ? 16 : param->u.crypt.key_len));
						/* only TKIP group key need to install this */
						if (param->u.crypt.key_len > 16) {
							_rtw_memcpy_22b(padapter->securitypriv.dot118021XGrptxmickey[param->u.crypt.idx].skey, &(param->u.crypt.key[16]), 8);
							_rtw_memcpy_22b(padapter->securitypriv.dot118021XGrprxmickey[param->u.crypt.idx].skey, &(param->u.crypt.key[24]), 8);
						}
						padapter->securitypriv.binstallGrpkey = _TRUE;
						if (param->u.crypt.idx < 4)
							_rtw_memcpy_22b(padapter->securitypriv.iv_seq[param->u.crypt.idx], param->u.crypt.seq, 8);
						padapter->securitypriv.dot118021XGrpKeyid = param->u.crypt.idx;
						rtw_set_key_22b(padapter, &padapter->securitypriv, param->u.crypt.idx, 1, _TRUE);

					#ifdef CONFIG_IEEE80211W
					} else if (strcmp(param->u.crypt.alg, "BIP") == 0) {
						RTW_INFO(FUNC_ADPT_FMT" set IGTK idx:%u, len:%u\n"
							, FUNC_ADPT_ARG(padapter), param->u.crypt.idx, param->u.crypt.key_len);
						_rtw_memcpy_22b(padapter->securitypriv.dot11wBIPKey[param->u.crypt.idx].skey,  param->u.crypt.key,
							(param->u.crypt.key_len > 16 ? 16 : param->u.crypt.key_len));
			...
			pbcmc_sta = rtw_get_bcmc_stainfo_22b(padapter);
			if (pbcmc_sta == NULL) {
...
static int rtw_set_wpa_ie(_adapter *padapter, char *pie, unsigned short ielen)
{
	...
	if (ielen) {
		...
		if (rtw_parse_wpa_ie_22b(buf, ielen, &group_cipher, &pairwise_cipher, NULL) == _SUCCESS) {
			padapter->securitypriv.dot11AuthAlgrthm = dot11AuthAlgrthm_8021X;
			padapter->securitypriv.ndisauthtype = Ndis802_11AuthModeWPAPSK;
			_rtw_memcpy_22b(padapter->securitypriv.supplicant_ie, &buf[0], ielen);
		}

		if (rtw_parse_wpa2_ie_22b(buf, ielen, &group_cipher, &pairwise_cipher, NULL, &mfp_opt) == _SUCCESS) {
			padapter->securitypriv.dot11AuthAlgrthm = dot11AuthAlgrthm_8021X;
		...
		{/* set wps_ie	 */
			...
			while (cnt < ielen) {
				...
				if ((eid == _VENDOR_SPECIFIC_IE_) && (_rtw_memcmp_22b(&buf[cnt + 2], wps_oui, 4) == _TRUE)) {
					RTW_INFO("SET WPS_IE\n");

					padapter->securitypriv.wps_ie_len = ((buf[cnt + 1] + 2) < MAX_WPS_IE_LEN) ? (buf[cnt + 1] + 2) : MAX_WPS_IE_LEN;

					_rtw_memcpy_22b(padapter->securitypriv.wps_ie, &buf[cnt], padapter->securitypriv.wps_ie_len);
	...
	if (padapter->securitypriv.dot11PrivacyAlgrthm == _TKIP_
		...
		rtw_hal_set_hwreg_22b(padapter, HW_VAR_OFF_RCR_AM, null_addr_22b);
			   ...
			   union iwreq_data *wrqu, char *extra)
{
	...
	if (check_fwstate(pmlmepriv, _FW_LINKED | WIFI_ADHOC_MASTER_STATE) == _TRUE) {
		/* parsing HT_CAP_IE */
		p = rtw_get_ie_22b(&pcur_bss->IEs[12], _HT_CAPABILITY_IE_, &ht_ielen, pcur_bss->IELength - 12);
		if (p && ht_ielen > 0)
			ht_cap = _TRUE;
		...
		if (rtw_is_cckratesonly_included_22b_22b((u8 *)prates) == _TRUE) {
			if (ht_cap == _TRUE)
				snprintf(wrqu->name, IFNAMSIZ, "IEEE 802.11bn");
			else
				snprintf(wrqu->name, IFNAMSIZ, "IEEE 802.11b");
		} else if ((rtw_is_cckrates_included_22b_22b((u8 *)prates)) == _TRUE) {
			if (ht_cap == _TRUE)
			   ...
			   union iwreq_data *wrqu, char *extra)
{
	...
	if (wrqu->freq.m <= 1000) {
		if (wrqu->freq.flags == IW_FREQ_AUTO) {
			if (rtw_chset_search_ch_22b(adapter_to_chset(padapter), wrqu->freq.m) > 0) {
		...
		/* If freq is invalid, rtw_freq2ch_22b() will return channel 1 */
		padapter->mlmeextpriv.cur_channel = rtw_freq2ch_22b(freq);
		RTW_INFO("%s: set to channel %d\n", __func__, padapter->mlmeextpriv.cur_channel);
	}
	set_channel_bwmode_22b(padapter, padapter->mlmeextpriv.cur_channel, HAL_PRIME_CHNL_OFFSET_DONT_CARE, CHANNEL_WIDTH_20);
exit:
			   ...
			   union iwreq_data *wrqu, char *extra)
{
	...
	if (check_fwstate(pmlmepriv, _FW_LINKED) == _TRUE && check_fwstate(pmlmepriv, WIFI_MONITOR_STATE) != _TRUE) {
		...
		wrqu->freq.m = rtw_ch2freq_22b(pcur_bss->Configuration.DSConfig) * 100000;
		wrqu->freq.e = 1;
		wrqu->freq.i = pcur_bss->Configuration.DSConfig;

	} else {
		wrqu->freq.m = rtw_ch2freq_22b(padapter->mlmeextpriv.cur_channel) * 100000;
		wrqu->freq.e = 1;
			   ...
			   union iwreq_data *wrqu, char *b)
{
	...
	if (wrqu->mode == IW_MODE_MONITOR) {
		rtw_ps_deny_22b(padapter, PS_DENY_MONITOR_MODE);
		LeaveAllPowerSaveMode_22b(padapter);
	} else {
		rtw_ps_deny_22b_cancel(padapter, PS_DENY_MONITOR_MODE);
	}
	...
	if (rtw_set_802_11_infrastructure_mode_22b(padapter, networkType) == _FALSE) {
	...
	rtw_setopmode_cmd_22b(padapter, networkType, RTW_CMDF_WAIT_ACK);
			    ...
			    union iwreq_data *wrqu, char *extra)
{
	...
	_rtw_memcpy_22b(strIssueBssid, pPMK->bssid.sa_data, ETH_ALEN);
	if (pPMK->cmd == IW_PMKSA_ADD) {
		RTW_INFO("[rtw_wx_set_pmkid] IW_PMKSA_ADD!\n");
		if (_rtw_memcmp_22b(strIssueBssid, strZeroMacAddress, ETH_ALEN) == _TRUE)
			return intReturn ;
		else
			intReturn = _TRUE;
		blInserted = _FALSE;
		...
		for (j = 0 ; j < NUM_PMKID_CACHE; j++) {
			if (_rtw_memcmp_22b(psecuritypriv->PMKIDList[j].Bssid, strIssueBssid, ETH_ALEN) == _TRUE) {
				/* BSSID is matched, the same AP => rewrite with new PMKID. */

				RTW_INFO("[rtw_wx_set_pmkid] BSSID exists in the PMKList.\n");

				_rtw_memcpy_22b(psecuritypriv->PMKIDList[j].PMKID, pPMK->pmkid, IW_PMKID_LEN);
		...
		if (!blInserted) {
			...
			_rtw_memcpy_22b(psecuritypriv->PMKIDList[psecuritypriv->PMKIDIndex].Bssid, strIssueBssid, ETH_ALEN);
			_rtw_memcpy_22b(psecuritypriv->PMKIDList[psecuritypriv->PMKIDIndex].PMKID, pPMK->pmkid, IW_PMKID_LEN);
		...
		for (j = 0 ; j < NUM_PMKID_CACHE; j++) {
			if (_rtw_memcmp_22b(psecuritypriv->PMKIDList[j].Bssid, strIssueBssid, ETH_ALEN) == _TRUE) {
				...
				_rtw_memset_22b(psecuritypriv->PMKIDList[j].Bssid, 0x00, ETH_ALEN);
		...
		_rtw_memset_22b(&psecuritypriv->PMKIDList[0], 0x00, sizeof(RT_PMKID_LIST) * NUM_PMKID_CACHE);
		psecuritypriv->PMKIDIndex = 0;
		intReturn = _TRUE;
	}
	return intReturn ;
			    ...
			    union iwreq_data *wrqu, char *extra)
{
	...
	_rtw_memset_22b(range, 0, sizeof(*range));
	...
	*	range->max_nwid;	 Maximal NWID we are able to set  */
	...
	for (i = 0; i < RATE_COUNT && i < IW_MAX_BITRATES; i++)
		range->bitrate[i] = rtw_rates_22b[i];
	...
	for (i = 0, val = 0; i < rfctl->max_chan_nums; i++) {
		...
		if (rfctl->channel_set[i].ChannelNum != 0) {
			...
			range->freq[val].m = rtw_ch2freq_22b(rfctl->channel_set[i].ChannelNum) * 100000;
			range->freq[val].e = 1;
			val++;
		}
...
/* set bssid flow
 * s1. rtw_set_802_11_infrastructure_mode_22b()
 * s2. rtw_set_802_11_authentication_mode_22b()
 * s3. set_802_11_encryption_mode()
 * s4. rtw_set_802_11_bssid_22b() */
			  ...
			  char *extra)
{
	...
	*/
...
#ifdef CONFIG_CONCURRENT_MODE
	if (rtw_mi_buddy_check_fwstate_22b(padapter, _FW_UNDER_SURVEY | _FW_UNDER_LINKING) == _TRUE) {
	...
	rtw_ps_deny_22b(padapter, PS_DENY_JOIN);
	if (_FAIL == rtw_pwr_wakeup(padapter)) {
		ret = -1;
		goto cancel_ps_deny;
	}
	...
	while (1) {
		...
		if ((rtw_end_of_queue_search_22b(phead, pmlmepriv->pscanned)) == _TRUE) {
#if 0
			ret = -EINVAL;
			goto cancel_ps_deny;

			if (check_fwstate(pmlmepriv, WIFI_ADHOC_STATE) == _TRUE) {
				rtw_set_802_11_bssid_22b(padapter, temp->sa_data);
				goto cancel_ps_deny;
			} else {
				ret = -EINVAL;
				goto cancel_ps_deny;
		...
		if ((_rtw_memcmp_22b(dst_bssid, src_bssid, ETH_ALEN)) == _TRUE) {
			if (!rtw_set_802_11_infrastructure_mode_22b(padapter, pnetwork->network.InfrastructureMode)) {
				ret = -1;
	...
	rtw_set_802_11_authentication_mode_22b(padapter, authmode);
	/* set_802_11_encryption_mode(padapter, padapter->securitypriv.ndisencryptstatus); */
	if (rtw_set_802_11_bssid_22b(padapter, temp->sa_data) == _FALSE) {
		ret = -1;
		goto cancel_ps_deny;
	}

cancel_ps_deny:
	rtw_ps_deny_22b_cancel(padapter, PS_DENY_JOIN);
			  ...
			  union iwreq_data *wrqu, char *extra)
{
	...
	if (((check_fwstate(pmlmepriv, _FW_LINKED)) == _TRUE) ||
		...
		_rtw_memcpy_22b(wrqu->ap_addr.sa_data, pcur_bss->MacAddress, ETH_ALEN);
	else
		_rtw_memset_22b(wrqu->ap_addr.sa_data, 0, ETH_ALEN);
			   ...
			   union iwreq_data *wrqu, char *extra)
{
	...
	switch (mlme->cmd) {
	case IW_MLME_DEAUTH:
		if (!rtw_set_802_11_disassociate_22b(padapter))
			ret = -1;
		break;
			   ...
			   union iwreq_data *wrqu, char *extra)
{
	...
	if (rtw_mp_mode_22b_check(padapter)) {
	...
	if (rtw_is_scan_deny_22b(padapter)) {
		indicate_wx_scan_complete_event_22b(padapter);
		goto exit;
	}

	rtw_ps_deny_22b(padapter, PS_DENY_SCAN);
	if (_FAIL == rtw_pwr_wakeup(padapter)) {
		ret = -1;
		goto cancel_ps_deny;
	}

	if (!rtw_is_adapter_up_22b(padapter)) {
		ret = -1;
		goto cancel_ps_deny;
	}
	...
	if (rtw_mi_busy_traffic_check_22b(padapter, _FALSE)) {
		indicate_wx_scan_complete_event_22b(padapter);
		goto cancel_ps_deny;
	}
#endif
...
#ifdef CONFIG_CONCURRENT_MODE
	if (rtw_mi_buddy_check_fwstate_22b(padapter,
	...
	if (wrqu->data.length == sizeof(struct iw_scan_req)) {
		...
		if (wrqu->data.flags & IW_SCAN_THIS_ESSID) {
			...
			_rtw_memcpy_22b(&parm.ssid[0].Ssid, &req->essid, len);
			parm.ssid[0].SsidLength = len;
			parm.ssid_num = 1;

			RTW_INFO("IW_SCAN_THIS_ESSID, ssid=%s, len=%d\n", req->essid, req->essid_len);

			_status = rtw_set_802_11_bssid_22b_list_scan(padapter, &parm);
		...
		if (wrqu->data.length >= WEXT_CSCAN_HEADER_SIZE
		    && _rtw_memcmp_22b(extra, WEXT_CSCAN_HEADER, WEXT_CSCAN_HEADER_SIZE) == _TRUE
		   ) {
			...
			while (len >= 1) {
				...
				case WEXT_CSCAN_SSID_SECTION:
					...
					if (sec_len > 0 && sec_len <= len) {
						...
						_rtw_memcpy_22b(&parm.ssid[ssid_index].Ssid, pos, sec_len);
			...
			_status = rtw_set_802_11_bssid_22b_list_scan(padapter, NULL);

	if (_status == _FALSE)
		ret = -1;

cancel_ps_deny:
	rtw_ps_deny_22b_cancel(padapter, PS_DENY_SCAN);
			   ...
			   union iwreq_data *wrqu, char *extra)
{
	...
	while (1) {
		...
		if (rtw_chset_search_ch_22b(adapter_to_chset(padapter), pnetwork->network.Configuration.DSConfig) >= 0
		    && rtw_mlme_band_check_22b(padapter, pnetwork->network.Configuration.DSConfig) == _TRUE
		    && _TRUE == rtw_validate_ssid_22b(&(pnetwork->network.Ssid))
		   )
			    ...
			    union iwreq_data *wrqu, char *extra)
{
...
#ifdef CONFIG_CONCURRENT_MODE
	if (rtw_mi_buddy_check_fwstate_22b(padapter, _FW_UNDER_SURVEY | _FW_UNDER_LINKING)) {
	...
	if (wrqu->essid.flags && wrqu->essid.length) {
		...
		_rtw_memset_22b(&ndis_ssid, 0, sizeof(NDIS_802_11_SSID));
		ndis_ssid.SsidLength = len;
		_rtw_memcpy_22b(ndis_ssid.Ssid, extra, len);
		...
		while (1) {
...
#if 0
				if (check_fwstate(pmlmepriv, WIFI_ADHOC_STATE) == _TRUE) {
					rtw_set_802_11_ssid_22b(padapter, &ndis_ssid);
			...
			if ((_rtw_memcmp_22b(dst_ssid, src_ssid, ndis_ssid.SsidLength) == _TRUE) &&
				...
				if (rtw_set_802_11_infrastructure_mode_22b(padapter, pnetwork->network.InfrastructureMode) == _FALSE) {
					ret = -1;
					_exit_critical_bh(&queue->lock, &irqL);
					goto cancel_ps_deny;
				}
		...
		rtw_set_802_11_authentication_mode_22b(padapter, authmode);
		/* set_802_11_encryption_mode(padapter, padapter->securitypriv.ndisencryptstatus); */
		if (rtw_set_802_11_ssid_22b(padapter, &ndis_ssid) == _FALSE) {
			ret = -1;
			goto cancel_ps_deny;
		}
	}
			    ...
			    union iwreq_data *wrqu, char *extra)
{
	...
	if ((check_fwstate(pmlmepriv, _FW_LINKED) == _TRUE) ||
		...
		_rtw_memcpy_22b(extra, pcur_bss->Ssid.Ssid, len);
...
set_rate:
	...
	if (rtw_setdatarate_cmd_22b(padapter, datarates) != _SUCCESS) {
		ret = -1;
	}
			   ...
			   union iwreq_data *wrqu, char *extra)
{
	...
	max_rate = rtw_get_cur_max_rate_22b((_adapter *)rtw_netdev_priv(dev));
			  ...
			  union iwreq_data *wrqu, char *keybuf)
{
	...
	_rtw_memset_22b(&wep, 0, sizeof(NDIS_802_11_WEP));
	...
	_rtw_memcpy_22b(wep.KeyMaterial, keybuf, wep.KeyLength);

	if (rtw_set_802_11_add_wep_22b(padapter, &wep) == _FALSE) {
			  ...
			  union iwreq_data *wrqu, char *keybuf)
{
	...
	case Ndis802_11Encryption1Enabled:
		...
		if (erq->length) {
			_rtw_memcpy_22b(keybuf, padapter->securitypriv.dot11DefKey[key].skey, padapter->securitypriv.dot11DefKeylen[key]);
	...
	case IW_AUTH_80211_AUTH_ALG:
		...
		*/
		if (check_fwstate(&padapter->mlmepriv, _FW_LINKED)) {
			...
			RTW_INFO("%s...call rtw_indicate_disconnect_22b\n ", __FUNCTION__);
			      ...
			      union iwreq_data *wrqu, char *extra)
{
	...
	_rtw_memset_22b(param, 0, param_len);

	param->cmd = IEEE_CMD_SET_ENCRYPTION;
	_rtw_memset_22b(param->sta_addr, 0xff, ETH_ALEN);
	...
	case IW_ENCODE_ALG_SM4:
		alg_name = "SMS4";
		_rtw_memcpy_22b(param->sta_addr, pext->addr.sa_data, ETH_ALEN);
	...
	if (pext->ext_flags & IW_ENCODE_EXT_RX_SEQ_VALID) {
		...
		if (pext->alg == IW_ENCODE_ALG_SM4)
			_rtw_memcpy_22b(param->u.crypt.seq, pext->rx_seq, 16);
		else
#endif /* CONFIG_IOCTL_CFG80211 */
#endif /* CONFIG_WAPI_SUPPORT */
			_rtw_memcpy_22b(param->u.crypt.seq, pext->rx_seq, 8);
	}

	if (pext->key_len) {
		param->u.crypt.key_len = pext->key_len;
		/* _rtw_memcpy_22b(param + 1, pext + 1, pext->key_len); */
		_rtw_memcpy_22b(param->u.crypt.key, pext + 1, pext->key_len);
	}
			   ...
			   union iwreq_data *wrqu, char *extra)
{
	...
	if (extra) {
		wrqu->data.length = 14;
		wrqu->data.flags = 1;
		_rtw_memcpy_22b(extra, "<WIFI@REALTEK>", 14);
	}
			  ...
			  union iwreq_data *wrqu, char *extra)
{
	...
	path = *(u32 *)extra;
	addr = *((u32 *)extra + 1);
	data32 = rtw_hal_read_rfreg_22b_22b_22b(padapter, path, addr, 0xFFFFF);
			   ...
			   union iwreq_data *wrqu, char *extra)
{
	...
	data32 = *((u32 *)extra + 2);
	/*	RTW_INFO("%s: path=%d addr=0x%02x data=0x%05x\n", __func__, path, addr, data32); */
	rtw_hal_write_rfreg_22b_22b_22b(padapter, path, addr, 0xFFFFF, data32);
				   ...
				   union iwreq_data *wrqu, char *extra)
{
	...
	if (_SUCCESS != rtw_set_channel_plan_22b(padapter, channel_plan_req))
		return -EPERM;
...
static void rtw_dbg_mode_hdl(_adapter *padapter, u32 id, u8 *pdata, u32 len)
{
	...
	case GEN_MP_IOCTL_SUBCODE(READ_RF_REG):
		...
		value = rtw_hal_read_rfreg_22b_22b_22b(padapter, path, offset, 0xffffffff);
	...
	case GEN_MP_IOCTL_SUBCODE(WRITE_RF_REG):
		...
		rtw_hal_write_rfreg_22b_22b_22b(padapter, path, offset, 0xffffffff, value);

		break;
	case GEN_MP_IOCTL_SUBCODE(TRIGGER_GPIO):
		RTW_INFO("==> trigger gpio 0\n");
		rtw_hal_set_hwreg_22b(padapter, HW_VAR_TRIGGER_GPIO_0, 0);
		break;
#ifdef CONFIG_BT_COEXIST
	case GEN_MP_IOCTL_SUBCODE(SET_DM_BT):
		RTW_INFO("==> set dm_bt_coexist:%x\n", *(u8 *)pdata);
		rtw_hal_set_hwreg_22b(padapter, HW_VAR_BT_SET_COEXIST, pdata);
		break;
	case GEN_MP_IOCTL_SUBCODE(DEL_BA):
		RTW_INFO("==> delete ba:%x\n", *(u8 *)pdata);
		rtw_hal_set_hwreg_22b(padapter, HW_VAR_BT_ISSUE_DELBA, pdata);
		break;
#endif
#ifdef DBG_CONFIG_ERROR_DETECT
	case GEN_MP_IOCTL_SUBCODE(GET_WIFI_STATUS):
		*pdata = rtw_hal_sreset_get_wifi_status_22b(padapter);
		break;
#endif
			   ...
			   union iwreq_data *wrqu, char *extra)
{
	...
	while (1) {
		...
		if (_rtw_memcmp_22b(bssid, pnetwork->network.MacAddress, ETH_ALEN) == _TRUE) { /* BSSID match, then check if supporting wpa/wpa2 */
			RTW_INFO("BSSID:" MAC_FMT "\n", MAC_ARG(bssid));

			pbuf = rtw_get_wpa_ie_22b(&pnetwork->network.IEs[12], &wpa_ielen, pnetwork->network.IELength - 12);
			if (pbuf && (wpa_ielen > 0)) {
				pdata->flags = 1;
				break;
			}

			pbuf = rtw_get_wpa2_ie_22b(&pnetwork->network.IEs[12], &wpa_ielen, pnetwork->network.IELength - 12);
			if (pbuf && (wpa_ielen > 0)) {
				pdata->flags = 2;
				break;
			}
			 ...
			 union iwreq_data *wrqu, char *extra)
{
	...
	if (u32wps_start == 1)   /* WPS Start */
		rtw_led_control_22b(padapter, LED_CTL_START_WPS);
	else if (u32wps_start == 2)   /* WPS Stop because of wps success */
		rtw_led_control_22b(padapter, LED_CTL_STOP_WPS);
	else if (u32wps_start == 3)   /* WPS Stop because of wps fail */
		rtw_led_control_22b(padapter, LED_CTL_STOP_WPS_FAIL);
			       ...
			       union iwreq_data *wrqu, char *extra)
{
	...
	if (_FAIL == rtw_p2p_enable_22b(padapter, init_role)) {
		ret = -EFAULT;
		goto exit;
	}
	...
	if (init_role != P2P_ROLE_DISABLE) {
		...
		set_channel_bwmode_22b(padapter, channel, ch_offset, bwmode);
	}
				    ...
				    union iwreq_data *wrqu, char *extra)
{
	...
	_rtw_memcpy_22b(pwdinfo->nego_ssid, extra, strlen(extra));
				...
				union iwreq_data *wrqu, char *extra)
{
	...
	if (!rtw_p2p_chk_state(pwdinfo, P2P_STATE_NONE)) {
		if (extra[0] == '0') {
			...
			_rtw_memset_22b(&pwdinfo->profileinfo[0], 0x00, sizeof(struct profile_info) * P2P_MAX_PERSISTENT_GROUP_NUM);
			...
			if (pwdinfo->profileindex >= P2P_MAX_PERSISTENT_GROUP_NUM)
				...
				for (jj = 0, kk = 1; jj < ETH_ALEN; jj++, kk += 3)
					pwdinfo->profileinfo[pwdinfo->profileindex].peermac[jj] = key_2char2num_22b(extra[kk], extra[kk + 1]);

				/* pwdinfo->profileinfo[pwdinfo->profileindex].ssidlen = ( extra[18] - '0' ) * 10 + ( extra[19] - '0' ); */
				/* _rtw_memcpy_22b( pwdinfo->profileinfo[pwdinfo->profileindex].ssid, &extra[20], pwdinfo->profileinfo[pwdinfo->profileindex].ssidlen ); */
			 ...
			 union iwreq_data *wrqu, char *extra)
{
	...
	RTW_INFO("[%s] %s %d\n", __FUNCTION__, extra, wrqu->data.length - 1);
	_rtw_memset_22b(pwdinfo->device_name, 0x00, WPS_MAX_DEVICE_NAME_LEN);
	_rtw_memcpy_22b(pwdinfo->device_name, extra, wrqu->data.length - 1);
			...
			union iwreq_data *wrqu, char *extra, char *subcmd)
{
	...
	while (1) {
		...
		pnetwork = LIST_CONTAINOR(plist, struct wlan_network, list);
		if (_rtw_memcmp_22b(pnetwork->network.MacAddress, peerMAC, ETH_ALEN)) {
			u8 *wpsie;
			uint	wpsie_len = 0;

			/*	The mac address is matched. */

			wpsie = rtw_get_wps_ie_22b_from_scan_queue(&pnetwork->network.IEs[0], pnetwork->network.IELength, NULL, &wpsie_len, pnetwork->network.Reserved[0]);
			if (wpsie) {
				rtw_get_wps_attr_22b_content(wpsie, wpsie_len, WPS_ATTR_CONF_METHOD, (u8 *)&attr_content, &attr_contentlen);
				if (attr_contentlen) {
	...
	_rtw_memcpy_22b(extra, attr_content_str, wrqu->data.length);
		...
		union iwreq_data *wrqu, char *extra, char *subcmd)
{
	...
	while (1) {
		...
		if (_rtw_memcmp_22b(pnetwork->network.MacAddress, peerMAC, ETH_ALEN)) {
			...
			p2pie = rtw_bss_ex_get_p2p_ie_22b(&pnetwork->network, NULL, &p2pielen);
			if (p2pie) {
				while (p2pie) {
					/*	The P2P Device ID attribute is included in the Beacon frame. */
					/*	The P2P Device Info attribute is included in the probe response frame. */

					_rtw_memset_22b(attr_content, 0x00, 100);
					if (rtw_get_p2p_attr_22b_content(p2pie, p2pielen, P2P_ATTR_DEVICE_ID, attr_content, &attr_contentlen)) {
						/*	Handle the P2P Device ID attribute of Beacon first */
						blnMatch = 1;
						break;

					} else if (rtw_get_p2p_attr_22b_content(p2pie, p2pielen, P2P_ATTR_DEVICE_INFO, attr_content, &attr_contentlen)) {
					...
					/* Get the next P2P IE */
					p2pie = rtw_get_p2p_ie_22b(p2pie + p2pielen, BSS_EX_TLV_IES_LEN(&pnetwork->network) - (p2pie + p2pielen - BSS_EX_TLV_IES(&pnetwork->network)), NULL, &p2pielen);
				}
			}
		}
	...
	_rtw_memcpy_22b(extra, go_devadd_str, wrqu->data.length);
			   ...
			   union iwreq_data *wrqu, char *extra, char *subcmd)
{
	...
	while (1) {
		...
		if (_rtw_memcmp_22b(pnetwork->network.MacAddress, peerMAC, ETH_ALEN)) {
			...
			wpsie = rtw_get_wps_ie_22b_from_scan_queue(&pnetwork->network.IEs[0], pnetwork->network.IELength, NULL, &wpsie_len, pnetwork->network.Reserved[0]);
			if (wpsie) {
				rtw_get_wps_attr_22b_content(wpsie, wpsie_len, WPS_ATTR_PRIMARY_DEV_TYPE, dev_type, &dev_type_len);
				if (dev_type_len) {
					u16	type = 0;

					_rtw_memcpy_22b(&type, dev_type, 2);
	...
	_rtw_memcpy_22b(extra, dev_type_str, wrqu->data.length);
			   ...
			   union iwreq_data *wrqu, char *extra, char *subcmd)
{
	...
	while (1) {
		...
		if (_rtw_memcmp_22b(pnetwork->network.MacAddress, peerMAC, ETH_ALEN)) {
			...
			wpsie = rtw_get_wps_ie_22b_from_scan_queue(&pnetwork->network.IEs[0], pnetwork->network.IELength, NULL, &wpsie_len, pnetwork->network.Reserved[0]);
			if (wpsie) {
				rtw_get_wps_attr_22b_content(wpsie, wpsie_len, WPS_ATTR_DEVICE_NAME, dev_name, &dev_len);
				if (dev_len) {
	...
	_rtw_memcpy_22b(extra, dev_name_str, wrqu->data.length);
		...
		union iwreq_data *wrqu, char *extra, char *subcmd)
{
	...
	while (1) {
		...
		if (_rtw_memcmp_22b(pnetwork->network.MacAddress, peerMAC, ETH_ALEN)) {
			...
			if (p2pie) {
				while (p2pie) {
					/* _rtw_memset_22b( attr_content, 0x00, 2); */
					if (rtw_get_p2p_attr_22b_content(p2pie, p2pielen, P2P_ATTR_CAPABILITY, attr_content, &attr_contentlen)) {
	...
	_rtw_memcpy_22b(extra, inv_proc_str, wrqu->data.length);
			   ...
			   union iwreq_data *wrqu, char *extra)
{
	...
	for (jj = 0, kk = 0; jj < ETH_ALEN; jj++, kk += 3)
		peerMAC[jj] = key_2char2num_22b(extra[kk], extra[kk + 1]);
	...
	if (uintPeerChannel) {
		...
		_rtw_memset_22b(&pwdinfo->nego_req_info, 0x00, sizeof(struct tx_nego_req_info));
		_rtw_memset_22b(&pwdinfo->groupid_info, 0x00, sizeof(struct group_id_info));

		pwdinfo->nego_req_info.peer_channel_num[0] = uintPeerChannel;
		_rtw_memcpy_22b(pwdinfo->nego_req_info.peerDevAddr, pnetwork->network.MacAddress, ETH_ALEN);
		...
		rtw_sitesurvey_cmd_22b(padapter, NULL);
			      ...
			      union iwreq_data *wrqu, char *extra)
{
	...
	if (rtw_p2p_chk_state(pwdinfo, P2P_STATE_NONE)) {
		...
		_rtw_memset_22b(pinvite_req_info->go_bssid, 0x00, ETH_ALEN);
		_rtw_memset_22b(pinvite_req_info->go_ssid, 0x00, WLAN_SSID_MAXLEN);
		pinvite_req_info->ssidlen = 0x00;
		pinvite_req_info->operating_ch = pwdinfo->operating_channel;
		_rtw_memset_22b(pinvite_req_info->peer_macaddr, 0x00, ETH_ALEN);
		pinvite_req_info->token = 3;
	}

	for (jj = 0, kk = 0; jj < ETH_ALEN; jj++, kk += 3)
		pinvite_req_info->peer_macaddr[jj] = key_2char2num_22b(extra[kk], extra[kk + 1]);
	...
	while (1) {
		...
		if (p2pie) {
			...
			if (rtw_get_p2p_attr_22b_content(p2pie, p2pielen, P2P_ATTR_DEVICE_ID, attr_content, &attr_contentlen)) {
				/*	Handle the P2P Device ID attribute of Beacon first */
				if (_rtw_memcmp_22b(attr_content, pinvite_req_info->peer_macaddr, ETH_ALEN)) {
					uintPeerChannel = pnetwork->network.Configuration.DSConfig;
					break;
				}
			} else if (rtw_get_p2p_attr_22b_content(p2pie, p2pielen, P2P_ATTR_DEVICE_INFO, attr_content, &attr_contentlen)) {
...
#ifdef CONFIG_WFD
	if (hal_chk_wl_func_22b(padapter, WL_FUNC_MIRACAST) && uintPeerChannel) {
	...
	if (uintPeerChannel) {
		...
		for (jj = 0, kk = 18; jj < ETH_ALEN; jj++, kk += 3)
			pinvite_req_info->go_bssid[jj] = key_2char2num_22b(extra[kk], extra[kk + 1]);

		/*	Store the GO's ssid */
		pinvite_req_info->ssidlen = wrqu->data.length - 36;
		_rtw_memcpy_22b(pinvite_req_info->go_ssid, &extra[36], (u32) pinvite_req_info->ssidlen);
		...
		if (rtw_mi_check_status_22b(padapter, MI_LINKED)) {
			...
			set_channel_bwmode_22b(padapter, uintPeerChannel, HAL_PRIME_CHNL_OFFSET_DONT_CARE, CHANNEL_WIDTH_20);
#else
		set_channel_bwmode_22b(padapter, uintPeerChannel, HAL_PRIME_CHNL_OFFSET_DONT_CARE, CHANNEL_WIDTH_20);
#endif/*CONFIG_CONCURRENT_MODE*/
	...
	while (1) {
		...
		if (p2pie) {
			...
			if (rtw_get_p2p_attr_22b_content(p2pie, p2pielen, P2P_ATTR_DEVICE_ID, attr_content, &attr_contentlen)) {
				...
				if (_rtw_memcmp_22b(attr_content, peerMAC, ETH_ALEN)) {
					uintPeerChannel = pnetwork->network.Configuration.DSConfig;
					break;
				}
			} else if (rtw_get_p2p_attr_22b_content(p2pie, p2pielen, P2P_ATTR_DEVICE_INFO, attr_content, &attr_contentlen)) {
				  ...
				  union iwreq_data *wrqu, char *extra)
{
	...
	if (*extra == '0')
		rtw_wfd_enable_22b(padapter, 0);
	else if (*extra == '1')
		rtw_wfd_enable_22b(padapter, 1);
			     ...
			     union iwreq_data *wrqu, char *extra)
{
	...
	if (pwdinfo->p2p_state == P2P_STATE_NONE) {
		...
		_rtw_memset_22b(pwdinfo->tx_prov_disc_info.peerDevAddr, 0x00, ETH_ALEN);
		_rtw_memset_22b(pwdinfo->tx_prov_disc_info.peerIFAddr, 0x00, ETH_ALEN);
		_rtw_memset_22b(&pwdinfo->tx_prov_disc_info.ssid, 0x00, sizeof(NDIS_802_11_SSID));
	...
	if (_rtw_memcmp_22b(&extra[18], "display", 7))
		pwdinfo->tx_prov_disc_info.wps_config_method_request = WPS_CM_DISPLYA;
	else if (_rtw_memcmp_22b(&extra[18], "keypad", 7))
		pwdinfo->tx_prov_disc_info.wps_config_method_request = WPS_CM_KEYPAD;
	else if (_rtw_memcmp_22b(&extra[18], "pbc", 3))
		pwdinfo->tx_prov_disc_info.wps_config_method_request = WPS_CM_PUSH_BUTTON;
	else if (_rtw_memcmp_22b(&extra[18], "label", 5))
	...
	if (uintPeerChannel) {
#ifdef CONFIG_WFD
		if (hal_chk_wl_func_22b(padapter, WL_FUNC_MIRACAST)) {
			struct wifi_display_info *pwfd_info = pwdinfo->wfd_info;
			u8 *wfd_ie;
			uint wfd_ielen = 0;
		...
		if (rtw_mi_check_status_22b(padapter, MI_LINKED))
			_cancel_timer_ex(&pwdinfo->ap_p2p_switch_timer);
#endif /* CONFIG_CONCURRENT_MODE */
		_rtw_memcpy_22b(pwdinfo->tx_prov_disc_info.peerIFAddr, pnetwork->network.MacAddress, ETH_ALEN);
		_rtw_memcpy_22b(pwdinfo->tx_prov_disc_info.peerDevAddr, peerMAC, ETH_ALEN);
		...
		if (rtw_p2p_chk_role(pwdinfo, P2P_ROLE_CLIENT))
			_rtw_memcpy_22b(&pwdinfo->tx_prov_disc_info.ssid, &pnetwork->network.Ssid, sizeof(NDIS_802_11_SSID));
		else if (rtw_p2p_chk_role(pwdinfo, P2P_ROLE_DEVICE) || rtw_p2p_chk_role(pwdinfo, P2P_ROLE_GO)) {
			_rtw_memcpy_22b(pwdinfo->tx_prov_disc_info.ssid.Ssid, pwdinfo->p2p_wildcard_ssid, P2P_WILDCARD_SSID_LEN);
		       ...
		       union iwreq_data *wrqu, char *extra)
{
	...
	if (_rtw_memcmp_22b(extra, "enable=", 7))
		rtw_wext_p2p_enable(dev, info, wrqu, &extra[7]);
	else if (_rtw_memcmp_22b(extra, "setDN=", 6)) {
		wrqu->data.length -= 6;
		rtw_p2p_setDN(dev, info, wrqu, &extra[6]);
	} else if (_rtw_memcmp_22b(extra, "profilefound=", 13)) {
		wrqu->data.length -= 13;
		rtw_p2p_profilefound(dev, info, wrqu, &extra[13]);
	} else if (_rtw_memcmp_22b(extra, "prov_disc=", 10)) {
		wrqu->data.length -= 10;
		rtw_p2p_prov_disc(dev, info, wrqu, &extra[10]);
	} else if (_rtw_memcmp_22b(extra, "nego=", 5)) {
		wrqu->data.length -= 5;
		rtw_p2p_connect(dev, info, wrqu, &extra[5]);
	} else if (_rtw_memcmp_22b(extra, "intent=", 7)) {
		/*	Commented by Albert 2011/03/23 */
		/*	The wrqu->data.length will include the null character */
		/*	So, we will decrease 7 + 1 */
		wrqu->data.length -= 8;
		rtw_p2p_set_intent(dev, info, wrqu, &extra[7]);
	} else if (_rtw_memcmp_22b(extra, "ssid=", 5)) {
		wrqu->data.length -= 5;
		rtw_p2p_set_go_nego_ssid(dev, info, wrqu, &extra[5]);
	} else if (_rtw_memcmp_22b(extra, "got_wpsinfo=", 12)) {
		wrqu->data.length -= 12;
		rtw_p2p_got_wpsinfo(dev, info, wrqu, &extra[12]);
	} else if (_rtw_memcmp_22b(extra, "listen_ch=", 10)) {
		/*	Commented by Albert 2011/05/24 */
		/*	The wrqu->data.length will include the null character */
		/*	So, we will decrease (10 + 1)	 */
		wrqu->data.length -= 11;
		rtw_p2p_set_listen_ch(dev, info, wrqu, &extra[10]);
	} else if (_rtw_memcmp_22b(extra, "op_ch=", 6)) {
		/*	Commented by Albert 2011/05/24 */
		/*	The wrqu->data.length will include the null character */
		/*	So, we will decrease (6 + 1)	 */
		wrqu->data.length -= 7;
		rtw_p2p_set_op_ch(dev, info, wrqu, &extra[6]);
	} else if (_rtw_memcmp_22b(extra, "invite=", 7)) {
		wrqu->data.length -= 8;
		rtw_p2p_invite_req(dev, info, wrqu, &extra[7]);
	} else if (_rtw_memcmp_22b(extra, "persistent=", 11)) {
		wrqu->data.length -= 11;
		rtw_p2p_set_persistent(dev, info, wrqu, &extra[11]);
	} else if (_rtw_memcmp_22b(extra, "uuid=", 5)) {
		wrqu->data.length -= 5;
...
#ifdef CONFIG_WFD
	if (hal_chk_wl_func_22b(padapter, WL_FUNC_MIRACAST)) {
		if (_rtw_memcmp_22b(extra, "sa=", 3)) {
			/* sa: WFD Session Available information */
			wrqu->data.length -= 3;
			rtw_p2p_set_sa(dev, info, wrqu, &extra[3]);
		} else if (_rtw_memcmp_22b(extra, "pc=", 3)) {
			/* pc: WFD Preferred Connection */
			wrqu->data.length -= 3;
			rtw_p2p_set_pc(dev, info, wrqu, &extra[3]);
		} else if (_rtw_memcmp_22b(extra, "wfd_type=", 9)) {
			wrqu->data.length -= 9;
			rtw_p2p_set_wfd_device_type(dev, info, wrqu, &extra[9]);
		} else if (_rtw_memcmp_22b(extra, "wfd_enable=", 11)) {
			wrqu->data.length -= 11;
			rtw_p2p_set_wfd_enable(dev, info, wrqu, &extra[11]);
		} else if (_rtw_memcmp_22b(extra, "driver_iface=", 13)) {
			wrqu->data.length -= 13;
		       ...
		       union iwreq_data *wrqu, char *extra)
{
	...
	if (_rtw_memcmp_22b(wrqu->data.pointer, "status", 6))
		rtw_p2p_get_status(dev, info, wrqu, extra);
	else if (_rtw_memcmp_22b(wrqu->data.pointer, "role", 4))
		rtw_p2p_get_role(dev, info, wrqu, extra);
	else if (_rtw_memcmp_22b(wrqu->data.pointer, "peer_ifa", 8))
		rtw_p2p_get_peer_ifaddr(dev, info, wrqu, extra);
	else if (_rtw_memcmp_22b(wrqu->data.pointer, "req_cm", 6))
		rtw_p2p_get_req_cm(dev, info, wrqu, extra);
	else if (_rtw_memcmp_22b(wrqu->data.pointer, "peer_deva", 9)) {
		/*	Get the P2P device address when receiving the provision discovery request frame. */
		rtw_p2p_get_peer_devaddr(dev, info, wrqu, extra);
	} else if (_rtw_memcmp_22b(wrqu->data.pointer, "group_id", 8))
		rtw_p2p_get_groupid(dev, info, wrqu, extra);
	else if (_rtw_memcmp_22b(wrqu->data.pointer, "inv_peer_deva", 13)) {
		/*	Get the P2P device address when receiving the P2P Invitation request frame. */
		rtw_p2p_get_peer_devaddr_by_invitation(dev, info, wrqu, extra);
	} else if (_rtw_memcmp_22b(wrqu->data.pointer, "op_ch", 5))
		rtw_p2p_get_op_ch(dev, info, wrqu, extra);

#ifdef CONFIG_WFD
	if (hal_chk_wl_func_22b(padapter, WL_FUNC_MIRACAST)) {
		if (_rtw_memcmp_22b(wrqu->data.pointer, "peer_port", 9))
			rtw_p2p_get_peer_wfd_port(dev, info, wrqu, extra);
		else if (_rtw_memcmp_22b(wrqu->data.pointer, "wfd_sa", 6))
			rtw_p2p_get_peer_wfd_session_available(dev, info, wrqu, extra);
		else if (_rtw_memcmp_22b(wrqu->data.pointer, "wfd_pc", 6))
			      ...
			      union iwreq_data *wrqu, char *extra)
{
	...
	rtw_hal_rcr_set_chk_bssid_22b(padapter, MLME_ACTION_NONE);

	return ret;
}

extern int rtw_change_ifname_22b_22b(_adapter *padapter, const char *ifname_22b);
			     ...
			     union iwreq_data *wrqu, char *extra)
{
	...
	char new_ifname_22b[IFNAMSIZ];

	if (rereg_priv->old_ifname_22b[0] == 0) {
		char *reg_ifname_22b;
#ifdef CONFIG_CONCURRENT_MODE
		if (padapter->isprimary)
			reg_ifname_22b = padapter->registrypriv.ifname_22b;
		else
#endif
			reg_ifname_22b = padapter->registrypriv.if2name_22b;

		strncpy(rereg_priv->old_ifname_22b, reg_ifname_22b, IFNAMSIZ);
		rereg_priv->old_ifname_22b[IFNAMSIZ - 1] = 0;
	}
	...
	if (copy_from_user(new_ifname_22b, wrqu->data.pointer, IFNAMSIZ))
		return -EFAULT;

	if (0 == strcmp(rereg_priv->old_ifname_22b, new_ifname_22b))
		return ret;

	RTW_INFO("%s new_ifname_22b:%s\n", __FUNCTION__, new_ifname_22b);
	rtw_set_rtnl_lock_holder_22b(dvobj, current);
	ret = rtw_change_ifname_22b_22b(padapter, new_ifname_22b);
	rtw_set_rtnl_lock_holder_22b(dvobj, NULL);
	if (0 != ret)
		goto exit;

	if (_rtw_memcmp_22b(rereg_priv->old_ifname_22b, "disable%d", 9) == _TRUE) {
		/* rtw_ips_mode_22b_req(&padapter->pwrctrlpriv, rereg_priv->old_ips_mode); */
	}

	strncpy(rereg_priv->old_ifname_22b, new_ifname_22b, IFNAMSIZ);
	rereg_priv->old_ifname_22b[IFNAMSIZ - 1] = 0;

	if (_rtw_memcmp_22b(new_ifname_22b, "disable%d", 9) == _TRUE) {
		...
		/* rtw_ips_mode_22b_req(&padapter->pwrctrlpriv, IPS_NORMAL); */
	}
exit:
	return ret;
			...
			union iwreq_data *wrqu, char *extra)
{
	...
	case 0x71: /* write_reg */
		...
		break;
	case 0x72: /* read_bb */
		RTW_INFO("read_bbreg_22b(0x%x)=0x%x\n", arg, rtw_hal_read_bbreg_22b_22b(padapter, arg, 0xffffffff));
		break;
	case 0x73: /* write_bb */
		rtw_hal_write_bbreg_22b_22b(padapter, arg, 0xffffffff, extra_arg);
		RTW_INFO("write_bbreg_22b(0x%x)=0x%x\n", arg, rtw_hal_read_bbreg_22b_22b(padapter, arg, 0xffffffff));
		break;
	case 0x74: /* read_rf */
		RTW_INFO("read RF_reg path(0x%02x),offset(0x%x),value(0x%08x)\n", minor_cmd, arg, rtw_hal_read_rfreg_22b_22b_22b(padapter, minor_cmd, arg, 0xffffffff));
		break;
	case 0x75: /* write_rf */
		rtw_hal_write_rfreg_22b_22b_22b(padapter, minor_cmd, arg, 0xffffffff, extra_arg);
	...
	case 0x7F:
		...
		case 0x04:
			...
			RTW_INFO("oper_bw=%d\n", rtw_get_oper_bw_22b(padapter));
			RTW_INFO("oper_ch_offet=%d\n", rtw_get_oper_ch_22boffset(padapter));

			break;
		case 0x05:
			psta = rtw_get_stainfo_22b(pstapriv, cur_network->network.MacAddress);
			if (psta) {
				...
				sta_rx_reorder_ctl_dump_22b(RTW_DBGDUMP, psta);
			} else
		...
		case 0x08: {
			...
			RTW_INFO("rx_urb_pending_cn=%d\n", ATOMIC_READ_22b(&(precvpriv->rx_pending_cnt)));
#endif
		}
			break;
		case 0x09: {
		...
		case 0x0c: { /* dump rx/tx packet */
			if (arg == 0) {
				...
				rtw_hal_set_def_var_22b(padapter, HAL_DEF_DBG_DUMP_RXPKT, &(extra_arg));
			} else if (arg == 1) {
				RTW_INFO("dump tx packet (%d)\n", extra_arg);
				rtw_hal_set_def_var_22b(padapter, HAL_DEF_DBG_DUMP_TXPKT, &(extra_arg));
			}
		}
			break;
		case 0x0e: {
		...
		case 0x0f: {
			if (extra_arg == 0) {
				...
				rtw_hal_sreset_reset_22b(padapter);
			} else {
		...
		case 0x11: { /* dump linked status */
			...
			/* linked_info_dump_22b(padapter,extra_arg); */
		...
		case 0x1b: {
			...
			if (arg == 0) {
...
#ifdef CONFIG_80211N_HT
				pregistrypriv->ht_enable = (u8)rtw_ht_enable_22b;
#endif /* CONFIG_80211N_HT */
			} else if (arg == 1) {
				...
				if (max_rx_rate < 0xc) { /* max_rx_rate < MSC0->B or G -> disable HT */
					...
					set_mcs_rate_by_mask_22b(pmlmeext->default_supported_mcs_set, mcs_bitmap);
				}
#endif /* CONFIG_80211N_HT							 */
			}
		}
		...
		case 0x20:
			{
				if (arg == 0xAA) {
					...
					rtw_dump_rsvd_page_22b(RTW_DBGDUMP, padapter, page_offset, page_num);
				}
		...
		case 0xdd: { /* registers dump , 0 for mac reg,1 for bb reg, 2 for rf reg */
			if (extra_arg == 0)
				mac_reg_dump_22b(RTW_DBGDUMP, padapter);
			else if (extra_arg == 1)
				bb_reg_dump_22b(RTW_DBGDUMP, padapter);
			else if (extra_arg == 2)
				rf_reg_dump_22b(RTW_DBGDUMP, padapter);
			else if (extra_arg == 11)
				bb_reg_dump_22b_ex(RTW_DBGDUMP, padapter);
		}
			break;
...
static int rtw_set_encryption(struct net_device *dev, struct ieee_param *param, u32 param_len)
{
	...
	if (param->sta_addr[0] == 0xff && param->sta_addr[1] == 0xff &&
		...
		psta = rtw_get_stainfo_22b(pstapriv, param->sta_addr);
		if (!psta) {
			/* ret = -EINVAL; */
	...
	if (strcmp(param->u.crypt.alg, "WEP") == 0 && (psta == NULL)) {
		...
		if (wep_key_len > 0) {
			...
			_rtw_memset_22b(pwep, 0, wep_total_len);
		...
		_rtw_memcpy_22b(pwep->KeyMaterial,  param->u.crypt.key, pwep->KeyLength);
		...
		if (param->u.crypt.set_tx) {
			...
			_rtw_memcpy_22b(&(psecuritypriv->dot11DefKey[wep_key_idx].skey[0]), pwep->KeyMaterial, pwep->KeyLength);

			psecuritypriv->dot11DefKeylen[wep_key_idx] = pwep->KeyLength;

			rtw_ap_set_wep_key_22b(padapter, pwep->KeyMaterial, pwep->KeyLength, wep_key_idx, 1);
		} else {
			RTW_INFO("wep, set_tx=0\n");

			/* don't update "psecuritypriv->dot11PrivacyAlgrthm" and  */
			/* "psecuritypriv->dot11PrivacyKeyIndex=keyid", but can rtw_set_key_22b to cam */

			_rtw_memcpy_22b(&(psecuritypriv->dot11DefKey[wep_key_idx].skey[0]), pwep->KeyMaterial, pwep->KeyLength);

			psecuritypriv->dot11DefKeylen[wep_key_idx] = pwep->KeyLength;

			rtw_ap_set_wep_key_22b(padapter, pwep->KeyMaterial, pwep->KeyLength, wep_key_idx, 0);
		}
		...
		if (param->u.crypt.set_tx == 1) {
			if (strcmp(param->u.crypt.alg, "WEP") == 0) {
				RTW_INFO(FUNC_ADPT_FMT" set WEP TX GTK idx:%u, len:%u\n"
					, FUNC_ADPT_ARG(padapter), param->u.crypt.idx, param->u.crypt.key_len);
				_rtw_memcpy_22b(psecuritypriv->dot118021XGrpKey[param->u.crypt.idx].skey,  param->u.crypt.key, (param->u.crypt.key_len > 16 ? 16 : param->u.crypt.key_len));
				psecuritypriv->dot118021XGrpPrivacy = _WEP40_;
				...
				_rtw_memcpy_22b(psecuritypriv->dot118021XGrpKey[param->u.crypt.idx].skey,  param->u.crypt.key, (param->u.crypt.key_len > 16 ? 16 : param->u.crypt.key_len));
				/* set mic key */
				_rtw_memcpy_22b(psecuritypriv->dot118021XGrptxmickey[param->u.crypt.idx].skey, &(param->u.crypt.key[16]), 8);
				_rtw_memcpy_22b(psecuritypriv->dot118021XGrprxmickey[param->u.crypt.idx].skey, &(param->u.crypt.key[24]), 8);
				...
				RTW_INFO(FUNC_ADPT_FMT" set TX IGTK idx:%u, len:%u\n"
					, FUNC_ADPT_ARG(padapter), param->u.crypt.idx, param->u.crypt.key_len);
				_rtw_memcpy_22b(padapter->securitypriv.dot11wBIPKey[param->u.crypt.idx].skey, param->u.crypt.key, (param->u.crypt.key_len > 16 ? 16 : param->u.crypt.key_len));
				psecuritypriv->dot11wBIPKeyid = param->u.crypt.idx;
			...
			rtw_ap_set_group_key_22b(padapter, param->u.crypt.key, psecuritypriv->dot118021XGrpPrivacy, param->u.crypt.idx);
		}
	...
	if (psecuritypriv->dot11AuthAlgrthm == dot11AuthAlgrthm_8021X && psta) { /* psk/802_1x */
		if (check_fwstate(pmlmepriv, WIFI_AP_STATE)) {
			if (param->u.crypt.set_tx == 1) {
				...
				psta->ieee8021x_blocked = _FALSE;
				psta->bpairwise_key_installed = _TRUE;
				rtw_ap_set_pairwise_key_22b(padapter, psta);
...
static int rtw_set_beacon(struct net_device *dev, struct ieee_param *param, int len)
{
	...
	_rtw_memcpy_22b(&pstapriv->max_num_sta, param->u.bcn_ie.reserved, 2);

	if ((pstapriv->max_num_sta > NUM_STA) || (pstapriv->max_num_sta <= 0))
		pstapriv->max_num_sta = NUM_STA;


	if (rtw_check_beacon_data_22b(padapter, pbuf, (len - 12 - 2)) == _SUCCESS) /* 12 = param header, 2:no packed */
		ret = 0;
	else
		ret = -EINVAL;
...
static int rtw_hostapd_sta_flush(struct net_device *dev)
{
	...
	flush_all_cam_entry_22b(padapter);	/* clear CAM */
#ifdef CONFIG_AP_MODE
	ret = rtw_sta_flush_22b(padapter, _TRUE);
#endif
	return ret;
...
#endif
	/* psta = rtw_alloc_stainfo_22b(pstapriv, param->sta_addr); */
	psta = rtw_get_stainfo_22b(pstapriv, param->sta_addr);
	if (psta) {
		...
		_rtw_memcpy_22b(psta->bssrateset, param->u.add_sta.tx_supp_rates, 16);
		...
		if (WLAN_STA_HT & flags) {
			...
			psta->qos_option = 1;
			_rtw_memcpy_22b((void *)&psta->htpriv.ht_cap, (void *)&param->u.add_sta.ht_cap, sizeof(struct rtw_ieee80211_ht_cap));
		} else
		...
		update_sta_info_22b_apmode(padapter, psta);
...
static int rtw_del_sta(struct net_device *dev, struct ieee_param *param)
{
	...
	if (psta) {
		...
		if (rtw_is_list_empty_22b(&psta->asoc_list) == _FALSE) {
			...
			updated = ap_free_sta_22b(padapter, psta, _TRUE, WLAN_REASON_DEAUTH_LEAVING, _TRUE);
...
static int rtw_ioctl_22b_get_sta_data(struct net_device *dev, struct ieee_param *param, int len)
{
	int ret = 0;
	struct sta_info *psta = NULL;
	_adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
	...
	RTW_INFO("rtw_ioctl_22b_get_sta_info, sta_addr: " MAC_FMT "\n", MAC_ARG(param_ex->sta_addr));
	...
	psta = rtw_get_stainfo_22b(pstapriv, param_ex->sta_addr);
	if (psta) {
#if 0
		struct {
			u16 aid;
		...
		*/
		...
		_rtw_memcpy_22b(psta_data->tx_supp_rates, psta->bssrateset, psta->bssratelen);
#ifdef CONFIG_80211N_HT
		_rtw_memcpy_22b(&psta_data->ht_cap, &psta->htpriv.ht_cap, sizeof(struct rtw_ieee80211_ht_cap));
#endif /* CONFIG_80211N_HT */
...
static int rtw_get_sta_wpaie(struct net_device *dev, struct ieee_param *param)
{
	...
	if (psta) {
		if ((psta->wpa_ie[0] == WLAN_EID_RSN) || (psta->wpa_ie[0] == WLAN_EID_GENERIC)) {
			...
			_rtw_memcpy_22b(param->u.wpa_ie.reserved, psta->wpa_ie, copy_len);
		} else {
			/* ret = -1; */
...
static int rtw_set_wps_beacon(struct net_device *dev, struct ieee_param *param, int len)
{
	...
	if (ie_len > 0) {
		...
		_rtw_memcpy_22b(pmlmepriv->wps_beacon_ie, param->u.bcn_ie.buf, ie_len);
...
static int rtw_set_wps_probe_resp(struct net_device *dev, struct ieee_param *param, int len)
{
	...
	if (ie_len > 0) {
		...
		_rtw_memcpy_22b(pmlmepriv->wps_probe_resp_ie, param->u.bcn_ie.buf, ie_len);
	}
...
static int rtw_set_wps_assoc_resp(struct net_device *dev, struct ieee_param *param, int len)
{
	...
	if (ie_len > 0) {
		...
		_rtw_memcpy_22b(pmlmepriv->wps_assoc_resp_ie, param->u.bcn_ie.buf, ie_len);
	}
...
static int rtw_set_hidden_ssid(struct net_device *dev, struct ieee_param *param, int len)
{
	...
	ie_len = len - 12 - 2; /* 12 = param header, 2:no packed */
	ssid_ie = rtw_get_ie_22b(param->u.bcn_ie.buf,  WLAN_EID_SSID, &ssid_len, ie_len);

	if (ssid_ie && ssid_len > 0 && ssid_len <= NDIS_802_11_LENGTH_SSID) {
		WLAN_BSSID_EX *pbss_network = &mlmepriv->cur_network.network;
		WLAN_BSSID_EX *pbss_network_ext = &mlmeinfo->network;

		_rtw_memcpy_22b(ssid, ssid_ie + 2, ssid_len);
		ssid[ssid_len] = 0x0;
		...
		_rtw_memcpy_22b(pbss_network->Ssid.Ssid, (void *)ssid, ssid_len);
		pbss_network->Ssid.SsidLength = ssid_len;
		_rtw_memcpy_22b(pbss_network_ext->Ssid.Ssid, (void *)ssid, ssid_len);
...
static int rtw_ioctl_22b_acl_remove_sta(struct net_device *dev, struct ieee_param *param, int len)
{
	int ret = 0;
	_adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
	struct mlme_priv *pmlmepriv = &(padapter->mlmepriv);
	...
	ret = rtw_acl_remove_sta_22b(padapter, RTW_ACL_PERIOD_BSS, param->sta_addr);

	return ret;

}

static int rtw_ioctl_22b_acl_add_sta(struct net_device *dev, struct ieee_param *param, int len)
{
	int ret = 0;
	_adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
	struct mlme_priv *pmlmepriv = &(padapter->mlmepriv);
	...
	ret = rtw_acl_add_sta_22b(padapter, RTW_ACL_PERIOD_BSS, param->sta_addr);

	return ret;

}

static int rtw_ioctl_22b_set_macaddr_acl(struct net_device *dev, struct ieee_param *param, int len)
{
	int ret = 0;
	_adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
	struct mlme_priv *pmlmepriv = &(padapter->mlmepriv);
	...
	rtw_set_macaddr_acl_22b(padapter, RTW_ACL_PERIOD_BSS, param->u.mlme.command);
...
static int rtw_hostapd_ioctl(struct net_device *dev, struct iw_point *p)
{
	...
	*/
	...
	case RTL871X_HOSTAPD_GET_INFO_STA:
		...
		ret = rtw_ioctl_22b_get_sta_data(dev, param, p->length);

		break;

#if CONFIG_RTW_MACADDR_ACL
	case RTL871X_HOSTAPD_SET_MACADDR_ACL:
		ret = rtw_ioctl_22b_set_macaddr_acl(dev, param, p->length);
		break;
	case RTL871X_HOSTAPD_ACL_ADD_STA:
		ret = rtw_ioctl_22b_acl_add_sta(dev, param, p->length);
		break;
	case RTL871X_HOSTAPD_ACL_REMOVE_STA:
		ret = rtw_ioctl_22b_acl_remove_sta(dev, param, p->length);
		break;
#endif /* CONFIG_RTW_MACADDR_ACL */
			   ...
			   char *extra)
{
	...
	_rtw_memcpy_22b(ext_dbg, ext, len);
#endif
	...
	if (dwrq->flags == 0x8766 && len > 8) {
		...
		if ((_VENDOR_SPECIFIC_IE_ == probereq_wpsie[0]) &&
		    (_rtw_memcmp_22b(&probereq_wpsie[2], wps_oui, 4) == _TRUE)) {
			...
			_rtw_memcpy_22b(pmlmepriv->wps_probe_req_ie, probereq_wpsie, cp_sz);
	...
	if (len >= WEXT_CSCAN_HEADER_SIZE
		&& _rtw_memcmp_22b(ext, WEXT_CSCAN_HEADER, WEXT_CSCAN_HEADER_SIZE) == _TRUE
	) {
	...
	i = rtw_android_cmdstr_to_num_22b(ext);
	...
	case ANDROID_WIFI_CMD_LINKSPEED: {
		u16 mbps = rtw_get_cur_max_rate_22b(padapter) / 10;
	...
	case ANDROID_WIFI_CMD_SCAN_ACTIVE: {
		/* rtw_set_scan_mode_22b(padapter, SCAN_ACTIVE); */
		sprintf(ext, "OK");
	}
		break;
	case ANDROID_WIFI_CMD_SCAN_PASSIVE: {
		/* rtw_set_scan_mode_22b(padapter, SCAN_PASSIVE); */
		sprintf(ext, "OK");
	}
		break;
	...
	case ANDROID_WIFI_CMD_COUNTRY: {
		...
		rtw_set_country_22b(padapter, country_code);
		sprintf(ext, "OK");
	}
		break;
	default:
			   ...
			   union iwreq_data *wrqu, char *extra)
{
	...
	if (check_fwstate(pmlmepriv, _FW_UNDER_SURVEY))
		rtw_scan_abort_22b(padapter);

	if (_rtw_memcmp_22b(extra, "enable", 6))


		rtw_suspend_common_22b(padapter);

	else if (_rtw_memcmp_22b(extra, "disable", 7)) {
#ifdef CONFIG_USB_HCI
		RTW_ENABLE_FUNC(padapter, DF_RX_BIT);
		RTW_ENABLE_FUNC(padapter, DF_TX_BIT);
#endif
		rtw_resume_common_22b(padapter);
				  ...
				  union iwreq_data *wrqu, char *extra)
{
	...
	if (wrqu->data.length <= 0) {
		...
		if (strncmp(input, "pattern=", 8) == 0) {
			...
			rtw_hal_set_hwreg_22b(padapter,
		      ...
		      union iwreq_data *wrqu, char *extra)
{
	...
	if (_rtw_memcmp_22b(extra, "lps=", 4)) {
		sscanf(extra + 4, "%u", &mode);
		ret = rtw_pm_set_lps_22b(padapter, mode);
	} else if (_rtw_memcmp_22b(extra, "ips=", 4)) {
		sscanf(extra + 4, "%u", &mode);
		ret = rtw_pm_set_ips_22b(padapter, mode);
	} else if (_rtw_memcmp_22b(extra, "lps_level=", 10)) {
		if (sscanf(extra + 10, "%u", &mode) > 0)
			ret = rtw_pm_set_lps_22b_level(padapter, mode);
	} else
		ret = -EINVAL;
							   ...
							   char *extra, u32 length)
{
	...
	_rtw_memset_22b(extra, 0, length);
...
int rtw_vendor_ie_set(struct net_device *dev, struct iw_request_info *info, union iwreq_data *wrqu, char *extra)
{
	...
	if (totoal_ie_len > 0) {
		for (i = 0  ; i < strlen(extra) ; i += 2) {
			pmlmepriv->vendor_ie[vendor_ie_num][len] = key_2char2num_22b(extra[i] , extra[i + 1]);
			if (len == 0) {
...
_clear_path:
	_rtw_memset_22b(pmlmepriv->vendor_ie[vendor_ie_num] , 0 , sizeof(u32) * WLAN_MAX_VENDOR_IE_LEN);
	pmlmepriv->vendor_ielen[vendor_ie_num] = 0;
			    ...
			    union iwreq_data *wdata, char *extra)
{
	...
	*(extra + wrqu->length) = '\0';
	...
	if (strcmp(tmp[0], "status") == 0) {
		...
		EFUSE_GetEfuseDefinition_22b(padapter, EFUSE_WIFI, TYPE_EFUSE_MAP_LEN, (void *)&mapLen, _FALSE);
		...
		EFUSE_GetEfuseDefinition_22b(padapter, EFUSE_WIFI, TYPE_EFUSE_MAP_LEN , (void *)&mapLen, _FALSE);
		efuse = pEfuseHal->fakeEfuseInitMap_22b;
		if (rtw_efuse_mask_map_read_22b(padapter, 0, mapLen, efuse) == _FAIL) {
		...
		EFUSE_GetEfuseDefinition_22b(padapter, EFUSE_WIFI, TYPE_EFUSE_MAP_LEN , (PVOID)&max_available_len, _FALSE);
		...
		if (rtw_efuse_mask_map_read_22b(padapter, addr, cnts, data) == _FAIL) {
			RTW_INFO("%s: rtw_efuse_mask_map_read_22b error!\n", __func__);
			err = -EFAULT;
			goto exit;
		}
		...
		*extra = 0;
		...
		addr = 0;
		EFUSE_GetEfuseDefinition_22b(padapter, EFUSE_WIFI, TYPE_EFUSE_REAL_CONTENT_LEN , (PVOID)&mapLen, _FALSE);
		RTW_INFO("Real content len = %d\n",mapLen );

		if (rtw_efuse_access_22b(padapter, _FALSE, addr, mapLen, rawdata) == _FAIL) {
			RTW_INFO("%s: rtw_efuse_access_22b Fail!!\n", __func__);
			err = -EFAULT;
			goto exit;
		}

		_rtw_memset_22b(extra, '\0', strlen(extra));
		...
		addr = 0;
		EFUSE_GetEfuseDefinition_22b(padapter, EFUSE_BT, TYPE_EFUSE_REAL_CONTENT_LEN, (PVOID)&mapLen, _FALSE);
		RTW_INFO("Real content len = %d\n", mapLen);
#ifdef RTW_HALMAC
		...
		if (hal_efuse_macaddr_offset_22b(padapter) == -1) {
			err = -EFAULT;
			goto exit;
		}

		addr = hal_efuse_macaddr_offset_22b(padapter);
		cnts = 6;
		...
		*extra = 0;
		...
		if (rtw_efuse_mask_map_read_22b(padapter, addr, cnts, data) == _FAIL) {
			RTW_INFO("%s: rtw_efuse_access_22b error!!\n", __FUNCTION__);
			err = -EFAULT;
			goto exit;
		}
		...
		*extra = 0;
...
#else
		efuse_GetCurrentSize_22b(padapter, &raw_cursize);
		raw_maxsize = efuse_GetMaxSize_22b(padapter);
#endif
		sprintf(extra, "[available raw size]= %d bytes\n", raw_maxsize - raw_cursize);
	} else if (strcmp(tmp[0], "btableraw") == 0) {
		efuse_bt_GetCurrentSize_22b(padapter, &raw_cursize);
		raw_maxsize = efuse_bt_GetMaxSize_22b(padapter);
		sprintf(extra, "[available raw size]= %d bytes\n", raw_maxsize - raw_cursize);
	} else if (strcmp(tmp[0], "btfmap") == 0) {

		BTEfuse_PowerSwitch_22b_22b(padapter, 1, _TRUE);

		mapLen = EFUSE_BT_MAX_MAP_LEN;
		if (rtw_BT_efuse_map_read_22b(padapter, 0, mapLen, pEfuseHal->BTEfuseInitMap_22b) == _FAIL) {
			RTW_INFO("%s: rtw_BT_efuse_map_read_22b Fail!!\n", __FUNCTION__);
			err = -EFAULT;
			goto exit;
		}
		...
		for (i = 0; i < 512; i += 16) { /* set 512 because the iwpriv's extra size have limit 0x7FF */
			...
			for (j = 0; j < 8; j++) {
				/*				RTW_INFO("%02X ", pEfuseHal->BTEfuseInitMap_22b[i+j]); */
				pextra += sprintf(pextra, "%02X ", pEfuseHal->BTEfuseInitMap_22b[i+j]);
			}
			/*			RTW_INFO("\t"); */
			pextra += sprintf(pextra, "\t");
			for (; j < 16; j++) {
		...
		RTW_INFO("%s: cnts=%d\n", __FUNCTION__, cnts);
#ifndef RTW_HALMAC
		EFUSE_GetEfuseDefinition_22b(padapter, EFUSE_BT, TYPE_EFUSE_MAP_LEN, (PVOID)&max_available_len, _FALSE);
		if ((addr + cnts) > max_available_len) {
			RTW_INFO("%s: addr(0x%X)+cnts(%d) parameter error!\n", __FUNCTION__, addr, cnts);
			err = -EFAULT;
			goto exit;
...
#endif
		if (rtw_BT_efuse_map_read_22b(padapter, addr, cnts, data) == _FAIL) {
			RTW_INFO("%s: rtw_BT_efuse_map_read_22b error!!\n", __FUNCTION__);
			err = -EFAULT;
			goto exit;
		}
		...
		*extra = 0;
		...
		for (i = 0; i < 512; i += 16) {
			...
			pextra += sprintf(pextra, "0x%03x\t", i);
			for (j = 0; j < 8; j++) {
				/*				RTW_INFO("%02X ", pEfuseHal->fakeBTEfuseModifiedMap_22b_22b[i+j]); */
				pextra += sprintf(pextra, "%02X ", pEfuseHal->fakeBTEfuseModifiedMap_22b_22b[i+j]);
			}
			/*			RTW_INFO("\t"); */
			pextra += sprintf(pextra, "\t");
			for (; j < 16; j++) {
		...
		EFUSE_GetEfuseDefinition_22b(padapter, EFUSE_WIFI, TYPE_EFUSE_MAP_LEN , (void *)&mapLen, _FALSE);
		efuse = pEfuseHal->fakeEfuseModifiedMap_22b;
		...
		*extra = 0;
		pextra = extra;
		for (i = 0; i < cnts; i++) {
			RTW_INFO("wlrfkrmap = 0x%02x\n", pEfuseHal->fakeEfuseModifiedMap_22b[addr + i]);
			pextra += sprintf(pextra, "0x%02X ", pEfuseHal->fakeEfuseModifiedMap_22b[addr+i]);
		}
		...
		*extra = 0;
		pextra = extra;
		for (i = 0; i < cnts; i++) {
			RTW_INFO("wlrfkrmap = 0x%02x\n", pEfuseHal->fakeBTEfuseModifiedMap_22b_22b[addr + i]);
			pextra += sprintf(pextra, "0x%02X ", pEfuseHal->fakeBTEfuseModifiedMap_22b_22b[addr+i]);
		}
	} else if (strcmp(tmp[0], "mask") == 0) {
		*extra = 0;
		mask_len = sizeof(u8) * rtw_get_efuse_mask_arraylen_22b(padapter);
		rtw_efuse_mask_array_22b(padapter, mask_buf);

		if (padapter->registrypriv.bFileMaskEfuse == _TRUE)
			_rtw_memcpy_22b(mask_buf, maskfileBuffer_22b, mask_len);
...
#ifdef CONFIG_IPS
		rtw_pm_set_ips_22b(padapter, ips_mode);
#endif /* CONFIG_IPS */

#ifdef CONFIG_LPS
		rtw_pm_set_lps_22b(padapter, lps_mode);
#endif /* CONFIG_LPS */
	}
			    ...
			    union iwreq_data *wdata, char *extra)
{
	...
	*(extra + wrqu->length) = '\0';
	...
	EFUSE_GetEfuseDefinition_22b(padapter, EFUSE_WIFI, TYPE_EFUSE_MAP_LEN , (void *)&wifimaplen, _FALSE);
	...
	if (strcmp(tmp[0], "wmap") == 0) {
		...
		for (jj = 0, kk = 0; jj < cnts; jj++, kk += 2)
			setdata[jj] = key_2char2num_22b(tmp[2][kk], tmp[2][kk + 1]);
		...
		if (rtw_efuse_map_write_22b(padapter, addr, cnts, setdata) == _FAIL) {
			RTW_INFO("%s: rtw_efuse_map_write_22b error!!\n", __FUNCTION__);
			err = -EFAULT;
			goto exit;
		}
		*extra = 0;
		RTW_INFO("%s: after rtw_efuse_map_write_22b to _rtw_memcmp_22b\n", __func__);
		if (rtw_efuse_mask_map_read_22b(padapter, addr, cnts, ShadowMapWiFi) == _SUCCESS) {
			if (_rtw_memcmp_22b((void *)ShadowMapWiFi , (void *)setdata, cnts)) {
		...
		for (jj = 0, kk = 0; jj < cnts; jj++, kk += 2)
			setrawdata[jj] = key_2char2num_22b(tmp[2][kk], tmp[2][kk + 1]);

		if (rtw_efuse_access_22b(padapter, _TRUE, addr, cnts, setrawdata) == _FAIL) {
			RTW_INFO("%s: rtw_efuse_access_22b error!!\n", __FUNCTION__);
			err = -EFAULT;
			goto exit;
		}
		...
		for (jj = 0, kk = 0; jj < cnts; jj++, kk += 2)
			setdata[jj] = key_2char2num_22b(tmp[1][kk], tmp[1][kk + 1]);
		...
		if (wifimaplen > EFUSE_MAX_MAP_LEN)
			...
			cnts = wifimaplen;
		if (rtw_efuse_mask_map_read_22b(padapter, 0, cnts, pEfuseHal->fakeEfuseModifiedMap_22b) == _SUCCESS)
		...
		rtw_msleep_os_22b(10);
		rtw_read8(padapter, EFUSE_CTRL);
		BTEfuse_PowerSwitch_22b_22b(padapter, 1, _FALSE);
#endif /* RTW_HALMAC */
...
#endif
		if (rtw_BT_efuse_map_write_22b(padapter, addr, cnts, setdata) == _FAIL) {
			RTW_INFO("%s: rtw_BT_efuse_map_write_22b error!!\n", __FUNCTION__);
			err = -EFAULT;
			goto exit;
		}
		*extra = 0;
		RTW_INFO("%s: after rtw_BT_efuse_map_write_22b to _rtw_memcmp_22b\n", __FUNCTION__);
		if ((rtw_BT_efuse_map_read_22b(padapter, addr, cnts, ShadowMapBT) == _SUCCESS)) {
			if (_rtw_memcmp_22b((void *)ShadowMapBT , (void *)setdata, cnts)) {
		...
		for (jj = 0, kk = 0; jj < cnts; jj++, kk += 2)
			pEfuseHal->fakeBTEfuseModifiedMap_22b_22b[addr + jj] = key_2char2num_22b(tmp[2][kk], tmp[2][kk + 1]);
	} else if (strcmp(tmp[0], "btdumpfake") == 0) {
		if (rtw_BT_efuse_map_read_22b(padapter, 0, EFUSE_BT_MAX_MAP_LEN, pEfuseHal->fakeBTEfuseModifiedMap_22b_22b) == _SUCCESS)
		...
		BTEfuse_PowerSwitch_22b_22b(padapter, 1, _FALSE);
#endif /* RTW_HALMAC */
		_rtw_memcpy_22b(pEfuseHal->BTEfuseModifiedMap_22b, pEfuseHal->fakeBTEfuseModifiedMap_22b_22b, EFUSE_BT_MAX_MAP_LEN);

		if (rtw_BT_efuse_map_write_22b(padapter, 0x00, EFUSE_BT_MAX_MAP_LEN, pEfuseHal->fakeBTEfuseModifiedMap_22b_22b) == _FAIL) {
			RTW_INFO("%s: rtw_BT_efuse_map_write_22b error!\n", __FUNCTION__);
			err = -EFAULT;
			goto exit;
		}

		RTW_INFO("pEfuseHal->fakeBTEfuseModifiedMap_22b_22b OFFSET\tVALUE(hex)\n");
		for (i = 0; i < EFUSE_BT_MAX_MAP_LEN; i += 16) {
			printk("0x%02x\t", i);
			for (j = 0; j < 8; j++)
				printk("%02X ", pEfuseHal->fakeBTEfuseModifiedMap_22b_22b[i + j]);
			printk("\t");
...
#if 1
		err = -EFAULT;
		RTW_INFO("%s: rtw_BT_efuse_map_read_22b _rtw_memcmp_22b\n", __FUNCTION__);
		if ((rtw_BT_efuse_map_read_22b(padapter, 0x00, EFUSE_BT_MAX_MAP_LEN, pEfuseHal->fakeBTEfuseInitMap_22b_22b) == _SUCCESS)) {
			if (_rtw_memcmp_22b((void *)pEfuseHal->fakeBTEfuseModifiedMap_22b_22b, (void *)pEfuseHal->fakeBTEfuseInitMap_22b_22b, EFUSE_BT_MAX_MAP_LEN)) {
				sprintf(extra, "BT write map compare OK");
				...
				sprintf(extra, "BT write map compare FAIL");
				if (rtw_BT_efuse_map_write_22b(padapter, 0x00, EFUSE_BT_MAX_MAP_LEN, pEfuseHal->fakeBTEfuseModifiedMap_22b_22b) == _FAIL)
					RTW_INFO("%s: rtw_BT_efuse_map_write_22b compare error,retry = %d!\n", __FUNCTION__, i);

				if (rtw_BT_efuse_map_read_22b(padapter, EFUSE_BT, EFUSE_BT_MAX_MAP_LEN, pEfuseHal->fakeBTEfuseInitMap_22b_22b) == _SUCCESS) {
					RTW_INFO("pEfuseHal->fakeBTEfuseInitMap_22b_22b OFFSET\tVALUE(hex)\n");

					for (i = 0; i < EFUSE_BT_MAX_MAP_LEN; i += 16) {
						printk("0x%02x\t", i);
						for (j = 0; j < 8; j++)
							printk("%02X ", pEfuseHal->fakeBTEfuseInitMap_22b_22b[i + j]);
						printk("\t");
						for (; j < 16; j++)
							printk("%02X ", pEfuseHal->fakeBTEfuseInitMap_22b_22b[i + j]);
						printk("\n");
		...
		*extra = 0;
		...
		if (wifimaplen > EFUSE_MAX_MAP_LEN)
			...
			cnts = wifimaplen;
		if (rtw_efuse_map_write_22b(padapter, 0x00, cnts, pEfuseHal->fakeEfuseModifiedMap_22b) == _FAIL) {
			RTW_INFO("%s: rtw_efuse_map_write_22b fakeEfuseModifiedMap_22b error!\n", __FUNCTION__);
			err = -EFAULT;
			goto exit;
		}

		if (rtw_efuse_mask_map_read_22b(padapter, 0x00, wifimaplen, ShadowMapWiFi) == _SUCCESS) {
			if (_rtw_memcmp_22b((void *)ShadowMapWiFi , (void *)pEfuseHal->fakeEfuseModifiedMap_22b, cnts)) {
		...
		for (jj = 0, kk = 0; jj < cnts; jj++, kk += 2)
			pEfuseHal->fakeEfuseModifiedMap_22b[addr + jj] = key_2char2num_22b(tmp[2][kk], tmp[2][kk + 1]);
		_rtw_memset_22b(extra, '\0', strlen(extra));
		...
		for (jj = 0, kk = 0; jj < cnts; jj++, kk += 2)
			pEfuseHal->fakeEfuseModifiedMap_22b[addr + jj] = key_2char2num_22b(tmp[1][kk], tmp[1][kk + 1]);
		...
		rtw_hal_read_chip_info_22b(padapter);
		/* set mac addr*/
		rtw_macaddr_cfg_22b(adapter_mac_addr(padapter), get_hal_mac_addr(padapter));
		_rtw_memcpy_22b(padapter->pnetdev->dev_addr, get_hal_mac_addr(padapter), ETH_ALEN); /* set mac addr to net_device */
...
#else
		rtw_hal_set_hwreg_22b(padapter, HW_VAR_MAC_ADDR, adapter_mac_addr(padapter)); /* set mac addr to mac register */
#endif
		...
		rtw_efuse_analyze_22b(padapter, EFUSE_WIFI, 0);
		_rtw_memset_22b(extra, '\0', strlen(extra));
	...
	union iwreq_data *wrqu, char *extra)
{
	...
	_rtw_memset_22b(param, 0xFF, RTW_CUSTOMER_STR_LEN);
	...
	if (strcmp(pch, "read") == 0) {
		...
		do {
			...
			if ((pch == NULL) || (strlen(pch) == 0))
				break;
			if (strlen(pch) != 2
				|| IsHexDigit_22b(*pch) == _FALSE
				|| IsHexDigit_22b(*(pch + 1)) == _FALSE
			   ...
			   union iwreq_data *wdata, char *extra)
{
	...
	case MP_DISABLE_BT_COEXIST:
		...
		rtw_mp_disable_bt_coexist_22b(dev, info, wdata, extra);
		break;
	case MP_IQK:
		RTW_INFO("set MP_IQK\n");
		rtw_mp_iqk_22b(dev, info, wrqu, extra);
		break;
	case MP_LCK:
		RTW_INFO("set MP_LCK\n");
		rtw_mp_lck_22b(dev, info, wrqu, extra);
	break;
			   ...
			   union iwreq_data *wdata, char *extra)
{
	...
	case MP_START:
		...
		rtw_mp_start_22b(dev, info, wrqu, extra);
		break;
	case MP_STOP:
		RTW_INFO("set case mp_stop\n");
		rtw_mp_stop_22b(dev, info, wrqu, extra);
		break;
	case MP_BANDWIDTH:
		RTW_INFO("set case mp_bandwidth\n");
		rtw_mp_bandwidth_22b(dev, info, wrqu, extra);
		break;
	case MP_RESET_STATS:
		RTW_INFO("set case MP_RESET_STATS\n");
		rtw_mp_reset_stats_22b(dev, info, wrqu, extra);
		break;
	case MP_SetRFPathSwh:
		RTW_INFO("set MP_SetRFPathSwitch\n");
		rtw_mp_SetRFPath_22b(dev, info, wrqu, extra);
		break;
	case WRITE_REG:
		rtw_mp_write_reg_22b(dev, info, wrqu, extra);
		break;
	case WRITE_RF:
		rtw_mp_write_rf_22b(dev, info, wrqu, extra);
		break;
	case MP_PHYPARA:
		RTW_INFO("mp_get  MP_PHYPARA\n");
		rtw_mp_phypara_22b(dev, info, wrqu, extra);
		break;
	case MP_CHANNEL:
		RTW_INFO("set case mp_channel\n");
		rtw_mp_channel_22b(dev , info, wrqu, extra);
		break;
    	case  MP_CHL_OFFSET:
	...
	case READ_REG:
		...
		rtw_mp_read_reg_22b(dev, info, wrqu, extra);
		break;
	case READ_RF:
		RTW_INFO("mp_get  READ_RF\n");
		rtw_mp_read_rf_22b(dev, info, wrqu, extra);
		break;
	case MP_RATE:
		RTW_INFO("set case mp_rate\n");
		rtw_mp_rate_22b(dev, info, wrqu, extra);
		break;
	case MP_TXPOWER:
		RTW_INFO("set case MP_TXPOWER\n");
		rtw_mp_tx_22bpower(dev, info, wrqu, extra);
		break;
	case MP_ANT_TX:
		RTW_INFO("set case MP_ANT_TX\n");
		rtw_mp_ant_tx_22b(dev, info, wrqu, extra);
		break;
	case MP_ANT_RX:
		RTW_INFO("set case MP_ANT_RX\n");
		rtw_mp_ant_rx_22b(dev, info, wrqu, extra);
		break;
	case MP_QUERY:
		rtw_mp_trx_query_22b(dev, info, wrqu, extra);
		break;
	case MP_CTX:
		RTW_INFO("set case MP_CTX\n");
		rtw_mp_ctx_22b(dev, info, wrqu, extra);
		break;
	case MP_ARX:
		RTW_INFO("set case MP_ARX\n");
		rtw_mp_arx_22b(dev, info, wrqu, extra);
		break;
	case MP_DUMP:
		RTW_INFO("set case MP_DUMP\n");
		rtw_mp_dump_22b(dev, info, wrqu, extra);
		break;
	case MP_PSD:
		RTW_INFO("set case MP_PSD\n");
		rtw_mp_psd_22b(dev, info, wrqu, extra);
		break;
	case MP_THER:
		RTW_INFO("set case MP_THER\n");
		rtw_mp_thermal_22b(dev, info, wrqu, extra);
		break;
	case MP_PwrCtlDM:
		RTW_INFO("set MP_PwrCtlDM\n");
		rtw_mp_PwrCtlDM_22b(dev, info, wrqu, extra);
		break;
	case MP_QueryDrvStats:
		RTW_INFO("mp_get MP_QueryDrvStats\n");
		rtw_mp_QueryDrv_22b(dev, info, wdata, extra);
		break;
	case MP_PWRTRK:
		RTW_INFO("set case MP_PWRTRK\n");
		rtw_mp_pwrtrk_22b(dev, info, wrqu, extra);
		break;
#ifdef CONFIG_MP_INCLUDED
	case EFUSE_SET:
	...
	case MP_GET_TXPOWER_INX:
		...
		rtw_mp_tx_22bpower_index(dev, info, wrqu, extra);
		break;
	case MP_GETVER:
		RTW_INFO("mp_get MP_GETVER\n");
		rtw_mp_getver_22b(dev, info, wdata, extra);
		break;
	case MP_MON:
		RTW_INFO("mp_get MP_MON\n");
		rtw_mp_mon_22b(dev, info, wdata, extra);
		break;
	case EFUSE_MASK:
		RTW_INFO("mp_get EFUSE_MASK\n");
		rtw_efuse_mask_file_22b(dev, info, wdata, extra);
		break;
	case  EFUSE_FILE:
		RTW_INFO("mp_get EFUSE_FILE\n");
		rtw_efuse_file_map_22b(dev, info, wdata, extra);
		break;
	case  MP_TX:
		RTW_INFO("mp_get MP_TX\n");
		rtw_mp_tx_22b(dev, info, wdata, extra);
		break;
	case  MP_RX:
		RTW_INFO("mp_get MP_RX\n");
		rtw_mp_rx_22b(dev, info, wdata, extra);
		break;
	case MP_HW_TX_MODE:
		RTW_INFO("mp_get MP_HW_TX_MODE\n");
		rtw_mp_hwtx_22b(dev, info, wdata, extra);
		break;
#ifdef CONFIG_RTW_CUSTOMER_STR
	...
	case MP_PWRLMT:
		...
		rtw_mp_pwrlmt_22b(dev, info, wdata, extra);
		break;
	case MP_PWRBYRATE:
		RTW_INFO("mp_get MP_SETPWRBYRATE\n");
		rtw_mp_pwrbyrate_22b(dev, info, wdata, extra);
		break;
	case  BT_EFUSE_FILE:
		RTW_INFO("mp_get BT EFUSE_FILE\n");
		rtw_bt_efuse_file_map_22b(dev, info, wdata, extra);
		break;
	case  MP_SWRFPath:
			   ...
			   , char *extra)
{
	...
	_rtw_memset_22b(extra, 0, wrqu->length);
			...
			union iwreq_data *wdata, char *extra)
{
	...
	rtw_msleep_os_22b(10); /* delay 5ms for sending pkt before exit adb shell operation */
	return 0;
}
				  ...
				  union iwreq_data *wrqu, char *extra)
{
	...
	if (extra[0] == '0')
		rtw_tdls_wfd_enable_22b(padapter, 0);
	else
		rtw_tdls_wfd_enable_22b(padapter, 1);
			  ...
			  union iwreq_data *wrqu, char *extra)
{
	...
	for (i = 0, j = 0 ; i < ETH_ALEN; i++, j += 3)
		txmgmt.peer[i] = key_2char2num_22b(*(extra + j), *(extra + j + 1));
			     ...
			     union iwreq_data *wrqu, char *extra)
{
	...
	ptdls_sta = rtw_get_stainfo_22b(&(padapter->stapriv), txmgmt.peer);
			      ...
			      union iwreq_data *wrqu, char *extra)
{
	...
	for (i = 0, j = 0 ; i < ETH_ALEN; i++, j += 3)
		pchsw_info->addr[i] = key_2char2num_22b(*(extra + j), *(extra + j + 1));

	ptdls_sta = rtw_get_stainfo_22b(&padapter->stapriv, pchsw_info->addr);
	if (ptdls_sta == NULL)
		return ret;
	...
	rtw_hal_get_hwreg_22b(padapter, HW_VAR_CH_SW_NEED_TO_TAKE_CARE_IQK_INFO, &take_care_iqk);
	if (take_care_iqk == _TRUE) {
		u8 central_chnl;
		u8 bw_mode;

		bw_mode = (pchsw_info->ch_offset) ? CHANNEL_WIDTH_40 : CHANNEL_WIDTH_20;
		central_chnl = rtw_get_center_ch_22b(pchsw_info->off_ch_num, bw_mode, pchsw_info->ch_offset);
		if (rtw_hal_ch_sw_iqk_info_search_22b(padapter, central_chnl, bw_mode) >= 0)
			rtw_tdls_cmd_22b(padapter, ptdls_sta->cmn.mac_addr, TDLS_CH_SW_START);
		else
			rtw_tdls_cmd_22b(padapter, ptdls_sta->cmn.mac_addr, TDLS_CH_SW_PREPARE);
	} else
		rtw_tdls_cmd_22b(padapter, ptdls_sta->cmn.mac_addr, TDLS_CH_SW_START);
				  ...
				  union iwreq_data *wrqu, char *extra)
{
	...
	if (wrqu->data.length >= 17) {
		for (i = 0, j = 0 ; i < ETH_ALEN; i++, j += 3)
			mac_addr[i] = key_2char2num_22b(*(extra + j), *(extra + j + 1));
		ptdls_sta = rtw_get_stainfo_22b(&padapter->stapriv, mac_addr);
	}
	...
	rtw_pm_set_lps_22b(padapter, PS_MODE_MAX);
#endif /* CONFIG_TDLS_CH_SW */
#endif /* CONFIG_TDLS */
			    ...
			    union iwreq_data *wrqu, char *extra)
{
	...
	extra[wrqu->data.length] = 0x00;
	ptdlsinfo->chsw_info.dump_stack = rtw_atoi_22b(extra);
			       ...
			       union iwreq_data *wrqu, char *extra)
{
	...
	extra[wrqu->data.length] = 0x00;
	ptdlsinfo->chsw_info.off_ch_num = rtw_atoi_22b(extra);
			 ...
			 union iwreq_data *wrqu, char *extra)
{
	...
	issue_nulldata_22b_to_TDLS_peer_STA(padapter, ptdls_sta->cmn.mac_addr, 1, 3, 500);
			  ...
			  union iwreq_data *wrqu, char *extra)
{
	...
	if (ptdls_sta)
		issue_nulldata_22b_to_TDLS_peer_STA(padapter, ptdls_sta->cmn.mac_addr, 0, 3, 500);
	...
	while (i < 4) {
		for (j = 0; j < 4; j++) {
			if (*(extra + j + tag) == '.' || *(extra + j + tag) == '\0') {
				if (j == 1)
					pwfd_info->ip_address[i] = convert_ip_addr_22b('0', '0', *(extra + (j - 1) + tag));
				if (j == 2)
					pwfd_info->ip_address[i] = convert_ip_addr_22b('0', *(extra + (j - 2) + tag), *(extra + (j - 1) + tag));
				if (j == 3)
					pwfd_info->ip_address[i] = convert_ip_addr_22b(*(extra + (j - 3) + tag), *(extra + (j - 2) + tag), *(extra + (j - 1) + tag));
			       ...
			       union iwreq_data *wrqu, char *extra)
{
	...
	sprintf(extra, "\nlink_established:%d\n"
		...
		ATOMIC_READ_22b(&padapter->tdlsinfo.chsw_info.chsw_on),
			   ...
			   union iwreq_data *wrqu, char *extra)
{
	...
	for (i = 0, j = 0 ; i < ETH_ALEN; i++, j += 3)
		addr[i] = key_2char2num_22b(*(charmac + j), *(charmac + j + 1));

	RTW_INFO("[%s] %d, charmac:%s, addr:"MAC_FMT"\n",
		 __FUNCTION__, __LINE__, charmac, MAC_ARG(addr));
	ptdls_sta = rtw_get_stainfo_22b(&padapter->stapriv, addr);
	if (ptdls_sta) {
		    ...
		    union iwreq_data *wrqu, char *extra)
{
	...
	if (hal_chk_wl_func_22b(padapter, WL_FUNC_TDLS) == _FALSE) {
	...
	if (hal_chk_wl_func_22b(padapter, WL_FUNC_MIRACAST)) {
		if (_rtw_memcmp_22b(extra, "wfdenable=", 10)) {
			wrqu->data.length -= 10;
	...
	if (_rtw_memcmp_22b(extra, "weaksec=", 8)) {
		wrqu->data.length -= 8;
		rtw_tdls_weaksec(dev, info, wrqu, &extra[8]);
		return ret;
	} else if (_rtw_memcmp_22b(extra, "tdlsenable=", 11)) {
		wrqu->data.length -= 11;
		rtw_tdls_enable(dev, info, wrqu, &extra[11]);
		return ret;
	}

	if (_rtw_memcmp_22b(extra, "setup=", 6)) {
		wrqu->data.length -= 6;
		rtw_tdls_setup(dev, info, wrqu, &extra[6]);
	} else if (_rtw_memcmp_22b(extra, "tear=", 5)) {
		wrqu->data.length -= 5;
		rtw_tdls_teardown(dev, info, wrqu, &extra[5]);
	} else if (_rtw_memcmp_22b(extra, "dis=", 4)) {
		wrqu->data.length -= 4;
		rtw_tdls_discovery(dev, info, wrqu, &extra[4]);
	} else if (_rtw_memcmp_22b(extra, "swoff=", 6)) {
		wrqu->data.length -= 6;
		rtw_tdls_ch_switch_off(dev, info, wrqu, &extra[6]);
	} else if (_rtw_memcmp_22b(extra, "sw=", 3)) {
		wrqu->data.length -= 3;
		rtw_tdls_ch_switch(dev, info, wrqu, &extra[3]);
	} else if (_rtw_memcmp_22b(extra, "dumpstack=", 10)) {
		wrqu->data.length -= 10;
		rtw_tdls_dump_ch(dev, info, wrqu, &extra[10]);
	} else if (_rtw_memcmp_22b(extra, "offchnum=", 9)) {
		wrqu->data.length -= 9;
		rtw_tdls_off_ch_num(dev, info, wrqu, &extra[9]);
	} else if (_rtw_memcmp_22b(extra, "choffset=", 9)) {
		wrqu->data.length -= 9;
		rtw_tdls_ch_offset(dev, info, wrqu, &extra[9]);
	} else if (_rtw_memcmp_22b(extra, "pson=", 5)) {
		wrqu->data.length -= 5;
		rtw_tdls_pson(dev, info, wrqu, &extra[5]);
	} else if (_rtw_memcmp_22b(extra, "psoff=", 6)) {
		wrqu->data.length -= 6;
...
#ifdef CONFIG_WFD
	if (hal_chk_wl_func_22b(padapter, WL_FUNC_MIRACAST)) {
		if (_rtw_memcmp_22b(extra, "setip=", 6)) {
			wrqu->data.length -= 6;
			rtw_tdls_setip(dev, info, wrqu, &extra[6]);
		} else if (_rtw_memcmp_22b(extra, "tprobe=", 6))
			...
			union iwreq_data *wrqu, char *extra)
{
	...
	if (_rtw_memcmp_22b(wrqu->data.pointer, "ip", 2))
		rtw_tdls_getip(dev, info, wrqu, extra);
	else if (_rtw_memcmp_22b(wrqu->data.pointer, "port", 4))
		rtw_tdls_getport(dev, info, wrqu, extra);
	/* WFDTDLS, for sigma test */
	else if (_rtw_memcmp_22b(wrqu->data.pointer, "dis", 3))
		rtw_tdls_dis_result(dev, info, wrqu, extra);
	else if (_rtw_memcmp_22b(wrqu->data.pointer, "status", 6))
		rtw_wfd_tdls_status(dev, info, wrqu, extra);
	else if (_rtw_memcmp_22b(wrqu->data.pointer, "tdls_sta=", 9))
		rtw_tdls_getsta(dev, info, wrqu, extra);
	else if (_rtw_memcmp_22b(wrqu->data.pointer, "best_ch", 7))
				      ...
				      union iwreq_data *wrqu, char *extra)
{
	...
	if (pbuf) {
		if (copy_from_user(pbuf, wrqu->data.pointer, wrqu->data.length))
			ret = -EFAULT;
		/* _rtw_memcpy_22b(pbuf, wrqu->data.pointer, wrqu->data.length); */
...
static s32 initLoopback(PADAPTER padapter)
{
	...
	if (padapter->ploopback == NULL) {
		...
		_rtw_init_sema_22b(&ploopback->sema, 0);
		ploopback->bstop = _TRUE;
		ploopback->cnt = 0;
		ploopback->size = 300;
		_rtw_memset_22b(ploopback->msg, 0, sizeof(ploopback->msg));
...
static s32 initpseudoadhoc(PADAPTER padapter)
{
	...
	networkType = Ndis802_11IBSS;
	err = rtw_set_802_11_infrastructure_mode_22b(padapter, networkType);
	if (err == _FALSE)
		return _FAIL;

	err = rtw_setopmode_cmd_22b(padapter, networkType, RTW_CMDF_WAIT_ACK);
	if (err == _FAIL)
		return _FAIL;
...
static s32 createpseudoadhoc(PADAPTER padapter)
{
	...
	authmode = Ndis802_11AuthModeOpen;
	err = rtw_set_802_11_authentication_mode_22b(padapter, authmode);
	if (err == _FALSE)
		return _FAIL;

	passoc_ssid = &pmlmepriv->assoc_ssid;
	_rtw_memset_22b(passoc_ssid, 0, sizeof(NDIS_802_11_SSID));
	passoc_ssid->SsidLength = sizeof(ssid) - 1;
	_rtw_memcpy_22b(passoc_ssid->Ssid, ssid, passoc_ssid->SsidLength);

	pdev_network = &padapter->registrypriv.dev_network;
	pibss = padapter->registrypriv.dev_network.MacAddress;
	_rtw_memcpy_22b(&pdev_network->Ssid, passoc_ssid, sizeof(NDIS_802_11_SSID));

	rtw_update_registrypriv_dev_network_22b(padapter);
	rtw_generate_random_ibss_22b(pibss);
...
#if 0
	err = rtw_create_ibss_cmd_22b(padapter, 0);
	if (err == _FAIL)
		return _FAIL;
#else
	{
		...
		psta = rtw_get_stainfo_22b(&padapter->stapriv, pcur_network->network.MacAddress);
		if (psta)
			rtw_free_stainfo_22b(padapter, psta);

		psta = rtw_alloc_stainfo_22b(&padapter->stapriv, pibss);
		if (psta == NULL)
			return _FAIL;
		...
		_rtw_memcpy_22b(&pcur_network->network, pdev_network, get_WLAN_BSSID_EX_sz(pdev_network));
...
static struct xmit_frame *createloopbackpkt(PADAPTER padapter, u32 size)
{
	...
	pframe = rtw_alloc_xmitframe_22b(pxmitpriv);
	if (pframe == NULL)
		return NULL;
	...
	pxmitbuf = rtw_alloc_xmitbuf_22b(pxmitpriv);
	_exit_critical_bh(&pxmitpriv->lock, &irqL);
	if (pxmitbuf == NULL) {
		rtw_free_xmitframe_22b(pxmitpriv, pframe);
		return NULL;
	}
	...
	_rtw_memset_22b(pattrib, 0, sizeof(struct pkt_attrib));

	pattrib->ether_type = 0x8723;
	_rtw_memcpy_22b(pattrib->src, adapter_mac_addr(padapter), ETH_ALEN);
	_rtw_memcpy_22b(pattrib->ta, pattrib->src, ETH_ALEN);
	_rtw_memset_22b(pattrib->dst, 0xFF, ETH_ALEN);
	_rtw_memcpy_22b(pattrib->ra, pattrib->dst, ETH_ALEN);
	...
	if (bmcast)
		pattrib->psta = rtw_get_bcmc_stainfo_22b(padapter);
	else
		pattrib->psta = rtw_get_stainfo_22b(&padapter->stapriv, get_bssid(&padapter->mlmepriv));
	...
	_rtw_memset_22b(desc, 0, TXDESC_SIZE);
 ...
 * ((PTXDESC)desc)->hwseq_sel = 0;  */ /* HWSEQ_SEL */
	...
	_rtw_memcpy_22b(hdr->addr1, pattrib->dst, ETH_ALEN); /* DA */
	_rtw_memcpy_22b(hdr->addr2, pattrib->src, ETH_ALEN); /* SA */
	_rtw_memcpy_22b(hdr->addr3, get_bssid(&padapter->mlmepriv), ETH_ALEN); /* RA, BSSID */
...
static void freeloopbackpkt(PADAPTER padapter, struct xmit_frame *pframe)
{
	...
	rtw_free_xmitframe_22b(pxmitpriv, pframe);
	rtw_free_xmitbuf_22b(pxmitpriv, pxmitbuf);
}
...
#if 0
		val = 0;
		_rtw_memcpy_22b(&val, pbuf + i, len - i);
		printk("%8X", val);
#else
		u8 str[9];
		u8 n;
...
static u8 pktcmp(PADAPTER padapter, u8 *txbuf, u32 txsz, u8 *rxbuf, u32 rxsz)
{
	...
	if (rtw_hal_rcr_check_22b(padapter, RCR_APPFCS))
	...
	if ((txsz - TXDESC_SIZE) != (rxpktsize - fcssize)) {
		...
		ret = _rtw_memcmp_22b(txbuf + TXDESC_SIZE, \
...
thread_return lbk_thread(thread_context context)
{
	...
	do {
		...
		_rtw_memcpy_22b(ploopback->txbuf, pxmitframe->buf_addr, ploopback->txsize);
		ff_hwaddr = rtw_get_ff_hwaddr_22b(pxmitframe);
		cnt++;
		...
		/* wait for rx pkt */
		_rtw_down_sema_22b(&ploopback->sema);
		...
		ploopback->txsize = 0;
		_rtw_memset_22b(ploopback->txbuf, 0, 0x8000);
		ploopback->rxsize = 0;
		_rtw_memset_22b(ploopback->rxbuf, 0, 0x8000);
...
static void loopbackTest(PADAPTER padapter, u32 cnt, u32 size, u8 *pmsg)
{
	...
	if (ploopback) {
		if (ploopback->bstop == _FALSE) {
			ploopback->bstop = _TRUE;
			_rtw_up_sema_22b(&ploopback->sema);
		}
		len = 0;
		do {
		...
		_rtw_memcpy_22b(pmsg, ploopback->msg, len + 1);
	...
	union iwreq_data *wrqu, char *extra)
{
	...
	if (strcmp(pch, "h2c") == 0) {
		...
		ret = rtw_test_h2c_cmd_22b(padapter, param, count);
...
static struct iw_statistics *rtw_get_wireless_stats(struct net_device *dev)
{
	...
	if (check_fwstate(&padapter->mlmepriv, _FW_LINKED) != _TRUE) {
...
#else
		{
			...
			tmp_level = (u8)phydm_signal_scale_mapping_22b(&pHal->odmpriv, padapter->recvpriv.signal_strength);
		}
#endif
#endif
		...
		if (IS_NM_ENABLE(padapter)) {
			...
			#ifndef CONFIG_SIGNAL_DISPLAY_DBM
			tmp_noise = translate_dbm_to_percentage_22b(tmp_noise);/*percentage*/
			#endif
		}
		#endif
...
struct iw_handler_def rtw_handlers_def_22b = {
...
static int _rtw_ioctl_22b_wext_private(struct net_device *dev, union iwreq_data *wrq_data)
{
	int err = 0;
	u8 *input = NULL;
	u32 input_len = 0;
	...
	_rtw_memcpy_22b(&wdata, wrq_data, sizeof(wdata));
	...
	if ((priv_args[k].set_args & IW_PRIV_TYPE_MASK) &&
		...
		case IW_PRIV_TYPE_CHAR:
			if (len > 0) {
				...
				/* Fetch string */
				_rtw_memcpy_22b(buffer, ptr, wdata.data.length);
			} else {
				wdata.data.length = 1;
				buffer[0] = '\0';
			}
	...
	* will have to handle the data */
	if ((priv_args[k].set_args & IW_PRIV_SIZE_FIXED) &&
		...
		if (offset)
			wdata.mode = subcmd;
		_rtw_memcpy_22b(wdata.name + offset, buffer, IFNAMSIZ - offset);
	} else {
	...
	if ((priv_args[k].get_args & IW_PRIV_TYPE_MASK) &&
		...
		case IW_PRIV_TYPE_BYTE:
			...
			for (j = 0; j < n; j++) {
				...
				_rtw_memcpy_22b(output + output_len, str, len);
			}
			break;
		...
		case IW_PRIV_TYPE_CHAR:
			/* Display args */
			_rtw_memcpy_22b(output, extra, n);
			break;
...
#ifdef CONFIG_COMPAT
static int rtw_ioctl_22b_compat_wext_private(struct net_device *dev, struct ifreq *rq)
{
	...
	err = _rtw_ioctl_22b_wext_private(dev, &wrq_data);
...
static int rtw_ioctl_22b_standard_wext_private(struct net_device *dev, struct ifreq *rq)
{
	struct iw_point *iwp;
	struct ifreq ifrq;
	union iwreq_data wrq_data;
...
static int rtw_ioctl_22b_wext_private(struct net_device *dev, struct ifreq *rq)
{
#ifdef CONFIG_COMPAT
...
#endif
		return rtw_ioctl_22b_compat_wext_private(dev, rq);
	else
#endif /* CONFIG_COMPAT */
		return rtw_ioctl_22b_standard_wext_private(dev, rq);
}
...
int rtw_ioctl_22b(struct net_device *dev, struct ifreq *rq, int cmd)
{
	...
	case SIOCDEVPRIVATE:
		ret = rtw_ioctl_22b_wext_private(dev, rq);
		break;
	case (SIOCDEVPRIVATE+1):
		ret = rtw_android_priv_cmd_22b(dev, rq, cmd);
		break;
	default:
		ret = -EOPNOTSUPP;
		break;
