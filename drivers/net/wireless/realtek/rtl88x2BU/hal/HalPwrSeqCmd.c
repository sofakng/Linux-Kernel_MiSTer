	...
	do {
		...
		if ((GET_PWR_CFG_FAB_MASK(PwrCfgCmd) & FabVersion) &&
			...
			case PWR_CMD_POLLING:
				...
				rtw_hal_get_hwreg_22b(padapter, HW_VAR_PWR_CMD, &bHWICSupport);
			...
			case PWR_CMD_DELAY:
				if (GET_PWR_CFG_VALUE(PwrCfgCmd) == PWRSEQ_DELAY_US)
					rtw_udelay_os_22b(GET_PWR_CFG_OFFSET(PwrCfgCmd));
				else
					rtw_udelay_os_22b(GET_PWR_CFG_OFFSET(PwrCfgCmd) * 1000);
				break;
