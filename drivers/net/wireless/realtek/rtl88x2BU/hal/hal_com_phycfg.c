...
const char *const _pg_txpwr_src_str_22b[] = {
	"PG_DATA",
	"IC_DEF",
	"DEF",
	"UNKNOWN"
...
#define pg_txpwr_src_str(src) (((src) >= PG_TXPWR_SRC_NUM) ? _pg_txpwr_src_str_22b[PG_TXPWR_SRC_NUM] : _pg_txpwr_src_str_22b[(src)])
...
const struct map_t pg_txpwr_def_info_22b =
	MAP_ENT(0xB8, 1, 0xFF
...
static const struct map_t rtl8188e_pg_txpwr_def_info_22b =
	MAP_ENT(0xB8, 1, 0xFF
...
static const struct map_t rtl8188f_pg_txpwr_def_info_22b =
	MAP_ENT(0xB8, 1, 0xFF
		, MAPSEG_ARRAY_ENT(0x10, 12,
...
static const struct map_t rtl8723b_pg_txpwr_def_info_22b =
	MAP_ENT(0xB8, 2, 0xFF
		, MAPSEG_ARRAY_ENT(0x10, 12,
...
static const struct map_t rtl8703b_pg_txpwr_def_info_22b =
	MAP_ENT(0xB8, 1, 0xFF
		, MAPSEG_ARRAY_ENT(0x10, 12,
...
static const struct map_t rtl8723d_pg_txpwr_def_info_22b =
	MAP_ENT(0xB8, 2, 0xFF
		, MAPSEG_ARRAY_ENT(0x10, 12,
			0x2D, 0x2D, 0x2D, 0x2D, 0x2D, 0x2D, 0x2D, 0x2D, 0x2D, 0x2D, 0x2D, 0x02)
		, MAPSEG_ARRAY_ENT(0x3A, 12,
...
static const struct map_t rtl8192e_pg_txpwr_def_info_22b =
	MAP_ENT(0xB8, 2, 0xFF
...
static const struct map_t rtl8821a_pg_txpwr_def_info_22b =
	MAP_ENT(0xB8, 1, 0xFF
...
static const struct map_t rtl8821c_pg_txpwr_def_info_22b =
	MAP_ENT(0xB8, 1, 0xFF
...
static const struct map_t rtl8812a_pg_txpwr_def_info_22b =
	MAP_ENT(0xB8, 1, 0xFF
...
static const struct map_t rtl8822b_pg_txpwr_def_info_22b =
	MAP_ENT(0xB8, 1, 0xFF
		, MAPSEG_ARRAY_ENT(0x10, 82,
			0x2D, 0x2D, 0x2D, 0x2D, 0x2D, 0x2D, 0x2D, 0x2D, 0x2D, 0x2D, 0x2D, 0x02, 0xEE, 0xEE, 0xFF, 0xFF,
			0xFF, 0xFF, 0x2A, 0x2A, 0x2A, 0x2A, 0x2A, 0x2A, 0x2A, 0x2A, 0x2A, 0x2A, 0x2A, 0x2A, 0x2A, 0x2A,
...
static const struct map_t rtl8814a_pg_txpwr_def_info_22b =
	MAP_ENT(0xB8, 1, 0xFF
		, MAPSEG_ARRAY_ENT(0x10, 168,
...
const struct map_t *hal_pg_txpwr_def_info_22b_22b(_adapter *adapter)
{
	u8 interface_type = 0;
...
#ifdef CONFIG_RTL8723B
	case RTL8723B:
		map = &rtl8723b_pg_txpwr_def_info_22b;
		break;
#endif
#ifdef CONFIG_RTL8703B
	case RTL8703B:
		map = &rtl8703b_pg_txpwr_def_info_22b;
		break;
#endif
#ifdef CONFIG_RTL8723D
	case RTL8723D:
		map = &rtl8723d_pg_txpwr_def_info_22b;
		break;
#endif
#ifdef CONFIG_RTL8188E
	case RTL8188E:
		map = &rtl8188e_pg_txpwr_def_info_22b;
		break;
#endif
#ifdef CONFIG_RTL8188F
	case RTL8188F:
		map = &rtl8188f_pg_txpwr_def_info_22b;
		break;
#endif
#ifdef CONFIG_RTL8812A
	case RTL8812:
		map = &rtl8812a_pg_txpwr_def_info_22b;
		break;
#endif
#ifdef CONFIG_RTL8821A
	case RTL8821:
		map = &rtl8821a_pg_txpwr_def_info_22b;
		break;
#endif
#ifdef CONFIG_RTL8192E
	case RTL8192E:
		map = &rtl8192e_pg_txpwr_def_info_22b;
		break;
#endif
#ifdef CONFIG_RTL8814A
	case RTL8814A:
		map = &rtl8814a_pg_txpwr_def_info_22b;
		break;
#endif
#ifdef CONFIG_RTL8822B
	case RTL8822B:
		map = &rtl8822b_pg_txpwr_def_info_22b;
		break;
#endif
#ifdef CONFIG_RTL8821C
	case RTL8821C:
		map = &rtl8821c_pg_txpwr_def_info_22b;
		break;
#endif
	}
...
static u8 hal_chk_pg_txpwr_info_2g(_adapter *adapter, TxPowerInfo24G *pwr_info)
{
	...
	if (pwr_info == NULL || !hal_chk_band_cap_22b(adapter, BAND_CAP_2G))
		return _SUCCESS;
...
static u8 hal_chk_pg_txpwr_info_5g(_adapter *adapter, TxPowerInfo5G *pwr_info)
{
	...
	if (pwr_info == NULL || !hal_chk_band_cap_22b(adapter, BAND_CAP_5G))
		return _SUCCESS;
...
static inline void hal_init_pg_txpwr_info_2g(_adapter *adapter, TxPowerInfo24G *pwr_info)
{
	...
	_rtw_memset_22b(pwr_info, 0, sizeof(TxPowerInfo24G));
...
static inline void hal_init_pg_txpwr_info_5g(_adapter *adapter, TxPowerInfo5G *pwr_info)
{
	...
	_rtw_memset_22b(pwr_info, 0, sizeof(TxPowerInfo5G));
...
u16 hal_load_pg_txpwr_info_22b_path_2g_22b(
	_adapter *adapter,
	...
	u16 pg_offset)
{
	...
	for (group = 0; group < MAX_CHNL_GROUP_24G; group++) {
		if (HAL_SPEC_CHK_RF_PATH_2G(hal_spec, path)) {
			tmp_base = map_read8_22b(txpwr_map, offset);
	...
	for (tx_idx = 0; tx_idx < MAX_TX_COUNT; tx_idx++) {
		if (tx_idx == 0) {
			if (HAL_SPEC_CHK_RF_PATH_2G(hal_spec, path) && HAL_SPEC_CHK_TX_CNT(hal_spec, tx_idx)) {
				val = map_read8_22b(txpwr_map, offset);
...
u16 hal_load_pg_txpwr_info_22b_path_5g_22b(
	_adapter *adapter,
...
void hal_load_pg_txpwr_info_22b(
	_adapter *adapter,
...
select_src:
	...
	case PG_TXPWR_SRC_IC_DEF:
		txpwr_map = hal_pg_txpwr_def_info_22b_22b(adapter);
		break;
	case PG_TXPWR_SRC_DEF:
	default:
		txpwr_map = &pg_txpwr_def_info_22b;
		break;
	};
	...
	for (path = 0; path < MAX_RF_PATH ; path++) {
		if (!HAL_SPEC_CHK_RF_PATH_2G(hal_spec, path) && !HAL_SPEC_CHK_RF_PATH_5G(hal_spec, path))
			break;
		pg_offset = hal_load_pg_txpwr_info_22b_path_2g_22b(adapter, pwr_info_2g, path, txpwr_src, txpwr_map, pg_offset);
		pg_offset = hal_load_pg_txpwr_info_22b_path_5g_22b(adapter, pwr_info_5g, path, txpwr_src, txpwr_map, pg_offset);
	}
...
static u8 _check_phy_efuse_tx_power_info_valid_22b(u8 *pg_data, int base_len, u16 pg_offset)
{
	int ff_cnt = 0;
	int i;
...
int check_phy_efuse_tx_power_info_valid_22b(_adapter *adapter)
{
	struct hal_spec_t *hal_spec = GET_HAL_SPEC(adapter);
	...
	for (path = 0; path < MAX_RF_PATH; path++) {
		...
		if (HAL_SPEC_CHK_RF_PATH_2G(hal_spec, path)) {
			ret = _check_phy_efuse_tx_power_info_valid_22b(pg_data, PG_TXPWR_BASE_BYTE_NUM_2G, pg_offset);
			if (ret == _TRUE)
		...
		#ifdef CONFIG_IEEE80211_BAND_5GHZ
		if (HAL_SPEC_CHK_RF_PATH_5G(hal_spec, path)) {
			ret = _check_phy_efuse_tx_power_info_valid_22b(pg_data, PG_TXPWR_BASE_BYTE_NUM_5G, pg_offset);
			if (ret == _TRUE)
	...
	if ((hal_chk_band_cap_22b(adapter, BAND_CAP_2G) && valid_2g_path_bmp)
		#ifdef CONFIG_IEEE80211_BAND_5GHZ
		|| (hal_chk_band_cap_22b(adapter, BAND_CAP_5G) && valid_5g_path_bmp)
		#endif
	)
		return _TRUE;
...
)
{
	...
	hal_load_pg_txpwr_info_22b(adapter, pwr_info_2g, pwr_info_5g, pg_data, _FALSE);
	...
	for (rfpath = 0; rfpath < MAX_RF_PATH; rfpath++) {
		...
		for (ch_idx = 0; ch_idx < CENTER_CH_2G_NUM; ch_idx++) {
			...
			if (rtw_get_ch_group_22b(ch_idx + 1, &group, &cck_group) != BAND_ON_2_4G)
				continue;
...
bypass_2g:
		...
		for (ch_idx = 0; ch_idx < CENTER_CH_5G_ALL_NUM; ch_idx++) {
			if (rtw_get_ch_group_22b(center_ch_5g_all_22b[ch_idx], &group, NULL) != BAND_ON_5G)
				continue;
		...
		for (ch_idx = 0 ; ch_idx < CENTER_CH_5G_80M_NUM; ch_idx++) {
			...
			if (rtw_get_ch_group_22b(center_ch_5g_80m_22b[ch_idx], &group, NULL) != BAND_ON_5G)
				continue;
...
void dump_hal_txpwr_info_2g_22b(void *sel, _adapter *adapter, u8 rfpath_num, u8 max_tx_cnt)
{
	HAL_DATA_TYPE *hal_data = GET_HAL_DATA(adapter);
	...
	RTW_PRINT_SEL(sel, "%4s ", "");
	for (ch_idx = 0; ch_idx < CENTER_CH_2G_NUM; ch_idx++)
		_RTW_PRINT_SEL(sel, "%2d ", center_ch_2g_22b[ch_idx]);
	_RTW_PRINT_SEL(sel, "\n");
	for (path = 0; path < MAX_RF_PATH && path < rfpath_num; path++) {
		RTW_PRINT_SEL(sel, "[%c]: ", rf_path_char(path));
		for (ch_idx = 0; ch_idx < CENTER_CH_2G_NUM; ch_idx++)
...
void dump_hal_txpwr_info_5g_22b(void *sel, _adapter *adapter, u8 rfpath_num, u8 max_tx_cnt)
{
#ifdef CONFIG_IEEE80211_BAND_5GHZ
	HAL_DATA_TYPE *hal_data = GET_HAL_DATA(adapter);
	int path, ch_idx, tx_idx;
	...
	do {
		...
		for (ch_idx = ch_idx_s; ch_idx < CENTER_CH_5G_ALL_NUM; ch_idx++) {
			_RTW_PRINT_SEL(sel, "%3d ", center_ch_5g_all_22b[ch_idx]);
			if (end[dump_section] == center_ch_5g_all_22b[ch_idx])
				break;
		}
		_RTW_PRINT_SEL(sel, "\n");
		for (path = 0; path < MAX_RF_PATH && path < rfpath_num; path++) {
	...
	RTW_PRINT_SEL(sel, "%4s ", "");
	for (ch_idx = 0; ch_idx < CENTER_CH_5G_80M_NUM; ch_idx++)
		_RTW_PRINT_SEL(sel, "%3d ", center_ch_5g_80m_22b[ch_idx]);
	_RTW_PRINT_SEL(sel, "\n");
	for (path = 0; path < MAX_RF_PATH && path < rfpath_num; path++) {
		RTW_PRINT_SEL(sel, "[%c]: ", rf_path_char(path));
		for (ch_idx = 0; ch_idx < CENTER_CH_5G_80M_NUM; ch_idx++)
...
/*
* rtw_regsty_get_target_tx_power_22b -
*
* Return dBm or -1 for undefined
*/
s8 rtw_regsty_get_target_tx_power_22b(
	IN	PADAPTER		Adapter,
	IN	u8				Band,
	IN	u8				RfPath,
...
bool rtw_regsty_chk_target_tx_power_valid_22b(_adapter *adapter)
{
	struct hal_spec_t *hal_spec = GET_HAL_SPEC(adapter);
	HAL_DATA_TYPE *hal_data = GET_HAL_DATA(adapter);
	...
	for (band = BAND_ON_2_4G; band <= BAND_ON_5G; band++) {
		if (!hal_is_band_support_22b(adapter, band))
			continue;
		...
		for (path = 0; path < RF_PATH_MAX; path++) {
			...
			for (rs = 0; rs < RATE_SECTION_NUM; rs++) {
				...
				target = rtw_regsty_get_target_tx_power_22b(adapter, band, path, rs);
				if (target == -1) {
...
VOID
phy_StoreTxPowerByRateBase_22b(
...
)
{
	...
	for (band = BAND_ON_2_4G; band <= BAND_ON_5G; band++) {
		if (!hal_is_band_support_22b(pAdapter, band))
			continue;
		...
		for (path = RF_PATH_A; path < RF_PATH_MAX; path++) {
			...
			for (rs = 0; rs < RATE_SECTION_NUM; rs++) {
				...
				if (regsty->target_tx_pwr_valid == _TRUE)
					base = 2 * rtw_regsty_get_target_tx_power_22b(pAdapter, band, path, rs);
				else
					base = _PHY_GetTxPowerByRate_22b(pAdapter, band, path, rate_sec_base[rs]);
				phy_SetTxPower_22bByRateBase(pAdapter, band, path, rs, base);
			}
		}
	}
}
...
)
{
	...
	PHY_GetRateValuesOfTxPowerByRate_22b(pAdapter, RegAddr, BitMask, Data, rates, PwrByRateVal, &rateNum);
	...
	for (i = 0; i < rateNum; ++i) {
		u8 rate_idx = PHY_GetRateIndexOfTxPowerByRate_22b(rates[i]);
...
VOID
phy_store_tx_power_by_rate_22b(
	IN	PADAPTER	pAdapter,
	IN	u32			Band,
	IN	u32			RfPath,
	IN	u32			TxNum,
...
)
{
	...
	if (pDM_Odm->phy_reg_pg_version > 0)
		PHY_StoreTxPowerByRateNew_22b(pAdapter, Band, RfPath, RegAddr, BitMask, Data);
	else
...
VOID
phy_ConvertTxPowerByRateInDbmToRelativeValues_22b(
	IN	PADAPTER	pAdapter
)
{
	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(pAdapter);
	...
	for (band = BAND_ON_2_4G; band <= BAND_ON_5G; ++band) {
		for (path = RF_PATH_A; path <= RF_PATH_D; ++path) {
			/* CCK */
			if (band == BAND_ON_2_4G) {
				base = PHY_GetTxPowerByRate_22bBase_22b(pAdapter, band, path, CCK);
				for (i = 0; i < sizeof(cckRates); ++i) {
					value = PHY_GetTxPowerByRate_22b(pAdapter, band, path, cckRates[i]);
					PHY_SetTxPower_22bByRate(pAdapter, band, path, cckRates[i], value - base);
				}
			}

			/* OFDM */
			base = PHY_GetTxPowerByRate_22bBase_22b(pAdapter, band, path, OFDM);
			for (i = 0; i < sizeof(ofdmRates); ++i) {
				value = PHY_GetTxPowerByRate_22b(pAdapter, band, path, ofdmRates[i]);
				PHY_SetTxPower_22bByRate(pAdapter, band, path, ofdmRates[i], value - base);
			}

			/* HT MCS0~7 */
			base = PHY_GetTxPowerByRate_22bBase_22b(pAdapter, band, path, HT_1SS);
			for (i = 0; i < sizeof(mcs0_7Rates); ++i) {
				value = PHY_GetTxPowerByRate_22b(pAdapter, band, path, mcs0_7Rates[i]);
				PHY_SetTxPower_22bByRate(pAdapter, band, path, mcs0_7Rates[i], value - base);
			}

			/* HT MCS8~15 */
			base = PHY_GetTxPowerByRate_22bBase_22b(pAdapter, band, path, HT_2SS);
			for (i = 0; i < sizeof(mcs8_15Rates); ++i) {
				value = PHY_GetTxPowerByRate_22b(pAdapter, band, path, mcs8_15Rates[i]);
				PHY_SetTxPower_22bByRate(pAdapter, band, path, mcs8_15Rates[i], value - base);
			}

			/* HT MCS16~23 */
			base = PHY_GetTxPowerByRate_22bBase_22b(pAdapter, band, path, HT_3SS);
			for (i = 0; i < sizeof(mcs16_23Rates); ++i) {
				value = PHY_GetTxPowerByRate_22b(pAdapter, band, path, mcs16_23Rates[i]);
				PHY_SetTxPower_22bByRate(pAdapter, band, path, mcs16_23Rates[i], value - base);
			}

			/* VHT 1SS */
			base = PHY_GetTxPowerByRate_22bBase_22b(pAdapter, band, path, VHT_1SS);
			for (i = 0; i < sizeof(vht1ssRates); ++i) {
				value = PHY_GetTxPowerByRate_22b(pAdapter, band, path, vht1ssRates[i]);
				PHY_SetTxPower_22bByRate(pAdapter, band, path, vht1ssRates[i], value - base);
			}

			/* VHT 2SS */
			base = PHY_GetTxPowerByRate_22bBase_22b(pAdapter, band, path, VHT_2SS);
			for (i = 0; i < sizeof(vht2ssRates); ++i) {
				value = PHY_GetTxPowerByRate_22b(pAdapter, band, path, vht2ssRates[i]);
				PHY_SetTxPower_22bByRate(pAdapter, band, path, vht2ssRates[i], value - base);
			}

			/* VHT 3SS */
			base = PHY_GetTxPowerByRate_22bBase_22b(pAdapter, band, path, VHT_3SS);
			for (i = 0; i < sizeof(vht3ssRates); ++i) {
				value = PHY_GetTxPowerByRate_22b(pAdapter, band, path, vht3ssRates[i]);
				PHY_SetTxPower_22bByRate(pAdapter, band, path, vht3ssRates[i], value - base);
			}
		}
	}
...
)
{
	...
	phy_StoreTxPowerByRateBase_22b(pAdapter);
	phy_ConvertTxPowerByRateInDbmToRelativeValues_22b(pAdapter);
}

VOID
phy_set_tx_power_index_by_rate_section_22b(
	IN	PADAPTER		pAdapter,
...
)
{
	...
	PHY_SetTxPower_22bIndexByRateArray(pAdapter, RFPath, pHalData->current_channel_bw, Channel,
		rates_by_sections_22b[RateSection].rates, rates_by_sections_22b[RateSection].rate_num);
	...
	if (Channel <= 14) {
		...
		*ChannelIdx = Channel - 1;
		...
		for (i = 0; i < CENTER_CH_5G_ALL_NUM; ++i) {
			if (center_ch_5g_all_22b[i] == Channel) {
				*ChannelIdx = i;
				return bIn24G;
			}
		}
...
)
{
	...
	if (HAL_IsLegalChannel_22b(pAdapter, Channel) == _FALSE) {
		chnlIdx = 0;
		RTW_INFO("Illegal channel!!\n");
	}

	*bIn24G = phy_GetChnlIndex_22b(Channel, &chnlIdx);
	...
	if (*bIn24G) {
		...
		if (Rate >= MGN_6M)
			...
			RTW_INFO("===>PHY_GetTxPowerIndexBase_22b: INVALID Rate(0x%02x).\n", Rate);
			goto exit;
		}
		...
		if (BandWidth == CHANNEL_WIDTH_80) {
			...
			for (i = 0; i < CENTER_CH_5G_80M_NUM; ++i) {
				if (center_ch_5g_80m_22b[i] == Channel) {
					chnlIdx = i;
					break;
				}
			}
...
/*The same as MRateToHwRate_22b in hal_com.c*/
u8
...
)
{
	HAL_DATA_TYPE *pHalData = GET_HAL_DATA(pAdapter);
	s8 value = 0;
	u8 rateIndex = PHY_GetRateIndexOfTxPowerByRate_22b(Rate);
...
{
	if (!phy_is_tx_power_by_rate_needed_22b(pAdapter))
		return 0;

	return _PHY_GetTxPowerByRate_22b(pAdapter, Band, RFPath, Rate);
}
...
VOID
phy_set_tx_power_level_by_path_22b(
	IN	PADAPTER	Adapter,
	IN	u8			channel,
	IN	u8			path
)
{
	...
	{
		if (bIsIn24G)
			phy_set_tx_power_index_by_rate_section_22b(Adapter, path, channel, CCK);

		phy_set_tx_power_index_by_rate_section_22b(Adapter, path, channel, OFDM);
		phy_set_tx_power_index_by_rate_section_22b(Adapter, path, channel, HT_MCS0_MCS7);

		if (IS_HARDWARE_TYPE_JAGUAR(Adapter) || IS_HARDWARE_TYPE_8814A(Adapter))
			phy_set_tx_power_index_by_rate_section_22b(Adapter, path, channel, VHT_1SSMCS0_1SSMCS9);

		if (pHalData->NumTotalRFPath >= 2) {
			phy_set_tx_power_index_by_rate_section_22b(Adapter, path, channel, HT_MCS8_MCS15);

			if (IS_HARDWARE_TYPE_JAGUAR(Adapter) || IS_HARDWARE_TYPE_8814A(Adapter))
				phy_set_tx_power_index_by_rate_section_22b(Adapter, path, channel, VHT_2SSMCS0_2SSMCS9);

			if (IS_HARDWARE_TYPE_8814A(Adapter)) {
				phy_set_tx_power_index_by_rate_section_22b(Adapter, path, channel, HT_MCS16_MCS23);
				phy_set_tx_power_index_by_rate_section_22b(Adapter, path, channel, VHT_3SSMCS0_3SSMCS9);
			}
		}
	}
}
...
)
{
	...
	for (i = 0; i < RateArraySize; ++i) {
		...
		powerIndex = rtw_hal_get_tx_power_index_22b(pAdapter, RFPath, Rates[i], BandWidth, Channel, &tic);
...
#else
		powerIndex = phy_get_tx_power_index_22b(pAdapter, RFPath, Rates[i], BandWidth, Channel);
#endif
		PHY_SetTxPower_22bIndex(pAdapter, powerIndex, RFPath, Rates[i]);
	}
}
...
)
{
	...
	if (lmt != MAX_POWER_INDEX) {
		/* return diff value */
		lmt = lmt - PHY_GetTxPowerByRate_22bBase_22b(adapter, band, rfpath, rs);
	}
	...
	, u8 rfpath, u8 rate, u8 ntx_idx, u8 cch)
{
	...
	if (rtw_mp_mode_22b_check(adapter) == _TRUE)
		no_sc = _TRUE;
#endif
	if (IS_CCK_RATE(rate)) {
		tlrs = TXPWR_LMT_RS_CCK;
	...
	if (no_sc == _TRUE) {
		...
		*/
		if (tlrs == TXPWR_LMT_RS_CCK || tlrs == TXPWR_LMT_RS_OFDM)
			...
			bw_bmp = rtw_get_tx_bw_bmp_of_ht_rate_22b(dvobj, rate, bw);
			if (bw_bmp == 0)
				bw_bmp = ch_width_to_bw_cap(bw > CHANNEL_WIDTH_40 ? CHANNEL_WIDTH_40 : bw);
		} else if (tlrs == TXPWR_LMT_RS_VHT) {
			bw_bmp = rtw_get_tx_bw_bmp_of_vht_rate_22b(dvobj, rate, bw);
			if (bw_bmp == 0)
	...
	if (min_lmt != MAX_POWER_INDEX) {
		/* return diff value */
		min_lmt = min_lmt - PHY_GetTxPowerByRate_22bBase_22b(adapter, band, rfpath, rs);
	}
...
void phy_txpwr_limit_bandwidth_chk(_adapter *adapter)
{
	...
	for (band = BAND_ON_2_4G; band <= BAND_ON_5G; band++) {
		...
		for (bw = CHANNEL_WIDTH_40; bw <= CHANNEL_WIDTH_80; bw++) {
			...
			if (band == BAND_ON_2_4G)
				ch_num = center_chs_2g_22b_num_22b(bw);
			else
				ch_num = center_chs_5g_22b_num_22b(bw);
			...
			for (tlrs = TXPWR_LMT_RS_HT; tlrs < TXPWR_LMT_RS_NUM; tlrs++) {
				...
				for (ntx_idx = RF_1TX; ntx_idx < MAX_TX_COUNT; ntx_idx++) {
					...
					for (n = 0; n < ch_num; n++) {
						...
						if (band == BAND_ON_2_4G)
							cch = center_chs_2g_22b(bw, n);
						else
							cch = center_chs_5g_22b(bw, n);
						...
						_rtw_memset_22b(cch_by_bw, 0, 3);
						cch_by_bw[bw] = cch;
						offset_by_bw = 0x01;

						do {
							for (bw_pos = bw; bw_pos >= CHANNEL_WIDTH_40; bw_pos--)
								cch_by_bw[bw_pos - 1] = rtw_get_scch_by_cch_offset_22b(cch_by_bw[bw_pos], bw_pos, offset_by_bw & BIT(bw_pos) ? HAL_PRIME_CHNL_OFFSET_UPPER : HAL_PRIME_CHNL_OFFSET_LOWER);
...
/*
* phy_set_tx_power_limit_22b - Parsing TX power limit from phydm array, called by odm_ConfigBB_TXPWR_LMT_XXX in phydm
*/
VOID
...
)
{
	...
	if (GetU1ByteIntegerFromStringInDecimal_22b((char *)Channel, &channel) == _FALSE
	...
	if (eqNByte_22b(RateSection, (u8 *)("CCK"), 3))
		tlrs = TXPWR_LMT_RS_CCK;
	else if (eqNByte_22b(RateSection, (u8 *)("OFDM"), 4))
		tlrs = TXPWR_LMT_RS_OFDM;
	else if (eqNByte_22b(RateSection, (u8 *)("HT"), 2))
		tlrs = TXPWR_LMT_RS_HT;
	else if (eqNByte_22b(RateSection, (u8 *)("VHT"), 3))
		tlrs = TXPWR_LMT_RS_VHT;
	else {
	...
	if (eqNByte_22b(ntx, (u8 *)("1T"), 2))
		ntx_idx = RF_1TX;
	else if (eqNByte_22b(ntx, (u8 *)("2T"), 2))
		ntx_idx = RF_2TX;
	else if (eqNByte_22b(ntx, (u8 *)("3T"), 2))
		ntx_idx = RF_3TX;
	else if (eqNByte_22b(ntx, (u8 *)("4T"), 2))
		ntx_idx = RF_4TX;
	else {
	...
	if (eqNByte_22b(Bandwidth, (u8 *)("20M"), 3))
		bandwidth = CHANNEL_WIDTH_20;
	else if (eqNByte_22b(Bandwidth, (u8 *)("40M"), 3))
		bandwidth = CHANNEL_WIDTH_40;
	else if (eqNByte_22b(Bandwidth, (u8 *)("80M"), 3))
		bandwidth = CHANNEL_WIDTH_80;
	else if (eqNByte_22b(Bandwidth, (u8 *)("160M"), 4))
...
#ifdef CONFIG_IEEE80211_BAND_5GHZ
	else if (eqNByte_22b(Band, (u8 *)("5G"), 2)) {
		band = BAND_ON_5G;
...
{
	return rtw_hal_get_tx_power_index_22b(pAdapter, RFPath, Rate, BandWidth, Channel, NULL);
}
...
)
{
	if (IS_HARDWARE_TYPE_8814A(pAdapter)) {
		...
		PHY_SetTxPower_22bIndex_8814A(pAdapter, PowerIndex, RFPath, Rate);
#endif
	} else if (IS_HARDWARE_TYPE_JAGUAR(pAdapter)) {
#if ((RTL8812A_SUPPORT == 1) || (RTL8821A_SUPPORT == 1))
		PHY_SetTxPower_22bIndex_8812A(pAdapter, PowerIndex, RFPath, Rate);
#endif
	} else if (IS_HARDWARE_TYPE_8723B(pAdapter)) {
#if (RTL8723B_SUPPORT == 1)
		PHY_SetTxPower_22bIndex_8723B(pAdapter, PowerIndex, RFPath, Rate);
#endif
	} else if (IS_HARDWARE_TYPE_8703B(pAdapter)) {
#if (RTL8703B_SUPPORT == 1)
		PHY_SetTxPower_22bIndex_8703B(pAdapter, PowerIndex, RFPath, Rate);
#endif
	} else if (IS_HARDWARE_TYPE_8723D(pAdapter)) {
#if (RTL8723D_SUPPORT == 1)
		PHY_SetTxPower_22bIndex_8723D(pAdapter, PowerIndex, RFPath, Rate);
#endif
	} else if (IS_HARDWARE_TYPE_8192E(pAdapter)) {
#if (RTL8192E_SUPPORT == 1)
		PHY_SetTxPower_22bIndex_8192E(pAdapter, PowerIndex, RFPath, Rate);
#endif
	} else if (IS_HARDWARE_TYPE_8188E(pAdapter)) {
#if (RTL8188E_SUPPORT == 1)
		PHY_SetTxPower_22bIndex_8188E(pAdapter, PowerIndex, RFPath, Rate);
#endif
	} else if (IS_HARDWARE_TYPE_8188F(pAdapter)) {
#if (RTL8188F_SUPPORT == 1)
		PHY_SetTxPower_22bIndex_8188F(pAdapter, PowerIndex, RFPath, Rate);
#endif
	} else if (IS_HARDWARE_TYPE_8822B(pAdapter))
		rtw_hal_set_tx_power_index_22b(pAdapter, PowerIndex, RFPath, Rate);
	else if (IS_HARDWARE_TYPE_8821C(pAdapter))
		rtw_hal_set_tx_power_index_22b(pAdapter, PowerIndex, RFPath, Rate);
}

void dump_tx_power_idx_22b_title_22b(void *sel, _adapter *adapter)
{
	HAL_DATA_TYPE *hal_data = GET_HAL_DATA(adapter);
...
void dump_tx_power_idx_22b_by_path_rs_22b(void *sel, _adapter *adapter, u8 rfpath, u8 rs)
{
	HAL_DATA_TYPE *hal_data = GET_HAL_DATA(adapter);
	struct hal_spec_t *hal_spec = GET_HAL_SPEC(adapter);
	u8 power_idx;
	...
	for (i = 0; i < rates_by_sections_22b[rs].rate_num; i++) {
		power_idx = rtw_hal_get_tx_power_index_22b(adapter, rfpath, rates_by_sections_22b[rs].rates[i], bw, cch, &tic);

		RTW_PRINT_SEL(sel, "%4c %9s %uT %3u %4u %3d (%3d %3d) %3d %5d\n"
			, rf_path_char(rfpath), MGN_RATE_STR(rates_by_sections_22b[rs].rates[i]), tic.ntx_idx + 1
			, power_idx, tic.base, (tic.by_rate > tic.limit ? tic.limit : tic.by_rate), tic.by_rate, tic.limit, tic.tpt, tic.ebias);
	}
}

void dump_tx_power_idx_22b(void *sel, _adapter *adapter)
{
	u8 rfpath, rs;

	dump_tx_power_idx_22b_title_22b(sel, adapter);
	for (rfpath = RF_PATH_A; rfpath < RF_PATH_MAX; rfpath++)
		for (rs = CCK; rs < RATE_SECTION_NUM; rs++)
			dump_tx_power_idx_22b_by_path_rs_22b(sel, adapter, rfpath, rs);
}

bool phy_is_tx_power_limit_needed_22b(_adapter *adapter)
{
	HAL_DATA_TYPE *hal_data = GET_HAL_DATA(adapter);
...
bool phy_is_tx_power_by_rate_needed_22b(_adapter *adapter)
{
	HAL_DATA_TYPE *hal_data = GET_HAL_DATA(adapter);
	struct registry_priv *regsty = dvobj_to_regsty(adapter_to_dvobj(adapter));
...
int phy_load_tx_power_by_rate_22b(_adapter *adapter, u8 chk_file)
{
	HAL_DATA_TYPE *hal_data = GET_HAL_DATA(adapter);
	struct registry_priv *regsty = dvobj_to_regsty(adapter_to_dvobj(adapter));
	int ret = _FAIL;
	...
	PHY_InitTxPowerByRate_22b(adapter);
	...
	if (chk_file
		&& phy_ConfigBBWithPgParaFile_22b(adapter, PHY_FILE_PHY_REG_PG) == _SUCCESS
	) {
	...
	if (HAL_STATUS_SUCCESS == odm_config_bb_with_header_file_22b(&hal_data->odmpriv, CONFIG_BB_PHY_REG_PG)) {
...
post_hdl:
	...
	PHY_TxPowerByRateConfiguration_22b(adapter);
...
#ifdef CONFIG_EMBEDDED_FWIMG
	if (odm_config_rf_with_header_file_22b(&hal_data->odmpriv, CONFIG_RF_TXPWR_LMT, RF_PATH_A) == HAL_STATUS_SUCCESS) {
...
void phy_load_tx_power_ext_info_22b(_adapter *adapter, u8 chk_file)
{
	struct registry_priv *regsty = adapter_to_regsty(adapter);

	/* check registy target tx power */
	regsty->target_tx_pwr_valid = rtw_regsty_chk_target_tx_power_valid_22b(adapter);

	/* power by rate and limit */
	if (phy_is_tx_power_by_rate_needed_22b(adapter)
		|| (phy_is_tx_power_limit_needed_22b(adapter) && regsty->target_tx_pwr_valid != _TRUE)
	)
		phy_load_tx_power_by_rate_22b(adapter, chk_file);

#ifdef CONFIG_TXPWR_LIMIT
	if (phy_is_tx_power_limit_needed_22b(adapter))
		phy_load_tx_power_limit(adapter, chk_file);
#endif
}

inline void phy_reload_tx_power_ext_info_22b(_adapter *adapter)
{
	phy_load_tx_power_ext_info_22b(adapter, 1);
}

inline void phy_reload_default_tx_power_ext_info_22b(_adapter *adapter)
{
	phy_load_tx_power_ext_info_22b(adapter, 0);
}

void dump_tx_power_ext_info_22b(void *sel, _adapter *adapter)
{
	struct registry_priv *regsty = adapter_to_regsty(adapter);
	HAL_DATA_TYPE *hal_data = GET_HAL_DATA(adapter);
	...
	else if (phy_is_tx_power_by_rate_needed_22b(adapter))
	...
	RTW_PRINT_SEL(sel, "tx_power_by_rate: %s, %s, %s\n"
		, phy_is_tx_power_by_rate_needed_22b(adapter) ? "enabled" : "disabled"
	...
	RTW_PRINT_SEL(sel, "tx_power_limit: %s, %s, %s\n"
		, phy_is_tx_power_limit_needed_22b(adapter) ? "enabled" : "disabled"
		, hal_data->txpwr_limit_loaded ? "loaded" : "unloaded"
		, hal_data->txpwr_limit_from_file ? "file" : "default"
	);
}

void dump_target_tx_power_22b(void *sel, _adapter *adapter)
{
	struct hal_spec_t *hal_spec = GET_HAL_SPEC(adapter);
	HAL_DATA_TYPE *hal_data = GET_HAL_DATA(adapter);
	struct registry_priv *regsty = adapter_to_regsty(adapter);
	...
	for (band = BAND_ON_2_4G; band <= BAND_ON_5G; band++) {
		...
		for (path = 0; path < RF_PATH_MAX; path++) {
			...
			for (rs = 0; rs < RATE_SECTION_NUM; rs++) {
				...
				target = PHY_GetTxPowerByRate_22bBase_22b(adapter, band, path, rs);

				if (target % 2)
					_RTW_PRINT_SEL(sel, "%7s: %2d.5\n", rate_section_str_22b(rs), target / 2);
				else
...
void dump_tx_power_by_rate_22b(void *sel, _adapter *adapter)
{
	struct hal_spec_t *hal_spec = GET_HAL_SPEC(adapter);
	HAL_DATA_TYPE *hal_data = GET_HAL_DATA(adapter);
	...
	for (band = BAND_ON_2_4G; band <= BAND_ON_5G; band++) {
		...
		for (path = 0; path < RF_PATH_MAX; path++) {
			...
			for (rs = 0; rs < RATE_SECTION_NUM; rs++) {
				...
				RTW_PRINT_SEL(sel, "%7s: ", rate_section_str_22b(rs));

				/* dump power by rate in db */
				for (n = rate_num - 1; n >= 0; n--) {
					by_rate_offset = PHY_GetTxPowerByRate_22b(adapter, band, path, rates_by_sections_22b[rs].rates[n]);
...
/*
 * phy file path is stored in global char array rtw_phy_para_file_path_22b
 * need to care about racing
 */
int rtw_get_phy_file_path_22b(_adapter *adapter, const char *file_name)
{
#ifdef CONFIG_LOAD_PHY_PARA_FROM_FILE
	struct hal_spec_t *hal_spec = GET_HAL_SPEC(adapter);
	int len = 0;
	...
	if (file_name) {
		len += snprintf(rtw_phy_para_file_path_22b, PATH_LENGTH_MAX, "%s", rtw_phy_file_path_22b);
		#if defined(CONFIG_MULTIDRV) || defined(REALTEK_CONFIG_PATH_WITH_IC_NAME_FOLDER)
		len += snprintf(rtw_phy_para_file_path_22b + len, PATH_LENGTH_MAX - len, "%s/", hal_spec->ic_name);
		#endif
		len += snprintf(rtw_phy_para_file_path_22b + len, PATH_LENGTH_MAX - len, "%s", file_name);
	...
	if ((pHalData->mac_reg_len == 0) && (pHalData->mac_reg == NULL)) {
		...
		if (rtw_is_file_readable_22b(rtw_phy_para_file_path_22b) == _TRUE) {
			...
			if (rlen > 0) {
				...
				if (pHalData->mac_reg) {
					_rtw_memcpy_22b(pHalData->mac_reg, pHalData->para_file_buf, rlen);
		...
		if ((pHalData->mac_reg_len != 0) && (pHalData->mac_reg != NULL)) {
			_rtw_memcpy_22b(pHalData->para_file_buf, pHalData->mac_reg, pHalData->mac_reg_len);
			rtStatus = _SUCCESS;
		} else
	...
	if (rtStatus == _SUCCESS) {
		...
		for (szLine = GetLineFromBuffer(ptmp); szLine != NULL; szLine = GetLineFromBuffer(ptmp)) {
			if (!IsCommentString_22b(szLine)) {
				...
				if (GetHexValueFromString_22b(szLine, &u4bRegOffset, &u4bMove)) {
					...
					szLine += u4bMove;
					if (GetHexValueFromString_22b(szLine, &u4bRegValue, &u4bMove))
	...
	if ((pBufLen != NULL) && (*pBufLen == 0) && (pBuf == NULL)) {
		...
		if (rtw_is_file_readable_22b(rtw_phy_para_file_path_22b) == _TRUE) {
			...
			if (rlen > 0) {
				...
				if (pBuf) {
					_rtw_memcpy_22b(pBuf, pHalData->para_file_buf, rlen);
					*pBufLen = rlen;
		...
		if ((pBufLen != NULL) && (*pBufLen != 0) && (pBuf != NULL)) {
			_rtw_memcpy_22b(pHalData->para_file_buf, pBuf, *pBufLen);
			rtStatus = _SUCCESS;
		} else
			RTW_INFO("%s(): Critical Error !!!\n", __FUNCTION__);
	}
...
)
{
	...
	/* RTW_INFO("=====>phy_DecryptBBPgParaFile_22b()\n"); */
...
)
{
	...
	/* RTW_INFO("=====>phy_ParseBBPgParaFile_22b()\n"); */

	if (Adapter->registrypriv.RegDecryptCustomFile == 1)
		phy_DecryptBBPgParaFile_22b(Adapter, buffer);

	ptmp = buffer;
	for (szLine = GetLineFromBuffer(ptmp); szLine != NULL; szLine = GetLineFromBuffer(ptmp)) {
		if (isAllSpaceOrTab_22b(szLine, sizeof(*szLine)))
			continue;

		if (!IsCommentString_22b(szLine)) {
			/* Get header info (relative value or exact value) */
			if (firstLine) {
				if (eqNByte_22b(szLine, (u8 *)("#[v1]"), 5)) {

					pHalData->odmpriv.phy_reg_pg_version = szLine[3] - '0';
					/* RTW_INFO("This is a new format PHY_REG_PG.txt\n"); */
				} else if (eqNByte_22b(szLine, (u8 *)("#[v0]"), 5)) {
					pHalData->odmpriv.phy_reg_pg_version = szLine[3] - '0';
			...
			if (pHalData->odmpriv.phy_reg_pg_version == 0) {
				...
				if (GetHexValueFromString_22b(szLine, &u4bRegOffset, &u4bMove)) {
					...
					if (GetHexValueFromString_22b(szLine, &u4bRegMask, &u4bMove))
						szLine += u4bMove;
					else
						return _FAIL;
					...
					if (pHalData->odmpriv.phy_reg_pg_value_type == PHY_REG_PG_RELATIVE_VALUE) {
						...
						if (GetHexValueFromString_22b(szLine, &u4bRegValue, &u4bMove)) {
							phy_store_tx_power_by_rate_22b(Adapter, 0, 0, 1, u4bRegOffset, u4bRegMask, u4bRegValue);
						...
						if (GetFractionValueFromString_22b(szLine, &integer, &fraction, &u4bMove))
							szLine += u4bMove;
						else
							return _FAIL;
						...
						combineValue |= (((integer / 10) << 4) + (integer % 10));
						/* RTW_INFO(" %d", integer ); */
						phy_store_tx_power_by_rate_22b(Adapter, 0, 0, 1, u4bRegOffset, u4bRegMask, combineValue);
				...
				if (eqNByte_22b(szLine, "0xffff", 6))
					break;

				if (!eqNByte_22b("#[END]#", szLine, 7)) {
					/* load the table label info */
					if (szLine[0] == '#') {
						index = 0;
						if (eqNByte_22b(szLine, "#[2.4G]" , 7)) {
							band = BAND_ON_2_4G;
							index += 8;
						} else if (eqNByte_22b(szLine, "#[5G]", 5)) {
							band = BAND_ON_5G;
							index += 6;
						} else {
						...
						if (pHalData->odmpriv.phy_reg_pg_value_type == PHY_REG_PG_RELATIVE_VALUE) {
							/* Get 3rd hex value as register value. */
							if (GetHexValueFromString_22b(szLine, &u4bRegValue, &u4bMove)) {
								phy_store_tx_power_by_rate_22b(Adapter, band, rf_path, tx_num, u4bRegOffset, u4bRegMask, u4bRegValue);
							...
							combineValue |= (((integer / 10) << 4) + (integer % 10));
							/* RTW_INFO(" %d", integer ); */
							phy_store_tx_power_by_rate_22b(Adapter, band, rf_path, tx_num, u4bRegOffset, u4bRegMask, combineValue);
...
int
phy_ConfigBBWithPgParaFile_22b(
	IN	PADAPTER	Adapter,
	IN	const char	*pFileName)
{
	...
	if (pHalData->bb_phy_reg_pg == NULL) {
		...
		if (rtw_is_file_readable_22b(rtw_phy_para_file_path_22b) == _TRUE) {
			...
			if (rlen > 0) {
				...
				if (pHalData->bb_phy_reg_pg) {
					_rtw_memcpy_22b(pHalData->bb_phy_reg_pg, pHalData->para_file_buf, rlen);
		...
		if ((pHalData->bb_phy_reg_pg_len != 0) && (pHalData->bb_phy_reg_pg != NULL)) {
			_rtw_memcpy_22b(pHalData->para_file_buf, pHalData->bb_phy_reg_pg, pHalData->bb_phy_reg_pg_len);
			rtStatus = _SUCCESS;
		} else
			RTW_INFO("%s(): Critical Error !!!\n", __FUNCTION__);
	}
	...
	if (rtStatus == _SUCCESS) {
		/* RTW_INFO("phy_ConfigBBWithPgParaFile_22b(): read %s ok\n", pFileName); */
		phy_ParseBBPgParaFile_22b(Adapter, pHalData->para_file_buf);
	} else
		RTW_INFO("%s(): No File %s, Load from HWImg Array!\n", __FUNCTION__, pFileName);
...
int
phy_ConfigBBWithMpParaFile_22b(
	IN	PADAPTER	Adapter,
	IN	char		*pFileName
)
{
	...
	if ((pHalData->bb_phy_reg_mp_len == 0) && (pHalData->bb_phy_reg_mp == NULL)) {
		...
		if (rtw_is_file_readable_22b(rtw_phy_para_file_path_22b) == _TRUE) {
			...
			if (rlen > 0) {
				...
				if (pHalData->bb_phy_reg_mp) {
					_rtw_memcpy_22b(pHalData->bb_phy_reg_mp, pHalData->para_file_buf, rlen);
		...
		if ((pHalData->bb_phy_reg_mp_len != 0) && (pHalData->bb_phy_reg_mp != NULL)) {
			_rtw_memcpy_22b(pHalData->para_file_buf, pHalData->bb_phy_reg_mp, pHalData->bb_phy_reg_mp_len);
			rtStatus = _SUCCESS;
		} else
			RTW_INFO("%s(): Critical Error !!!\n", __FUNCTION__);
	}
	...
	if (rtStatus == _SUCCESS) {
		/* RTW_INFO("phy_ConfigBBWithMpParaFile_22b(): read %s ok\n", pFileName); */
			...
			if (!IsCommentString_22b(szLine)) {
				...
				if (GetHexValueFromString_22b(szLine, &u4bRegOffset, &u4bMove)) {
					if (u4bRegOffset == 0xfe || u4bRegOffset == 0xffe) {
						...
						/* delay_ms(5); */
						for (i = 0; i < 100; i++)
							rtw_udelay_os_22b(MAX_STALL_TIME);
					} else if (u4bRegOffset == 0xfc) {
						/* delay_ms(1); */
						for (i = 0; i < 20; i++)
							rtw_udelay_os_22b(MAX_STALL_TIME);
...
#define STORE_SWING_TABLE(_array, _iteratedIdx) \
	...
	/* RTW_INFO("===>initDeltaSwingIndexTables_22b(): Band: %s;\nPath: %s;\nSign: %s;\nChannel: %s;\nRate: %s;\n, Data: %s;\n",  */
	...
	if (STR_EQUAL_2G("2G", "A", "+", "CCK"))
		...
		RTW_INFO("===>initDeltaSwingIndexTables_22b(): The input is invalid!!\n");
}

int
PHY_ConfigRFWithTxPwrTrackParaFile_22b(
	IN	PADAPTER		Adapter,
	IN	char			*pFileName
)
{
	...
	if ((pHalData->rf_tx_pwr_track_len == 0) && (pHalData->rf_tx_pwr_track == NULL)) {
		...
		if (rtw_is_file_readable_22b(rtw_phy_para_file_path_22b) == _TRUE) {
			...
			if (rlen > 0) {
				...
				if (pHalData->rf_tx_pwr_track) {
					_rtw_memcpy_22b(pHalData->rf_tx_pwr_track, pHalData->para_file_buf, rlen);
		...
		if ((pHalData->rf_tx_pwr_track_len != 0) && (pHalData->rf_tx_pwr_track != NULL)) {
			_rtw_memcpy_22b(pHalData->para_file_buf, pHalData->rf_tx_pwr_track, pHalData->rf_tx_pwr_track_len);
			rtStatus = _SUCCESS;
		} else
			RTW_INFO("%s(): Critical Error !!!\n", __FUNCTION__);
	}
	...
	if (rtStatus == _SUCCESS) {
		...
		for (szLine = GetLineFromBuffer(ptmp); szLine != NULL; szLine = GetLineFromBuffer(ptmp)) {
			if (!IsCommentString_22b(szLine)) {
				...
				i = 10; /* szLine+10 */
				if (!ParseQualifiedString_22b(szLine, &i, rate, '[', ']')) {
					/* RTW_INFO("Fail to parse rate!\n"); */
				}
				if (!ParseQualifiedString_22b(szLine, &i, chnl, '[', ']')) {
					/* RTW_INFO("Fail to parse channel group!\n"); */
				}
				while (szLine[i] != '{' && i < strlen(szLine))
					i++;
				if (!ParseQualifiedString_22b(szLine, &i, data, '{', '}')) {
					/* RTW_INFO("Fail to parse data!\n"); */
				}

				initDeltaSwingIndexTables_22b(Adapter, band, path, sign, chnl, rate, data);
			}
		}
	} else
		RTW_INFO("%s(): No File %s, Load from HWImg Array!\n", __FUNCTION__, pFileName);
...
#define REG_PREFIX "Regulation="
	...
	if (strncmp(szLine + i, VER_PREFIX, ver_prefix_len) == 0)
		...
		for (j = i_val_s; j < i_val_e; j++)
			if (IsHexDigit_22b(szLine[j]) == _FALSE)
				return PARSE_RET_FAIL;
		...
		/* check if all alpha */
		for (j = i_val_s; j < i_val_e; j++)
			if (is_alpha_22b(szLine[j]) == _FALSE)
				return PARSE_RET_FAIL;
...
#define LD_STAGE_CH_ROW			4
	...
	for (szLine = GetLineFromBuffer(ptmp); szLine != NULL; szLine = GetLineFromBuffer(ptmp)) {
		...
		if (loadingStage == LD_STAGE_TAB_DEFINE) {
			...
			_rtw_memset_22b((PVOID) band, 0, 10);
			_rtw_memset_22b((PVOID) bandwidth, 0, 10);
			_rtw_memset_22b((PVOID) ntx, 0, 10);
			_rtw_memset_22b((PVOID) rateSection, 0, 10);
			if (!ParseQualifiedString_22b(szLine, &i, band, ' ', ',')) {
				RTW_ERR("Fail to parse band!\n");
				goto exit;
			}
			if (!ParseQualifiedString_22b(szLine, &i, bandwidth, ' ', ',')) {
				RTW_ERR("Fail to parse bandwidth!\n");
				goto exit;
			}
			if (!ParseQualifiedString_22b(szLine, &i, ntx, ' ', ',')) {
				RTW_ERR("Fail to parse ntx!\n");
				goto exit;
			}
			if (!ParseQualifiedString_22b(szLine, &i, rateSection, ' ', ',')) {
			...
			if (!eqNByte_22b((u8 *)(szLine + i), (u8 *)("START"), 5)) {
			...
			_rtw_memset_22b((PVOID) colNumBuf, 0, 10);
			if (!ParseQualifiedString_22b(szLine, &i, colNumBuf, '#', '#')) {
				RTW_ERR("Fail to parse column number!\n");
				goto exit;
			}
			if (!GetU1ByteIntegerFromStringInDecimal_22b(colNumBuf, &colNum)) {
			...
			for (forCnt = 0; forCnt < colNum; ++forCnt) {
				...
				_rtw_memcpy_22b(regulation[forCnt], szLine + i_ns, i - i_ns);
			...
			if (szLine[0] == '#' && szLine[1] == '#') {
				...
				if (eqNByte_22b((u8 *)(szLine + i), (u8 *)("END"), 3)) {
					loadingStage = LD_STAGE_TAB_DEFINE;
					if (regulation) {
						for (forCnt = 0; forCnt < colNum; ++forCnt) {
							if (regulation[forCnt]) {
			...
			for (forCnt = 0; forCnt < colNum; ++forCnt) {
				...
				fraction = 0;
				negative = 0;
				_rtw_memset_22b((PVOID) powerLimit, 0, 10);
				...
				if (powerLimit[0] == '\0') {
					...
					if (!GetU1ByteIntegerFromStringInDecimal_22b(powerLimit, &limitValue)) {
				...
				phy_set_tx_power_limit_22b(pDM_Odm, (u8 *)regulation[forCnt], (u8 *)band,
...
)
{
	...
	if (pHalData->rf_tx_pwr_lmt == NULL) {
		...
		if (rtw_is_file_readable_22b(rtw_phy_para_file_path_22b) == _TRUE) {
			...
			if (rlen > 0) {
				...
				if (pHalData->rf_tx_pwr_lmt) {
					_rtw_memcpy_22b(pHalData->rf_tx_pwr_lmt, pHalData->para_file_buf, rlen);
		...
		if ((pHalData->rf_tx_pwr_lmt_len != 0) && (pHalData->rf_tx_pwr_lmt != NULL)) {
			_rtw_memcpy_22b(pHalData->para_file_buf, pHalData->rf_tx_pwr_lmt, pHalData->rf_tx_pwr_lmt_len);
			rtStatus = _SUCCESS;
		} else
			RTW_INFO("%s(): Critical Error !!!\n", __FUNCTION__);
	}
...
inline void phy_free_filebuf_22b(_adapter *padapter)
{
	phy_free_filebuf_22b_mask(padapter, 0xFF);
}
