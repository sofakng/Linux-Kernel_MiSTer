...
#define usb_write_port_22b_complete_not_xmitframe(purb, regs)	usb_write_port_22b_complete_not_xmitframe(purb)
#endif
static void usb_write_port_22b_complete_not_xmitframe(struct urb *purb, struct pt_regs *regs)
{
...
static u32 usb_write_port_22b_not_xmitframe(struct dvobj_priv *d, u8 addr, u32 cnt, u8 *wmem)
{
	...
	usb_fill_bulk_urb(purb, pusbd, pipe,
			  wmem,
			  cnt,
			  usb_write_port_22b_complete_not_xmitframe,
			  padapter);
...
static u8 usb_write_data_not_xmitframe(void *d, u8 *pBuf, u32 size, u8 qsel)
{
	...
	if (qsel == HALMAC_TXDESC_QSEL_BEACON) {
		...
		if (add_pkt_offset == 1)
			_rtw_memcpy_22b(buf + desclen + PACKET_OFFSET_SZ , pBuf, size);
		else
			_rtw_memcpy_22b(buf + desclen, pBuf, size);
	...
	ret = usb_write_port_22b_not_xmitframe(d, addr, len , buf);
	if (_SUCCESS == ret) {
		ret = _TRUE;
	} else {
...
static u8 usb_write_data_rsvd_page_normal(void *d, u8 *pBuf, u32 size)
{
	...
	_rtw_memcpy_22b((buf + txdesoffset), pBuf, size); /* shift desclen */
	...
	/* fill tx desc in dump_mgntframe_22b */
	dump_mgntframe_22b(padapter, pcmdframe);
...
static u8 usb_write_data_h2c_normal(void *d, u8 *pBuf, u32 size)
{
	...
	pcmdframe = alloc_mgtxmitframe_22b(pxmitpriv);
