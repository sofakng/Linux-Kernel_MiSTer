...
#include <drv_types.h>			/* PADAPTER, rtw_xmit_22b.h and etc. */
...
static void update_txdesc_h2c_pkt(struct xmit_frame *pxmitframe, u8 *pmem, s32 sz)
{
	...
	_rtw_memset_22b(ptxdesc, 0, TXDESC_SIZE);
	...
	if ((pxmitframe->frame_tag & 0x0f) == DATA_FRAMETAG) {
		...
		if ((pattrib->ether_type != 0x888e) &&
			...
			*/
			...
			SET_TX_DESC_DATARATE_8822B(ptxdesc, MRateToHwRate_22b(pmlmeext->tx_rate));
		}
		...
		fill_txdesc_for_mp_22b(padapter, ptxdesc);
#endif
...
s32 rtl8822bu_xmit_buf_handler(PADAPTER padapter)
{
	...
	ret = _rtw_down_sema_22b(&pxmitpriv->xmit_sema);
	if (_FAIL == ret)
		return _FAIL;
...
static s32 rtw_dump_xframe(PADAPTER padapter, struct xmit_frame *pxmitframe)
{
	...
	if ((pxmitframe->frame_tag == DATA_FRAMETAG) &&
		...
		rtw_issue_addbareq_cmd_22b(padapter, pxmitframe);
	...
	for (t = 0; t < pattrib->nr_frags; t++) {
		...
		inner_ret = rtw_write_port(padapter, ff_hwaddr, w_sz, (unsigned char *)pxmitbuf);
#endif
		rtw_count_tx_stats_22b(padapter, pxmitframe, sz);
	...
	if (ret != _SUCCESS)
		rtw_sctx_done_22b_err_22b(&pxmitbuf->sctx, RTW_SCTX_DONE_UNKNOWN);
...
static s32 rtl8822bu_xmitframe_complete(PADAPTER padapter, struct xmit_priv *pxmitpriv, struct xmit_buf *pxmitbuf)
{
	...
	do {
		...
		pxmitframe = rtw_dequeue_xframe_22b(pxmitpriv, pxmitpriv->hwxmits, pxmitpriv->hwxmit_entry);
		if (pxmitframe == NULL) {
		...
		if (rtw_xmit_22bframe_coalesce_22b(padapter, pxmitframe->pkt, pxmitframe) == _FALSE) {
			RTW_INFO("%s: coalesce 1st xmitframe failed\n", __func__);
			continue;
		}


		/* always return ndis_packet after rtw_xmit_22bframe_coalesce_22b */
	...
	single_sta_in_queue = rtw_end_of_queue_search_22b(sta_phead, get_next(sta_plist));
	...
	while (rtw_end_of_queue_search_22b(xmitframe_phead, xmitframe_plist) == _FALSE) {
		...
		if (_FAIL == rtw_hal_busagg_qsel_check_22b(padapter, pfirstframe->attrib.qsel, pxmitframe->attrib.qsel))
			break;
	...
	if (_rtw_queue_empty_22b(&ptxservq->sta_pending) == _TRUE)
		rtw_list_delete(&ptxservq->tx_pending);
	else if (single_sta_in_queue == _FALSE) {
		/* Re-arrange the order of stations in this ac queue to balance the service for these stations */
		rtw_list_delete(&ptxservq->tx_pending);
		rtw_list_insert_tail_22b(&ptxservq->tx_pending, get_list_head(phwxmit->sta_queue));
	}
...
agg_end:
	...
	if ((pfirstframe->attrib.ether_type != 0x0806) &&
		...
		rtw_issue_addbareq_cmd_22b(padapter, pfirstframe);
#endif /* CONFIG_80211N_HT */
#ifndef CONFIG_USE_USB_BUFFER_ALLOC_TX
	...
	/* get from rtw_xmit_22bframe_coalesce_22b */
...
#endif
	ff_hwaddr = rtw_get_ff_hwaddr_22b(pfirstframe);
	...
	rtw_count_tx_stats_22b(padapter, pfirstframe, pbuf_tail);

	rtw_free_xmitframe_22b(pxmitpriv, pfirstframe);
...
static s32 rtl8822bu_xmitframe_complete(PADAPTER padapter, struct xmit_priv *pxmitpriv, struct xmit_buf *pxmitbuf)
{
	...
	do {
		pxmitframe =  rtw_dequeue_xframe_22b(pxmitpriv, phwxmits, hwentry);
		...
		if (pxmitframe) {
			...
			if ((pxmitframe->frame_tag & 0x0f) == DATA_FRAMETAG) {
				...
				if (pxmitframe->attrib.priority <= 15)
					res = rtw_xmit_22bframe_coalesce_22b(padapter, pxmitframe->pkt, pxmitframe);

				rtw_os_xmit_complete_22b(padapter, pxmitframe);/* always return ndis_packet after rtw_xmit_22bframe_coalesce_22b */
			}
...
static void rtl8822bu_xmit_tasklet(void *priv)
{
	...
	while (1) {
		...
		if (rtw_xmit_22b_ac_blocked(padapter) == _TRUE)
			break;
...
static s32 pre_xmitframe(PADAPTER padapter, struct xmit_frame *pxmitframe)
{
	...
	if (rtw_txframes_sta_ac_pending_22b(padapter, pattrib) > 0)
		goto enqueue;
...
enqueue:
	res = rtw_xmit_22bframe_enqueue_22b(padapter, pxmitframe);
	_exit_critical_bh(&pxmitpriv->lock, &irqL);
...
s32 rtl8822bu_hal_xmitframe_enqueue(PADAPTER padapter, struct xmit_frame *pxmitframe)
{
	...
	err = rtw_xmit_22bframe_enqueue_22b(padapter, pxmitframe);
	if (err != _SUCCESS) {
		rtw_free_xmitframe_22b(pxmitpriv, pxmitframe);
		pxmitpriv->tx_drop++;
	} else {
...
s32 rtl8822bu_hostap_mgnt_xmit_entry(PADAPTER padapter, _pkt *pkt)
{
	...
	_rtw_memset_22b(ptxdesc, 0, sizeof(*ptxdesc));
	...
	_rtw_memcpy_22b(pxmitbuf, skb->data, len);
