...
u8 rtl8822b_phy_init_mac_register(PADAPTER adapter)
{
	...
	if (_FALSE == ret) {
		status = odm_config_mac_with_header_file_22b(&hal->odmpriv);
...
#endif
	if (_FALSE == ret) {
		status = odm_config_bb_with_header_file_22b(&hal->odmpriv, CONFIG_BB_PHY_REG);
		if (HAL_STATUS_SUCCESS == status)
			ret = _TRUE;
	}
	if (_FALSE == ret) {
...
#endif
		if (_FALSE == ret) {
			status = odm_config_bb_with_header_file_22b(&hal->odmpriv, CONFIG_BB_PHY_REG_MP);
			if (HAL_STATUS_SUCCESS == status)
				ret = _TRUE;
		}
		if (_FALSE == ret) {
...
#endif
	if (_FALSE == ret) {
		status = odm_config_bb_with_header_file_22b(&hal->odmpriv, CONFIG_BB_AGC_TAB);
		if (HAL_STATUS_SUCCESS == status)
			ret = _TRUE;
	}
	if (_FALSE == ret) {
...
static u8 init_bb_reg(PADAPTER adapter)
{
	...
	hal_set_crystal_cap_22b(adapter, hal->crystal_cap);
...
#endif
		if (_FALSE == ret) {
			status = odm_config_rf_with_header_file_22b(&hal->odmpriv, CONFIG_RF_RADIO, phydm_path);
			if (HAL_STATUS_SUCCESS != status)
				goto exit;
			ret = _TRUE;
		}
...
#endif
	if (_FALSE == ret) {
		status = odm_config_rf_with_tx_pwr_track_header_file_22b(&hal->odmpriv);
		if (HAL_STATUS_SUCCESS != status) {
...
static void init_phydm_cominfo(PADAPTER adapter)
{
	...
	odm_cmn_info_init_22b(p_dm_odm, ODM_CMNINFO_PACKAGE_TYPE, hal->PackageType);
	odm_cmn_info_init_22b(p_dm_odm, ODM_CMNINFO_IC_TYPE, ODM_RTL8822B);
	...
	odm_cmn_info_init_22b(p_dm_odm, ODM_CMNINFO_FAB_VER, fab_ver);
	odm_cmn_info_init_22b(p_dm_odm, ODM_CMNINFO_CUT_VER, cut_ver);
...
void rtl8822b_phy_init_dm_priv(PADAPTER adapter)
{
	...
	odm_init_all_timers_22b(podmpriv);
}
...
void rtl8822b_phy_deinit_dm_priv(PADAPTER adapter)
{
	...
	odm_cancel_all_timers_22b(podmpriv);
}
...
void rtl8822b_phy_haldm_watchdog(PADAPTER adapter)
{
	...
	goto skip_dm;
#endif
	rtw_phydm_watchdog_22b_22b(adapter);
...
void rtl8822b_switch_chnl_and_set_bw(PADAPTER adapter)
{
	...
	if (switch_band) {
		...
		if (hal->EEPROMBluetoothCoexist) {
			...
			rtw_btcoex_wifionly_switchband_notify_22b(adapter);
	...
	phydm_config_kfree_22b(p_dm_odm, hal->current_channel);

	/* TX Power Setting */
	odm_clear_txpowertracking_state_22b(p_dm_odm);
