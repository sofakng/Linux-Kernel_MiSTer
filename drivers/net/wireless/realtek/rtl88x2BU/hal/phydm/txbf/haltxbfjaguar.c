...
)
{
	...
	odm_write_1byte_22b(dm, REG_NDPA_OPT_CTRL_8812A, (rate << 2 | BW));
...
)
{
	...
	odm_set_rf_reg_22b(dm, RF_PATH_A, 0xef, 0x80000, 0x1);	/*RF mode table write enable*/
	odm_set_rf_reg_22b(dm, RF_PATH_B, 0xef, 0x80000, 0x1);	/*RF mode table write enable*/

	if (beam_info->beamformee_su_cnt > 0) {
		/* Paath_A */
		odm_set_rf_reg_22b(dm, RF_PATH_A, 0x30, 0x78000, 0x3);		/*Select RX mode*/
		odm_set_rf_reg_22b(dm, RF_PATH_A, 0x31, 0xfffff, 0x3F7FF);	/*Set Table data*/
		odm_set_rf_reg_22b(dm, RF_PATH_A, 0x32, 0xfffff, 0xE26BF);	/*Enable TXIQGEN in RX mode*/
		/* Path_B */
		odm_set_rf_reg_22b(dm, RF_PATH_B, 0x30, 0x78000, 0x3);		/*Select RX mode*/
		odm_set_rf_reg_22b(dm, RF_PATH_B, 0x31, 0xfffff, 0x3F7FF);	/*Set Table data*/
		odm_set_rf_reg_22b(dm, RF_PATH_B, 0x32, 0xfffff, 0xE26BF);	/*Enable TXIQGEN in RX mode*/
	} else {
		/* Paath_A */
		odm_set_rf_reg_22b(dm, RF_PATH_A, 0x30, 0x78000, 0x3);		/*Select RX mode*/
		odm_set_rf_reg_22b(dm, RF_PATH_A, 0x31, 0xfffff, 0x3F7FF);	/*Set Table data*/
		odm_set_rf_reg_22b(dm, RF_PATH_A, 0x32, 0xfffff, 0xC26BF);	/*Disable TXIQGEN in RX mode*/
		/* Path_B */
		odm_set_rf_reg_22b(dm, RF_PATH_B, 0x30, 0x78000, 0x3);		/*Select RX mode*/
		odm_set_rf_reg_22b(dm, RF_PATH_B, 0x31, 0xfffff, 0x3F7FF);	/*Set Table data*/
		odm_set_rf_reg_22b(dm, RF_PATH_B, 0x32, 0xfffff, 0xC26BF);	/*Disable TXIQGEN in RX mode*/
	}

	odm_set_rf_reg_22b(dm, RF_PATH_A, 0xef, 0x80000, 0x0);	/*RF mode table write disable*/
	odm_set_rf_reg_22b(dm, RF_PATH_B, 0xef, 0x80000, 0x0);	/*RF mode table write disable*/

	if (beam_info->beamformee_su_cnt > 0)
		odm_set_bb_reg_22b(dm, 0x80c, MASKBYTE1, 0x33);
	else
		odm_set_bb_reg_22b(dm, 0x80c, MASKBYTE1, 0x11);
}
...
)
{
	...
	*dm->is_fw_dw_rsvd_page_in_progress = true;
	...
	phydm_get_hal_def_var_handler_interface_22b(dm, HAL_DEF_TX_PAGE_BOUNDARY, (u8 *)&tx_page_bndy);

	/*Set REG_CR bit 8. DMA beacon by SW.*/
	u1b_tmp = odm_read_1byte_22b(dm, REG_CR_8812A + 1);
	odm_write_1byte_22b(dm,  REG_CR_8812A + 1, (u1b_tmp | BIT(0)));


	/*Set FWHW_TXQ_CTRL 0x422[6]=0 to tell Hw the packet is not a real beacon frame.*/
	tmp_reg422 = odm_read_1byte_22b(dm, REG_FWHW_TXQ_CTRL_8812A + 2);
	odm_write_1byte_22b(dm, REG_FWHW_TXQ_CTRL_8812A + 2,  tmp_reg422 & (~BIT(6)));
	...
	odm_write_1byte_22b(dm, REG_TDECTRL_8812A + 1, head_page);

	do {
		/*Clear beacon valid check bit.*/
		bcn_valid_reg = odm_read_1byte_22b(dm, REG_TDECTRL_8812A + 2);
		odm_write_1byte_22b(dm, REG_TDECTRL_8812A + 2, (bcn_valid_reg | BIT(0)));
	...
	/*TDECTRL[15:8] 0x209[7:0] = 0xF6	Beacon Head for TXDMA*/
	odm_write_1byte_22b(dm, REG_TDECTRL_8812A + 1, tx_page_bndy);
	...
	if (is_send_beacon_22b)
		odm_write_1byte_22b(dm, REG_FWHW_TXQ_CTRL_8812A + 2, tmp_reg422);
...
)
{
	...
	if (dm->rf_type == RF_2T2R)
		odm_set_bb_reg_22b(dm, ODM_REG_CSI_CONTENT_VALUE, MASKDWORD, 0x00000000);	/*nc =2*/
	else
		odm_set_bb_reg_22b(dm, ODM_REG_CSI_CONTENT_VALUE, MASKDWORD, 0x01081008);	/*nc =1*/
	...
	if ((beamforming_info->beamformer_su_cnt > 0) && (bfer_idx < BEAMFORMER_ENTRY_NUM)) {
		...
		odm_write_1byte_22b(dm, REG_SND_PTCL_CTRL_8812A, 0xCB);

		/*MAC address/Partial AID of Beamformer*/
		if (bfer_idx == 0) {
			for (i = 0; i < 6 ; i++)
				odm_write_1byte_22b(dm, (REG_BFMER0_INFO_8812A + i), beamformer_entry.mac_addr[i]);
			...
			for (i = 0; i < 6 ; i++)
				odm_write_1byte_22b(dm, (REG_BFMER1_INFO_8812A + i), beamformer_entry.mac_addr[i]);
			/*CSI report use legacy ofdm so don't need to fill P_AID.*/
		...
		odm_write_4byte_22b(dm, REG_CSI_RPT_PARAM_BW20_8812A, csi_param);
		odm_write_4byte_22b(dm, REG_CSI_RPT_PARAM_BW40_8812A, csi_param);
		odm_write_4byte_22b(dm, REG_CSI_RPT_PARAM_BW80_8812A, csi_param);

		/*Timeout value for MAC to leave NDP_RX_standby_state (60 us, Test chip) (80 us,  MP chip)*/
		odm_write_1byte_22b(dm, REG_SND_PTCL_CTRL_8812A + 3, 0x50);
	}
	...
	if ((beamforming_info->beamformee_su_cnt > 0) && (bfee_idx < BEAMFORMEE_ENTRY_NUM)) {
		...
		if (bfee_idx == 0) {
			odm_write_2byte_22b(dm, REG_TXBF_CTRL_8812A, sta_id);
			odm_write_1byte_22b(dm, REG_TXBF_CTRL_8812A + 3, odm_read_1byte_22b(dm, REG_TXBF_CTRL_8812A + 3) | BIT(4) | BIT(6) | BIT(7));
		} else
			odm_write_2byte_22b(dm, REG_TXBF_CTRL_8812A + 2, sta_id | BIT(12) | BIT(14) | BIT(15));

		/*CSI report parameters of Beamformee*/
		if (bfee_idx == 0) {
			/*Get BIT24 & BIT25*/
			u8	tmp = odm_read_1byte_22b(dm, REG_BFMEE_SEL_8812A + 3) & 0x3;

			odm_write_1byte_22b(dm, REG_BFMEE_SEL_8812A + 3, tmp | 0x60);
			odm_write_2byte_22b(dm, REG_BFMEE_SEL_8812A, sta_id | BIT(9));
		} else {
			/*Set BIT25*/
			odm_write_2byte_22b(dm, REG_BFMEE_SEL_8812A + 2, sta_id | 0xE200);
		}
...
)
{
	...
	if (beamformer_entry.beamform_entry_cap == BEAMFORMING_CAP_NONE) {
		odm_write_1byte_22b(dm, REG_SND_PTCL_CTRL_8812A, 0xC8);
		if (idx == 0) {
			odm_write_4byte_22b(dm, REG_BFMER0_INFO_8812A, 0);
			odm_write_2byte_22b(dm, REG_BFMER0_INFO_8812A + 4, 0);
			odm_write_2byte_22b(dm, REG_CSI_RPT_PARAM_BW20_8812A, 0);
			odm_write_2byte_22b(dm, REG_CSI_RPT_PARAM_BW40_8812A, 0);
			odm_write_2byte_22b(dm, REG_CSI_RPT_PARAM_BW80_8812A, 0);
		} else {
			odm_write_4byte_22b(dm, REG_BFMER1_INFO_8812A, 0);
			odm_write_2byte_22b(dm, REG_BFMER1_INFO_8812A + 4, 0);
			odm_write_2byte_22b(dm, REG_CSI_RPT_PARAM_BW20_8812A, 0);
			odm_write_2byte_22b(dm, REG_CSI_RPT_PARAM_BW40_8812A, 0);
			odm_write_2byte_22b(dm, REG_CSI_RPT_PARAM_BW80_8812A, 0);
		}
	...
	if (beamformee_entry.beamform_entry_cap == BEAMFORMING_CAP_NONE) {
		hal_txbf_jaguar_rf_mode(dm, beamforming_info);
		if (idx == 0) {
			odm_write_2byte_22b(dm, REG_TXBF_CTRL_8812A, 0x0);
			odm_write_2byte_22b(dm, REG_BFMEE_SEL_8812A, 0);
		} else {
			odm_write_2byte_22b(dm, REG_TXBF_CTRL_8812A + 2, odm_read_2byte_22b(dm, REG_TXBF_CTRL_8812A + 2) & 0xF000);
			odm_write_2byte_22b(dm, REG_BFMEE_SEL_8812A + 2, odm_read_2byte_22b(dm, REG_BFMEE_SEL_8812A + 2) & 0x60);
		}
	}
...
)
{
	...
	/*Stop PCIe TxDMA*/
	odm_write_1byte_22b(dm, REG_PCIE_CTRL_REG_8812A + 1, 0xFE);
#endif
	...
	for (count = 0; count < 100; count++) {
		u2btmp = odm_read_2byte_22b(dm, REG_TXPKT_EMPTY_8812A);
		u2btmp = u2btmp & 0xfff;
		if (u2btmp != 0xfff) {
			ODM_delay_ms_22b(10);
			continue;
	...
	/*TX pause*/
	odm_write_1byte_22b(dm, REG_TXPAUSE_8812A, 0xFF);

	/*Wait TX state Machine OK*/
	for (count = 0; count < 100; count++) {
		if (odm_read_4byte_22b(dm, REG_SCH_TXCMD_8812A) != 0)
			continue;
		else
			break;
	}
	...
	/*Stop RX DMA path*/
	u1btmp = odm_read_1byte_22b(dm, REG_RXDMA_CONTROL_8812A);
	odm_write_1byte_22b(dm, REG_RXDMA_CONTROL_8812A, u1btmp | BIT(2));
	...
	/*Disable clock*/
	odm_write_1byte_22b(dm, REG_SYS_CLKR_8812A + 1, 0xf0);
	/*Disable 320M*/
	odm_write_1byte_22b(dm, REG_AFE_PLL_CTRL_8812A + 3, 0x8);
	/*Enable 320M*/
	odm_write_1byte_22b(dm, REG_AFE_PLL_CTRL_8812A + 3, 0xa);
	/*Enable clock*/
	odm_write_1byte_22b(dm, REG_SYS_CLKR_8812A + 1, 0xfc);


	/*Release Tx pause*/
	odm_write_1byte_22b(dm, REG_TXPAUSE_8812A, 0);
...
#if DEV_BUS_TYPE == RT_PCI_INTERFACE
	/*Enable PCIe TxDMA*/
	odm_write_1byte_22b(dm, REG_PCIE_CTRL_REG_8812A + 1, 0);
#endif
	/*Start Usb TxDMA*/
