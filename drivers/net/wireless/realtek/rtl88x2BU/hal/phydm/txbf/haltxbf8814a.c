...
)
{
	...
	for (i = RF_PATH_A ; i < MAX_RF_PATH ; i++)
		odm_set_rf_reg_22b(dm, i, RF_WE_LUT, 0x80000, 0x1);	/*RF mode table write enable*/

	while (1) {
		counter++;
		for (i = RF_PATH_A; i < MAX_RF_PATH; i++)
			odm_set_rf_reg_22b(dm, i, RF_RCK_OS, 0xfffff, 0x18000);	/*Select Rx mode*/

		ODM_delay_us_22b(2);

		for (i = RF_PATH_A; i < MAX_RF_PATH; i++)
			rf_mode[i] = odm_get_rf_reg_22b(dm, i, RF_RCK_OS, 0xfffff);
	...
	for (i = RF_PATH_A ; i < MAX_RF_PATH ; i++) {
		odm_set_rf_reg_22b(dm, i, RF_TXPA_G1, 0xfffff, 0xBE77F); /*Set Table data*/
		odm_set_rf_reg_22b(dm, i, RF_TXPA_G2, 0xfffff, 0x226BF); /*Enable TXIQGEN in Rx mode*/
	}
	odm_set_rf_reg_22b(dm, RF_PATH_A, RF_TXPA_G2, 0xfffff, 0xE26BF); /*Enable TXIQGEN in Rx mode*/

	for (i = RF_PATH_A; i < MAX_RF_PATH; i++)
		odm_set_rf_reg_22b(dm, i, RF_WE_LUT, 0x80000, 0x0);	/*RF mode table write disable*/
...
)
{
	...
	odm_write_1byte_22b(dm, REG_NDPA_OPT_CTRL_8814A, BW);
	odm_write_1byte_22b(dm, REG_NDPA_RATE_8814A, (u8) rate);
...
)
{
	...
	odm_write_2byte_22b(dm, REG_PKTBUF_DBG_CTRL_8814A, PHYDM_CTRL_INFO_PAGE);
	...
	for (i = 0; i < data_len; i++)
		*(data + i) = odm_read_2byte_22b(dm, x_read_data_addr + i);
...
)
{
	...
	if ((beamforming_info->last_usb_hub) != (*dm->hub_usb_mode)) {
		...
		if (tx_ss == 0xf) {
			odm_set_bb_reg_22b(dm, REG_BB_TX_PATH_SEL_1_8814A, MASKBYTE3 | MASKBYTE2HIGHNIBBLE, 0x93f);
			odm_set_bb_reg_22b(dm, REG_BB_TX_PATH_SEL_1_8814A, MASKDWORD, 0x93f93f0);
		} else if (tx_ss == 0xe) {
			odm_set_bb_reg_22b(dm, REG_BB_TX_PATH_SEL_1_8814A, MASKBYTE3 | MASKBYTE2HIGHNIBBLE, 0x93e);
			odm_set_bb_reg_22b(dm, REG_BB_TX_PATH_SEL_2_8814A, MASKDWORD, 0x93e93e0);
		} else if (tx_ss == 0x6) {
			odm_set_bb_reg_22b(dm, REG_BB_TX_PATH_SEL_1_8814A, MASKBYTE3 | MASKBYTE2HIGHNIBBLE, 0x936);
			odm_set_bb_reg_22b(dm, REG_BB_TX_PATH_SEL_2_8814A, MASKLWORD, 0x9360);
		}
		...
		if (idx == 0) {
			...
			case 1:			/*Nsts = 2	BC*/
				odm_set_bb_reg_22b(dm, REG_BB_TXBF_ANT_SET_BF0_8814A, MASKBYTE3LOWNIBBLE | MASKL3BYTES, 0x9366);		/*tx2path, BC*/
				break;

			case 2:			/*Nsts = 3	BCD*/
				odm_set_bb_reg_22b(dm, REG_BB_TXBF_ANT_SET_BF0_8814A, MASKBYTE3LOWNIBBLE | MASKL3BYTES, 0x93e93ee);	/*tx3path, BCD*/
				break;

			default:			/*nr>3, same as Case 3*/
				odm_set_bb_reg_22b(dm, REG_BB_TXBF_ANT_SET_BF0_8814A, MASKBYTE3LOWNIBBLE | MASKL3BYTES, 0x93f93ff);	/*tx4path, ABCD*/
				break;
			}
		} else	{
			switch (nr_index) {
			...
			case 1:			/*Nsts = 2	BC*/
				odm_set_bb_reg_22b(dm, REG_BB_TXBF_ANT_SET_BF1_8814A, MASKBYTE3LOWNIBBLE | MASKL3BYTES, 0x9366);		/*tx2path, BC*/
				break;

			case 2:			/*Nsts = 3	BCD*/
				odm_set_bb_reg_22b(dm, REG_BB_TXBF_ANT_SET_BF1_8814A, MASKBYTE3LOWNIBBLE | MASKL3BYTES, 0x93e93ee);	/*tx3path, BCD*/
				break;

			default:			/*nr>3, same as Case 3*/
				odm_set_bb_reg_22b(dm, REG_BB_TXBF_ANT_SET_BF1_8814A, MASKBYTE3LOWNIBBLE | MASKL3BYTES, 0x93f93ff);	/*tx4path, ABCD*/
				break;
			}
		}
...
)
{
	...
	if (beamforming_info->beamformee_su_cnt > 0) {
		...
		odm_set_bb_reg_22b(dm, REG_BB_TXBF_ANT_SET_BF1_8814A, BIT(28) | BIT29, 0x2);			/*enable BB TxBF ant mapping register*/
		odm_set_bb_reg_22b(dm, REG_BB_TXBF_ANT_SET_BF1_8814A, BIT30, 0x1);			/*if Nsts > Nc don't apply V matrix*/
	...
	if ((beamforming_info->beamformee_su_cnt == 0) && (beamforming_info->beamformer_su_cnt == 0)) {
		odm_set_bb_reg_22b(dm, REG_BB_TX_PATH_SEL_1_8814A, MASKBYTE3 | MASKBYTE2HIGHNIBBLE, 0x932);	/*set tx_path selection for 8814a BFer bug refine*/
		odm_set_bb_reg_22b(dm, REG_BB_TX_PATH_SEL_2_8814A, MASKDWORD, 0x93e9360);
	}
}
#if 0
void
...
)
{
	...
	*dm->is_fw_dw_rsvd_page_in_progress = true;
	...
	u1b_tmp = odm_read_1byte_22b(dm, REG_CR_8814A + 1);
	odm_write_1byte_22b(dm,  REG_CR_8814A + 1, (u1b_tmp | BIT(0)));


	/*Set FWHW_TXQ_CTRL 0x422[6]=0 to tell Hw the packet is not a real beacon frame.*/
	tmp_reg422 = odm_read_1byte_22b(dm, REG_FWHW_TXQ_CTRL_8814A + 2);
	odm_write_1byte_22b(dm, REG_FWHW_TXQ_CTRL_8814A + 2,  tmp_reg422 & (~BIT(6)));
	...
	odm_write_2byte_22b(dm, REG_FIFOPAGE_CTRL_2_8814A, head_page);

	do {
		/*Clear beacon valid check bit.*/
		bcn_valid_reg = odm_read_1byte_22b(dm, REG_FIFOPAGE_CTRL_2_8814A + 1);
		odm_write_1byte_22b(dm, REG_FIFOPAGE_CTRL_2_8814A + 1, (bcn_valid_reg | BIT(7)));
		...
		while (!(bcn_valid_reg & BIT(7)) && count < 20) {
			count++;
			ODM_delay_ms_22b(10);
			bcn_valid_reg = odm_read_1byte_22b(dm, REG_FIFOPAGE_CTRL_2_8814A + 2);
		}
		dl_bcn_count++;
	...
	/*0x204[11:0]	Beacon Head for TXDMA*/
	odm_write_2byte_22b(dm, REG_FIFOPAGE_CTRL_2_8814A, tx_page_bndy);
	...
	if (is_send_beacon_22b)
		odm_write_1byte_22b(dm, REG_FWHW_TXQ_CTRL_8814A + 2, tmp_reg422);
...
)
{
	...
	odm_fill_h2c_cmd_22b(dm, PHYDM_H2C_TXBF, 3, u1_tx_bf_parm);
...
)
{
	...
	odm_set_mac_reg_22b(dm, REG_SND_PTCL_CTRL_8814A, MASKBYTE1 | MASKBYTE2, 0x0202);
	...
	if ((beamforming_info->beamformer_su_cnt > 0) && (bfer_idx < BEAMFORMER_ENTRY_NUM)) {
		...
		odm_write_1byte_22b(dm, REG_SND_PTCL_CTRL_8814A, 0xDB);

		/*MAC address/Partial AID of Beamformer*/
		if (bfer_idx == 0) {
			for (i = 0; i < 6 ; i++)
				odm_write_1byte_22b(dm, (REG_ASSOCIATED_BFMER0_INFO_8814A + i), beamformer_entry.mac_addr[i]);
		} else {
			for (i = 0; i < 6 ; i++)
				odm_write_1byte_22b(dm, (REG_ASSOCIATED_BFMER1_INFO_8814A + i), beamformer_entry.mac_addr[i]);
		}
		...
		if (bfer_idx == 0)
			odm_write_2byte_22b(dm, REG_CSI_RPT_PARAM_BW20_8814A, csi_param);
		else
			odm_write_2byte_22b(dm, REG_CSI_RPT_PARAM_BW20_8814A + 2, csi_param);
		/*ndp_rx_standby_timer, 8814 need > 0x56, suggest from Dvaid*/
		odm_write_1byte_22b(dm, REG_SND_PTCL_CTRL_8814A + 3, 0x40);
	...
	if ((beamforming_info->beamformee_su_cnt > 0) && (bfee_idx < BEAMFORMEE_ENTRY_NUM)) {
		...
		if (bfee_idx == 0) {
			odm_write_2byte_22b(dm, REG_TXBF_CTRL_8814A, sta_id);
			odm_write_1byte_22b(dm, REG_TXBF_CTRL_8814A + 3, odm_read_1byte_22b(dm, REG_TXBF_CTRL_8814A + 3) | BIT(4) | BIT(6) | BIT(7));
		} else
			odm_write_2byte_22b(dm, REG_TXBF_CTRL_8814A + 2, sta_id | BIT(14) | BIT(15) | BIT(12));

		/*CSI report parameters of Beamformee*/
		if (bfee_idx == 0) {
			/*Get BIT24 & BIT25*/
			u8	tmp = odm_read_1byte_22b(dm, REG_ASSOCIATED_BFMEE_SEL_8814A + 3) & 0x3;

			odm_write_1byte_22b(dm, REG_ASSOCIATED_BFMEE_SEL_8814A + 3, tmp | 0x60);
			odm_write_2byte_22b(dm, REG_ASSOCIATED_BFMEE_SEL_8814A, sta_id | BIT(9));
		} else
			odm_write_2byte_22b(dm, REG_ASSOCIATED_BFMEE_SEL_8814A + 2, sta_id | 0xE200);	/*Set BIT25*/
...
)
{
	...
	if (beamformer_entry.beamform_entry_cap == BEAMFORMING_CAP_NONE) {
		odm_write_1byte_22b(dm, REG_SND_PTCL_CTRL_8814A, 0xD8);
		if (idx == 0) {
			odm_write_4byte_22b(dm, REG_ASSOCIATED_BFMER0_INFO_8814A, 0);
			odm_write_2byte_22b(dm, REG_ASSOCIATED_BFMER0_INFO_8814A + 4, 0);
			odm_write_2byte_22b(dm, REG_CSI_RPT_PARAM_BW20_8814A, 0);
		} else {
			odm_write_4byte_22b(dm, REG_ASSOCIATED_BFMER1_INFO_8814A, 0);
			odm_write_2byte_22b(dm, REG_ASSOCIATED_BFMER1_INFO_8814A + 4, 0);
			odm_write_2byte_22b(dm, REG_CSI_RPT_PARAM_BW20_8814A + 2, 0);
		}
	}
	...
	if (beamformee_entry.beamform_entry_cap == BEAMFORMING_CAP_NONE) {
		...
		if (idx == 0) {
			odm_write_2byte_22b(dm, REG_TXBF_CTRL_8814A, 0x0);
			odm_write_1byte_22b(dm, REG_TXBF_CTRL_8814A + 3, odm_read_1byte_22b(dm, REG_TXBF_CTRL_8814A + 3) | BIT(4) | BIT(6) | BIT(7));
			odm_write_2byte_22b(dm, REG_ASSOCIATED_BFMEE_SEL_8814A, 0);
		} else {
			odm_write_2byte_22b(dm, REG_TXBF_CTRL_8814A + 2, 0x0 | BIT(14) | BIT(15) | BIT(12));

			odm_write_2byte_22b(dm, REG_ASSOCIATED_BFMEE_SEL_8814A + 2, odm_read_2byte_22b(dm, REG_ASSOCIATED_BFMEE_SEL_8814A + 2) & 0x60);
		}
	}
}
...
)
{
	...
	tmp_val = odm_read_2byte_22b(dm, REG_TXBF_CTRL_8814A);
	odm_write_2byte_22b(dm, REG_TXBF_CTRL_8814A, tmp_val | BIT(15));
