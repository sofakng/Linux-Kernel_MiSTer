...
)
{
	...
	for (i = RF_PATH_A; i < RF_PATH_B; i++) {
		odm_set_rf_reg_22b(dm, (enum rf_path)i, rf_welut_jaguar, 0x80000, 0x1);
		/*RF mode table write enable*/
	}

	if ((beamforming_info->beamformee_su_cnt > 0) || (beamforming_info->beamformee_mu_cnt > 0)) {
		for (i = RF_PATH_A; i < RF_PATH_B; i++) {
			odm_set_rf_reg_22b(dm, (enum rf_path)i, rf_mode_table_addr, 0xfffff, 0x18000);
			/*Select RX mode*/
			odm_set_rf_reg_22b(dm, (enum rf_path)i, rf_mode_table_data0, 0xfffff, 0xBE77F);
			/*Set Table data*/
			odm_set_rf_reg_22b(dm, (enum rf_path)i, rf_mode_table_data1, 0xfffff, 0x226BF);
			/*Enable TXIQGEN in RX mode*/
		}
		odm_set_rf_reg_22b(dm, RF_PATH_A, rf_mode_table_data1, 0xfffff, 0xE26BF);
		/*Enable TXIQGEN in RX mode*/
	}

	for (i = RF_PATH_A; i < RF_PATH_B; i++) {
		odm_set_rf_reg_22b(dm, (enum rf_path)i, rf_welut_jaguar, 0x80000, 0x0);
	...
	if (beamforming_info->beamformee_su_cnt > 0) {
		...
		odm_set_bb_reg_22b(dm, REG_BB_TXBF_ANT_SET_BF1_8822B, BIT(28) | BIT29, 0x2);			/*enable BB TxBF ant mapping register*/

		if (idx == 0) {
			/*Nsts = 2	AB*/
			odm_set_bb_reg_22b(dm, REG_BB_TXBF_ANT_SET_BF0_8822B, 0xffff, 0x0433);
			odm_set_bb_reg_22b(dm, REG_BB_TX_PATH_SEL_1_8822B, 0xfff00000, 0x043);
			/*odm_set_bb_reg_22b(dm, REG_BB_TX_PATH_SEL_2, MASKLWORD, 0x430);*/

		} else {/*IDX =1*/
			odm_set_bb_reg_22b(dm, REG_BB_TXBF_ANT_SET_BF1_8822B, 0xffff, 0x0433);
			odm_set_bb_reg_22b(dm, REG_BB_TX_PATH_SEL_1_8822B, 0xfff00000, 0x043);
			/*odm_set_bb_reg_22b(dm, REG_BB_TX_PATH_SEL_2, MASKLWORD, 0x430;*/
		}
	} else {
		odm_set_bb_reg_22b(dm, REG_BB_TX_PATH_SEL_1_8822B, 0xfff00000, 0x1); /*1SS by path-A*/
		odm_set_bb_reg_22b(dm, REG_BB_TX_PATH_SEL_2_8822B, MASKLWORD, 0x430); /*2SS by path-A,B*/
	}

	if (beamforming_info->beamformee_mu_cnt > 0) {
		/*MU STAs share the common setting*/
		odm_set_bb_reg_22b(dm, REG_BB_TXBF_ANT_SET_BF1_8822B, BIT(31), 1);
		odm_set_bb_reg_22b(dm, REG_BB_TXBF_ANT_SET_BF1_8822B, 0xffff, 0x0433);
		odm_set_bb_reg_22b(dm, REG_BB_TX_PATH_SEL_1_8822B, 0xfff00000, 0x043);
	}
#endif
...
)
{
	...
	phydm_get_hal_def_var_handler_interface_22b(dm, HAL_DEF_TX_PAGE_BOUNDARY, (u16 *)&tx_page_bndy);
...
)
{
	...
	odm_set_bb_reg_22b(dm, 0x14c0, BIT(16), 1); /*Enable P1 aggr new packet according to P0 transfer time*/
	odm_set_bb_reg_22b(dm, 0x14c0, BIT(15) | BIT14 | BIT13 | BIT12, 10); /*MU Retry Limit*/
	odm_set_bb_reg_22b(dm, 0x14c0, BIT(7), 0); /*Disable Tx MU-MIMO until sounding done*/
	odm_set_bb_reg_22b(dm, 0x14c0, 0x3F, 0); /* Clear validity of MU STAs */
	odm_write_1byte_22b(dm, 0x167c, 0x70); /*MU-MIMO Option as default value*/
	odm_write_2byte_22b(dm, 0x1680, 0); /*MU-MIMO Control as default value*/

	/* Set MU NDPA rate & BW source */
	/* 0x42C[30] = 1 (0: from Tx desc, 1: from 0x45F) */
	u1b_tmp = odm_read_1byte_22b(dm, 0x42C);
	odm_write_1byte_22b(dm, REG_TXBF_CTRL_8822B, (u1b_tmp | BIT(6)));
	/* 0x45F[7:0] = 0x10 (rate=OFDM_6M, BW20) */
	odm_write_1byte_22b(dm, REG_NDPA_OPT_CTRL_8822B, 0x10);

	/*Temp Settings*/
	odm_set_bb_reg_22b(dm, 0x6DC, 0x3F000000, 4); /*STA2's CSI rate is fixed at 6M*/
	odm_set_bb_reg_22b(dm, 0x1C94, MASKDWORD, 0xAFFFAFFF); /*Grouping bitmap parameters*/

	/* Init HW variable */
	beamforming_info->reg_mu_tx_ctrl = odm_read_4byte_22b(dm, 0x14c0);
	...
	/* odm_set_bb_reg_22b(dm, 0xB00, BIT(9), 1);*/ /* For A-cut only. RegB00[9] = 1 (enable PMAC Rx) */
	odm_set_bb_reg_22b(dm, 0xB54, BIT(30), 1); /* RegB54[30] = 1 (force user position) */
	odm_set_bb_reg_22b(dm, 0xB54, (BIT(29) | BIT28), adapter->MgntInfo.sniff_user_position); /* RegB54[29:28] = user position (0~3) */
...
)
{
	...
	if ((beamforming_info->beamformer_su_cnt > 0) && (bfer_idx < BEAMFORMER_ENTRY_NUM)) {
		...
		odm_write_1byte_22b(dm, REG_SND_PTCL_CTRL_8822B, 0xDB);
		...
		if (beamformer_entry->su_reg_index == 0) {
			for (i = 0; i < 6 ; i++)
				odm_write_1byte_22b(dm, (REG_ASSOCIATED_BFMER0_INFO_8822B + i), beamformer_entry->mac_addr[i]);
		} else {
			for (i = 0; i < 6 ; i++)
				odm_write_1byte_22b(dm, (REG_ASSOCIATED_BFMER1_INFO_8822B + i), beamformer_entry->mac_addr[i]);
		}
		...
		if (bfer_idx == 0)
			odm_write_2byte_22b(dm, REG_TX_CSI_RPT_PARAM_BW20_8822B, csi_param);
		else
			odm_write_2byte_22b(dm, REG_TX_CSI_RPT_PARAM_BW20_8822B + 2, csi_param);
		/*ndp_rx_standby_timer, 8814 need > 0x56, suggest from Dvaid*/
		odm_write_1byte_22b(dm, REG_SND_PTCL_CTRL_8822B + 3, 0x70);
	...
	if ((beamforming_info->beamformee_su_cnt > 0) && (bfee_idx < BEAMFORMEE_ENTRY_NUM)) {
		...
		if (p_beamformee_entry->su_reg_index == 0) {
			odm_write_2byte_22b(dm, REG_TXBF_CTRL_8822B, sta_id);
			odm_write_1byte_22b(dm, REG_TXBF_CTRL_8822B + 3, odm_read_1byte_22b(dm, REG_TXBF_CTRL_8822B + 3) | BIT(4) | BIT(6) | BIT(7));
		} else
			odm_write_2byte_22b(dm, REG_TXBF_CTRL_8822B + 2, sta_id | BIT(14) | BIT(15) | BIT(12));

		/*CSI report parameters of Beamformee*/
		if (p_beamformee_entry->su_reg_index == 0) {
			/*Get BIT24 & BIT25*/
			u8	tmp = odm_read_1byte_22b(dm, REG_ASSOCIATED_BFMEE_SEL_8822B + 3) & 0x3;

			odm_write_1byte_22b(dm, REG_ASSOCIATED_BFMEE_SEL_8822B + 3, tmp | 0x60);
			odm_write_2byte_22b(dm, REG_ASSOCIATED_BFMEE_SEL_8822B, sta_id | BIT(9));
		} else
			odm_write_2byte_22b(dm, REG_ASSOCIATED_BFMEE_SEL_8822B + 2, sta_id | 0xE200);	/*Set BIT25*/
	...
	if ((beamforming_info->beamformer_mu_cnt > 0) && (bfer_idx < BEAMFORMER_ENTRY_NUM)) {
		...
		/* Set partial AID */
		odm_write_2byte_22b(dm, (REG_ASSOCIATED_BFMER0_INFO_8822B + 6), beamformer_entry->p_aid);

		/* Fill our AID to 0x1680[11:0] and [13:12] = 2b'00, BF report segment select to 3895 bytes*/
		u1b_tmp = odm_read_1byte_22b(dm, 0x1680);
		u1b_tmp = (beamformer_entry->p_aid) & 0xFFF;
		odm_write_1byte_22b(dm, 0x1680, u1b_tmp);

		/* Set 80us for leaving ndp_rx_standby_state */
		odm_write_1byte_22b(dm, 0x71B, 0x50);

		/* Set 0x6A0[14] = 1 to accept action_no_ack */
		u1b_tmp = odm_read_1byte_22b(dm, REG_RXFLTMAP0_8822B + 1);
		u1b_tmp |= 0x40;
		odm_write_1byte_22b(dm, REG_RXFLTMAP0_8822B + 1, u1b_tmp);
		/* Set 0x6A2[5:4] = 1 to NDPA and BF report poll */
		u1b_tmp = odm_read_1byte_22b(dm, REG_RXFLTMAP1_8822B);
		u1b_tmp |= 0x30;
		odm_write_1byte_22b(dm, REG_RXFLTMAP1_8822B, u1b_tmp);
		...
		csi_param = (u16)((coefficientsize << 10) | (codebookinfo << 8) | (grouping << 6) | (nr_index << 3) | (nc_index));
		odm_write_2byte_22b(dm, 0x6F4, csi_param);

		/*for B-cut*/
		odm_set_bb_reg_22b(dm, 0x6A0, BIT(20), 0);
		odm_set_bb_reg_22b(dm, 0x688, BIT(20), 0);
	...
	if ((beamforming_info->beamformee_mu_cnt > 0) && (bfee_idx < BEAMFORMEE_ENTRY_NUM)) {
		...
		odm_write_4byte_22b(dm, 0x14c0, beamforming_info->reg_mu_tx_ctrl);

		odm_set_bb_reg_22b(dm, 0x14c4, MASKDWORD, 0); /*Reset gid_valid table*/
		odm_set_bb_reg_22b(dm, 0x14c8, MASKDWORD, user_position_l);
		odm_set_bb_reg_22b(dm, 0x14cc, MASKDWORD, user_position_h);
		...
		value16 = odm_read_2byte_22b(dm, mu_reg[p_beamformee_entry->mu_reg_index]);
		value16 &= 0xFE00; /*Clear PAID*/
		value16 |= BIT(9); /*Enable MU BFee*/
		value16 |= p_beamformee_entry->p_aid;
		odm_write_2byte_22b(dm, mu_reg[p_beamformee_entry->mu_reg_index], value16);

		/* 0x42C[30] = 1 (0: from Tx desc, 1: from 0x45F) */
		u1b_tmp = odm_read_1byte_22b(dm, REG_TXBF_CTRL_8822B + 3);
		u1b_tmp |= 0xD0; /* Set bit 28, 30, 31 to 3b'111*/
		odm_write_1byte_22b(dm, REG_TXBF_CTRL_8822B + 3, u1b_tmp);
		/* Set NDPA to 6M*/
		odm_write_1byte_22b(dm, REG_NDPA_RATE_8822B, 0x4);

		u1b_tmp = odm_read_1byte_22b(dm, REG_NDPA_OPT_CTRL_8822B);
		u1b_tmp &= 0xFC; /* Clear bit 0, 1*/
		odm_write_1byte_22b(dm, REG_NDPA_OPT_CTRL_8822B, u1b_tmp);

		u4b_tmp = odm_read_4byte_22b(dm, REG_SND_PTCL_CTRL_8822B);
		u4b_tmp = ((u4b_tmp & 0xFF0000FF) | 0x020200); /* Set [23:8] to 0x0202*/
		odm_write_4byte_22b(dm, REG_SND_PTCL_CTRL_8822B, u4b_tmp);
...
#if 1
		{
			...
			/*Set Ctrl Info*/
			odm_write_2byte_22b(dm, 0x140, 0x660);
			ctrl_info_offset = 0x8000 + 32 * p_beamformee_entry->mac_id;
			/*Reset Ctrl Info*/
			for (index = 0; index < 8; index++)
				odm_write_4byte_22b(dm, ctrl_info_offset + index * 4, 0);

			odm_write_4byte_22b(dm, ctrl_info_offset, (p_beamformee_entry->mu_reg_index + 1) << 16);
			odm_write_1byte_22b(dm, 0x81, 0x80); /*RPTBUF ready*/
...
)
{
	...
	if (beamformer_entry->beamform_entry_cap == BEAMFORMING_CAP_NONE) {
		odm_write_1byte_22b(dm, REG_SND_PTCL_CTRL_8822B, 0xD8);
		if (beamformer_entry->is_mu_ap == 0) { /*SU BFer */
			if (beamformer_entry->su_reg_index == 0) {
				odm_write_4byte_22b(dm, REG_ASSOCIATED_BFMER0_INFO_8822B, 0);
				odm_write_2byte_22b(dm, REG_ASSOCIATED_BFMER0_INFO_8822B + 4, 0);
				odm_write_2byte_22b(dm, REG_TX_CSI_RPT_PARAM_BW20_8822B, 0);
			} else {
				odm_write_4byte_22b(dm, REG_ASSOCIATED_BFMER1_INFO_8822B, 0);
				odm_write_2byte_22b(dm, REG_ASSOCIATED_BFMER1_INFO_8822B + 4, 0);
				odm_write_2byte_22b(dm, REG_TX_CSI_RPT_PARAM_BW20_8822B + 2, 0);
			}
			...
			odm_memory_set_22b(dm, beamformer_entry->gid_valid, 0, 8);
			odm_memory_set_22b(dm, beamformer_entry->user_position, 0, 16);
			beamformer_entry->is_mu_ap = false;
		}
	}
	...
	if (p_beamformee_entry->beamform_entry_cap == BEAMFORMING_CAP_NONE) {
		...
		if (p_beamformee_entry->is_mu_sta == 0) { /*SU BFee*/
			if (p_beamformee_entry->su_reg_index == 0) {
				odm_write_2byte_22b(dm, REG_TXBF_CTRL_8822B, 0x0);
				odm_write_1byte_22b(dm, REG_TXBF_CTRL_8822B + 3, odm_read_1byte_22b(dm, REG_TXBF_CTRL_8822B + 3) | BIT(4) | BIT(6) | BIT(7));
				odm_write_2byte_22b(dm, REG_ASSOCIATED_BFMEE_SEL_8822B, 0);
			} else {
				odm_write_2byte_22b(dm, REG_TXBF_CTRL_8822B + 2, 0x0 | BIT(14) | BIT(15) | BIT(12));

				odm_write_2byte_22b(dm, REG_ASSOCIATED_BFMEE_SEL_8822B + 2,
					odm_read_2byte_22b(dm, REG_ASSOCIATED_BFMEE_SEL_8822B + 2) & 0x60);
			}
			...
			odm_write_2byte_22b(dm, mu_reg[p_beamformee_entry->mu_reg_index], 0);
...
)
{
	...
	if (is_mu_sounding == false) {
		...
		odm_write_2byte_22b(dm, beam_ctrl_reg, beam_ctrl_val);
		/*disable NDP packet use beamforming */
		tmp_val = odm_read_2byte_22b(dm, REG_TXBF_CTRL_8822B);
		odm_write_2byte_22b(dm, REG_TXBF_CTRL_8822B, tmp_val | BIT(15));
	} else {
		...
		if (1) { /* (beamform_entry->beamform_entry_state == BEAMFORMING_ENTRY_STATE_PROGRESSED) { */
			...
			value32 = odm_get_bb_reg_22b(dm, 0xF4C, 0xFFFF0000);
			/* odm_set_bb_reg_22b(dm, 0x19E0, MASKHWORD, 0xFFFF);Let MAC ignore bitmap */
			...
			for (idx = 0; idx < 6; idx++) {
				...
				odm_write_4byte_22b(dm, 0x14c0, beamforming_info->reg_mu_tx_ctrl);
				odm_set_mac_reg_22b(dm, 0x14C4, MASKDWORD, gid_valid[idx]); /*set MU STA gid valid table*/
			}
...
)
{
	...
	beamforming_info->reg_mu_tx_ctrl &= ~(BIT(8) | BIT(9) | BIT(10));
	odm_write_4byte_22b(dm, 0x14c0, beamforming_info->reg_mu_tx_ctrl);
	odm_set_bb_reg_22b(dm, 0x14c4, MASKDWORD, gid_valid);
	odm_set_bb_reg_22b(dm, 0x14c8, MASKDWORD, user_position_l);
	odm_set_bb_reg_22b(dm, 0x14cc, MASKDWORD, user_position_h);
...
)
{
	...
	if ((su_bfee_cnt > 0) || (mu_bfee_cnt > 0)) {
		for (i = RF_PATH_A; i <= RF_PATH_B; i++) {
			odm_set_rf_reg_22b(dm, (enum rf_path)i, 0xEF, BIT(19), 0x1); /*RF mode table write enable*/
			odm_set_rf_reg_22b(dm, (enum rf_path)i, 0x33, 0xF, 3); /*Select RX mode*/
			odm_set_rf_reg_22b(dm, (enum rf_path)i, 0x3E, 0xfffff, 0x00036); /*Set Table data*/
			odm_set_rf_reg_22b(dm, (enum rf_path)i, 0x3F, 0xfffff, 0x5AFCE); /*Set Table data*/
			odm_set_rf_reg_22b(dm, (enum rf_path)i, 0xEF, BIT(19), 0x0); /*RF mode table write disable*/
		}
	}

	odm_set_bb_reg_22b(dm, REG_BB_TXBF_ANT_SET_BF1_8822B, BIT(30), 1);			/*if Nsts > Nc, don't apply V matrix*/

	if (su_bfee_cnt > 0 || mu_bfee_cnt > 0) {
		/*for 8814 19ac(idx 1), 19b4(idx 0), different Tx ant setting*/
		odm_set_bb_reg_22b(dm, REG_BB_TXBF_ANT_SET_BF1_8822B, BIT(28) | BIT29, 0x2);	/*enable BB TxBF ant mapping register*/
		odm_set_bb_reg_22b(dm, REG_BB_TXBF_ANT_SET_BF1_8822B, BIT(31), 1);			/*ignore user since 8822B only 2Tx*/
		...
		odm_set_bb_reg_22b(dm, REG_BB_TXBF_ANT_SET_BF1_8822B, BIT(28) | BIT29, 0x0);	/*enable BB TxBF ant mapping register*/
		odm_set_bb_reg_22b(dm, REG_BB_TXBF_ANT_SET_BF1_8822B, BIT(31), 0);			/*ignore user since 8822B only 2Tx*/
...
)
{
	...
	if (enable_su_bfer) {
		odm_set_bb_reg_22b(dm, 0x19f8, BIT(22) | BIT(21) | BIT(20), 0x1);
		odm_set_bb_reg_22b(dm, 0x19f8, BIT(25) | BIT(24) | BIT(23), 0x0);
		odm_set_bb_reg_22b(dm, 0x19f8, BIT(16), 0x1);

		if (is_vht)
			odm_set_bb_reg_22b(dm, 0x19f0, BIT(5) | BIT(4) | BIT(3) | BIT(2) | BIT(1) | BIT(0), 0x1f);
		else
			odm_set_bb_reg_22b(dm, 0x19f0, BIT(5) | BIT(4) | BIT(3) | BIT(2) | BIT(1) | BIT(0), 0x22);

		odm_set_bb_reg_22b(dm, 0x19f0, BIT(7) | BIT(6), nc);
		odm_set_bb_reg_22b(dm, 0x19f0, BIT(9) | BIT(8), nr);
		odm_set_bb_reg_22b(dm, 0x19f0, BIT(11) | BIT(10), ng);
		odm_set_bb_reg_22b(dm, 0x19f0, BIT(13) | BIT(12), CB);

		odm_set_bb_reg_22b(dm, 0xb58, BIT(3) | BIT(2), BW);
		odm_set_bb_reg_22b(dm, 0xb58, BIT(7) | BIT(6) | BIT(5) | BIT(4), 0x0);
		odm_set_bb_reg_22b(dm, 0xb58, BIT(9) | BIT(8), BW);
		odm_set_bb_reg_22b(dm, 0xb58, BIT(13) | BIT(12) | BIT(11) | BIT(10), 0x0);
	} else
		odm_set_bb_reg_22b(dm, 0x19f8, BIT(16), 0x0);
