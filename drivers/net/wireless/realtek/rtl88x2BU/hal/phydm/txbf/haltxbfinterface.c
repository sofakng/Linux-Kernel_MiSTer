...
)
{
	...
	ndp_tx_rate = beamforming_get_htndp_tx_rate_22b(dm, beamform_entry->comp_steering_num_of_bfer);
...
)
{
	...
	odm_move_memory_22b(dm, p_ndpa_frame + 16, &sequence, 1);
	...
	odm_move_memory_22b(dm, p_ndpa_frame + 17, (u8 *)&sta_info, 2);
...
)
{
	...
	ndp_tx_rate = beamforming_get_vht_ndp_tx_rate_22b(dm, beamform_entry->comp_steering_num_of_bfer);
	PHYDM_DBG(dm, DBG_TXBF, "[%s] ndp_tx_rate =%d\n", __func__, ndp_tx_rate);
	...
	*p_length = 17;
	...
	for (idx = 0; idx < BEAMFORMEE_ENTRY_NUM; idx++) {
		...
		if (entry->is_mu_sta) {
			...
			odm_move_memory_22b(dm, p_ndpa_frame + (*p_length), (u8 *)&sta_info, 2);
			*p_length += 2;
		}
	}
...
)
{
	...
	*(fctrl) = 0;
	...
	_rtw_memcpy_22b(pwlanhdr->addr2, beamform_entry->my_mac_addr, ETH_ALEN);
	...
	_rtw_memcpy_22b(pframe + 28, action_hdr, 4);
...
)
{
	...
	/* update attribute */
	pattrib = &pmgntframe->attrib;
	_rtw_memcpy_22b(pattrib->ra, RA, ETH_ALEN);
	update_mgntframe_attrib_22b(adapter, pattrib);
...
)
{
	...
	*(fctrl) = 0;
	...
	_rtw_memcpy_22b(pframe + 17, (u8 *)&ndpa_sta_info, 2);
