...
void phydm_set_noise_data_sum_22b(struct noise_level *noise_data, u8 max_rf_path)
{
	u8 rf_path;
...
s16 odm_inband_noise_monitor_22b_n_series(struct dm_struct	*dm, u8 is_pause_dig, u8 igi_value, u32 max_time)
{
	u32				tmp4b;
	...
	func_start = odm_get_current_time_22b(dm);
	...
	odm_memory_set_22b(dm, &noise_data, 0, sizeof(struct noise_level));
	/* step 1. Disable DIG && Set initial gain. */

	if (is_pause_dig)
		odm_pause_dig_22b(dm, PHYDM_PAUSE, PHYDM_PAUSE_LEVEL_1, igi_value);

	/* step 3. Get noise power level */
	start = odm_get_current_time_22b(dm);
	while (1) {
		/* Stop updating idle time pwer report (for driver read) */
		odm_set_bb_reg_22b(dm, REG_FPGA0_TX_GAIN_STAGE, BIT(25), 1);

		/* Read Noise Floor Report */
		tmp4b = odm_get_bb_reg_22b(dm, 0x8f8, MASKDWORD);

		/* update idle time pwer report per 5us */
		odm_set_bb_reg_22b(dm, REG_FPGA0_TX_GAIN_STAGE, BIT(25), 0);
		...
		if ((valid_done == max_rf_path) || (odm_get_progressing_time_22b(dm, start) > max_time)) {
			phydm_set_noise_data_sum_22b(&noise_data, max_rf_path);
			break;
		}
	}
	reg_c50 = (u8)odm_get_bb_reg_22b(dm, REG_OFDM_0_XA_AGC_CORE1, MASKBYTE0);
	reg_c50 &= ~BIT(7);
	...
	if (max_rf_path == 2) {
		reg_c58 = (u8)odm_get_bb_reg_22b(dm, REG_OFDM_0_XB_AGC_CORE1, MASKBYTE0);
		reg_c58 &= ~BIT(7);
	...
	if (is_pause_dig)
		odm_pause_dig_22b(dm, PHYDM_RESUME, PHYDM_PAUSE_LEVEL_1, igi_value);
	func_end = odm_get_progressing_time_22b(dm, func_start);
...
s16
phydm_idle_noise_measurement_ac_22b(
	struct dm_struct	*dm,
	u8	is_pause_dig,
	u8	igi_value,
	u32	max_time
	)
{
	...
	PHYDM_DBG(dm, DBG_ENV_MNTR, "phydm_idle_noise_measurement_ac_22b==>\n");
	...
	while (1) {
		/*Stop updating idle time pwer report (for driver read)*/
		odm_set_bb_reg_22b(dm, 0x9e4, BIT(30), 0x1);

		/*Read Noise Floor Report*/
		tmp4b = odm_get_bb_reg_22b(dm, 0xff0, MASKDWORD);

		/*update idle time pwer report per 5us*/
		odm_set_bb_reg_22b(dm, 0x9e4, BIT(30), 0x0);
	...
	reg_c50 = (u8)odm_get_bb_reg_22b(dm, 0xc50, MASKBYTE0);
	reg_c50 &= ~BIT(7);
	dm->noise_level.noise[RF_PATH_A] = (s8)(-110 + reg_c50 + noise_data.sum[RF_PATH_A]);
	dm->noise_level.noise_all += dm->noise_level.noise[RF_PATH_A];

	if (max_rf_path == 2) {
		reg_e50 = (u8)odm_get_bb_reg_22b(dm, 0xe50, MASKBYTE0);
		reg_e50 &= ~BIT(7);
...
s16
odm_inband_noise_monitor_22b_ac_series_22b(
	struct dm_struct	*dm,
	u8 is_pause_dig,
	u8 igi_value,
	u32 max_time
	)
{
	...
	if (dm->support_ic_type & (ODM_RTL8822B | ODM_RTL8821C))
		return phydm_idle_noise_measurement_ac_22b(dm, is_pause_dig, igi_value, max_time);
	...
	PHYDM_DBG(dm, DBG_ENV_MNTR, "odm_inband_noise_monitor_22b_ac_series_22b() ==>\n");
	...
	while (1) {
		/*Set IGI=0x1C */
		odm_write_dig_22b(dm, 0x1C);
		/*stop CK320&CK88 */
		odm_set_bb_reg_22b(dm, 0x8B4, BIT(6), 1);
		/*Read path-A */
		odm_set_bb_reg_22b(dm, 0x8FC, MASKDWORD, 0x200); /*set debug port*/
		value32 = odm_get_bb_reg_22b(dm, 0xFA0, MASKDWORD); /*read debug port*/
		...
		if ((!pd_flag) || (rxi_buf_anta != 0x200)) {
			/*sign conversion*/
			rxi_buf_anta = odm_sign_conversion_22b(rxi_buf_anta, 10);
			rxq_buf_anta = odm_sign_conversion_22b(rxq_buf_anta, 10);

			pwdb_A = odm_pwdb_conversion_22b(rxi_buf_anta * rxi_buf_anta + rxq_buf_anta * rxq_buf_anta, 20, 18); /*S(10,9)*S(10,9)=S(20,18)*/

			PHYDM_DBG(dm, DBG_ENV_MNTR, "pwdb_A= %d dB, rxi_buf_anta= 0x%x, rxq_buf_anta= 0x%x\n", pwdb_A, rxi_buf_anta & 0x3FF, rxq_buf_anta & 0x3FF);
		}
		/*Start CK320&CK88*/
		odm_set_bb_reg_22b(dm, 0x8B4, BIT(6), 0);
		/*BB Reset*/
		odm_write_1byte_22b(dm, 0x02, odm_read_1byte_22b(dm, 0x02) & (~BIT(0)));
		odm_write_1byte_22b(dm, 0x02, odm_read_1byte_22b(dm, 0x02) | BIT(0));
		/*PMAC Reset*/
		odm_write_1byte_22b(dm, 0xB03, odm_read_1byte_22b(dm, 0xB03) & (~BIT(0)));
		odm_write_1byte_22b(dm, 0xB03, odm_read_1byte_22b(dm, 0xB03) | BIT(0));
		/*CCK Reset*/
		if (odm_read_1byte_22b(dm, 0x80B) & BIT(4)) {
			odm_write_1byte_22b(dm, 0x80B, odm_read_1byte_22b(dm, 0x80B) & (~BIT(4)));
			odm_write_1byte_22b(dm, 0x80B, odm_read_1byte_22b(dm, 0x80B) | BIT(4));
		}
		...
		if ((sval < 0 && sval >= -27) && (valid_cnt < VALID_CNT)){
			...
			PHYDM_DBG(dm, DBG_ENV_MNTR, "Sum of sval = %d,\n", sum);
			if ((valid_cnt >= VALID_CNT) || (odm_get_progressing_time_22b(dm, start) > max_time)) {
				sum /= VALID_CNT;
	...
	)
{
	...
	if (dm->support_ic_type & ODM_IC_11AC_SERIES)
		return odm_inband_noise_monitor_22b_ac_series_22b(dm, is_pause_dig, igi_value, max_time);
	else
		return odm_inband_noise_monitor_22b_n_series(dm, is_pause_dig, igi_value, max_time);
}
