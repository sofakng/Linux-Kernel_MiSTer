...
void configure_txpower_track_22b(
	void		*dm_void,
...
)
{
	...
	if (dm->support_ic_type == ODM_RTL8812)
		configure_txpower_track_22b_8812a(config);
	/* else */
#endif
#endif
	...
	if (dm->support_ic_type == ODM_RTL8814A)
		configure_txpower_track_22b_8814a(config);
#endif


#if RTL8188E_SUPPORT
	if (dm->support_ic_type == ODM_RTL8188E)
		configure_txpower_track_22b_8188e(config);
#endif

#if RTL8197F_SUPPORT
	if (dm->support_ic_type == ODM_RTL8197F)
		configure_txpower_track_22b_8197f(config);
#endif

#if RTL8822B_SUPPORT
	if (dm->support_ic_type == ODM_RTL8822B)
		configure_txpower_track_22b_8822b(config);
#endif
...
)
{
	...
	thermal_value = (unsigned char)odm_get_rf_reg_22b(dm, RF_PATH_A, ODM_RF_T_METER_92E, 0xfc00);	/* 0x42: RF Reg[15:10] 88E */
	...
	for (i = 0; i < OFDM_TABLE_SIZE_92E; i++) {
		if (ele_D == (ofdm_swing_table_22b_92e[i] >> 22)) {
	...
	if (thermal_value != priv->pshare->thermal_value) {
		...
		PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK, "ofdm_swing_table_22b_92e[(unsigned int)OFDM_index[0]] = %x\n", ofdm_swing_table_22b_92e[(unsigned int)OFDM_index[0]]);
		PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK, "ofdm_swing_table_22b_92e[(unsigned int)OFDM_index[1]] = %x\n", ofdm_swing_table_22b_92e[(unsigned int)OFDM_index[1]]);

		/* Adujst OFDM Ant_A according to IQK result */
		ele_D = (ofdm_swing_table_22b_92e[(unsigned int)OFDM_index[0]] & 0xFFC00000) >> 22;
		...
		if (X != 0) {
			...
			phy_set_bb_reg(priv, REG_OFDM_0_XA_TX_IQ_IMBALANCE, MASKDWORD, ofdm_swing_table_22b_92e[(unsigned int)OFDM_index[0]]);
		...
		if (rf == 2) {
			ele_D = (ofdm_swing_table_22b_92e[(unsigned int)OFDM_index[1]] & 0xFFC00000) >> 22;
			...
			if (X != 0) {
				...
				phy_set_bb_reg(priv, REG_OFDM_0_XB_TX_IQ_IMBALANCE, MASKDWORD, ofdm_swing_table_22b_92e[(unsigned int)OFDM_index[1]]);
...
)
{
	...
	configure_txpower_track_22b(dm, &c);
	...
	thermal_value = (u8)odm_get_rf_reg_22b(dm, RF_PATH_A, c.thermal_reg_addr, 0xfc00); /*0x42: RF Reg[15:10] 88E*/
#ifdef THER_TRIM
...
)
{
	...
	is_tssi_enable = (boolean)odm_get_rf_reg_22b(dm, RF_PATH_A, REG_RF_TX_GAIN_OFFSET, BIT(7));	/* check TSSI enable */

	/* 4 Query OFDM BB swing default setting 	Bit[31:21] */
	for (p = RF_PATH_A ; p < c.rf_path_count ; p++) {
		ele_D = odm_get_bb_reg_22b(dm, bb_swing_reg[p], 0xffe00000);
		PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,"0x%x:0x%x ([31:21] = 0x%x)\n", bb_swing_reg[p], odm_get_bb_reg_22b(dm, bb_swing_reg[p], MASKDWORD), ele_D);

		for (bb_swing_idx = 0; bb_swing_idx < TXSCALE_TABLE_SIZE; bb_swing_idx++) {/* 4 */
			if (ele_D == tx_scaling_table_jaguar_22b[bb_swing_idx]) {
...
)
{
	...
	for (rf_path = 0 ; rf_path < max_rf_path ; rf_path++) {
		...
		for (i = 0; i < OFDM_TABLE_SIZE_8812; i++) {/* 4 */
			if (ele_D == ofdm_swing_table_22b_8812[i]) {
	...
	if (thermal_value != priv->pshare->thermal_value) {
		...
		/* if (*dm->band_type == ODM_BAND_5G) */
		{
			...
			for (rf_path = 0; rf_path < max_rf_path; rf_path++) {
				phy_set_bb_reg(priv, bb_swing_reg[rf_path], 0xffe00000, ofdm_swing_table_22b_8812[(unsigned int)OFDM_index[rf_path]]);
...
)
{
	...
	if (dm->support_ic_type == ODM_RTL8197F || dm->support_ic_type == ODM_RTL8822B
		...
		odm_txpowertracking_callback_thermal_meter_22b_jaguar_series3(dm);
		return;
	}
#endif
#if (RTL8814A_SUPPORT == 1)		/*use this function to do power tracking after 8814 by YuChen*/
	if (dm->support_ic_type & ODM_RTL8814A) {
		odm_txpowertracking_callback_thermal_meter_22b_jaguar_series2(dm);
		return;
	}
#endif
#if (RTL8881A_SUPPORT || RTL8812A_SUPPORT == 1)
	if (dm->support_ic_type & ODM_RTL8812 || dm->support_ic_type & ODM_RTL8881A) {
		odm_txpowertracking_callback_thermal_meter_22b_jaguar_series(dm);
		return;
	}
#endif
	...
	if (dm->support_ic_type == ODM_RTL8192E) {
		odm_txpowertracking_callback_thermal_meter_22b_92e(dm);
		return;
	}
#endif
...
#endif
	PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK, "===>odm_txpowertracking_callback_thermal_meter_22b_8188e, dm->bb_swing_idx_cck_base: %d, dm->bb_swing_idx_ofdm_base: %d\n", cali_info->bb_swing_idx_cck_base, cali_info->bb_swing_idx_ofdm_base);
	/*
		if (!dm->rf_calibrate_info.tm_trigger) {
			odm_set_rf_reg_22b(dm, RF_PATH_A, c.thermal_reg_addr, BIT(17) | BIT(16), 0x3);
...
u8 odm_get_right_chnl_place_for_iqk_22b(u8 chnl)
{
...
{
	struct dm_struct		*dm = (struct dm_struct *)dm_void;
	odm_txpowertracking_init_22b(dm);
...
void phydm_rf_watchdog_22b(void		*dm_void)
{
	...
	odm_txpowertracking_check_22b(dm);
	if (dm->support_ic_type & ODM_IC_11AC_SERIES)
		odm_iq_calibrate_22b(dm);
#endif
}
