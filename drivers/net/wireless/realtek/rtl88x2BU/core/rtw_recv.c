...
u8 signal_stat_calc_profile_22b[SIGNAL_STAT_CALC_PROFILE_MAX][2] = {
...
void _rtw_init_sta_recv_priv_22b(struct sta_recv_priv *psta_recvpriv)
{



	_rtw_memset_22b((u8 *)psta_recvpriv, 0, sizeof(struct sta_recv_priv));

	_rtw_spinlock_22b_init_22b(&psta_recvpriv->lock);

	/* for(i=0; i<MAX_RX_NUMBLKS; i++) */
	/*	_rtw_init_queue_22b(&psta_recvpriv->blk_strms[i]); */

	_rtw_init_queue_22b(&psta_recvpriv->defrag_q);


}

sint _rtw_init_recv_priv_22b(struct recv_priv *precvpriv, _adapter *padapter)
{
	sint i;
	...
	/* _rtw_memset_22b((unsigned char *)precvpriv, 0, sizeof (struct  recv_priv)); */

	_rtw_spinlock_22b_init_22b(&precvpriv->lock);

#ifdef CONFIG_RECV_THREAD_MODE
	_rtw_init_sema_22b(&precvpriv->recv_sema, 0);

#endif

	_rtw_init_queue_22b(&precvpriv->free_recv_queue);
	_rtw_init_queue_22b(&precvpriv->recv_pending_queue);
	_rtw_init_queue_22b(&precvpriv->uc_swdec_pending_queue);
	...
	rtw_os_recv_resource_init_22b(precvpriv, padapter);
	...
	/* _rtw_memset_22b(precvpriv->pallocated_frame_buf, 0, NR_RECVFRAME * sizeof(union recv_frame) + RXFRAME_ALIGN_SZ); */
	...
	for (i = 0; i < NR_RECVFRAME ; i++) {
		_rtw_init_listhead_22b(&(precvframe->u.list));

		rtw_list_insert_tail_22b(&(precvframe->u.list), &(precvpriv->free_recv_queue.queue));

		res = rtw_os_recv_resource_alloc_22b(padapter, precvframe);
	...
	ATOMIC_SET_22b(&(precvpriv->rx_pending_cnt), 1);

	_rtw_init_sema_22b(&precvpriv->allrxreturnevt, 0);

#endif

	res = rtw_hal_init_22b_recv_priv_22b(padapter);

#ifdef CONFIG_NEW_SIGNAL_STAT_PROCESS
	rtw_init_timer_22b(&precvpriv->signal_stat_timer, padapter, rtw_signal_stat_timer_hdl, padapter);
...
void rtw_mfree_recv_priv_lock_22b(struct recv_priv *precvpriv);
void rtw_mfree_recv_priv_lock_22b(struct recv_priv *precvpriv)
{
	_rtw_spinlock_22b_free_22b(&precvpriv->lock);
#ifdef CONFIG_RECV_THREAD_MODE
	_rtw_free_sema_22b(&precvpriv->recv_sema);
#endif

	_rtw_spinlock_22b_free_22b(&precvpriv->free_recv_queue.lock);
	_rtw_spinlock_22b_free_22b(&precvpriv->recv_pending_queue.lock);

	_rtw_spinlock_22b_free_22b(&precvpriv->free_recv_buf_queue.lock);

#ifdef CONFIG_USE_USB_BUFFER_ALLOC_RX
	_rtw_spinlock_22b_free_22b(&precvpriv->recv_buf_pending_queue.lock);
#endif /* CONFIG_USE_USB_BUFFER_ALLOC_RX */
}

void _rtw_free_recv_priv_22b(struct recv_priv *precvpriv)
{
	...
	rtw_mfree_recv_priv_lock_22b(precvpriv);

	rtw_os_recv_resource_free_22b(precvpriv);

	if (precvpriv->pallocated_frame_buf)
		rtw_vmfree(precvpriv->pallocated_frame_buf, NR_RECVFRAME * sizeof(union recv_frame) + RXFRAME_ALIGN_SZ);

	rtw_hal_free_recv_priv_22b(padapter);


}

bool rtw_rframe_del_wfd_ie_22b(union recv_frame *rframe, u8 ies_offset)
{
#define DBG_RFRAME_DEL_WFD_IE 0
	...
	ies_len = rtw_del_wfd_ie_22b(ies, ies_len_ori, DBG_RFRAME_DEL_WFD_IE ? __func__ : NULL);
	rframe->u.hdr.len -= ies_len_ori - ies_len;

	return ies_len_ori != ies_len;
}

union recv_frame *_rtw_alloc_recvframe_22b_22b(_queue *pfree_recv_queue)
{
	...
	if (_rtw_queue_empty_22b(pfree_recv_queue) == _TRUE)
		precvframe = NULL;
	else {
...
union recv_frame *rtw_alloc_recvframe_22b(_queue *pfree_recv_queue)
{
	_irqL irqL;
	union recv_frame  *precvframe;
	...
	precvframe = _rtw_alloc_recvframe_22b_22b(pfree_recv_queue);
...
void rtw_init_recvframe_22b(union recv_frame *precvframe, struct recv_priv *precvpriv)
{
	/* Perry: This can be removed */
	_rtw_init_listhead_22b(&precvframe->u.hdr.list);

	precvframe->u.hdr.len = 0;
}

int rtw_free_recvframe_22b(union recv_frame *precvframe, _queue *pfree_recv_queue)
{
	_irqL irqL;
	...
	rtw_os_free_recvframe_22b(precvframe);
	...
	rtw_list_insert_tail_22b(&(precvframe->u.hdr.list), get_list_head(pfree_recv_queue));
...
sint _rtw_enqueue_recvframe_22b_22b(union recv_frame *precvframe, _queue *queue)
{
	...
	rtw_list_insert_tail_22b(&(precvframe->u.hdr.list), get_list_head(queue));
...
sint rtw_enqueue_recvframe_22b(union recv_frame *precvframe, _queue *queue)
{
	sint ret;
	_irqL irqL;
	...
	ret = _rtw_enqueue_recvframe_22b_22b(precvframe, queue);
	/* _rtw_spinunlock_22b(&pfree_recv_queue->lock); */
...
sint	rtw_enqueue_recvframe_22b(union recv_frame *precvframe, _queue *queue)
{
	return rtw_free_recvframe_22b(precvframe, queue);
}
*/
...
/*
caller : defrag ; recvframe_chk_defrag_22b in recv_thread  (passive)
...
void rtw_free_recvframe_22b_queue(_queue *pframequeue,  _queue *pfree_recv_queue)
{
	union	recv_frame	*precvframe;
	_list	*plist, *phead;

	_rtw_spinlock_22b(&pframequeue->lock);
	...
	while (rtw_end_of_queue_search_22b(phead, plist) == _FALSE) {
		...
		/* rtw_list_delete(&precvframe->u.hdr.list); */ /* will do this in rtw_free_recvframe_22b() */

		rtw_free_recvframe_22b(precvframe, pfree_recv_queue);
	}

	_rtw_spinunlock_22b(&pframequeue->lock);
...
u32 rtw_free_uc_swdec_pending_queue_22b(_adapter *adapter)
{
	...
	while ((pending_frame = rtw_alloc_recvframe_22b(&adapter->recvpriv.uc_swdec_pending_queue))) {
		rtw_free_recvframe_22b(pending_frame, &adapter->recvpriv.free_recv_queue);
		cnt++;
	}
...
sint rtw_enqueue_recvbuf_22b_to_head_22b(struct recv_buf *precvbuf, _queue *queue)
{
	...
	rtw_list_insert_head_22b(&precvbuf->list, get_list_head(queue));
...
sint rtw_enqueue_recvbuf_22b(struct recv_buf *precvbuf, _queue *queue)
{
	...
	rtw_list_insert_tail_22b(&precvbuf->list, get_list_head(queue));
#ifdef CONFIG_SDIO_HCI
	_exit_critical_bh(&queue->lock, &irqL);
#else
...
sint recvframe_chkmic_22b(_adapter *adapter,  union recv_frame *precvframe);
sint recvframe_chkmic_22b(_adapter *adapter,  union recv_frame *precvframe)
{
	...
	stainfo = rtw_get_stainfo_22b(&adapter->stapriv , &prxattrib->ta[0]);
	...
	if (prxattrib->encrypt == _TKIP_) {
		...
		if (stainfo != NULL) {
			if (IS_MCAST(prxattrib->ra)) {
				...
				/* RTW_INFO("\n recvframe_chkmic_22b: bcmc key psecuritypriv->dot118021XGrpKeyid(%d),pmlmeinfo->key_index(%d) ,recv key_id(%d)\n", */
				/*								psecuritypriv->dot118021XGrpKeyid,pmlmeinfo->key_index,rxdata_key_idx); */

				if (psecuritypriv->binstallGrpkey == _FALSE) {
					res = _FAIL;
					RTW_INFO("\n recvframe_chkmic_22b:didn't install group key!!!!!!!!!!\n");
					goto exit;
				}
			} else {
			...
			/* rtw_seccalctkipmic_22b(&stainfo->dot11tkiprxmickey.skey[0],pframe,payload, datalen ,&miccode[0],(unsigned char)prxattrib->priority); */ /* care the length of the data */

			rtw_seccalctkipmic_22b(mickey, pframe, payload, datalen , &miccode[0], (unsigned char)prxattrib->priority); /* care the length of the data */
			...
			if (bmic_err == _TRUE) {
				...
				if ((prxattrib->bdecrypted == _TRUE) && (brpt_micerror == _TRUE)) {
					rtw_handle_tkip_mic_err_22b(adapter, stainfo, (u8)IS_MCAST(prxattrib->ra));
...
union recv_frame *decryptor_22b(_adapter *padapter, union recv_frame *precv_frame);
union recv_frame *decryptor_22b(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if (prxattrib->encrypt && !prxattrib->bdecrypted) {
		...
		RTW_INFO(ADPT_FMT" - sec_type:%s DO SW decryption\n",
			ADPT_ARG(padapter), security_type_str_22b(prxattrib->encrypt));
#endif
		...
		case _TKIP_:
			...
			res = rtw_tkip_decrypt_22b(padapter, (u8 *)precv_frame);
			break;
		case _AES_:
			DBG_COUNTER(padapter->rx_logs.core_rx_post_decrypt_aes);
			res = rtw_aes_decrypt_22b(padapter, (u8 *)precv_frame);
			break;
		...
		if ((prxstat->icv == 1) && (prxattrib->encrypt != _AES_)) {
			...
			rtw_free_recvframe_22b(precv_frame, &padapter->recvpriv.free_recv_queue);
	...
	if (res == _FAIL) {
		rtw_free_recvframe_22b(return_packet, &padapter->recvpriv.free_recv_queue);
		return_packet = NULL;
	} else
		prxattrib->bdecrypted = _TRUE;
	/* recvframe_chkmic_22b(adapter, precv_frame);   */ /* move to recvframme_defrag function */
...
union recv_frame *portctrl_22b(_adapter *adapter, union recv_frame *precv_frame);
union recv_frame *portctrl_22b(_adapter *adapter, union recv_frame *precv_frame)
{
	u8 *psta_addr = NULL;
	u8 *ptr;
	...
	psta = rtw_get_stainfo_22b(pstapriv, psta_addr);
	...
	if (auth_alg == dot11AuthAlgrthm_8021X) {
		if ((psta != NULL) && (psta->ieee8021x_blocked)) {
			...
			_rtw_memcpy_22b(&ether_type, ptr, 2);
			...
			if (ether_type == eapol_type)
				...
				/* free this frame */
				rtw_free_recvframe_22b(precv_frame, &adapter->recvpriv.free_recv_queue);
				prtnframe = NULL;
			}
		} else {
			/* allowed */
...
sint recv_decache_22b(union recv_frame *precv_frame, u16 *tid_rxseq)
{
...
sint recv_ucast_pn_decache_22b(union recv_frame *precv_frame);
sint recv_ucast_pn_decache_22b(union recv_frame *precv_frame)
{
	...
	if (pattrib->encrypt == _AES_) {
		...
		if (!VALID_PN_CHK(pkt_pn, curr_pn)) {
			...
			_rtw_memcpy_22b(prxcache->iv[tid],
...
sint recv_bcast_pn_decache_22b(union recv_frame *precv_frame);
sint recv_bcast_pn_decache_22b(union recv_frame *precv_frame)
{
	_adapter *padapter = precv_frame->u.hdr.adapter;
...
void process_pwrbit_data_22b(_adapter *padapter, union recv_frame *precv_frame, struct sta_info *psta)
{
	...
	if (pwrbit) {
		if (!(psta->state & WIFI_SLEEP_STATE)) {
			...
			stop_sta_xmit_22b(padapter, psta);
...
void process_wmmps_data_22b(_adapter *padapter, union recv_frame *precv_frame, struct sta_info *psta)
{
#ifdef CONFIG_AP_MODE
	struct rx_pkt_attrib *pattrib = &precv_frame->u.hdr.attrib;
	struct sta_priv *pstapriv = &padapter->stapriv;
	...
	if (psta->state & WIFI_SLEEP_STATE) {
		...
		if (wmmps_ac) {
			if (psta->sleepq_ac_len > 0) {
				...
				xmit_delivery_enabled_frames_22b(padapter, psta);
			} else {
				/* issue one qos null frame with More data bit = 0 and the EOSP bit set (=1) */
				issue_qos_nulldata_22b(padapter, psta->cmn.mac_addr, (u16)pattrib->priority, 0, 0, 0);
			}
		}
...
sint OnTDLS(_adapter *adapter, union recv_frame *precv_frame)
{
	...
	if (hal_chk_wl_func_22b(adapter, WL_FUNC_TDLS) == _FALSE) {
	...
	ptdls_sta = rtw_get_stainfo_22b(pstapriv, get_sa(ptr));
	if (ptdls_sta == NULL) {
		switch (*paction) {
	...
	case 0x50:
		if (_rtw_memcmp_22b(WFA_OUI, paction, 3)) {
			/* Probe request frame */
			if (*(paction + 3) == 0x04) {
...
void count_rx_stats_22b(_adapter *padapter, union recv_frame *prframe, struct sta_info *sta)
{
	...
	traffic_check_for_leave_lps_22b(padapter, _FALSE, 0);
#endif /* CONFIG_LPS */

}

sint sta2sta_data_frame_22b(
	_adapter *adapter,
...
)
{
	...
	if ((check_fwstate(pmlmepriv, WIFI_ADHOC_STATE) == _TRUE) ||
		...
		if (_rtw_memcmp_22b(myhwaddr, pattrib->src, ETH_ALEN)) {
			ret = _FAIL;
			goto exit;
		}

		if ((!_rtw_memcmp_22b(myhwaddr, pattrib->dst, ETH_ALEN))	&& (!bmcast)) {
			ret = _FAIL;
			goto exit;
		}

		if (_rtw_memcmp_22b(pattrib->bssid, "\x0\x0\x0\x0\x0\x0", ETH_ALEN) ||
		    _rtw_memcmp_22b(mybssid, "\x0\x0\x0\x0\x0\x0", ETH_ALEN) ||
		    (!_rtw_memcmp_22b(pattrib->bssid, mybssid, ETH_ALEN))) {
			ret = _FAIL;
			goto exit;
		}
		...
		if (ptdlsinfo->link_established == _TRUE) {
			*psta = ptdls_sta = rtw_get_stainfo_22b(pstapriv, pattrib->ta);
			if (ptdls_sta == NULL) {
				ret = _FAIL;
				goto exit;
...
#ifdef CONFIG_TDLS_CH_SW
				if (ATOMIC_READ_22b(&pchsw_info->chsw_on) == _TRUE) {
					if (adapter->mlmeextpriv.cur_channel != rtw_get_oper_ch_22b(adapter)) {
				...
				process_pwrbit_data_22b(adapter, precv_frame, ptdls_sta);
				...
				if (_rtw_memcmp_22b(psnap_type, SNAP_ETH_TYPE_TDLS, 2)) {
					ret = OnTDLS(adapter, precv_frame);
					goto exit;
				}

				if ((get_frame_sub_type(ptr) & WIFI_QOS_DATA_TYPE) == WIFI_QOS_DATA_TYPE)
					process_wmmps_data_22b(adapter, precv_frame, ptdls_sta);
...
#endif /* CONFIG_TDLS */
		{
			...
			if (!_rtw_memcmp_22b(pattrib->bssid, pattrib->src, ETH_ALEN)) {
				ret = _FAIL;
				goto exit;
			}
		}
		...
		if (bmcast) {
			...
			if (!_rtw_memcmp_22b(pattrib->bssid, pattrib->dst, ETH_ALEN)) {
				ret = _FAIL;
				goto exit;
			}
		}
		...
		_rtw_memcpy_22b(pattrib->dst, GetAddr1Ptr(ptr), ETH_ALEN);
		_rtw_memcpy_22b(pattrib->src, get_addr2_ptr(ptr), ETH_ALEN);
		_rtw_memcpy_22b(pattrib->bssid, GetAddr3Ptr(ptr), ETH_ALEN);
	...
	if (ptdls_sta == NULL)
#endif
		*psta = rtw_get_stainfo_22b(pstapriv, sta_addr);
	...
	struct sta_info **psta)
{
	...
	if ((check_fwstate(pmlmepriv, WIFI_STATION_STATE) == _TRUE)
		...
		if (_rtw_memcmp_22b(pattrib->bssid, "\x0\x0\x0\x0\x0\x0", ETH_ALEN) ||
			...
			if (!bmcast) {
				RTW_INFO(ADPT_FMT" -issue_deauth_22b to the nonassociated ap=" MAC_FMT " for the reason(7)\n", ADPT_ARG(adapter), MAC_ARG(pattrib->bssid));
				issue_deauth_22b(adapter, pattrib->bssid, WLAN_REASON_CLASS3_FRAME_FROM_NONASSOC_STA);
			}
		...
		*psta = rtw_get_stainfo_22b(pstapriv, pattrib->ta);
		if (*psta == NULL) {
			#ifdef DBG_RX_DROP_FRAME
		...
		*/
		...
		if (get_frame_sub_type(ptr) & BIT(6)) {
			...
			count_rx_stats_22b(adapter, precv_frame, *psta);
			ret = RTW_RX_HANDLED;
			goto exit;
		}
		...
		*psta = rtw_get_stainfo_22b(pstapriv, pattrib->bssid); /* get sta_info */
		if (*psta == NULL) {
			#ifdef DBG_RX_DROP_FRAME
		...
		if (_rtw_memcmp_22b(myhwaddr, pattrib->dst, ETH_ALEN) && (!bmcast)) {
			*psta = rtw_get_stainfo_22b(pstapriv, pattrib->ta);
			if (*psta == NULL) {
				...
				static systime send_issue_deauth_22b_time = 0;

				/* RTW_INFO("After send deauth , %u ms has elapsed.\n", rtw_get_passing_time_ms(send_issue_deauth_22b_time)); */

				if (rtw_get_passing_time_ms(send_issue_deauth_22b_time) > 10000 || send_issue_deauth_22b_time == 0) {
					send_issue_deauth_22b_time = rtw_get_current_time();

					RTW_INFO("issue_deauth_22b to the ap=" MAC_FMT " for the reason(7)\n", MAC_ARG(pattrib->bssid));
	...
	struct sta_info **psta)
{
	...
	if (check_fwstate(pmlmepriv, WIFI_AP_STATE) == _TRUE) {
		...
		if (!_rtw_memcmp_22b(pattrib->bssid, mybssid, ETH_ALEN)) {
			ret = _FAIL;
			goto exit;
		}
		...
		*psta = rtw_get_stainfo_22b(pstapriv, pattrib->ta);
		if (*psta == NULL) {
			...
			RTW_INFO("issue_deauth_22b to sta=" MAC_FMT " for the reason(7)\n", MAC_ARG(pattrib->src));

			issue_deauth_22b(adapter, pattrib->src, WLAN_REASON_CLASS3_FRAME_FROM_NONASSOC_STA);
...
bypass_deauth7:
		...
		process_pwrbit_data_22b(adapter, precv_frame, *psta);

		if ((get_frame_sub_type(ptr) & WIFI_QOS_DATA_TYPE) == WIFI_QOS_DATA_TYPE)
			process_wmmps_data_22b(adapter, precv_frame, *psta);
		...
		*psta = rtw_get_stainfo_22b(pstapriv, pattrib->bssid); /* get sta_info */
		...
		u8 *myhwaddr = adapter_mac_addr(adapter);
		if (!_rtw_memcmp_22b(pattrib->ra, myhwaddr, ETH_ALEN)) {
			ret = RTW_RX_HANDLED;
			goto exit;
		}
		RTW_INFO("issue_deauth_22b to sta=" MAC_FMT " for the reason(7)\n", MAC_ARG(pattrib->src));
...
sint validate_recv_ctrl_frame_22b(_adapter *padapter, union recv_frame *precv_frame);
sint validate_recv_ctrl_frame_22b(_adapter *padapter, union recv_frame *precv_frame)
{
	struct rx_pkt_attrib *pattrib = &precv_frame->u.hdr.attrib;
	struct sta_priv *pstapriv = &padapter->stapriv;
	...
	/* RTW_INFO("+validate_recv_ctrl_frame_22b\n"); */
	...
	if (get_frame_sub_type(pframe) == WIFI_PSPOLL) {
		...
		if ((psta->state & WIFI_SLEEP_STATE) && (rtw_tim_map_is_set(padapter, pstapriv->sta_dz_bitmap, psta->cmn.aid))) {
			...
			if ((rtw_end_of_queue_search_22b(xmitframe_phead, xmitframe_plist)) == _FALSE) {
				...
				if (rtw_tim_map_is_set(padapter, pstapriv->tim_bitmap, psta->cmn.aid)) {
					if (psta->sleepq_len == 0) {
						...
						issue_nulldata_22b(padapter, psta->cmn.mac_addr, 0, 0, 0);
					} else {
...
bip_verify:
	...
	_rtw_memcpy_22b(pattrib->ra, GetAddr1Ptr(ptr), ETH_ALEN);
	_rtw_memcpy_22b(pattrib->ta, get_addr2_ptr(ptr), ETH_ALEN);
	...
	precv_frame = decryptor_22b(adapter, precv_frame);
	/* save actual management data frame body */
	_rtw_memcpy_22b(mgmt_DATA, ptr + pattrib->hdrlen + pattrib->iv_len, data_len);
	/* overwrite the iv field */
	_rtw_memcpy_22b(ptr + pattrib->hdrlen, mgmt_DATA, data_len);
...
union recv_frame *recvframe_chk_defrag_22b(PADAPTER padapter, union recv_frame *precv_frame);

sint validate_recv_mgnt_frame_22b(PADAPTER padapter, union recv_frame *precv_frame)
{
	struct sta_info *psta = precv_frame->u.hdr.psta
		= rtw_get_stainfo_22b(&padapter->stapriv, get_addr2_ptr(precv_frame->u.hdr.rx_data));
	...
	precv_frame = recvframe_chk_defrag_22b(padapter, precv_frame);
	if (precv_frame == NULL)
		return _SUCCESS;
	...
	if (psta) {
		...
		if (get_frame_sub_type(precv_frame->u.hdr.rx_data) == WIFI_BEACON)
			...
			if (_rtw_memcmp_22b(adapter_mac_addr(padapter), GetAddr1Ptr(precv_frame->u.hdr.rx_data), ETH_ALEN) == _TRUE)
	...
	if (padapter->proximity.proxim_on == _TRUE) {
		...
		_rtw_memcpy_22b(pattrib->dst, pda, ETH_ALEN);
		_rtw_memcpy_22b(pattrib->src, psa, ETH_ALEN);

		_rtw_memcpy_22b(pattrib->bssid, pbssid, ETH_ALEN);

		switch (pattrib->to_fr_ds) {
		case 0:
			_rtw_memcpy_22b(pattrib->ra, pda, ETH_ALEN);
			_rtw_memcpy_22b(pattrib->ta, psa, ETH_ALEN);
			break;

		case 1:
			_rtw_memcpy_22b(pattrib->ra, pda, ETH_ALEN);
			_rtw_memcpy_22b(pattrib->ta, pbssid, ETH_ALEN);
			break;

		case 2:
			_rtw_memcpy_22b(pattrib->ra, pbssid, ETH_ALEN);
			_rtw_memcpy_22b(pattrib->ta, psa, ETH_ALEN);
			break;
...
#endif
	mgt_dispatcher_22b(padapter, precv_frame);
...
sint validate_recv_data_frame_22b(_adapter *adapter, union recv_frame *precv_frame)
{
	u8 bretry, a4_shift;
	struct sta_info *psta = NULL;
	u8 *ptr = precv_frame->u.hdr.rx_data;
	...
	case 0:
		...
		_rtw_memcpy_22b(pattrib->src, get_addr2_ptr(ptr), ETH_ALEN);
		_rtw_memcpy_22b(pattrib->bssid, GetAddr3Ptr(ptr), ETH_ALEN);
		ret = sta2sta_data_frame_22b(adapter, precv_frame, &psta);
		break;
	...
	case 1:
		...
		_rtw_memcpy_22b(pattrib->ta, get_addr2_ptr(ptr), ETH_ALEN);
		_rtw_memcpy_22b(pattrib->dst, GetAddr1Ptr(ptr), ETH_ALEN);
		_rtw_memcpy_22b(pattrib->src, GetAddr3Ptr(ptr), ETH_ALEN);
		_rtw_memcpy_22b(pattrib->bssid, get_addr2_ptr(ptr), ETH_ALEN);
		ret = ap2sta_data_frame_22b(adapter, precv_frame, &psta);
		break;

	case 2:
		_rtw_memcpy_22b(pattrib->ra, GetAddr1Ptr(ptr), ETH_ALEN);
		_rtw_memcpy_22b(pattrib->ta, get_addr2_ptr(ptr), ETH_ALEN);
		_rtw_memcpy_22b(pattrib->dst, GetAddr3Ptr(ptr), ETH_ALEN);
		_rtw_memcpy_22b(pattrib->src, get_addr2_ptr(ptr), ETH_ALEN);
		_rtw_memcpy_22b(pattrib->bssid, GetAddr1Ptr(ptr), ETH_ALEN);
		ret = sta2ap_data_frame_22b(adapter, precv_frame, &psta);
		break;
...
pre_validate_status_chk:
	...
	if (!IS_MCAST(pattrib->ra)) {
		...
		if (recv_decache_22b(precv_frame, psta->sta_recvpriv.rxcache.tid_rxseq) == _FAIL) {
			#ifdef DBG_RX_DROP_FRAME
			RTW_INFO("DBG_RX_DROP_FRAME "FUNC_ADPT_FMT" recv_decache_22b uc return _FAIL for sta="MAC_FMT"\n"
				, FUNC_ADPT_ARG(adapter), MAC_ARG(psta->cmn.mac_addr));
			#endif
			ret = _FAIL;
			goto exit;
		}
		if (recv_ucast_pn_decache_22b(precv_frame) == _FAIL) {
			#ifdef DBG_RX_DROP_FRAME
			RTW_INFO("DBG_RX_DROP_FRAME "FUNC_ADPT_FMT" recv_ucast_pn_decache_22b return _FAIL for sta="MAC_FMT"\n"
				, FUNC_ADPT_ARG(adapter), MAC_ARG(psta->cmn.mac_addr));
			#endif
			ret = _FAIL;
			goto exit;
		...
		if (recv_decache_22b(precv_frame, psta->sta_recvpriv.bmc_tid_rxseq) == _FAIL) {
			#ifdef DBG_RX_DROP_FRAME
			RTW_INFO("DBG_RX_DROP_FRAME "FUNC_ADPT_FMT" recv_decache_22b bmc return _FAIL for sta="MAC_FMT"\n"
				, FUNC_ADPT_ARG(adapter), MAC_ARG(psta->cmn.mac_addr));
			#endif
			ret = _FAIL;
			goto exit;
		}
		if (recv_bcast_pn_decache_22b(precv_frame) == _FAIL) {
			#ifdef DBG_RX_DROP_FRAME
			RTW_INFO("DBG_RX_DROP_FRAME "FUNC_ADPT_FMT" recv_bcast_pn_decache_22b return _FAIL for sta="MAC_FMT"\n"
				, FUNC_ADPT_ARG(adapter), MAC_ARG(psta->cmn.mac_addr));
			#endif
			ret = _FAIL;
			goto exit;
...
sint validate_recv_frame_22b(_adapter *adapter, union recv_frame *precv_frame);
sint validate_recv_frame_22b(_adapter *adapter, union recv_frame *precv_frame)
{
	...
	if (pmlmeext->sitesurvey_res.state == SCAN_PROCESS) {
		int ch_set_idx = rtw_chset_search_ch_22b(rfctl->channel_set, rtw_get_oper_ch_22b(adapter));
		if (ch_set_idx >= 0)
...
#if 1 /* Dump rx packets */
	{
		...
		rtw_hal_get_def_var_22b(adapter, HAL_DEF_DBG_DUMP_RXPKT, &(bDumpRxPkt));
	...
	case WIFI_MGT_TYPE: /* mgnt */
		...
		retval = validate_recv_mgnt_frame_22b(adapter, precv_frame);
		if (retval == _FAIL) {
	...
	case WIFI_CTRL_TYPE: /* ctrl */
		...
		retval = validate_recv_ctrl_frame_22b(adapter, precv_frame);
		if (retval == _FAIL) {
	...
	case WIFI_DATA_TYPE: /* data */
		...
		retval = validate_recv_data_frame_22b(adapter, precv_frame);
		if (retval == _FAIL) {
			...
			if (!pattrib->encrypt || pattrib->bdecrypted) {
				...
				rtw_hal_get_def_var_22b(adapter, HAL_DEF_DBG_DUMP_RXPKT, &(bDumpRxPkt));
				/* get ether_type */
				_rtw_memcpy_22b(&eth_type, ptr + pattrib->hdrlen + pattrib->iv_len + RATTRIB_GET_MCTRL_LEN(pattrib) + LLC_HEADER_SIZE, 2);
...
#if 1
sint wlanhdr_to_ethhdr_22b(union recv_frame *precvframe)
{
	sint	rmv_len;
	u16	eth_type, len;
	u8	bsnaphdr;
	...
	if ((_rtw_memcmp_22b(psnap, rtw_rfc1042_header, SNAP_SIZE) &&
	     (_rtw_memcmp_22b(psnap_type, SNAP_ETH_TYPE_IPX, 2) == _FALSE) &&
	     (_rtw_memcmp_22b(psnap_type, SNAP_ETH_TYPE_APPLETALK_AARP, 2) == _FALSE)) ||
	    /* eth_type != ETH_P_AARP && eth_type != ETH_P_IPX) || */
	    _rtw_memcmp_22b(psnap, rtw_bridge_tunnel_header, SNAP_SIZE)) {
	...
	_rtw_memcpy_22b(&eth_type, ptr + rmv_len, 2);
	...
	if ((check_fwstate(pmlmepriv, WIFI_MP_STATE) == _TRUE)) {
		...
		*(ptr + 1) = 0x12;
		...
		_rtw_memcpy_22b(ptr, get_rxmem(precvframe), 24);
		ptr += 24;
	} else {
	...
	if (ptr) {
		_rtw_memcpy_22b(ptr, pattrib->dst, ETH_ALEN);
		_rtw_memcpy_22b(ptr + ETH_ALEN, pattrib->src, ETH_ALEN);

		if (!bsnaphdr) {
			len = htons(len);
			_rtw_memcpy_22b(ptr + 12, &len, 2);
		}
...
sint wlanhdr_to_ethhdr_22b(union recv_frame *precvframe)
{
	...
	if (psnap->dsap == 0xaa && psnap->ssap == 0xaa && psnap->ctrl == 0x03) {
		if (_rtw_memcmp_22b(psnap->oui, oui_rfc1042, WLAN_IEEE_OUI_LEN))
			bsnaphdr = _TRUE; /* wlan_pkt_format = WLAN_PKT_FORMAT_SNAP_RFC1042;	 */
		else if (_rtw_memcmp_22b(psnap->oui, SNAP_HDR_APPLETALK_DDP, WLAN_IEEE_OUI_LEN) &&
			_rtw_memcmp_22b(psnap_type, SNAP_ETH_TYPE_APPLETALK_DDP, 2))
			bsnaphdr = _TRUE;	/* wlan_pkt_format = WLAN_PKT_FORMAT_APPLETALK; */
		else if (_rtw_memcmp_22b(psnap->oui, oui_8021h, WLAN_IEEE_OUI_LEN))
	...
	_rtw_memcpy_22b(&eth_type, ptr, 2);
	eth_type = ntohs((unsigned short)eth_type); /* pattrib->ether_type */
	ptr += 2;
	...
	if (eth_type == 0x0800) { /* ip */
		...
		/* ptr -= 16; */
		/* _rtw_memcpy_22b(ptr, get_rxmem(precvframe), 16); */
	} else {
	...
	_rtw_memcpy_22b(ptr + 12, &eth_type, 2);
	...
	union recv_frame *prframe)
{
	...
	*	alloc_sz = _RND(1536 + RXDESC_SIZE + pfhdr->attrib.drvinfosize + shift_sz + 8, 128); */
	...
	if (ptr)
		_rtw_memcpy_22b(ptr, pfhdr->rx_data, pfhdr->len);
...
/* perform defrag */
union recv_frame *recvframe_defrag_22b(_adapter *adapter, _queue *defrag_q);
union recv_frame *recvframe_defrag_22b(_adapter *adapter, _queue *defrag_q)
{
	_list	*plist, *phead;
	...
	if (curfragnum != pfhdr->attrib.frag_num) {
		...
		rtw_free_recvframe_22b(prframe, pfree_recv_queue);
		rtw_free_recvframe_22b_queue(defrag_q, pfree_recv_queue);
	...
	while (rtw_end_of_queue_search_22b(phead, plist) == _FALSE) {
		...
		/* memcpy */
		_rtw_memcpy_22b(pfhdr->rx_tail, pnfhdr->rx_data, pnfhdr->len);
...
union recv_frame *recvframe_chk_defrag_22b(PADAPTER padapter, union recv_frame *precv_frame)
{
	...
	if (ismfrag == 1) {
		...
		if (pdefrag_q != NULL) {
			if (fragnum == 0) {
				/* the first fragment */
				if (_rtw_queue_empty_22b(pdefrag_q) == _FALSE) {
					/* free current defrag_q */
					rtw_free_recvframe_22b_queue(pdefrag_q, pfree_recv_queue);
				}
			}
			...
			/* _rtw_spinlock_22b(&pdefrag_q->lock); */
			phead = get_list_head(pdefrag_q);
			rtw_list_insert_tail_22b(&pfhdr->list, phead);
			/* _rtw_spinunlock_22b(&pdefrag_q->lock); */
			...
			rtw_free_recvframe_22b(precv_frame, pfree_recv_queue);
			prtnframe = NULL;
		}
	...
	if ((ismfrag == 0) && (fragnum != 0)) {
		...
		if (pdefrag_q != NULL) {
			...
			/* call recvframe_defrag_22b to defrag */
			precv_frame = recvframe_defrag_22b(padapter, pdefrag_q);
			prtnframe = precv_frame;
	...
	if ((prtnframe != NULL) && (prtnframe->u.hdr.attrib.privacy)) {
		...
		if (recvframe_chkmic_22b(padapter,  prtnframe) == _FAIL) {
			rtw_free_recvframe_22b(prtnframe, pfree_recv_queue);
			prtnframe = NULL;
		}
	}
...
static int rtw_recv_indicatepkt_22b_check(union recv_frame *rframe, u8 *ehdr_pos, u32 pkt_len)
{
	...
	if (rframe->u.hdr.psta)
		rtw_st_ctl_rx_22b(rframe->u.hdr.psta, ehdr_pos);

	if (ntohs(ehdr->h_proto) == 0x888e)
		RTW_PRINT("recv eapol packet\n");

	if (recvpriv->sink_udpport > 0)
		rtw_sink_rtp_seq_dbg_22b(adapter, ehdr_pos);
...
static void recv_free_fwd_resource(_adapter *adapter, struct xmit_frame *fwd_frame, _list *b2u_list)
{
	...
	if (fwd_frame)
		rtw_free_xmitframe_22b(xmitpriv, fwd_frame);

#ifdef CONFIG_RTW_MESH
#if CONFIG_RTW_MESH_DATA_BMC_TO_UC
	if (!rtw_is_list_empty_22b(b2u_list)) {
		struct xmit_frame *b2uframe;
		_list *list;

		list = get_next(b2u_list);
		while (rtw_end_of_queue_search_22b(b2u_list, list) == _FALSE) {
			b2uframe = LIST_CONTAINOR(list, struct xmit_frame, list);
			list = get_next(list);
			rtw_list_delete(&b2uframe->list);
			rtw_free_xmitframe_22b(xmitpriv, b2uframe);
		}
	}
#endif
	...
	, u8 act, struct xmit_frame *fwd_frame, _list *b2u_list)
{
	...
	if (!rtw_is_list_empty_22b(b2u_list)) {
		...
		while (rtw_end_of_queue_search_22b(b2u_list, list) == _FALSE) {
			...
			if (!fwd_frame && rtw_is_list_empty_22b(b2u_list)) /* the last fwd_pkt */
			...
			rtw_xmit_22b_posthandle(adapter, b2uframe, b2uframe->pkt);
		}
	}
#endif
#endif /* CONFIG_RTW_MESH */
	...
	if (fwd_frame) {
		...
		if (rtw_xmit_22b_posthandle(adapter, fwd_frame, fwd_pkt) < 0) {
			#ifdef DBG_TX_DROP_FRAME
			RTW_INFO("DBG_TX_DROP_FRAME %s rtw_xmit_22b_posthandle fail\n", __func__);
			#endif
			xmitpriv->tx_drop++;
		}
	}
...
int amsdu_to_msdu_22b(_adapter *padapter, union recv_frame *prframe)
{
	...
	while (a_len > ETH_HLEN) {
		...
		sub_pkt = rtw_os_alloc_msdu_pkt_22b(prframe, da, sa
			, pdata + ETH_HLEN + mctrl_len, nSubframe_Length - mctrl_len);
		if (sub_pkt == NULL) {
			if (act & RTW_RX_MSDU_ACT_INDICATE) {
				#ifdef DBG_RX_DROP_FRAME
				RTW_INFO("DBG_RX_DROP_FRAME %s rtw_os_alloc_msdu_pkt_22b fail\n", __func__);
				#endif
			}
			if (act & RTW_RX_MSDU_ACT_FORWARD) {
				#ifdef DBG_TX_DROP_FRAME
				RTW_INFO("DBG_TX_DROP_FRAME %s rtw_os_alloc_msdu_pkt_22b fail\n", __func__);
				#endif
		...
		if (rtw_recv_indicatepkt_22b_check(prframe, rtw_os_pkt_data_22b(sub_pkt), rtw_os_pkt_len_22b(sub_pkt)) == _SUCCESS)
			subframes[nr_subframes++] = sub_pkt;
		else
			rtw_os_pkt_free_22b(sub_pkt);
...
move_to_next:
	...
	for (i = 0; i < nr_subframes; i++) {
		...
		if (sub_pkt)
			rtw_os_recv_indicate_pkt_22b(padapter, sub_pkt, prframe);
	}

	prframe->u.hdr.len = 0;
	rtw_free_recvframe_22b(prframe, pfree_recv_queue);/* free this recv_frame */
...
static int recv_process_mpdu(_adapter *padapter, union recv_frame *prframe)
{
	...
	if (pattrib->amsdu) {
		ret = amsdu_to_msdu_22b(padapter, prframe);
		if (ret != _SUCCESS) {
			#ifdef DBG_RX_DROP_FRAME
			RTW_INFO("DBG_RX_DROP_FRAME "FUNC_ADPT_FMT" amsdu_to_msdu_22b fail\n"
		...
		ret = wlanhdr_to_ethhdr_22b(prframe);
		if (ret != _SUCCESS) {
			if (act & RTW_RX_MSDU_ACT_INDICATE) {
				#ifdef DBG_RX_DROP_FRAME
				RTW_INFO("DBG_RX_DROP_FRAME "FUNC_ADPT_FMT" wlanhdr_to_ethhdr_22b: drop pkt\n"
					, FUNC_ADPT_ARG(padapter));
				#endif
			}
			if (act & RTW_RX_MSDU_ACT_FORWARD) {
				#ifdef DBG_TX_DROP_FRAME
				RTW_INFO("DBG_TX_DROP_FRAME %s wlanhdr_to_ethhdr_22b fail\n", __func__);
				#endif
				recv_free_fwd_resource(padapter, fwd_frame, &b2u_list);
			}
			rtw_free_recvframe_22b(prframe, pfree_recv_queue);
		...
		if (!RTW_CANNOT_RUN(padapter)) {
			ret = rtw_recv_indicatepkt_22b_check(prframe
			...
			ret = rtw_recv_indicatepkt_22b(padapter, prframe);
			if (ret != _SUCCESS) {
				#ifdef DBG_RX_DROP_FRAME
				RTW_INFO("DBG_RX_DROP_FRAME "FUNC_ADPT_FMT" rtw_recv_indicatepkt_22b fail!\n"
					, FUNC_ADPT_ARG(padapter));
				#endif
				goto exit;
			}
...
static int enqueue_reorder_recvframe(struct recv_reorder_ctrl *preorder_ctrl, union recv_frame *prframe)
{
	...
	/* _rtw_spinlock_22b_ex(&ppending_recvframe_queue->lock); */
	...
	rtw_list_insert_tail_22b(&(prframe->u.hdr.list), plist);

	/* _rtw_spinunlock_22b_ex(&ppending_recvframe_queue->lock); */
	/* _exit_critical_ex(&ppending_recvframe_queue->lock, &irql); */
...
static int recv_indicatepkts_in_order(_adapter *padapter, struct recv_reorder_ctrl *preorder_ctrl, int bforced)
{
	...
	if (bforced == _TRUE) {
		...
		if (rtw_is_list_empty_22b(phead)) {
			/* _exit_critical_ex(&ppending_recvframe_queue->lock, &irql); */
			/* _rtw_spinunlock_22b_ex(&ppending_recvframe_queue->lock); */
			return _TRUE;
		}
	...
	while (!rtw_is_list_empty_22b(phead)) {
...
static int recv_indicatepkt_reorder(_adapter *padapter, union recv_frame *prframe)
{
	...
	if (!check_indicate_seq(preorder_ctrl, pattrib->seq_num)) {
		...
		#endif
#if 0
		rtw_recv_indicatepkt_22b(padapter, prframe);
...
void rtw_reordering_ctrl_timeout_handler_22b(void *pcontext)
{
	...
	/* RTW_INFO("+rtw_reordering_ctrl_timeout_handler_22b()=>\n"); */
...
#ifdef CONFIG_MP_INCLUDED
int validate_mp_recv_frame_22b_22b(_adapter *adapter, union recv_frame *precv_frame)
{
	int ret = _SUCCESS;
	u8 *ptr = precv_frame->u.hdr.rx_data;
	u8 type, subtype;
	...
	if (pmppriv->bloopback) {
		if (_rtw_memcmp_22b(ptr + 24, pmptx->buf + 24, precv_frame->u.hdr.len - 24) == _FALSE) {
 	...
 	if (pmppriv->bSetRxBssid == _TRUE) {
		...
		if (_rtw_memcmp_22b(bs, adapter->mppriv.network_macaddr, ETH_ALEN) == _FALSE)
			ret = _FAIL;
	...
	if (_rtw_memcmp_22b(get_addr2_ptr(ptr), adapter->mppriv.mac_filter, ETH_ALEN) == _FALSE)
		ret = _FAIL;

	return ret;
}

static sint MPwlanhdr_to_ethhdr_22b(union recv_frame *precvframe)
{
	sint	rmv_len;
	u16 eth_type, len;
	u8	bsnaphdr;
	...
	if (adapter->mppriv.bRTWSmbCfg == _TRUE) {
		/* if(_rtw_memcmp_22b(mcastheadermac, pattrib->dst, 3) == _TRUE) */ /* SimpleConfig Dest. */
		/*			_rtw_memcpy_22b(ptr+ETH_ALEN, pattrib->bssid, ETH_ALEN); */

		if (_rtw_memcmp_22b(mcastheadermac, pattrib->bssid, 3) == _TRUE) /* SimpleConfig Dest. */
			_rtw_memcpy_22b(ptr, pattrib->bssid, ETH_ALEN);
...
int mp_recv_frame_22b(_adapter *padapter, union recv_frame *rframe)
{
	int ret = _SUCCESS;
	...
	if ((check_fwstate(pmlmepriv, _FW_LINKED) == _TRUE)) { /* &&(padapter->mppriv.check_mp_pkt == 0)) */
		if (pattrib->crc_err == 1)
			...
			if (_SUCCESS == validate_mp_recv_frame_22b_22b(padapter, rframe))
		...
		if (pmppriv->rx_bindicatePkt == _FALSE) {
			ret = _FAIL;
			rtw_free_recvframe_22b(rframe, pfree_recv_queue);/* free this recv_frame */
			goto exit;
		} else {
			type =	GetFrameType(ptr);
			pattrib->to_fr_ds = get_tofr_ds(ptr);
			...
			if (type == WIFI_DATA_TYPE) {
				...
				case 0:
					_rtw_memcpy_22b(pattrib->ra, pda, ETH_ALEN);
					_rtw_memcpy_22b(pattrib->ta, psa, ETH_ALEN);
					ret = sta2sta_data_frame_22b(padapter, rframe, &psta);
					break;
				...
				case 1:
					...
					_rtw_memcpy_22b(pattrib->ra, pda, ETH_ALEN);
					_rtw_memcpy_22b(pattrib->ta, pbssid, ETH_ALEN);
					ret = ap2sta_data_frame_22b(padapter, rframe, &psta);
				...
				case 2:
					_rtw_memcpy_22b(pattrib->ra, pbssid, ETH_ALEN);
					_rtw_memcpy_22b(pattrib->ta, psa, ETH_ALEN);
					ret = sta2ap_data_frame_22b(padapter, rframe, &psta);
					break;
				...
				ret = MPwlanhdr_to_ethhdr_22b(rframe);
				...
				if (!RTW_CANNOT_RUN(padapter)) {
					/* indicate this recv_frame */
					ret = rtw_recv_indicatepkt_22b(padapter, rframe);
					if (ret != _SUCCESS) {
						#ifdef DBG_RX_DROP_FRAME
						RTW_INFO("DBG_RX_DROP_FRAME "FUNC_ADPT_FMT" rtw_recv_indicatepkt_22b fail!\n"
							, FUNC_ADPT_ARG(padapter));
...
#define IEEE80211_RADIOTAP_F_BADFCS 0x40 /* bad FCS */
	...
	tmp_16bit = CHAN2FREQ(rtw_get_oper_ch_22b(padapter));
...
int recv_frame_monitor_22b(_adapter *padapter, union recv_frame *rframe)
{
	int ret = _SUCCESS;
	struct rx_pkt_attrib *pattrib = &rframe->u.hdr.attrib;
	struct recv_priv *precvpriv = &padapter->recvpriv;
	...
	if (!RTW_CANNOT_RUN(padapter)) {
		/* indicate this recv_frame */
		ret = rtw_recv_monitor_22b(padapter, rframe);
		if (ret != _SUCCESS) {
			ret = _FAIL;
			rtw_free_recvframe_22b(rframe, pfree_recv_queue); /* free this recv_frame */
			goto exit;
...
#endif
int recv_func_22b_prehandle_22b(_adapter *padapter, union recv_frame *rframe)
{
	int ret = _SUCCESS;
	struct rx_pkt_attrib *pattrib = &rframe->u.hdr.attrib;
	struct recv_priv *precvpriv = &padapter->recvpriv;
	...
	if (padapter->registrypriv.mp_mode == 1 || padapter->mppriv.bRTWSmbCfg == _TRUE) {
		mp_recv_frame_22b(padapter, rframe);
		ret = _FAIL;
		goto exit;
	} else
#endif
	{
		...
		ret = validate_recv_frame_22b(padapter, rframe);
		if (ret != _SUCCESS) {
			rtw_free_recvframe_22b(rframe, pfree_recv_queue);/* free this recv_frame */
			goto exit;
		}
...
int recv_func_22b_posthandle_22b(_adapter *padapter, union recv_frame *prframe)
{
	int ret = _SUCCESS;
	...
	prframe = decryptor_22b(padapter, prframe);
	if (prframe == NULL) {
		#ifdef DBG_RX_DROP_FRAME
		RTW_INFO("DBG_RX_DROP_FRAME "FUNC_ADPT_FMT" decryptor_22b: drop pkt\n"
			, FUNC_ADPT_ARG(padapter));
		#endif
		ret = _FAIL;
	...
	if ((_rtw_memcmp_22b(psnap_type, SNAP_ETH_TYPE_TDLS, ETH_TYPE_LEN)) &&
	    ((*pcategory == RTW_WLAN_CATEGORY_TDLS) || (*pcategory == RTW_WLAN_CATEGORY_P2P))) {
		ret = OnTDLS(padapter, prframe);
		if (ret == _FAIL)
			goto _exit_recv_func_22b;
	}
#endif /* CONFIG_TDLS */

	prframe = recvframe_chk_defrag_22b(padapter, prframe);
	if (prframe == NULL)	{
		#ifdef DBG_RX_DROP_FRAME
		RTW_INFO("DBG_RX_DROP_FRAME "FUNC_ADPT_FMT" recvframe_chk_defrag_22b: drop pkt\n"
			, FUNC_ADPT_ARG(padapter));
		#endif
	...
	prframe = portctrl_22b(padapter, prframe);
	if (prframe == NULL) {
		#ifdef DBG_RX_DROP_FRAME
		RTW_INFO("DBG_RX_DROP_FRAME "FUNC_ADPT_FMT" portctrl_22b: drop pkt\n"
			, FUNC_ADPT_ARG(padapter));
		#endif
		ret = _FAIL;
		DBG_COUNTER(padapter->rx_logs.core_rx_post_portctrl_22b_err);
		goto _recv_data_drop;
	}

	count_rx_stats_22b(padapter, prframe, NULL);
	...
	if (ret == _FAIL) {
		rtw_free_recvframe_22b(orig_prframe, pfree_recv_queue);
		goto _recv_data_drop;
...
int recv_func_22b(_adapter *padapter, union recv_frame *rframe)
{
	int ret;
	...
	if (check_fwstate(mlmepriv, WIFI_MONITOR_STATE)) {
		/* monitor mode */
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 24))
		recv_frame_monitor_22b(padapter, rframe);
#endif
		ret = _SUCCESS;
		goto exit;
	} else
		...
		if (check_fwstate(mlmepriv, WIFI_STATION_STATE) && psecuritypriv->busetkipkey) {
			...
			while ((pending_frame = rtw_alloc_recvframe_22b(&padapter->recvpriv.uc_swdec_pending_queue))) {
				cnt++;
				DBG_COUNTER(padapter->rx_logs.core_rx_dequeue);
				recv_func_22b_posthandle_22b(padapter, pending_frame);
			}
	...
	ret = recv_func_22b_prehandle_22b(padapter, rframe);
	...
	if (ret == _SUCCESS) {
		...
		if (check_fwstate(mlmepriv, WIFI_STATION_STATE) &&
			...
			rtw_enqueue_recvframe_22b(rframe, &padapter->recvpriv.uc_swdec_pending_queue);
			/* RTW_INFO("%s: no key, enqueue uc_swdec_pending_queue\n", __func__); */

			if (recvpriv->free_recvframe_cnt < NR_RECVFRAME / 4) {
				/* to prevent from recvframe starvation, get recvframe from uc_swdec_pending_queue to free_recvframe_cnt */
				rframe = rtw_alloc_recvframe_22b(&padapter->recvpriv.uc_swdec_pending_queue);
				if (rframe)
					goto do_posthandle;
			}
			goto exit;
...
do_posthandle:
		ret = recv_func_22b_posthandle_22b(padapter, rframe);
	}
...
s32 rtw_recv_entry_22b(union recv_frame *precvframe)
{
	_adapter *padapter;
	struct recv_priv *precvpriv;
	s32 ret = _SUCCESS;
	...
	ret = recv_func_22b(padapter, precvframe);
	if (ret == _FAIL) {
	...
	if (adapter->recvpriv.is_signal_dbg) {
		...
		if (num_signal_strength == 0) {
			if (rtw_get_on_cur_ch_time_22b(adapter) == 0
			    || rtw_get_passing_time_ms(rtw_get_on_cur_ch_time_22b(adapter)) < 2 * adapter->mlmeextpriv.mlmext_info.bcn_interval
			   )
				goto set_timer;
		}
...
#ifdef CONFIG_CONCURRENT_MODE
		if (rtw_mi_buddy_check_fwstate_22b(adapter, _FW_UNDER_SURVEY) == _TRUE)
			goto set_timer;
#endif
		...
		ratio_pre_stat = signal_stat_calc_profile_22b[ratio_profile][0];
		ratio_curr_stat = signal_stat_calc_profile_22b[ratio_profile][1];
		...
		RTW_INFO(FUNC_ADPT_FMT" signal_strength:%3u, rssi:%3d, signal_qual:%3u"
			...
			, rtw_get_on_cur_ch_time_22b(adapter) ? rtw_get_passing_time_ms(rtw_get_on_cur_ch_time_22b(adapter)) : 0
			);
#endif
	}
...
void rx_process_phy_info_22b(_adapter *padapter, union recv_frame *rframe)
{
	/* Check RSSI */
	...
	rtw_store_phy_info_22b(padapter, rframe);
}

void rx_query_phy_status_22b(
	union recv_frame	*precvframe,
	u8 *pphy_status)
{
	...
	if (_rtw_memcmp_22b(adapter_mac_addr(padapter), ta, ETH_ALEN) == _TRUE) {
	...
	pkt_info.is_packet_match_bssid = (!IsFrameTypeCtrl(wlanhdr))
		...
		&& ((!MLME_IS_MESH(padapter) && _rtw_memcmp_22b(get_hdr_bssid(wlanhdr), get_bssid(&padapter->mlmepriv), ETH_ALEN))
			|| (MLME_IS_MESH(padapter) && psta));

	pkt_info.is_to_self = (!pattrib->icv_err) && (!pattrib->crc_err)
		&& _rtw_memcmp_22b(ra, adapter_mac_addr(padapter), ETH_ALEN);
	...
	odm_phy_status_query_22b(&pHalData->odmpriv, p_phy_info, pphy_status, &pkt_info);
...

	{
		...
		if ((!MLME_IS_MESH(padapter) && pkt_info.is_packet_match_bssid)
			...
			if (psta) {
				...
				rx_process_phy_info_22b(padapter, precvframe);
			}
	...
	rtw_odm_parse_rx_phy_status_chinfo_22b(precvframe, pphy_status);
}
/*
...
* @return _FALSE:
*/
int rtw_inc_and_chk_continual_no_rx_packet_22b(struct sta_info *sta, int tid_index)
{

	int ret = _FALSE;
	int value = ATOMIC_INC_22b_RETURN(&sta->continual_no_rx_packet[tid_index]);
...
*/
void rtw_reset_continual_no_rx_packet_22b(struct sta_info *sta, int tid_index)
{
	ATOMIC_SET_22b(&sta->continual_no_rx_packet[tid_index], 0);
}

u8 adapter_allow_bmc_data_rx_22b(_adapter *adapter)
{
	...
	if (rtw_linked_check_22b(adapter) == _FALSE)
		return 0;

	return 1;
}

s32 pre_recv_entry_22b(union recv_frame *precvframe, u8 *pphy_status)
{
	s32 ret = _SUCCESS;
	...
	#ifdef CONFIG_MP_INCLUDED
	if (rtw_mp_mode_22b_check(primary_padapter))
		goto bypass_concurrent_hdl;
	#endif

	if (ra_is_bmc == _FALSE) { /*unicast packets*/
		iface = rtw_get_iface_by_macddr_22b(primary_padapter , pda);
		if (NULL == iface) {
		...
		rtw_mi_buddy_clone_bcmc_packet_22b(primary_padapter, precvframe, pphy_status);
bypass_concurrent_hdl:
	...
	if (ra_is_bmc == _TRUE && GetFrameType(pbuf) == WIFI_DATA_TYPE
		&& !adapter_allow_bmc_data_rx_22b(precvframe->u.hdr.adapter)
	) {
		rtw_free_recvframe_22b(precvframe, &precvframe->u.hdr.adapter->recvpriv.free_recv_queue);
		goto exit;
	}

	if (pphy_status)
		rx_query_phy_status_22b(precvframe, pphy_status);
	ret = rtw_recv_entry_22b(precvframe);
...
thread_return rtw_recv_thread(thread_context context)
{
	...
	do {
		err = _rtw_down_sema_22b(&recvpriv->recv_sema);
		if (_FAIL == err) {
		...
		if (err == RTW_RFRAME_UNAVAIL
			...
			rtw_msleep_os_22b(1);
			_rtw_up_sema_22b(&recvpriv->recv_sema);
		}
