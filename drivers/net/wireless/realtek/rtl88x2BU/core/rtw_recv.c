...
sint validate_recv_frame_22b(_adapter *adapter, union recv_frame *precv_frame)
{
	...
	if (pmlmeext->sitesurvey_res.state == SCAN_PROCESS) {
		int ch_set_idx = rtw_chset_search_ch(rfctl->channel_set, rtw_get_oper_ch_22b(adapter));
		if (ch_set_idx >= 0)
...
int amsdu_to_msdu_22b(_adapter *padapter, union recv_frame *prframe)
{
	...
	while (a_len > ETH_HLEN) {
		...
		if (rtw_recv_indicatepkt_22b_check(prframe, rtw_os_pkt_data(sub_pkt), rtw_os_pkt_len(sub_pkt)) == _SUCCESS)
