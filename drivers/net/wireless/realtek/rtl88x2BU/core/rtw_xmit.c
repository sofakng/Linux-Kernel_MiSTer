...
{
	_rtw_init_listhead_22b(&ptxservq->tx_pending);
	_rtw_init_queue_22b(&ptxservq->sta_pending);
	ptxservq->qcnt = 0;
}
...
void	_rtw_init_sta_xmit_priv_22b(struct sta_xmit_priv *psta_xmitpriv)
{
	...
	_rtw_memset_22b((unsigned char *)psta_xmitpriv, 0, sizeof(struct sta_xmit_priv));

	_rtw_spinlock_22b_init_22b(&psta_xmitpriv->lock);
	...
	_rtw_init_listhead_22b(&psta_xmitpriv->legacy_dz);
	_rtw_init_listhead_22b(&psta_xmitpriv->apsd);


}

void rtw_init_xmit_block_22b(_adapter *padapter)
{
	struct dvobj_priv *dvobj = adapter_to_dvobj(padapter);

	_rtw_spinlock_22b_init_22b(&dvobj->xmit_block_lock);
	dvobj->xmit_block = XMIT_BLOCK_NONE;

}
void rtw_free_xmit_block_22b(_adapter *padapter)
{
	struct dvobj_priv *dvobj = adapter_to_dvobj(padapter);

	_rtw_spinlock_22b_free_22b(&dvobj->xmit_block_lock);
}
...
s32	_rtw_init_xmit_priv_22b(struct xmit_priv *pxmitpriv, _adapter *padapter)
{
	...
	/* _rtw_memset_22b((unsigned char *)pxmitpriv, 0, sizeof(struct xmit_priv)); */

	_rtw_spinlock_22b_init_22b(&pxmitpriv->lock);
	_rtw_spinlock_22b_init_22b(&pxmitpriv->lock_sctx);
	_rtw_init_sema_22b(&pxmitpriv->xmit_sema, 0);

	/*
	Please insert all the queue initializaiton using _rtw_init_queue_22b below
	*/

	pxmitpriv->adapter = padapter;

	/* for(i = 0 ; i < MAX_NUMBLKS; i++) */
	/*	_rtw_init_queue_22b(&pxmitpriv->blk_strms[i]); */

	_rtw_init_queue_22b(&pxmitpriv->be_pending);
	_rtw_init_queue_22b(&pxmitpriv->bk_pending);
	_rtw_init_queue_22b(&pxmitpriv->vi_pending);
	_rtw_init_queue_22b(&pxmitpriv->vo_pending);
	_rtw_init_queue_22b(&pxmitpriv->bm_pending);

	/* _rtw_init_queue_22b(&pxmitpriv->legacy_dz_queue); */
	/* _rtw_init_queue_22b(&pxmitpriv->apsd_queue); */

	_rtw_init_queue_22b(&pxmitpriv->free_xmit_queue);
	...
	*/
	...
	for (i = 0; i < NR_XMITFRAME; i++) {
		_rtw_init_listhead_22b(&(pxframe->list));
		...
		rtw_list_insert_tail_22b(&(pxframe->list), &(pxmitpriv->free_xmit_queue.queue));
	...
	/* init xmit_buf */
	_rtw_init_queue_22b(&pxmitpriv->free_xmitbuf_queue);
	_rtw_init_queue_22b(&pxmitpriv->pending_xmitbuf_queue);
	...
	for (i = 0; i < NR_XMITBUFF; i++) {
		_rtw_init_listhead_22b(&pxmitbuf->list);
		...
		res = rtw_os_xmit_resource_alloc_22b(padapter, pxmitbuf, (MAX_XMITBUF_SZ + XMITBUF_ALIGN_SZ), _TRUE);
		if (res == _FAIL) {
		...
		rtw_list_insert_tail_22b(&pxmitbuf->list, &(pxmitpriv->free_xmitbuf_queue.queue));
#ifdef DBG_XMIT_BUF
		pxmitbuf->no = i;
#endif
	...
	_rtw_init_queue_22b(&pxmitpriv->free_xframe_ext_queue);
	...
	for (i = 0; i < NR_XMIT_EXTBUFF; i++) {
		...
		rtw_list_insert_tail_22b(&(pxframe->list), &(pxmitpriv->free_xframe_ext_queue.queue));
	...
	_rtw_init_queue_22b(&pxmitpriv->free_xmit_extbuf_queue);
	...
	for (i = 0; i < NR_XMIT_EXTBUFF; i++) {
		...
		res = rtw_os_xmit_resource_alloc_22b(padapter, pxmitbuf, MAX_XMIT_EXTBUF_SZ + XMITBUF_ALIGN_SZ, _TRUE);
		if (res == _FAIL) {
			res = _FAIL;
			goto exit;
		}
		...
		rtw_list_insert_tail_22b(&pxmitbuf->list, &(pxmitpriv->free_xmit_extbuf_queue.queue));
	...
	for (i = 0; i < CMDBUF_MAX; i++) {
		...
		if (pxmitbuf) {
			...
			res = rtw_os_xmit_resource_alloc_22b(padapter, pxmitbuf, MAX_CMDBUF_SZ + XMITBUF_ALIGN_SZ, _TRUE);
			if (res == _FAIL) {
				res = _FAIL;
				goto exit;
			}
	...
	rtw_alloc_hwxmits_22b(padapter);
	rtw_init_hwxmits_22b(pxmitpriv->hwxmits, pxmitpriv->hwxmit_entry);
	...
	_rtw_init_sema_22b(&(pxmitpriv->tx_retevt), 0);
	...
	_rtw_mutex_init_22b(&pxmitpriv->ack_tx_mutex);
	rtw_sctx_init_22b(&pxmitpriv->ack_tx_ops, 0);
#endif

#ifdef CONFIG_TX_AMSDU
	rtw_init_timer_22b(&(pxmitpriv->amsdu_vo_timer), padapter,
		rtw_amsdu_vo_timeout_handler, padapter);
	pxmitpriv->amsdu_vo_timeout = RTW_AMSDU_TIMER_UNSET;

	rtw_init_timer_22b(&(pxmitpriv->amsdu_vi_timer), padapter,
		rtw_amsdu_vi_timeout_handler, padapter);
	pxmitpriv->amsdu_vi_timeout = RTW_AMSDU_TIMER_UNSET;

	rtw_init_timer_22b(&(pxmitpriv->amsdu_be_timer), padapter,
		rtw_amsdu_be_timeout_handler, padapter);
	pxmitpriv->amsdu_be_timeout = RTW_AMSDU_TIMER_UNSET;

	rtw_init_timer_22b(&(pxmitpriv->amsdu_bk_timer), padapter,
...
#endif
	rtw_init_xmit_block_22b(padapter);
	rtw_hal_init_22b_xmit_priv_22b(padapter);
...
void  rtw_mfree_xmit_priv_lock_22b(struct xmit_priv *pxmitpriv);
void  rtw_mfree_xmit_priv_lock_22b(struct xmit_priv *pxmitpriv)
{
	_rtw_spinlock_22b_free_22b(&pxmitpriv->lock);
	_rtw_free_sema_22b(&pxmitpriv->xmit_sema);

	_rtw_spinlock_22b_free_22b(&pxmitpriv->be_pending.lock);
	_rtw_spinlock_22b_free_22b(&pxmitpriv->bk_pending.lock);
	_rtw_spinlock_22b_free_22b(&pxmitpriv->vi_pending.lock);
	_rtw_spinlock_22b_free_22b(&pxmitpriv->vo_pending.lock);
	_rtw_spinlock_22b_free_22b(&pxmitpriv->bm_pending.lock);

	/* _rtw_spinlock_22b_free_22b(&pxmitpriv->legacy_dz_queue.lock); */
	/* _rtw_spinlock_22b_free_22b(&pxmitpriv->apsd_queue.lock); */

	_rtw_spinlock_22b_free_22b(&pxmitpriv->free_xmit_queue.lock);
	_rtw_spinlock_22b_free_22b(&pxmitpriv->free_xmitbuf_queue.lock);
	_rtw_spinlock_22b_free_22b(&pxmitpriv->pending_xmitbuf_queue.lock);
}
...
void _rtw_free_xmit_priv_22b(struct xmit_priv *pxmitpriv)
{
	...
	rtw_hal_free_xmit_priv_22b(padapter);

	rtw_mfree_xmit_priv_lock_22b(pxmitpriv);
	...
	for (i = 0; i < NR_XMITBUFF; i++) {
		rtw_os_xmit_resource_free_22b(padapter, pxmitbuf, (MAX_XMITBUF_SZ + XMITBUF_ALIGN_SZ), _TRUE);
	...
	_rtw_spinlock_22b_free_22b(&pxmitpriv->free_xframe_ext_queue.lock);

	/* free xmit extension buff */
	_rtw_spinlock_22b_free_22b(&pxmitpriv->free_xmit_extbuf_queue.lock);

	pxmitbuf = (struct xmit_buf *)pxmitpriv->pxmit_extbuf;
	for (i = 0; i < NR_XMIT_EXTBUFF; i++) {
		rtw_os_xmit_resource_free_22b(padapter, pxmitbuf, (MAX_XMIT_EXTBUF_SZ + XMITBUF_ALIGN_SZ), _TRUE);
	...
	for (i = 0; i < CMDBUF_MAX; i++) {
		pxmitbuf = &pxmitpriv->pcmd_xmitbuf[i];
		if (pxmitbuf != NULL)
			rtw_os_xmit_resource_free_22b(padapter, pxmitbuf, MAX_CMDBUF_SZ + XMITBUF_ALIGN_SZ , _TRUE);
	}

	rtw_free_hwxmits_22b(padapter);

#ifdef CONFIG_XMIT_ACK
	_rtw_mutex_free_22b(&pxmitpriv->ack_tx_mutex);
#endif
	rtw_free_xmit_block_22b(padapter);
out:
	return;
}

u8 rtw_get_tx_bw_mode_22b(_adapter *adapter, struct sta_info *sta)
{
	u8 bw;
...
void rtw_get_adapter_tx_rate_bmp_by_bw_22b(_adapter *adapter, u8 bw, u16 *r_bmp_cck_ofdm, u32 *r_bmp_ht, u32 *r_bmp_vht)
{
	...
	for (i = 0; i < macid_ctl->num; i++) {
		if (!rtw_macid_is_used_22b(macid_ctl, i))
			continue;
		if (!rtw_macid_is_iface_specific_22b(macid_ctl, i, adapter))
			continue;
...
void rtw_get_shared_macid_tx_rate_bmp_by_bw_22b(struct dvobj_priv *dvobj, u8 bw, u16 *r_bmp_cck_ofdm, u32 *r_bmp_ht, u32 *r_bmp_vht)
{
	struct macid_ctl_t *macid_ctl = dvobj_to_macidctl(dvobj);
	u16 bmp_cck_ofdm = 0;
	u32 bmp_ht = 0;
	...
	for (i = 0; i < macid_ctl->num; i++) {
		if (!rtw_macid_is_used_22b(macid_ctl, i))
			continue;
		if (!rtw_macid_is_iface_shared_22b(macid_ctl, i))
			continue;
...
void rtw_update_tx_rate_bmp_22b(struct dvobj_priv *dvobj)
{
	...
	for (bw = CHANNEL_WIDTH_20; bw <= CHANNEL_WIDTH_160; bw++) {
		...
		if (hal_is_bw_support_22b(dvobj_get_primary_adapter(dvobj), bw)) {
			for (i = 0; i < dvobj->iface_nums; i++) {
				if (!dvobj->padapters[i])
					continue;
				rtw_get_adapter_tx_rate_bmp_by_bw_22b(dvobj->padapters[i], bw, &tmp_cck_ofdm, &tmp_ht, &tmp_vht);
				bmp_cck_ofdm |= tmp_cck_ofdm;
				bmp_ht |= tmp_ht;
				bmp_vht |= tmp_vht;
			}
			rtw_get_shared_macid_tx_rate_bmp_by_bw_22b(dvobj, bw, &tmp_cck_ofdm, &tmp_ht, &tmp_vht);
			bmp_cck_ofdm |= tmp_cck_ofdm;
			bmp_ht |= tmp_ht;
			bmp_vht |= tmp_vht;
		}
...
#define DBG_HIGHEST_RATE_BMP_BW_CHANGE 0
...

	{
		...
		if (update_ht_rs == _TRUE || update_vht_rs == _TRUE)
			rtw_hal_set_tx_power_level_22b(dvobj_get_primary_adapter(dvobj), hal_data->current_channel);
	}
}

inline u16 rtw_get_tx_rate_bmp_cck_ofdm_22b(struct dvobj_priv *dvobj)
{
	struct rf_ctl_t *rf_ctl = dvobj_to_rfctl(dvobj);
...
inline u32 rtw_get_tx_rate_bmp_ht_by_bw_22b(struct dvobj_priv *dvobj, u8 bw)
{
	struct rf_ctl_t *rf_ctl = dvobj_to_rfctl(dvobj);
...
inline u32 rtw_get_tx_rate_bmp_vht_by_bw_22b(struct dvobj_priv *dvobj, u8 bw)
{
	struct rf_ctl_t *rf_ctl = dvobj_to_rfctl(dvobj);
...
u8 rtw_get_tx_bw_bmp_of_ht_rate_22b(struct dvobj_priv *dvobj, u8 rate, u8 max_bw)
{
	struct rf_ctl_t *rf_ctl = dvobj_to_rfctl(dvobj);
	u8 bw;
	u8 bw_bmp = 0;
...
u8 rtw_get_tx_bw_bmp_of_vht_rate_22b(struct dvobj_priv *dvobj, u8 rate, u8 max_bw)
{
	struct rf_ctl_t *rf_ctl = dvobj_to_rfctl(dvobj);
	u8 bw;
	u8 bw_bmp = 0;
...
static void update_attrib_vcs_info(_adapter *padapter, struct xmit_frame *pxmitframe)
{
	...
	/*
		...
		{
			RTW_INFO("%s, call rtw_get_stainfo_22b()\n", __func__);
			psta=rtw_get_stainfo_22b(&padapter->stapriv ,&pattrib->ra[0] );
		}
...
static void update_attrib_phy_info(_adapter *padapter, struct pkt_attrib *pattrib, struct sta_info *psta)
{
	...
	bw = rtw_get_tx_bw_mode_22b(padapter, psta);
	pattrib->bwmode = rtw_min(bw, mlmeext->cur_bwmode);
	pattrib->sgi = query_ra_short_GI_22b(psta, pattrib->bwmode);
	...
	if (pattrib->direct_link == _TRUE) {
		...
		pattrib->raid = psta->cmn.ra_info.rate_id;
#ifdef CONFIG_80211N_HT
		pattrib->bwmode = rtw_get_tx_bw_mode_22b(padapter, psta);
		pattrib->ht_en = psta->htpriv.ht_option;
		pattrib->ch_offset = psta->htpriv.ch_offset;
		pattrib->sgi = query_ra_short_GI_22b(psta, pattrib->bwmode);
#endif /* CONFIG_80211N_HT */
...
static s32 update_attrib_sec_info(_adapter *padapter, struct pkt_attrib *pattrib, struct sta_info *psta)
{
	...
	_rtw_memset_22b(pattrib->dot118021x_UncstKey.skey,  0, 16);
	_rtw_memset_22b(pattrib->dot11tkiptxmickey.skey,  0, 16);
	...
	case _TKIP_:
		...
		_rtw_memcpy_22b(pattrib->dot11tkiptxmickey.skey, psta->dot11tkiptxmickey.skey, 16);
	...
	if (pattrib->encrypt > 0)
		_rtw_memcpy_22b(pattrib->dot118021x_UncstKey.skey, psta->dot118021x_UncstKey.skey, 16);
	...
	if (pattrib->encrypt && bmcast && _rtw_camctl_chk_flags_22b(padapter, SEC_STATUS_STA_PK_GK_CONFLICT_DIS_BMC_SEARCH))
		pattrib->bswenc = _TRUE;
...
u8	qos_acm_22b(u8 acm_mask, u8 priority)
{
	...
	case 7:
		...
		break;
	default:
		RTW_INFO("qos_acm_22b(): invalid pattrib->priority: %d!!!\n", priority);
		break;
	}
...
static void set_qos(struct pkt_file *ppktfile, struct pkt_attrib *pattrib)
{
	...
	_rtw_open_pktfile_22b(ppktfile->pkt, ppktfile);
	_rtw_pktfile_read_22b(ppktfile, (unsigned char *)&etherhdr, ETH_HLEN);

	/* get UserPriority from IP hdr */
	if (pattrib->ether_type == 0x0800) {
		_rtw_pktfile_read_22b(ppktfile, (u8 *)&ip_hdr, sizeof(ip_hdr));
...
u8 rtw_check_tdls_established(_adapter *padapter, struct pkt_attrib *pattrib)
{
	...
	if (padapter->tdlsinfo.link_established == _TRUE) {
		pattrib->ptdls_sta = rtw_get_stainfo_22b(&padapter->stapriv, pattrib->dst);
#if 1
...
inline u8 rtw_get_hwseq_no_22b(_adapter *padapter)
{
	u8 hwseq_num = 0;
...
static s32 update_attrib(_adapter *padapter, _pkt *pkt, struct pkt_attrib *pattrib)
{
	...
	_rtw_open_pktfile_22b(pkt, &pktfile);
	i = _rtw_pktfile_read_22b(&pktfile, (u8 *)&etherhdr, ETH_HLEN);
	...
	_rtw_memcpy_22b(pattrib->dst, &etherhdr.h_dest, ETH_ALEN);
	_rtw_memcpy_22b(pattrib->src, &etherhdr.h_source, ETH_ALEN);

	if ((check_fwstate(pmlmepriv, WIFI_ADHOC_STATE) == _TRUE) ||
	    (check_fwstate(pmlmepriv, WIFI_ADHOC_MASTER_STATE) == _TRUE)) {
		_rtw_memcpy_22b(pattrib->ra, pattrib->dst, ETH_ALEN);
		_rtw_memcpy_22b(pattrib->ta, adapter_mac_addr(padapter), ETH_ALEN);
		DBG_COUNTER(padapter->tx_logs.core_tx_upd_attrib_adhoc);
	} else if (check_fwstate(pmlmepriv, WIFI_STATION_STATE)) {
#ifdef CONFIG_TDLS
		if (rtw_check_tdls_established(padapter, pattrib) == _TRUE)
			_rtw_memcpy_22b(pattrib->ra, pattrib->dst, ETH_ALEN);	/* For TDLS direct link Tx, set ra to be same to dst */
		else
#endif
			_rtw_memcpy_22b(pattrib->ra, get_bssid(pmlmepriv), ETH_ALEN);
		_rtw_memcpy_22b(pattrib->ta, adapter_mac_addr(padapter), ETH_ALEN);
		DBG_COUNTER(padapter->tx_logs.core_tx_upd_attrib_sta);
	} else if (check_fwstate(pmlmepriv, WIFI_AP_STATE)) {
		_rtw_memcpy_22b(pattrib->ra, pattrib->dst, ETH_ALEN);
		_rtw_memcpy_22b(pattrib->ta, get_bssid(pmlmepriv), ETH_ALEN);
...
get_sta_info:
	...
	if (ETH_P_IP == pattrib->ether_type) {
		...
		_rtw_pktfile_read_22b(&pktfile, ip, 20);

		if (GET_IPV4_IHL(ip) * 4 > 20)
			_rtw_pktfile_read_22b(&pktfile, NULL, GET_IPV4_IHL(ip) - 20);
		...
		if (GET_IPV4_PROTOCOL(ip) == 0x01) { /* ICMP */
			...
			_rtw_pktfile_read_22b(&pktfile, udp, 8);
			...
			&& rtw_st_ctl_chk_reg_s_proto_22b(&psta->st_ctl, 0x06) == _TRUE
		) {
			u8 tcp[20];

			_rtw_pktfile_read_22b(&pktfile, tcp, 20);

			if (rtw_st_ctl_chk_reg_rule_22b(&psta->st_ctl, padapter, IPV4_SRC(ip), TCP_SRC(tcp), IPV4_DST(ip), TCP_DST(tcp)) == _TRUE) {
				if (GET_TCP_SYN(tcp) && GET_TCP_ACK(tcp)) {
					session_tracker_add_cmd_22b(padapter, psta
				...
				if (GET_TCP_FIN(tcp)) {
					session_tracker_del_cmd_22b(padapter, psta
						, IPV4_SRC(ip), TCP_SRC(tcp)
						, IPV4_SRC(ip), TCP_DST(tcp));
					if (DBG_SESSION_TRACKER)
...
#endif
	{
		...
		rtw_lps_ctrl_wk_cmd_22b(padapter, LPS_CTRL_SPECIAL_PACKET, 1);
	}
#endif /* CONFIG_LPS */
...
#endif
		{
			if (pqospriv->qos_option) {
				...
				if (pmlmepriv->acm_mask != 0)
					pattrib->priority = qos_acm_22b(pmlmepriv->acm_mask, pattrib->priority);
			}
		}
	}
	...
	rtw_set_tx_chksum_offload_22b(pkt, pattrib);
...
static s32 xmitframe_addmic(_adapter *padapter, struct xmit_frame *pxmitframe)
{
	...
	/*
		...
		{
			RTW_INFO("%s, call rtw_get_stainfo_22b()\n", __func__);
			stainfo=rtw_get_stainfo_22b(&padapter->stapriv ,&pattrib->ra[0]);
		}
	...
	*/
	...
	if (pattrib->encrypt == _TKIP_) { /* if(psecuritypriv->dot11PrivacyAlgrthm==_TKIP_PRIVACY_) */
		...
		/* if(stainfo!= NULL) */
		{
			...
			if (bmcst) {
				if (_rtw_memcmp_22b(psecuritypriv->dot118021XGrptxmickey[psecuritypriv->dot118021XGrpKeyid].skey, null_key, 16) == _TRUE) {
					/* DbgPrint("\nxmitframe_addmic:stainfo->dot11tkiptxmickey==0\n"); */
					/* rtw_msleep_os_22b(10); */
					return _FAIL;
				}
				/* start to calculate the mic code */
				rtw_secmicsetkey_22b(&micdata, psecuritypriv->dot118021XGrptxmickey[psecuritypriv->dot118021XGrpKeyid].skey);
			} else {
				if (_rtw_memcmp_22b(&pattrib->dot11tkiptxmickey.skey[0], null_key, 16) == _TRUE) {
					/* DbgPrint("\nxmitframe_addmic:stainfo->dot11tkiptxmickey==0\n"); */
					/* rtw_msleep_os_22b(10); */
					return _FAIL;
				}
				/* start to calculate the mic code */
				rtw_secmicsetkey_22b(&micdata, &pattrib->dot11tkiptxmickey.skey[0]);
			}

			if (pframe[1] & 1) { /* ToDS==1 */
				rtw_secmicappend_22b(&micdata, &pframe[16], 6);  /* DA */
				if (pframe[1] & 2) /* From Ds==1 */
					rtw_secmicappend_22b(&micdata, &pframe[24], 6);
				else
					rtw_secmicappend_22b(&micdata, &pframe[10], 6);
			} else {	/* ToDS==0 */
				rtw_secmicappend_22b(&micdata, &pframe[4], 6);   /* DA */
				if (pframe[1] & 2) /* From Ds==1 */
					rtw_secmicappend_22b(&micdata, &pframe[16], 6);
				else
					rtw_secmicappend_22b(&micdata, &pframe[10], 6);
			...
			for (curfragnum = 0; curfragnum < pattrib->nr_frags; curfragnum++) {
				...
				if ((curfragnum + 1) == pattrib->nr_frags) {
					...
					rtw_secmicappend_22b(&micdata, payload, length);
...
static s32 xmitframe_swencrypt(_adapter *padapter, struct xmit_frame *pxmitframe)
{
	...
	if (pattrib->bswenc) {
		...
		RTW_INFO(ADPT_FMT" - sec_type:%s DO SW encryption\n",
			ADPT_ARG(padapter), security_type_str_22b(pattrib->encrypt));
#endif

		switch (pattrib->encrypt) {
		case _WEP40_:
		case _WEP104_:
			rtw_wep_encrypt_22b(padapter, (u8 *)pxmitframe);
			break;
		case _TKIP_:
			rtw_tkip_encrypt_22b(padapter, (u8 *)pxmitframe);
			break;
		case _AES_:
			rtw_aes_encrypt_22b(padapter, (u8 *)pxmitframe);
			break;
#ifdef CONFIG_WAPI_SUPPORT
		case _SMS4_:
...
s32 rtw_make_wlanhdr_22b(_adapter *padapter , u8 *hdr, struct pkt_attrib *pattrib)
{
	u16 *qc;
	...
	*/
	...
	_rtw_memset_22b(hdr, 0, WLANHDR_OFFSET);
	...
	if (pattrib->subtype & WIFI_DATA_TYPE) {
		if ((check_fwstate(pmlmepriv,  WIFI_STATION_STATE) == _TRUE)) {
			...
			if (pattrib->direct_link == _TRUE) {
				...
				_rtw_memcpy_22b(pwlanhdr->addr1, pattrib->dst, ETH_ALEN);
				_rtw_memcpy_22b(pwlanhdr->addr2, pattrib->src, ETH_ALEN);
				_rtw_memcpy_22b(pwlanhdr->addr3, get_bssid(pmlmepriv), ETH_ALEN);
...
#endif /* CONFIG_TDLS */
			{
				...
				SetToDs(fctrl);
				_rtw_memcpy_22b(pwlanhdr->addr1, get_bssid(pmlmepriv), ETH_ALEN);
				_rtw_memcpy_22b(pwlanhdr->addr2, pattrib->ta, ETH_ALEN);
				_rtw_memcpy_22b(pwlanhdr->addr3, pattrib->dst, ETH_ALEN);
			...
			SetFrDs(fctrl);
			_rtw_memcpy_22b(pwlanhdr->addr1, pattrib->dst, ETH_ALEN);
			_rtw_memcpy_22b(pwlanhdr->addr2, get_bssid(pmlmepriv), ETH_ALEN);
			_rtw_memcpy_22b(pwlanhdr->addr3, pattrib->src, ETH_ALEN);
...
s32 rtw_txframes_pending_22b(_adapter *padapter)
{
	...
	return ((_rtw_queue_empty_22b(&pxmitpriv->be_pending) == _FALSE) ||
		(_rtw_queue_empty_22b(&pxmitpriv->bk_pending) == _FALSE) ||
		(_rtw_queue_empty_22b(&pxmitpriv->vi_pending) == _FALSE) ||
		(_rtw_queue_empty_22b(&pxmitpriv->vo_pending) == _FALSE));
}
...
s32 rtw_make_tdls_wlanhdr(_adapter *padapter , u8 *hdr, struct pkt_attrib *pattrib, struct tdls_txmgmt *ptxmgmt)
{
	...
	if (tdls_seq == 1) {
		ptdls_sta = rtw_get_stainfo_22b(pstapriv, pattrib->dst);
		if (ptdls_sta) {
...
s32 rtw_xmit_22b_tdls_coalesce(_adapter *padapter, struct xmit_frame *pxmitframe, struct tdls_txmgmt *ptxmgmt)
{
	...
	if (pattrib->iv_len) {
		...
		_rtw_memcpy_22b(pframe, pattrib->iv, pattrib->iv_len);
	...
	if ((pattrib->icv_len > 0) && (pattrib->bswenc)) {
		...
		_rtw_memcpy_22b(pframe, pattrib->icv, pattrib->icv_len);
 ...
 */
u32 rtw_calculate_wlan_pkt_size_by_attribue_22b(struct pkt_attrib *pattrib)
{
	u32	len = 0;
...
s32 rtw_xmit_22bframe_coalesce_22b_amsdu(_adapter *padapter, struct xmit_frame *pxmitframe, struct xmit_frame *pxmitframe_queue)
{
	...
	if (rtw_make_wlanhdr_22b(padapter, mem_start, pattrib) == _FAIL) {
		RTW_INFO("rtw_xmit_22bframe_coalesce_22b: rtw_make_wlanhdr_22b fail; drop pkt\n");
		res = _FAIL;
		goto exit;
	}
	...
	/* adding icv, if necessary... */
	if (pattrib->iv_len) {
		_rtw_memcpy_22b(pframe, pattrib->iv, pattrib->iv_len); // queue or new?

		RTW_DBG("rtw_xmit_22bframe_coalesce_22b: keyid=%d pattrib->iv[3]=%.2x pframe=%.2x %.2x %.2x %.2x\n",
	...
	if(pxmitframe_queue)
	{
		...
		_rtw_open_pktfile_22b(pkt_queue, &pktfile_queue);
		_rtw_pktfile_read_22b(&pktfile_queue, NULL, pattrib_queue->pkt_hdrlen);

		#ifdef CONFIG_RTW_MESH
		if (MLME_IS_MESH(padapter)) {
			/* mDA(6), mSA(6), len(2), mctrl */
			_rtw_memcpy_22b(pframe, pattrib_queue->mda, ETH_ALEN);
			pframe += ETH_ALEN;
			_rtw_memcpy_22b(pframe, pattrib_queue->msa, ETH_ALEN);
			pframe += ETH_ALEN;
			len = (u16*)pframe;
			pframe += 2;
		...
		#endif
		{
			...
			_rtw_memcpy_22b(pframe, pattrib_queue->dst, ETH_ALEN);
			pframe += ETH_ALEN;
			_rtw_memcpy_22b(pframe, pattrib_queue->src, ETH_ALEN);
			pframe += ETH_ALEN;
			len = (u16*)pframe;
			pframe += 2;
		}

		llc_sz = rtw_put_snap_22b(pframe, pattrib_queue->ether_type);
		pframe += llc_sz;

		mem_sz = _rtw_pktfile_read_22b(&pktfile_queue, pframe, pattrib_queue->pktlen);
		pframe += mem_sz;
		...
		*len = htons(XATTRIB_GET_MCTRL_LEN(pattrib_queue) + llc_sz + mem_sz);
		...
		//_rtw_memset_22b(pframe,0xaa, padding);
		pframe += padding;
	...
	pkt = pxmitframe->pkt;
	_rtw_open_pktfile_22b(pkt, &pktfile);
	_rtw_pktfile_read_22b(&pktfile, NULL, pattrib->pkt_hdrlen);

#ifdef CONFIG_RTW_MESH
	if (MLME_IS_MESH(padapter)) {
		/* mDA(6), mSA(6), len(2), mctrl */
		_rtw_memcpy_22b(pframe, pattrib->mda, ETH_ALEN);
		pframe += ETH_ALEN;
		_rtw_memcpy_22b(pframe, pattrib->msa, ETH_ALEN);
		pframe += ETH_ALEN;
		len = (u16*)pframe;
		pframe += 2;
	...
	{
		/* 802.3 MAC Header  DA(6)  SA(6)  Len(2) */
		_rtw_memcpy_22b(pframe, pattrib->dst, ETH_ALEN);
		pframe += ETH_ALEN;
		_rtw_memcpy_22b(pframe, pattrib->src, ETH_ALEN);
		pframe += ETH_ALEN;
		len = (u16*)pframe;
		pframe += 2;
	}
	...
	mem_sz = _rtw_pktfile_read_22b(&pktfile, pframe, pattrib->pktlen);
...
*/
s32 rtw_xmit_22bframe_coalesce_22b(_adapter *padapter, _pkt *pkt, struct xmit_frame *pxmitframe)
{
	struct pkt_file pktfile;
	...
	*/
	...
	while (1) {
		...
		if (bmcst) {
			...
			mem_sz = _rtw_pktfile_read_22b(&pktfile, pframe, mpdu_len);
		...
		if (bmcst || (rtw_endofpktfile_22b(&pktfile) == _TRUE)) {
		...
		_rtw_memcpy_22b(mem_start, pbuf_start + hw_hdr_offset, pattrib->hdrlen);
...
#define DBG_MGMT_XMIT_ENC_DUMP 0
	...
	if (!bmcst) {
		if (pattrib->psta)
			...
			pattrib->psta = psta = rtw_get_stainfo_22b(&padapter->stapriv, pattrib->ra);
		if (psta == NULL) {
	...
	if (bmcst) {
		if (subtype == WIFI_ACTION && CATEGORY_IS_GROUP_PRIVACY(category)) {
			...
			if (pattrib->psta)
				...
				pattrib->psta = psta = rtw_get_bcmc_stainfo_22b(padapter);
			if (psta == NULL) {
			...
			_rtw_memset_22b(MME, 0, _MME_IE_LENGTH_);
			...
			pframe = rtw_set_ie_22b(pframe, _MME_IE_ , 16 , MME, &(pattrib->pktlen));
			pattrib->last_txcmdsz = pattrib->pktlen;
			/* total frame length - header length */
			frame_body_len = pattrib->pktlen - sizeof(struct rtw_ieee80211_hdr_3addr);

			/* conscruct AAD, copy frame control field */
			_rtw_memcpy_22b(BIP_AAD, &pwlanhdr->frame_ctl, 2);
			ClearRetry(BIP_AAD);
			ClearPwrMgt(BIP_AAD);
			ClearMData(BIP_AAD);
			/* conscruct AAD, copy address 1 to address 3 */
			_rtw_memcpy_22b(BIP_AAD + 2, pwlanhdr->addr1, 18);
			/* copy management fram body */
			_rtw_memcpy_22b(BIP_AAD + BIP_AAD_SIZE, MGMT_body, frame_body_len);
			...
			/* calculate mic */
			if (omac1_aes_128_22b(padapter->securitypriv.dot11wBIPKey[padapter->securitypriv.dot11wBIPKeyid].skey
			...
			_rtw_memcpy_22b(pframe - 8, mic, 8);
	...
	_rtw_memcpy_22b(tmp_buf, pframe, pattrib->pktlen);
	...
	_rtw_memcpy_22b(pframe, tmp_buf + pattrib->hdrlen, pattrib->pktlen - pattrib->hdrlen);
 ...
 */
s32 rtw_put_snap_22b(u8 *data, u16 h_proto)
{
...
void rtw_update_protection_22b(_adapter *padapter, u8 *ie, uint ie_len)
{
	...
	case AUTO_VCS:
	default:
		perp = rtw_get_ie_22b(ie, _ERPINFO_IE_, &erp_len, ie_len);
		if (perp == NULL)
			pxmitpriv->vcs = NONE_VCS;
		else {
...
void rtw_count_tx_stats_22b(PADAPTER padapter, struct xmit_frame *pxmitframe, int sz)
{
	struct sta_info *psta = NULL;
	...
	if ((pxmitframe->frame_tag & 0x0f) == DATA_FRAMETAG) {
		...
		/* traffic_check_for_leave_lps_22b(padapter, _TRUE); */
#endif /* CONFIG_LPS */
		...
		enum cmdbuf_type buf_type)
{
	...
	if (pxmitbuf !=  NULL) {
		...
		if (pxmitbuf->sctx) {
			...
			rtw_sctx_done_22b_err_22b(&pxmitbuf->sctx, RTW_SCTX_DONE_BUF_ALLOC);
		}
	} else
...
struct xmit_frame *__rtw_alloc_cmdxmitframe_22b(struct xmit_priv *pxmitpriv,
		enum cmdbuf_type buf_type)
{
	struct xmit_frame		*pcmdframe;
	struct xmit_buf		*pxmitbuf;

	pcmdframe = rtw_alloc_xmitframe_22b(pxmitpriv);
	if (pcmdframe == NULL) {
	...
	_rtw_memset_22b(pcmdframe->buf_addr, 0, MAX_CMDBUF_SZ);
...
struct xmit_buf *rtw_alloc_xmitbuf_22b_ext_22b(struct xmit_priv *pxmitpriv)
{
	_irqL irqL;
	struct xmit_buf *pxmitbuf =  NULL;
	_list *plist, *phead;
	...
	if (_rtw_queue_empty_22b(pfree_queue) == _TRUE)
		pxmitbuf = NULL;
	else {
...
s32 rtw_free_xmitbuf_22b_ext_22b(struct xmit_priv *pxmitpriv, struct xmit_buf *pxmitbuf)
{
	_irqL irqL;
	_queue *pfree_queue = &pxmitpriv->free_xmit_extbuf_queue;
	...
	rtw_list_insert_tail_22b(&(pxmitbuf->list), get_list_head(pfree_queue));
	pxmitpriv->free_xmit_extbuf_cnt++;
#ifdef DBG_XMIT_BUF_EXT
...
struct xmit_buf *rtw_alloc_xmitbuf_22b(struct xmit_priv *pxmitpriv)
{
	_irqL irqL;
	struct xmit_buf *pxmitbuf =  NULL;
	_list *plist, *phead;
	...
	/* RTW_INFO("+rtw_alloc_xmitbuf_22b\n"); */

	_enter_critical(&pfree_xmitbuf_queue->lock, &irqL);

	if (_rtw_queue_empty_22b(pfree_xmitbuf_queue) == _TRUE)
		pxmitbuf = NULL;
	else {
	...
	if (pxmitbuf !=  NULL) {
		...
		RTW_INFO("DBG_XMIT_BUF rtw_alloc_xmitbuf_22b return NULL\n");
#endif
...
s32 rtw_free_xmitbuf_22b(struct xmit_priv *pxmitpriv, struct xmit_buf *pxmitbuf)
{
	_irqL irqL;
	_queue *pfree_xmitbuf_queue = &pxmitpriv->free_xmitbuf_queue;


	/* RTW_INFO("+rtw_free_xmitbuf_22b\n"); */
	...
	if (pxmitbuf->sctx) {
		RTW_INFO("%s pxmitbuf->sctx is not NULL\n", __func__);
		rtw_sctx_done_22b_err_22b(&pxmitbuf->sctx, RTW_SCTX_DONE_BUF_FREE);
	}

	if (pxmitbuf->buf_tag == XMITBUF_CMD) {
	} else if (pxmitbuf->buf_tag == XMITBUF_MGNT)
		rtw_free_xmitbuf_22b_ext_22b(pxmitpriv, pxmitbuf);
	else {
		_enter_critical(&pfree_xmitbuf_queue->lock, &irqL);

		rtw_list_delete(&pxmitbuf->list);

		rtw_list_insert_tail_22b(&(pxmitbuf->list), get_list_head(pfree_xmitbuf_queue));
...
void rtw_init_xmitframe_22b(struct xmit_frame *pxframe)
{
	if (pxframe !=  NULL) { /* default value setting */
		pxframe->buf_addr = NULL;
		pxframe->pxmitbuf = NULL;

		_rtw_memset_22b(&pxframe->attrib, 0, sizeof(struct pkt_attrib));
...
*/
struct xmit_frame *rtw_alloc_xmitframe_22b(struct xmit_priv *pxmitpriv)/* (_queue *pfree_xmit_queue) */
{
	/*
	...
	*/
	...
	if (_rtw_queue_empty_22b(pfree_xmit_queue) == _TRUE) {
		pxframe =  NULL;
	} else {
	...
	rtw_init_xmitframe_22b(pxframe);
...
s32 rtw_free_xmitframe_22b(struct xmit_priv *pxmitpriv, struct xmit_frame *pxmitframe)
{
	_irqL irqL;
	_queue *queue = NULL;
	_adapter *padapter = pxmitpriv->adapter;
	...
	rtw_list_insert_tail_22b(&pxmitframe->list, get_list_head(queue));
	if (pxmitframe->ext_tag == 0) {
		pxmitpriv->free_xmitframe_cnt++;
	} else if (pxmitframe->ext_tag == 1) {
		pxmitpriv->free_xframe_ext_cnt++;
...
check_pkt_complete:
	...
	if (pndis_pkt)
		rtw_os_pkt_complete_22b(padapter, pndis_pkt);
...
void rtw_free_xmitframe_22b_queue(struct xmit_priv *pxmitpriv, _queue *pframequeue)
{
	_irqL irqL;
	_list	*plist, *phead;
	...
	while (rtw_end_of_queue_search_22b(phead, plist) == _FALSE) {
		...
		rtw_free_xmitframe_22b(pxmitpriv, pxmitframe);
...
s32 rtw_xmit_22bframe_enqueue_22b(_adapter *padapter, struct xmit_frame *pxmitframe)
{
	DBG_COUNTER(padapter->tx_logs.core_tx_enqueue);
	if (rtw_xmit_22b_classifier_22b(padapter, pxmitframe) == _FAIL) {
		/*		pxmitframe->pkt = NULL; */
		return _FAIL;
	}
...
static struct xmit_frame *dequeue_one_xmitframe(struct xmit_priv *pxmitpriv, struct hw_xmit *phwxmit, struct tx_servq *ptxservq, _queue *pframe_queue)
{
	...
	while ((rtw_end_of_queue_search_22b(xmitframe_phead, xmitframe_plist)) == _FALSE) {
		...
		/* rtw_list_insert_tail_22b(&pxmitframe->list, &phwxmit->pending); */
...
struct xmit_frame *rtw_get_xframe_22b(struct xmit_priv *pxmitpriv, int *num_frame)
{
	_irqL irqL0;
	...
	*num_frame = 0;
	...
	for (i = 0; i < entry; i++) {
		...
		while ((rtw_end_of_queue_search_22b(sta_phead, sta_plist)) == _FALSE) {
...
struct xmit_frame *rtw_dequeue_xframe_22b(struct xmit_priv *pxmitpriv, struct hw_xmit *phwxmit_i, sint entry)
{
	_irqL irqL0;
	_list *sta_plist, *sta_phead;
	struct hw_xmit *phwxmit;
	...
	for (i = 0; i < entry; i++) {
		...
		while ((rtw_end_of_queue_search_22b(sta_phead, sta_plist)) == _FALSE) {
			...
			if (pxmitframe) {
				...
				if (_rtw_queue_empty_22b(pframe_queue)) /* must be done after get_next and before break */
...
#else
__inline static struct tx_servq *rtw_get_sta_pending_22b
 ...
 */
s32 rtw_xmit_22b_classifier_22b(_adapter *padapter, struct xmit_frame *pxmitframe)
{
	/* _irqL irqL0; */
	u8	ac_index;
	struct sta_info	*psta;
	...
	*/
	...
	if (psta == NULL) {
		...
		res = _FAIL;
		RTW_INFO("rtw_xmit_22b_classifier_22b: psta == NULL\n");
		goto exit;
	}
	...
	ptxservq = rtw_get_sta_pending_22b(padapter, psta, pattrib->priority, (u8 *)(&ac_index));

	/* _enter_critical(&pstapending->lock, &irqL0); */

	if (rtw_is_list_empty_22b(&ptxservq->tx_pending))
		rtw_list_insert_tail_22b(&ptxservq->tx_pending, get_list_head(phwxmits[ac_index].sta_queue));

	/* _enter_critical(&ptxservq->sta_pending.lock, &irqL1); */

	rtw_list_insert_tail_22b(&pxmitframe->list, get_list_head(&ptxservq->sta_pending));
	ptxservq->qcnt++;
...
void rtw_init_hwxmits_22b(struct hw_xmit *phwxmit, sint entry)
{
	...
	for (i = 0; i < entry; i++, phwxmit++) {
		/* _rtw_spinlock_22b_init_22b(&phwxmit->xmit_lock); */
		/* _rtw_init_listhead_22b(&phwxmit->pending);		 */
		/* phwxmit->txcmdcnt = 0; */
		phwxmit->accnt = 0;
	}
}
...
#ifdef CONFIG_BR_EXT
int rtw_br_client_tx_22b(_adapter *padapter, struct sk_buff **pskb)
{
	...
	{
		void dhcp_flag_bcast_22b(_adapter *priv, struct sk_buff *skb);
			...
			/* if (!priv->pmib->ethBrExtInfo.nat25_disable)		 */
		{
			...
			if (*((unsigned short *)(skb->data + MACADDRLEN * 2)) == __constant_htons(ETH_P_IP)) {
				if (memcmp(padapter->scdb_mac, skb->data + MACADDRLEN, MACADDRLEN)) {
					void *scdb_findEntry_22b(_adapter *priv, unsigned char *macAddr, unsigned char *ipAddr);

					padapter->scdb_entry = (struct nat25_network_db_entry *)scdb_findEntry_22b(padapter,
...
#endif /* 1 */
			if (do_nat25) {
				int nat25_db_handle_22b(_adapter *priv, struct sk_buff *skb, int method);
				if (nat25_db_handle_22b(padapter, skb, NAT25_CHECK) == 0) {
				...
				res = nat25_db_handle_22b(padapter, skb, NAT25_INSERT);
				if (res < 0) {
					if (res == -2) {
						/* priv->ext_stats.tx_drops++; */
						DEBUG_ERR("TX DROP: nat25_db_handle_22b fail!\n");
						/* goto free_and_stop; */
						return -1;

					}
					/* we just print warning message and let it go */
					/* DEBUG_WARN("%s()-%d: nat25_db_handle_22b INSERT Warning!\n", __FUNCTION__, __LINE__); */
			...
			dhcp_flag_bcast_22b(padapter, skb);
...
s32 rtw_monitor_xmit_entry_22b(struct sk_buff *skb, struct net_device *ndev)
{
	u16 frame_ctl;
	...
	_rtw_open_pktfile_22b((_pkt *)skb, &pktfile);
	_rtw_pktfile_read_22b(&pktfile, (u8 *)(&rtap_hdr), sizeof(struct ieee80211_radiotap_header));
	...
	_rtw_pktfile_read_22b(&pktfile, dummybuf, rtap_len-sizeof(struct ieee80211_radiotap_header));
	len = len - rtap_len;

	pmgntframe = alloc_mgtxmitframe_22b(pxmitpriv);
	if (pmgntframe == NULL) {
		rtw_udelay_os_22b(500);
		goto fail;
	}

	_rtw_memset_22b(pmgntframe->buf_addr, 0, WLANHDR_OFFSET + TXDESC_OFFSET);
	pframe = (u8 *)(pmgntframe->buf_addr) + TXDESC_OFFSET;
//	_rtw_memcpy_22b(pframe, (void *)checking, len);
	_rtw_pktfile_read_22b(&pktfile, pframe, len);
	...
	if ((frame_ctl & RTW_IEEE80211_FCTL_FTYPE) == RTW_IEEE80211_FTYPE_DATA) {
		...
		update_monitor_frame_attrib_22b(padapter, pattrib);
...
s32 rtw_xmit_22b_posthandle(_adapter *padapter, struct xmit_frame *pxmitframe, _pkt *pkt)
{
...
#ifdef CONFIG_AP_MODE
	_enter_critical_bh(&pxmitpriv->lock, &irqL0);
	if (xmitframe_enqueue_for_sleeping_sta_22b(padapter, pxmitframe) == _TRUE) {
		_exit_critical_bh(&pxmitpriv->lock, &irqL0);
	...
	/* pre_xmitframe */
	if (rtw_hal_xmit_22b(padapter, pxmitframe) == _FALSE)
		return 1;
 ...
 *	<0	fail
 */
s32 rtw_xmit_22b(_adapter *padapter, _pkt **ppkt)
{
	...
	pxmitframe = rtw_alloc_xmitframe_22b(pxmitpriv);
	...
	if (check_fwstate(&padapter->mlmepriv, WIFI_STATION_STATE | WIFI_ADHOC_STATE) == _TRUE) {
		...
		if (br_port) {
			res = rtw_br_client_tx_22b(padapter, ppkt);
			if (res == -1) {
				rtw_free_xmitframe_22b(pxmitpriv, pxmitframe);
			...
			while ((rtw_end_of_queue_search_22b(&b2u_list, list)) == _FALSE) {
				...
				if (!b2uframe->pkt) {
					if (res == RTW_BMC_NO_NEED)
						res = _SUCCESS;
					rtw_free_xmitframe_22b(pxmitpriv, b2uframe);
					continue;
				}

				rtw_xmit_22b_posthandle(padapter, b2uframe, b2uframe->pkt);
			}
		}
		#endif /* CONFIG_RTW_MESH_DATA_BMC_TO_UC */

		if (res == RTW_BMC_NO_NEED) {
			rtw_free_xmitframe_22b(&padapter->xmitpriv, pxmitframe);
			return 0;
		}
	}
#endif /* CONFIG_RTW_MESH */

	pxmitframe->pkt = NULL; /* let rtw_xmit_22b_posthandle not to free pkt inside */
	res = rtw_xmit_22b_posthandle(padapter, pxmitframe, *ppkt);
...
sint xmitframe_enqueue_for_tdls_sleeping_sta(_adapter *padapter, struct xmit_frame *pxmitframe)
{
	...
	if (ptdls_sta == NULL)
		...
		if (ptdls_sta->state & WIFI_SLEEP_STATE) {
			...
			rtw_list_insert_tail_22b(&pxmitframe->list, get_list_head(&ptdls_sta->sleep_q));
			...
			if (ptdls_sta->sleepq_len == 1)
				rtw_tdls_cmd_22b(padapter, ptdls_sta->cmn.mac_addr, TDLS_ISSUE_PTI);
...
inline bool xmitframe_hiq_filter_22b(struct xmit_frame *xmitframe)
{
	bool allow = _FALSE;
...
sint xmitframe_enqueue_for_sleeping_sta_22b(_adapter *padapter, struct xmit_frame *pxmitframe)
{
	...
	*/
	...
	if (pattrib->triggered == 1) {
		...
		/* pattrib->triggered=0; */
		if (bmcst && xmitframe_hiq_filter_22b(pxmitframe) == _TRUE)
	...
	if (bmcst) {
		...
		if (rtw_tim_map_anyone_be_set(padapter, pstapriv->sta_dz_bitmap)) { /* if anyone sta is in ps mode */
			...
			rtw_list_insert_tail_22b(&pxmitframe->list, get_list_head(&psta->sleep_q));
			...
			if (update_tim == _TRUE) {
				if (is_broadcast_mac_addr(pattrib->ra))
					_update_beacon_22b(padapter, _TIM_IE_, NULL, _TRUE, "buffer BC");
				else
					_update_beacon_22b(padapter, _TIM_IE_, NULL, _TRUE, "buffer MC");
			} else
				chk_bmc_sleepq_cmd_22b(padapter);
		...
		if (rtw_tim_map_is_set(padapter, pstapriv->sta_dz_bitmap, psta->cmn.aid)) {
			...
			if (((psta->has_legacy_ac) && (!wmmps_ac)) || ((!psta->has_legacy_ac) && (wmmps_ac))) {
				...
				if (update_tim == _TRUE) {
					...
					_update_beacon_22b(padapter, _TIM_IE_, NULL, _TRUE, "buffer UC");
				}
			}
...
static void dequeue_xmitframes_to_sleeping_queue(_adapter *padapter, struct sta_info *psta, _queue *pframequeue)
{
	...
	while (rtw_end_of_queue_search_22b(phead, plist) == _FALSE) {
		...
		ret = xmitframe_enqueue_for_sleeping_sta_22b(padapter, pxmitframe);
		...
		if (_TRUE == ret) {
			...
			/* RTW_INFO("xmitframe_enqueue_for_sleeping_sta_22b return _FALSE\n"); */
		}
...
_exit:
	...
	if (update_mask) {
		...
		if ((update_mask & (BIT(0) | BIT(1))) == (BIT(0) | BIT(1)))
			_update_beacon_22b(padapter, _TIM_IE_, NULL, _TRUE, "clear UC&BMC");
		else if ((update_mask & BIT(1)) == BIT(1))
			_update_beacon_22b(padapter, _TIM_IE_, NULL, _TRUE, "clear BMC");
		else
			_update_beacon_22b(padapter, _TIM_IE_, NULL, _TRUE, "clear UC");
	}
	...
	struct xmit_buf *pxmitbuf)
{
	...
	rtw_list_delete(&pxmitbuf->list);
	rtw_list_insert_tail_22b(&pxmitbuf->list, get_list_head(pqueue));
	_exit_critical_bh(&pqueue->lock, &irql);

#if defined(CONFIG_SDIO_HCI) && defined(CONFIG_CONCURRENT_MODE)
	pri_adapter = GET_PRIMARY_ADAPTER(pri_adapter);
#endif /*SDIO_HCI + CONCURRENT*/
	_rtw_up_sema_22b(&(pri_adapter->xmitpriv.xmit_sema));
}
	...
	struct xmit_buf *pxmitbuf)
{
	...
	_enter_critical_bh(&pqueue->lock, &irql);
	rtw_list_delete(&pxmitbuf->list);
	rtw_list_insert_head_22b(&pxmitbuf->list, get_list_head(pqueue));
	_exit_critical_bh(&pqueue->lock, &irql);
}
	...
	struct xmit_priv *pxmitpriv)
{
	...
	if (_rtw_queue_empty_22b(pqueue) == _FALSE) {
		_list *plist, *phead;
...
struct xmit_buf *select_and_dequeue_pending_xmitbuf(_adapter *padapter)
{
	...
	if (_TRUE == rtw_is_xmit_blocked_22b(padapter))
		return pxmitbuf;

	pxmitbuf = dequeue_pending_xmitbuf_ext(pxmitpriv);
	if (pxmitbuf == NULL && rtw_xmit_22b_ac_blocked(padapter) != _TRUE)
...
thread_return rtw_xmit_22b_thread(thread_context context)
{
	...
	do {
		err = rtw_hal_xmit_22b_thread_handler(padapter);
...
void rtw_set_xmit_block_22b(_adapter *padapter, enum XMIT_BLOCK_REASON reason)
{
	_irqL irqL;
	struct dvobj_priv *dvobj = adapter_to_dvobj(padapter);
...
void rtw_clr_xmit_block_22b(_adapter *padapter, enum XMIT_BLOCK_REASON reason)
{
	_irqL irqL;
	struct dvobj_priv *dvobj = adapter_to_dvobj(padapter);
...
bool rtw_is_xmit_blocked_22b(_adapter *padapter)
{
	struct dvobj_priv *dvobj = adapter_to_dvobj(padapter);
...
#ifdef CONFIG_USB_HCI
	if (rtw_hal_sreset_inprogress_22b(adapter) == _TRUE) {
		blocked = _TRUE;
		goto exit;
	}
#endif/* #ifdef CONFIG_USB_HCI */
...
void rtw_tx_desc_backup(_adapter *padapter, struct xmit_frame *pxmitframe, u8 desc_size, u8 hwq)
{
	...
	_rtw_memcpy_22b(tx_backup[hwq][backup_idx[hwq]].tx_bak_desc, pxmit_buf, desc_size);
	_rtw_memcpy_22b(tx_backup[hwq][backup_idx[hwq]].tx_bak_data_hdr, pxmit_buf+desc_size, TX_BAK_DATA_LEN);
...
void rtw_tx_desc_backup_reset(void)
{
	...
	for (i = 0; i < HW_QUEUE_ENTRY; i++) {
		for (j = 0; j < TX_BAK_FRMAE_CNT; j++)
			_rtw_memset_22b(&tx_backup[i][j], 0, sizeof(struct rtw_tx_desc_backup));
...
bool rtw_sctx_chk_waring_status_22b(int status)
{
	switch (status) {
...
{
	if (*sctx) {
		if (rtw_sctx_chk_waring_status_22b(status))
			RTW_INFO("%s status:%d\n", __func__, status);
		(*sctx)->status = status;
#ifdef PLATFORM_LINUX
		complete(&((*sctx)->done));
...
{
	rtw_sctx_done_22b_err_22b(sctx, RTW_SCTX_DONE_SUCCESS);
}
...
int rtw_ack_tx_wait_22b(struct xmit_priv *pxmitpriv, u32 timeout_ms)
{
	...
	return rtw_sctx_wait_22b(pack_tx_ops, __func__);
}
...
void rtw_ack_tx_done_22b(struct xmit_priv *pxmitpriv, int status)
{
	...
	if (pxmitpriv->ack_tx)
		rtw_sctx_done_22b_err_22b(&pack_tx_ops, status);
	else
