...
void rm_timer_callback(void *data)
{
	...
	for (i=0;i<RM_TIMER_NUM;i++) {
		...
		if (pclock->prm == NULL
			||(ATOMIC_READ_22b(&(pclock->counter)) == 0))
			continue;

		ATOMIC_DEC_22b(&(pclock->counter));

		if (ATOMIC_READ_22b(&(pclock->counter)) == 0)
...
int rtw_init_rm(_adapter *padapter)
{
	...
	RTW_INFO("RM: %s\n",__func__);
	_rtw_init_queue_22b(&(prmpriv->rm_queue));
	_rtw_init_queue_22b(&(prmpriv->ev_queue));
	...
	/* clock timer */
	rtw_init_timer_22b(&prmpriv->rm_timer,
...
int rtw_deinit_rm(_adapter *padapter)
{
	...
	_rtw_deinit_queue_22b(&(prmpriv->rm_queue));
	_rtw_deinit_queue_22b(&(prmpriv->ev_queue));
...
{
	ATOMIC_SET_22b(&(prm->pclock->counter), (ms/CLOCK_UNIT));
...
static struct rm_clock *rm_alloc_clock(_adapter *padapter, struct rm_obj *prm)
{
	...
	for (i=0;i<RM_TIMER_NUM;i++) {
		...
		if (pclock->prm == NULL) {
			pclock->prm = prm;
			ATOMIC_SET_22b(&(pclock->counter), 0);
...
{
	ATOMIC_SET_22b(&(prm->pclock->counter), 0);
...
struct rm_obj *rm_alloc_rmobj(_adapter *padapter)
{
	...
	_rtw_memset_22b(prm, 0, sizeof(struct rm_obj));
...
int rm_enqueue_rmobj(_adapter *padapter, struct rm_obj *prm, bool to_head)
{
	...
	_enter_critical(&queue->lock, &irqL);
	if (to_head)
		rtw_list_insert_head_22b(&prm->list, &queue->queue);
	else
		rtw_list_insert_tail_22b(&prm->list, &queue->queue);
	_exit_critical(&queue->lock, &irqL);
...
static int rm_state_do_meas(struct rm_obj *prm, enum RM_EV_ID evid)
{
	...
	case RM_EV_start_meas:
		if (prm->q.action_code == RM_ACT_RADIO_MEAS_REQ) {
			...
			rtw_hal_get_hwreg_22b(padapter, HW_VAR_TSF, (u8 *)&val64);
	...
	case RM_EV_state_out:
		...
		rtw_hal_get_hwreg_22b(padapter, HW_VAR_TSF, (u8 *)&val64);
		_rtw_memcpy_22b(&prm->meas_end_time, (char *)&val64, sizeof(u64));

		val8 = 0; /* Disable free run counter */
		rtw_hal_set_hwreg_22b(padapter, HW_VAR_FREECNT, &val8);
		break;
	default:
		break;
	}
