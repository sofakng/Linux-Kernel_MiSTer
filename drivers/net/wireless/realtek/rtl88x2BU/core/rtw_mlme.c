...
extern void indicate_wx_scan_complete_event_22b(_adapter *padapter);
extern u8 rtw_do_join_22b(_adapter *padapter);


void rtw_init_mlme_timer_22b(_adapter *padapter)
{
	struct	mlme_priv *pmlmepriv = &padapter->mlmepriv;

	rtw_init_timer_22b(&(pmlmepriv->assoc_timer), padapter, rtw_join_timeout_handler_22b, padapter);
	rtw_init_timer_22b(&(pmlmepriv->scan_to_timer), padapter, rtw_scan_timeout_handler_22b, padapter);

#ifdef CONFIG_DFS_MASTER
	rtw_init_timer_22b(&(pmlmepriv->dfs_master_timer), padapter, rtw_dfs_master_timer_hdl, padapter);
#endif

#ifdef CONFIG_SET_SCAN_DENY_TIMER
	rtw_init_timer_22b(&(pmlmepriv->set_scan_deny_timer), padapter, rtw_set_scan_deny_22b_timer_hdl_22b, padapter);
#endif
...
sint	_rtw_init_mlme_priv_22b_22b(_adapter *padapter)
{
	sint	i;
	u8	*pbuf;
	...
	/* _rtw_memset_22b((u8 *)pmlmepriv, 0, sizeof(struct mlme_priv)); */
	...
	/*pmlmepriv->fw_state = WIFI_STATION_STATE; */ /*Must sync with rtw_wdev_alloc_22b()*/
	...
	_rtw_spinlock_22b_init_22b(&(pmlmepriv->lock));
	_rtw_init_queue_22b(&(pmlmepriv->free_bss_pool));
	_rtw_init_queue_22b(&(pmlmepriv->scanned_queue));

	set_scanned_network_val(pmlmepriv, 0);

	_rtw_memset_22b(&pmlmepriv->assoc_ssid, 0, sizeof(NDIS_802_11_SSID));
	...
	for (i = 0; i < MAX_BSS_CNT; i++) {
		_rtw_init_listhead_22b(&(pnetwork->list));

		rtw_list_insert_tail_22b(&(pnetwork->list), &(pmlmepriv->free_bss_pool.queue));
...
#endif	
#endif
	rtw_init_mlme_timer_22b(padapter);
...
void rtw_mfree_mlme_priv_lock_22b(struct mlme_priv *pmlmepriv);
void rtw_mfree_mlme_priv_lock_22b(struct mlme_priv *pmlmepriv)
{
	_rtw_spinlock_22b_free_22b(&pmlmepriv->lock);
	_rtw_spinlock_22b_free_22b(&(pmlmepriv->free_bss_pool.lock));
	_rtw_spinlock_22b_free_22b(&(pmlmepriv->scanned_queue.lock));
}
...
void rtw_free_mlme_priv_22b_ie_data_22b(struct mlme_priv *pmlmepriv)
{
...
#if defined(CONFIG_WFD) && defined(CONFIG_IOCTL_CFG80211)
int rtw_mlme_update_wfd_ie_data_22b(struct mlme_priv *mlme, u8 type, u8 *ie, u32 ie_len)
{
	...
	if (!hal_chk_wl_func_22b(adapter, WL_FUNC_MIRACAST))
		goto success;
	...
	if (!clear) {
		*t_ie = rtw_malloc(ie_len);
		...
		_rtw_memcpy_22b(*t_ie, ie, ie_len);
		*t_ie_len = ie_len;
	}
	...
	if (*t_ie && *t_ie_len) {
		...
		attr_content = rtw_get_wfd_attr_22b_content_22b(*t_ie, *t_ie_len, WFD_ATTR_DEVICE_INFO, NULL, &attr_contentlen);
...
void _rtw_free_mlme_priv_22b_22b(struct mlme_priv *pmlmepriv)
{
	if (NULL == pmlmepriv) {
		rtw_warn_on(1);
		goto exit;
	}
	rtw_free_mlme_priv_22b_ie_data_22b(pmlmepriv);

	if (pmlmepriv) {
		rtw_mfree_mlme_priv_lock_22b(pmlmepriv);
...
sint	_rtw_enqueue_network_22b_22b(_queue *queue, struct wlan_network *pnetwork)
{
	_irqL irqL;
	...
	rtw_list_insert_tail_22b(&pnetwork->list, &queue->queue);
...
struct	wlan_network *_rtw_dequeue_network(_queue *queue)
{
	...
	if (_rtw_queue_empty_22b(queue) == _TRUE)
...
struct	wlan_network *_rtw_alloc_network_22b_22b(struct	mlme_priv *pmlmepriv) /* (_queue *free_queue) */
{
	_irqL	irqL;
	struct	wlan_network	*pnetwork;
	...
	if (_rtw_queue_empty_22b(free_queue) == _TRUE) {
		pnetwork = NULL;
		goto exit;
	}
...
void _rtw_free_network_22b_22b(struct	mlme_priv *pmlmepriv , struct wlan_network *pnetwork, u8 isfreeall)
{
	u32 delta_time;
	...
	rtw_list_insert_tail_22b(&(pnetwork->list), &(free_queue->queue));

	pmlmepriv->num_of_scanned--;


	/* RTW_INFO("_rtw_free_network_22b_22b:SSID=%s\n", pnetwork->network.Ssid.Ssid); */
...
void _rtw_free_network_22b_22b_nolock(struct	mlme_priv *pmlmepriv, struct wlan_network *pnetwork)
{
	...
	rtw_list_insert_tail_22b(&(pnetwork->list), get_list_head(free_queue));
...
void _rtw_free_network_22b_22b_queue(_adapter *padapter, u8 isfreeall)
{
	_irqL irqL;
	_list *phead, *plist;
	struct wlan_network *pnetwork;
	...
	while (rtw_end_of_queue_search_22b(phead, plist) == _FALSE) {
		...
		_rtw_free_network_22b_22b(pmlmepriv, pnetwork, isfreeall);
...
sint rtw_if_up_22b(_adapter *padapter)
{
...
void rtw_generate_random_ibss_22b(u8 *pibss)
{
	*((u32 *)(&pibss[2])) = rtw_random32_22b();
	pibss[0] = 0x02; /* in ad-hoc mode local bit must set to 1 */
	pibss[1] = 0x11;
	pibss[2] = 0x87;
}

u8 *rtw_get_capability_22b_from_ie_22b(u8 *ie)
{
	return ie + 8 + 2;
}


u16 rtw_get_capability_22b(WLAN_BSSID_EX *bss)
{
	u16	val;

	_rtw_memcpy_22b((u8 *)&val, rtw_get_capability_22b_from_ie_22b(bss->IEs), 2);

	return le16_to_cpu(val);
}

u8 *rtw_get_timestampe_from_ie_22b(u8 *ie)
{
	return ie + 0;
}

u8 *rtw_get_beacon_interval_22b_from_ie_22b(u8 *ie)
{
	return ie + 8;
}


int	rtw_init_mlme_priv_22b(_adapter *padapter) /* (struct	mlme_priv *pmlmepriv) */
{
	int	res;
	res = _rtw_init_mlme_priv_22b_22b(padapter);/* (pmlmepriv); */
	return res;
}

void rtw_free_mlme_priv_22b(struct mlme_priv *pmlmepriv)
{
	_rtw_free_mlme_priv_22b_22b(pmlmepriv);
}

int	rtw_enqueue_network_22b(_queue *queue, struct wlan_network *pnetwork);
int	rtw_enqueue_network_22b(_queue *queue, struct wlan_network *pnetwork)
{
	int	res;
	res = _rtw_enqueue_network_22b_22b(queue, pnetwork);
	return res;
}
...
struct	wlan_network *rtw_alloc_network_22b(struct	mlme_priv *pmlmepriv);
struct	wlan_network *rtw_alloc_network_22b(struct	mlme_priv *pmlmepriv) /* (_queue	*free_queue) */
{
	struct	wlan_network	*pnetwork;
	pnetwork = _rtw_alloc_network_22b_22b(pmlmepriv);
	return pnetwork;
}

void rtw_free_network_22b(struct mlme_priv *pmlmepriv, struct	wlan_network *pnetwork, u8 is_freeall);
void rtw_free_network_22b(struct mlme_priv *pmlmepriv, struct	wlan_network *pnetwork, u8 is_freeall)/* (struct	wlan_network *pnetwork, _queue	*free_queue) */
{
	_rtw_free_network_22b_22b(pmlmepriv, pnetwork, is_freeall);
}

void rtw_free_network_22b_nolock(_adapter *padapter, struct wlan_network *pnetwork);
void rtw_free_network_22b_nolock(_adapter *padapter, struct wlan_network *pnetwork)
{
	_rtw_free_network_22b_22b_nolock(&(padapter->mlmepriv), pnetwork);
#ifdef CONFIG_IOCTL_CFG80211
	rtw_cfg80211_unlink_bss_22b(padapter, pnetwork);
#endif /* CONFIG_IOCTL_CFG80211 */
}


void rtw_free_network_22b_queue(_adapter *dev, u8 isfreeall)
{
	_rtw_free_network_22b_22b_queue(dev, isfreeall);
}

struct wlan_network *_rtw_find_network_22b_22b(_queue *scanned_queue, const u8 *addr)
{
	_list	*phead, *plist;
	struct	wlan_network *pnetwork = NULL;
	u8 zero_addr[ETH_ALEN] = {0, 0, 0, 0, 0, 0};

	if (_rtw_memcmp_22b(zero_addr, addr, ETH_ALEN)) {
		pnetwork = NULL;
		goto exit;
	}
	...
	while (plist != phead) {
		...
		if (_rtw_memcmp_22b(addr, pnetwork->network.MacAddress, ETH_ALEN) == _TRUE)
			break;
...
struct wlan_network *rtw_find_network_22b(_queue *scanned_queue, const u8 *addr)
{
	struct	wlan_network *pnetwork;
	_irqL irqL;

	 _enter_critical_bh(&scanned_queue->lock, &irqL);
	pnetwork = _rtw_find_network_22b_22b(scanned_queue, addr);
	_exit_critical_bh(&scanned_queue->lock, &irqL);

	return pnetwork;
}

int rtw_is_same_ibss_22b(_adapter *adapter, struct wlan_network *pnetwork)
{
	int ret = _TRUE;
...
inline int is_same_ess_22b(WLAN_BSSID_EX *a, WLAN_BSSID_EX *b)
{
	return (a->Ssid.SsidLength == b->Ssid.SsidLength)
	       &&  _rtw_memcmp_22b(a->Ssid.Ssid, b->Ssid.Ssid, a->Ssid.SsidLength) == _TRUE;
}

int is_same_network_22b(WLAN_BSSID_EX *src, WLAN_BSSID_EX *dst, u8 feature)
{
	u16 s_cap, d_cap;
	...
	_rtw_memcpy_22b((u8 *)&s_cap, rtw_get_capability_22b_from_ie_22b(src->IEs), 2);
	_rtw_memcpy_22b((u8 *)&d_cap, rtw_get_capability_22b_from_ie_22b(dst->IEs), 2);
	...
	if ((feature == 1) && /* 1: P2P supported */
	    (_rtw_memcmp_22b(src->MacAddress, dst->MacAddress, ETH_ALEN) == _TRUE)
	   )
		return _TRUE;
#endif
	...
	if (((_rtw_memcmp_22b(src->MacAddress, dst->MacAddress, ETH_ALEN)) == _TRUE) &&
		...
		if ((src->Ssid.SsidLength == dst->Ssid.SsidLength) && 
			(((_rtw_memcmp_22b(src->Ssid.Ssid, dst->Ssid.Ssid, src->Ssid.SsidLength)) == _TRUE) || //Case of normal AP
			(is_all_null_22b(src->Ssid.Ssid, src->Ssid.SsidLength) == _TRUE || is_all_null_22b(dst->Ssid.Ssid, dst->Ssid.SsidLength) == _TRUE))) //Case of hidden AP
			return _TRUE;
...
struct wlan_network *_rtw_find_same_network_22b_22b(_queue *scanned_queue, struct wlan_network *network)
{
	_list *phead, *plist;
	...
	while (plist != phead) {
		...
		if (is_same_network_22b(&network->network, &found->network, 0))
			break;
...
struct wlan_network *rtw_find_same_network_22b(_queue *scanned_queue, struct wlan_network *network)
{
	_irqL irqL;
	struct wlan_network *found = NULL;
	...
	_enter_critical_bh(&scanned_queue->lock, &irqL);
	found = _rtw_find_same_network_22b_22b(scanned_queue, network);
	_exit_critical_bh(&scanned_queue->lock, &irqL);
...
struct	wlan_network	*rtw_get_oldest_wlan_network_22b(_queue *scanned_queue)
{
	_list	*plist, *phead;
...
void update_network_22b(WLAN_BSSID_EX *dst, WLAN_BSSID_EX *src,
		    _adapter *padapter, bool update_ie)
{
	...
	if (check_fwstate(&padapter->mlmepriv, _FW_LINKED) && is_same_network_22b(&(padapter->mlmepriv.cur_network.network), src, 0)) {
	...
	if (update_ie) {
		...
		_rtw_memcpy_22b((u8 *)dst, (u8 *)src, get_WLAN_BSSID_EX_sz(src));
	}
 ...
 * 	RTW_INFO("update_network_22b: rssi=0x%lx dst->Rssi=%d ,dst->Rssi=0x%lx , src->Rssi=0x%lx",(dst->Rssi+src->Rssi)/2,dst->Rssi,dst->Rssi,src->Rssi); */
	if (check_fwstate(&padapter->mlmepriv, _FW_LINKED) && is_same_network_22b(&(padapter->mlmepriv.cur_network.network), src)) {

		/* RTW_INFO("b:ssid=%s update_network_22b: src->rssi=0x%d padapter->recvpriv.ui_rssi=%d\n",src->Ssid.Ssid,src->Rssi,padapter->recvpriv.signal); */
		...
		/*	RTW_INFO("ELSE:ssid=%s update_network_22b: src->rssi=0x%d dst->rssi=%d\n",src->Ssid.Ssid,src->Rssi,dst->Rssi); */
		src->Rssi = (src->Rssi + dst->Rssi) / 2; /* dBM */
	}

	/*	RTW_INFO("a:update_network_22b: src->rssi=0x%d padapter->recvpriv.ui_rssi=%d\n",src->Rssi,padapter->recvpriv.signal); */
...
static void update_current_network(_adapter *adapter, WLAN_BSSID_EX *pnetwork)
{
	...
	if ((check_fwstate(pmlmepriv, _FW_LINKED) == _TRUE) && (is_same_network_22b(&(pmlmepriv->cur_network.network), pnetwork, 0))) {

		/* if(pmlmepriv->cur_network.network.IELength<= pnetwork->IELength) */
		{
			update_network_22b(&(pmlmepriv->cur_network.network), pnetwork, adapter, _TRUE);
			rtw_update_protection_22b(adapter, (pmlmepriv->cur_network.network.IEs) + sizeof(NDIS_802_11_FIXED_IEs),
...
*/
bool rtw_update_scanned_network_22b(_adapter *adapter, WLAN_BSSID_EX *target)
{
	_irqL irqL;
	_list	*plist, *phead;
	ULONG	bssid_ex_sz;
...
#ifdef CONFIG_P2P
		if (!rtw_p2p_chk_state(pwdinfo, P2P_STATE_NONE) &&
		    (_rtw_memcmp_22b(pnetwork->network.MacAddress, target->MacAddress, ETH_ALEN) == _TRUE)) {
			target_find = 1;
			break;
		}
#endif

		if (is_same_network_22b(&(pnetwork->network), target, feature)) {
			target_find = 1;
			break;
		}
	...
	/* if (rtw_end_of_queue_search_22b(phead,plist)== _TRUE) { */
	if (!target_find) {
		if (_rtw_queue_empty_22b(&(pmlmepriv->free_bss_pool)) == _TRUE) {
		...
		RTW_DBG("%s => ssid:%s ,bssid:"MAC_FMT" will be deleted from scanned_queue\n",
...
#ifdef CONFIG_ANTENNA_DIVERSITY
			rtw_hal_get_odm_var_22b(adapter, HAL_ODM_ANTDIV_SELECT, &(target->PhyInfo.Optimum_antenna), NULL);
#endif
			_rtw_memcpy_22b(&(pnetwork->network), target,  get_WLAN_BSSID_EX_sz(target));
			/* pnetwork->last_scanned = rtw_get_current_time(); */
			/* variable initialize */
			pnetwork->fixed = _FALSE;
			pnetwork->last_scanned = rtw_get_current_time();
			...
			pnetwork = rtw_alloc_network_22b(pmlmepriv); /* will update scan_time */
			if (pnetwork == NULL)
				goto unlock_scan_queue;
			...
			rtw_hal_get_odm_var_22b(adapter, HAL_ODM_ANTDIV_SELECT, &(target->PhyInfo.Optimum_antenna), NULL);
#endif
			_rtw_memcpy_22b(&(pnetwork->network), target, bssid_ex_sz);
		...
		update_network_22b(&(pnetwork->network), target, adapter, update_ie);
	}
...
void rtw_add_network_22b(_adapter *adapter, WLAN_BSSID_EX *pnetwork);
void rtw_add_network_22b(_adapter *adapter, WLAN_BSSID_EX *pnetwork)
{
	_irqL irqL;
	...
	if (adapter->registrypriv.wifi_spec == 0)
		rtw_bss_ex_del_p2p_attr_22b(pnetwork, P2P_ATTR_GROUP_INFO);
#endif

	if (!hal_chk_wl_func_22b(adapter, WL_FUNC_MIRACAST))
		rtw_bss_ex_del_wfd_ie_22b(pnetwork);

	/* Wi-Fi driver will update the current network if the scan result of the connected AP be updated by scan. */
	update_ie = rtw_update_scanned_network_22b(adapter, pnetwork);
 ...
 *			   (4) HT
 * (5) others */
int rtw_is_desired_network_22b(_adapter *adapter, struct wlan_network *pnetwork);
int rtw_is_desired_network_22b(_adapter *adapter, struct wlan_network *pnetwork)
{
	struct security_priv *psecuritypriv = &adapter->securitypriv;
	struct mlme_priv *pmlmepriv = &adapter->mlmepriv;
	...
	if (check_fwstate(pmlmepriv, WIFI_UNDER_WPS)) {
		if (rtw_get_wps_ie_22b(pnetwork->network.IEs + _FIXED_IE_LENGTH_, pnetwork->network.IELength - _FIXED_IE_LENGTH_, NULL, &wps_ielen) != NULL)
			return _TRUE;
		else
			return _FALSE;
	}
...
void rtw_atimdone_event_callback_22b(_adapter	*adapter , u8 *pbuf)
{
...
void rtw_survey_event_callback_22b(_adapter	*adapter, u8 *pbuf)
{
	_irqL  irqL;
	u32 len;
	...
	if ((check_fwstate(pmlmepriv, WIFI_ADHOC_MASTER_STATE)) == _TRUE) {
		if (_rtw_memcmp_22b(&(pmlmepriv->cur_network.network.MacAddress), pnetwork->MacAddress, ETH_ALEN)) {
			struct wlan_network *ibss_wlan = NULL;
			_irqL	irqL;

			_rtw_memcpy_22b(pmlmepriv->cur_network.network.IEs, pnetwork->IEs, 8);
			_enter_critical_bh(&(pmlmepriv->scanned_queue.lock), &irqL);
			ibss_wlan = _rtw_find_network_22b_22b(&pmlmepriv->scanned_queue,  pnetwork->MacAddress);
			if (ibss_wlan) {
				_rtw_memcpy_22b(ibss_wlan->network.IEs , pnetwork->IEs, 8);
	...
	if ((check_fwstate(pmlmepriv, _FW_UNDER_LINKING)) == _FALSE) {
		...
		rtw_add_network_22b(adapter, pnetwork);
	}
...
void rtw_surveydone_event_callback_22b(_adapter	*adapter, u8 *pbuf)
{
	_irqL  irqL;
	...
	if (pmlmepriv->to_join == _TRUE) {
		if ((check_fwstate(pmlmepriv, WIFI_ADHOC_STATE) == _TRUE)) {
			if (check_fwstate(pmlmepriv, _FW_LINKED) == _FALSE) {
				...
				if (rtw_select_and_join_from_scanned_queue_22b(pmlmepriv) == _SUCCESS)
					...
					_rtw_memset_22b(&pdev_network->Ssid, 0, sizeof(NDIS_802_11_SSID));
					_rtw_memcpy_22b(&pdev_network->Ssid, &pmlmepriv->assoc_ssid, sizeof(NDIS_802_11_SSID));

					rtw_update_registrypriv_dev_network_22b(adapter);
					rtw_generate_random_ibss_22b(pibss);

					/*pmlmepriv->fw_state = WIFI_ADHOC_MASTER_STATE;*/
					init_fwstate(pmlmepriv, WIFI_ADHOC_MASTER_STATE);

					if (rtw_create_ibss_cmd_22b(adapter, 0) != _SUCCESS)
						RTW_ERR("rtw_create_ibss_cmd_22b FAIL\n");
			...
			if (_SUCCESS == s_ret)
				...
				rtw_indicate_connect_22b(adapter);
			} else {
				RTW_INFO("try_to_join, but select scanning queue fail, to_roam:%d\n", rtw_to_roam_22b(adapter));

				if (rtw_to_roam_22b(adapter) != 0) {

					rtw_init_sitesurvey_parm(adapter, &parm);
					_rtw_memcpy_22b(&parm.ssid[0], &pmlmepriv->assoc_ssid, sizeof(NDIS_802_11_SSID));
					parm.ssid_num = 1;

					if (rtw_dec_to_roam_22b(adapter) == 0
					    || _SUCCESS != rtw_sitesurvey_cmd_22b(adapter, &parm)
					   ) {
						rtw_set_to_roam_22b(adapter, 0);
						...
						rtw_free_assoc_resources_22b(adapter, 1);
						rtw_indicate_disconnect_22b(adapter, 0, _FALSE);
					} else
...
#else
					receive_disconnect_22b(adapter, pmlmepriv->cur_network.network.MacAddress
	...
	if (check_fwstate(pmlmepriv, _FW_LINKED) == _TRUE)
		p2p_ps_wk_cmd_22b(adapter, P2P_PS_SCAN_DONE, 0);
#endif /* CONFIG_P2P_PS */
...
#ifdef DBG_CONFIG_ERROR_DETECT
	{
		...
		if (pmlmeext->sitesurvey_res.bss_cnt == 0) {
			/* rtw_hal_sreset_reset_22b(adapter); */
		}
	}
#endif

#ifdef CONFIG_IOCTL_CFG80211
	rtw_cfg80211_surveydone_event_callback_22b(adapter);
#endif /* CONFIG_IOCTL_CFG80211 */

	rtw_indicate_scan_done_22b(adapter, _FALSE);

#if defined(CONFIG_CONCURRENT_MODE) && defined(CONFIG_IOCTL_CFG80211)
	rtw_cfg80211_indicate_scan_done_22b_for_buddy(adapter, _FALSE);
#endif
	...
	if (rtw_mesh_offch_candidate_accepted(adapter)) {
		...
		if (ch && pmlmepriv->cur_network.network.Configuration.DSConfig != ch) {
			...
			rtw_change_bss_chbw_cmd_22b(adapter, RTW_CMDF_DIRECTLY, ch, adapter->mlmepriv.ori_bw, -1);
			issue_probereq_22b_ex(adapter, &pmlmepriv->cur_network.network.mesh_id, NULL, 0, 0, 0, 0);
		}
	}
	#endif
...
void rtw_dummy_event_callback_22b(_adapter *adapter , u8 *pbuf)
{

}

void rtw_fwdbg_event_callback_22b(_adapter *adapter , u8 *pbuf)
{
...
static void free_scanqueue(struct	mlme_priv *pmlmepriv)
{
	...
	while (plist != phead) {
		...
		rtw_list_insert_tail_22b(plist, &free_queue->queue);
		plist = ptemp;
		pmlmepriv->num_of_scanned--;
	}
...
void rtw_reset_rx_info_22b(_adapter *adapter)
{
...
/*
*rtw_free_assoc_resources_22b: the caller has to lock pmlmepriv->lock
*/
void rtw_free_assoc_resources_22b(_adapter *adapter, int lock_scanned_queue)
{
	_irqL irqL;
	struct wlan_network *pwlan = NULL;
	struct	mlme_priv *pmlmepriv = &adapter->mlmepriv;
	...
	if (check_fwstate(pmlmepriv, WIFI_STATION_STATE)) {
		...
		psta = rtw_get_stainfo_22b(&adapter->stapriv, tgt_network->network.MacAddress);
		...
		if (ptdlsinfo->link_established == _TRUE)
			rtw_tdls_cmd_22b(adapter, NULL, TDLS_RS_RCR);
#endif /* CONFIG_TDLS */
	...
	if (check_fwstate(pmlmepriv, WIFI_ADHOC_STATE | WIFI_ADHOC_MASTER_STATE)) {
		...
		rtw_free_all_stainfo_22b(adapter);

		psta = rtw_get_bcmc_stainfo_22b(adapter);
		/* _enter_critical_bh(&(pstapriv->sta_hash_lock), &irqL);		 */
		rtw_free_stainfo_22b(adapter, psta);
		/* _exit_critical_bh(&(pstapriv->sta_hash_lock), &irqL);		 */

		rtw_init_bcmc_stainfo_22b(adapter);
	}

	if (lock_scanned_queue)
		_enter_critical_bh(&(pmlmepriv->scanned_queue.lock), &irqL);

	pwlan = _rtw_find_same_network_22b_22b(&pmlmepriv->scanned_queue, tgt_network);
	if ((pwlan)  && (!check_fwstate(pmlmepriv, WIFI_UNDER_WPS))) {
		pwlan->fixed = _FALSE;

		RTW_INFO("free disconnecting network of scanned_queue\n");
		rtw_free_network_22b_nolock(adapter, pwlan);
#ifdef CONFIG_P2P
		if (!rtw_p2p_chk_state(&adapter->wdinfo, P2P_STATE_NONE)) {
			rtw_mi_set_scan_deny_22b(adapter, 2000);
			/* rtw_clear_scan_deny_22b(adapter);			 */
		}
#endif /* CONFIG_P2P */
	} else {
		if (pwlan == NULL)
	...
	rtw_reset_rx_info_22b(adapter);


}

/*
*rtw_indicate_connect_22b: the caller has to lock pmlmepriv->lock
*/
void rtw_indicate_connect_22b(_adapter *padapter)
{
	struct mlme_priv	*pmlmepriv = &padapter->mlmepriv;
	...
	if (!check_fwstate(&padapter->mlmepriv, _FW_LINKED)) {
		...
		rtw_os_indicate_connect_22b(padapter);
	}
	...
	if (!MLME_IS_AP(padapter) && !MLME_IS_MESH(padapter))
		rtw_mi_set_scan_deny_22b(padapter, 3000);
...
/*
*rtw_indicate_disconnect_22b: the caller has to lock pmlmepriv->lock
*/
void rtw_indicate_disconnect_22b(_adapter *padapter, u16 reason, u8 locally_generated)
{
	struct	mlme_priv *pmlmepriv = &padapter->mlmepriv;
	...
	if (pmlmepriv->cur_network_scanned) {
		...
		if (current_joined_bss) {
			wps_ie = rtw_get_wps_ie_22b(current_joined_bss->IEs + _FIXED_IE_LENGTH_,
				current_joined_bss->IELength - _FIXED_IE_LENGTH_, NULL, &wpsie_len);
			if (wps_ie && wpsie_len > 0) {
				u8 *attr = NULL;
				u32 attr_len;
				attr = rtw_get_wps_attr_22b(wps_ie, wpsie_len, WPS_ATTR_SELECTED_REGISTRAR,
							NULL, &attr_len);
				if (attr)
					*(attr + 4) = 0;
			}
	...
	if (check_fwstate(&padapter->mlmepriv, _FW_LINKED)
	    || (rtw_to_roam_22b(padapter) <= 0)
	   ) {

		rtw_os_indicate_disconnect_22b(padapter, reason, locally_generated);

		/* set ips_deny_time to avoid enter IPS before LPS leave */
		rtw_set_ips_deny_22b(padapter, 3000);
...
#ifdef CONFIG_P2P_PS
	p2p_ps_wk_cmd_22b(padapter, P2P_PS_DISABLE, 1);
#endif /* CONFIG_P2P_PS */

#ifdef CONFIG_LPS
	rtw_lps_ctrl_wk_cmd_22b(padapter, LPS_CTRL_DISCONNECT, 1);
#endif
...
inline void rtw_indicate_scan_done_22b(_adapter *padapter, bool aborted)
{
	RTW_INFO(FUNC_ADPT_FMT"\n", FUNC_ADPT_ARG(padapter));

	rtw_os_indicate_scan_done_22b(padapter, aborted);
	...
	if (is_primary_adapter(padapter)
		...
		rtw_set_ips_deny_22b(padapter, 0);
...
#endif
			rtw_indicate_scan_done_22b(adapter, _TRUE);
		}
	}
...
void rtw_scan_wait_completed_22b(_adapter *adapter)
{
	struct mlme_ext_priv *pmlmeext = &adapter->mlmeextpriv;
	struct mlme_ext_info *pmlmeinfo = &pmlmeext->mlmext_info;
...
u32 rtw_scan_abort_22b_timeout(_adapter *adapter, u32 timeout_ms)
{
	return _rtw_wait_scan_done(adapter, _TRUE, timeout_ms);
}

void rtw_scan_abort_22b_no_wait(_adapter *adapter)
{
	struct mlme_priv *pmlmepriv = &(adapter->mlmepriv);
	struct mlme_ext_priv *pmlmeext = &(adapter->mlmeextpriv);
...
void rtw_scan_abort_22b(_adapter *adapter)
{
	rtw_scan_abort_22b_timeout(adapter, 200);
}
...
static struct sta_info *rtw_joinbss_update_stainfo(_adapter *padapter, struct wlan_network *pnetwork)
{
	...
	psta = rtw_get_stainfo_22b(pstapriv, pnetwork->network.MacAddress);
	if (psta == NULL)
		psta = rtw_alloc_stainfo_22b(pstapriv, pnetwork->network.MacAddress);
	...
	if (psta) { /* update ptarget_sta */
		...
		psta->bssratelen = rtw_get_rateset_len_22b(pnetwork->network.SupportedRates);
		_rtw_memcpy_22b(psta->bssrateset, pnetwork->network.SupportedRates, psta->bssratelen);
		rtw_hal_update_sta_ra_info_22b(padapter, psta);

		psta->wireless_mode = pmlmeext->cur_wireless_mode;
		rtw_hal_update_sta_wset_22b(padapter, psta);
		...
		if (padapter->securitypriv.dot11AuthAlgrthm == dot11AuthAlgrthm_8021X) {
			...
			ie = rtw_get_ie_22b(pnetwork->network.IEs + _BEACON_IE_OFFSET_, WLAN_EID_RSN
				, &ie_len, (pnetwork->network.IELength - _BEACON_IE_OFFSET_));
			if (ie && ie_len > 0
				&& rtw_parse_wpa2_ie_22b(ie, ie_len + 2, NULL, NULL, NULL, &mfp_opt) == _SUCCESS
			) {
			...
			_rtw_memset_22b((u8 *)&psta->dot118021x_UncstKey, 0, sizeof(union Keytype));
			_rtw_memset_22b((u8 *)&psta->dot11tkiprxmickey, 0, sizeof(union Keytype));
			_rtw_memset_22b((u8 *)&psta->dot11tkiptxmickey, 0, sizeof(union Keytype));
		}
	...
	_rtw_memcpy_22b(&psta->rm_en_cap, pnetwork->network.PhyInfo.rm_en_cap, 5);
#endif
...
/* pnetwork : returns from rtw_joinbss_event_callback_22b
 * ptarget_wlan: found from scanned_queue */
static void rtw_joinbss_update_network_22b(_adapter *padapter, struct wlan_network *ptarget_wlan, struct wlan_network  *pnetwork)
{
	struct mlme_priv	*pmlmepriv = &(padapter->mlmepriv);
	...
	_rtw_memcpy_22b(&cur_network->network, &pnetwork->network, pnetwork->network.Length);
	/* some IEs in pnetwork is wrong, so we should use ptarget_wlan IEs */
	cur_network->network.IELength = ptarget_wlan->network.IELength;
	_rtw_memcpy_22b(&cur_network->network.IEs[0], &ptarget_wlan->network.IEs[0], MAX_IE_SZ);
	...
	rtw_update_protection_22b(padapter, (cur_network->network.IEs) + sizeof(NDIS_802_11_FIXED_IEs),
			      (cur_network->network.IELength));

#ifdef CONFIG_80211N_HT
	rtw_update_ht_cap_22b(padapter, cur_network->network.IEs, cur_network->network.IELength, (u8) cur_network->network.Configuration.DSConfig);
#endif
}
 ...
 */
/* #define REJOIN */
void rtw_joinbss_event_prehandle_22b(_adapter *adapter, u8 *pbuf)
{
	_irqL irqL, irqL2;
	static u8 retry = 0;
	...
	rtw_get_encrypt_decrypt_from_registrypriv_22b(adapter);



	the_same_macaddr = _rtw_memcmp_22b(pnetwork->network.MacAddress, cur_network->network.MacAddress, ETH_ALEN);
	...
	if (pnetwork->join_res > 0) {
		...
		if (check_fwstate(pmlmepriv, _FW_UNDER_LINKING)) {
			...
			if (check_fwstate(pmlmepriv, _FW_LINKED)) {
				if (the_same_macaddr == _TRUE)
					ptarget_wlan = _rtw_find_network_22b_22b(&pmlmepriv->scanned_queue, cur_network->network.MacAddress);
				else {
					pcur_wlan = _rtw_find_network_22b_22b(&pmlmepriv->scanned_queue, cur_network->network.MacAddress);
					if (pcur_wlan)
						pcur_wlan->fixed = _FALSE;

					pcur_sta = rtw_get_stainfo_22b(pstapriv, cur_network->network.MacAddress);
					if (pcur_sta) {
						/* _enter_critical_bh(&(pstapriv->sta_hash_lock), &irqL2); */
						rtw_free_stainfo_22b(adapter,  pcur_sta);
						/* _exit_critical_bh(&(pstapriv->sta_hash_lock), &irqL2); */
					}

					ptarget_wlan = _rtw_find_network_22b_22b(&pmlmepriv->scanned_queue, pnetwork->network.MacAddress);
				...
				ptarget_wlan = _rtw_find_same_network_22b_22b(&pmlmepriv->scanned_queue, pnetwork);
				if (check_fwstate(pmlmepriv, WIFI_STATION_STATE) == _TRUE) {
					if (ptarget_wlan)
						ptarget_wlan->fixed = _TRUE;
				}
			...
			if (ptarget_wlan)
				rtw_joinbss_update_network_22b(adapter, ptarget_wlan, pnetwork);
			else {
		...
		rtw_reset_securitypriv_22b(adapter);
		...
		res = _FAIL;
		if (retry < 2) {
			res = rtw_select_and_join_from_scanned_queue_22b(pmlmepriv);
		}
...
void rtw_joinbss_event_callback_22b(_adapter *adapter, u8 *pbuf)
{
	struct wlan_network	*pnetwork	= (struct wlan_network *)pbuf;


	mlmeext_joinbss_event_callback_22b(adapter, pnetwork->join_res);

	rtw_mi_os_xmit_schedule_22b(adapter);

}

void rtw_sta_media_status_rpt_22b(_adapter *adapter, struct sta_info *sta, bool connected)
{
	...
	if (!rtw_macid_is_used_22b(macid_ctl, sta->cmn.mac_id)) {
	...
	if (connected && !rtw_macid_is_bmc_22b(macid_ctl, sta->cmn.mac_id)) {
		miracast_enabled = STA_OP_WFD_MODE(sta) != 0 && is_miracast_enabled_22b(adapter);
		...
		if (role == H2C_MSR_ROLE_GC
			...
			if (adapter->wfd_info.rtsp_ctrlport
				...
				rtw_wfd_st_switch_22b(sta, 1);
		}
#endif
	}

	rtw_hal_set_FwMediaStatusRpt_single_cmd_22b(adapter
		, connected
		, miracast_enabled
		, miracast_sink
		, role
...
u8 rtw_sta_media_status_rpt_22b_cmd(_adapter *adapter, struct sta_info *sta, bool connected)
{
	...
	res = rtw_enqueue_cmd_22b(cmdpriv, cmdobj);

exit:
	return res;
}

inline void rtw_sta_media_status_rpt_22b_cmd_hdl(_adapter *adapter, struct sta_media_status_rpt_cmd_parm *parm)
{
	rtw_sta_media_status_rpt_22b(adapter, parm->sta, parm->connected);
}

void rtw_stassoc_event_callback_22b(_adapter *adapter, u8 *pbuf)
{
	_irqL irqL;
	struct sta_info *psta;
	struct mlme_priv *pmlmepriv = &(adapter->mlmepriv);
	...
	if (rtw_access_ctrl_22b(adapter, pstassoc->macaddr) == _FALSE)
		return;
#endif

#if defined(CONFIG_AP_MODE) && defined (CONFIG_NATIVEAP_MLME)
	if (MLME_IS_AP(adapter) || MLME_IS_MESH(adapter)) {
		psta = rtw_get_stainfo_22b(&adapter->stapriv, pstassoc->macaddr);
		if (psta) {
			u8 *passoc_req = NULL;
			u32 assoc_req_len = 0;
			...
			ap_sta_info_defer_update_22b(adapter, psta);
			...
			if (!MLME_IS_MESH(adapter)) {
				...
				if (psta->passoc_req && psta->assoc_req_len > 0) {
					...
					if (passoc_req) {
						...
						_rtw_memcpy_22b(passoc_req, psta->passoc_req, assoc_req_len);
				...
				if (passoc_req && assoc_req_len > 0) {
					rtw_cfg80211_indicate_sta_assoc_22b(adapter, passoc_req, assoc_req_len);
					rtw_mfree(passoc_req, assoc_req_len);
				}
				#else /* !CONFIG_IOCTL_CFG80211	 */
				rtw_indicate_sta_assoc_event_22b(adapter, psta);
				#endif /* !CONFIG_IOCTL_CFG80211 */
			}
			...
			if (is_wep_enc(adapter->securitypriv.dot11PrivacyAlgrthm))
				rtw_ap_wep_pk_setting_22b(adapter, psta);
		}
		goto exit;
	}
	...
	mlmeext_sta_add_event_callback_22b(adapter, psta);

#ifdef CONFIG_RTL8711
	/* submit SetStaKey_cmd to tell fw, fw will allocate an CAM entry for this sta	 */
	rtw_setstakey_cmd_22b(adapter, psta, GROUP_KEY, _TRUE);
#endif
...
void rtw_sta_timeout_event_callback(_adapter *adapter, u8 *pbuf)
{
	...
	psta = rtw_get_stainfo_22b(&adapter->stapriv, pstadel->macaddr);
	...
	if (psta) {
		...
		if (rtw_is_list_empty_22b(&psta->asoc_list) == _FALSE) {
			...
			updated = ap_free_sta_22b(adapter, psta, _TRUE, WLAN_REASON_PREV_AUTH_NOT_VALID, _TRUE);
		}
...
{
	_rtw_memset_22b(pft, 0, sizeof(struct ft_roam_info));
	pft->ft_flags = 0
		| RTW_FT_EN
		| RTW_FT_OTD_EN
	...
	_adapter *padapter, struct wlan_network *competitor)
{
	...
	if (!(pmdie = rtw_get_ie_22b(&competitor->network.IEs[12],
			_MDIE_, &mdie_len, competitor->network.IELength-12)))
		return _FALSE;

	if (!_rtw_memcmp_22b(&pft_roam->mdid, (pmdie+2), 2))
		return _FALSE;
...
void rtw_ft_update_stainfo(_adapter *padapter, WLAN_BSSID_EX *pnetwork)
{
	...
	psta = rtw_get_stainfo_22b(pstapriv, pnetwork->MacAddress);
	if (psta == NULL)
		psta = rtw_alloc_stainfo_22b(pstapriv, pnetwork->MacAddress);
...
void rtw_ft_reassoc_event_callback(_adapter *padapter, u8 *pbuf)
{
	...
	if (ft_evt_parms.target_ap)
		_rtw_memcpy_22b((void *)ft_evt_parms.target_ap, pstassoc->macaddr, ETH_ALEN);
	else
		goto err_1;
...
void rtw_roam_nb_info_init(_adapter *padapter)
{
	...
	rtw_init_timer_22b(&pnb->roam_scan_timer, 
...
set_bssid_list:
	rtw_set_802_11_bssid_22b_list_scan(padapter, pparm);
	return ret;
}
#endif

void rtw_sta_mstatus_disc_rpt_22b(_adapter *adapter, u8 mac_id)
{
	struct macid_ctl_t *macid_ctl = &adapter->dvobj->macid_ctl;
	...
	if (mac_id >= 0 && mac_id < macid_ctl->num) {
		...
		if (!id_is_shared) {
			rtw_hal_set_FwMediaStatusRpt_single_cmd_22b(adapter, 0, 0, 0, 0, mac_id);
			/*
			...
			/*if (MLME_IS_STA(adapter))*/
			rtw_hal_macid_wakeup_22b(adapter, mac_id);
		}
	} else {
		RTW_PRINT(FUNC_ADPT_FMT" invalid macid:%u\n"
...
void rtw_sta_mstatus_report_22b(_adapter *adapter)
{
	struct mlme_priv *pmlmepriv = &adapter->mlmepriv;
	struct wlan_network *tgt_network = &pmlmepriv->cur_network;
	struct sta_info *psta = NULL;
	...
	if (check_fwstate(pmlmepriv, WIFI_STATION_STATE) && check_fwstate(pmlmepriv, WIFI_ASOC_STATE)) {
		psta = rtw_get_stainfo_22b(&adapter->stapriv, tgt_network->network.MacAddress);
		if (psta)
			rtw_sta_mstatus_disc_rpt_22b(adapter, psta->cmn.mac_id);
		else {
...
void rtw_stadel_event_callback_22b(_adapter *adapter, u8 *pbuf)
{
	_irqL irqL, irqL2;
	...
	rtw_sta_mstatus_disc_rpt_22b(adapter, pstadel->mac_id);
	...
	if (MLME_IS_AP(adapter)) {
		...
		rtw_cfg80211_indicate_sta_disassoc_22b(adapter, pstadel->macaddr, *(u16 *)pstadel->rsvd);
	...
	mlmeext_sta_del_event_callback_22b(adapter);
	...
	if (check_fwstate(pmlmepriv, WIFI_STATION_STATE)) {
		...
		if (roam == _TRUE) {
			if (rtw_to_roam_22b(adapter) > 0)
				rtw_dec_to_roam_22b(adapter); /* this stadel_event is caused by roaming, decrease to_roam */
			else if (rtw_to_roam_22b(adapter) == 0)
				rtw_set_to_roam_22b(adapter, adapter->registrypriv.max_roaming_times);
		} else
			rtw_set_to_roam_22b(adapter, 0);
#endif /* CONFIG_LAYER2_ROAMING */

		rtw_free_uc_swdec_pending_queue_22b(adapter);
		...
		pwlan = _rtw_find_network_22b_22b(&pmlmepriv->scanned_queue, tgt_network->network.MacAddress);
		if ((pwlan)  && (!check_fwstate(pmlmepriv, WIFI_UNDER_WPS))) {
			pwlan->fixed = _FALSE;
			rtw_free_network_22b_nolock(adapter, pwlan);
		}
		...
		rtw_indicate_disconnect_22b(adapter, *(u16 *)pstadel->rsvd, pstadel->locally_generated);
#ifdef CONFIG_INTEL_WIDI
		if (!rtw_to_roam_22b(adapter))
			process_intel_widi_disconnect(adapter, 1);
#endif /* CONFIG_INTEL_WIDI */

		_rtw_roaming_22b_22b(adapter, roam_target);
	}
	...
	if (check_fwstate(pmlmepriv, WIFI_ADHOC_MASTER_STATE) ||
		...
		if (adapter->stapriv.asoc_sta_count == 1) { /* a sta + bc/mc_stainfo (not Ibss_stainfo) */
			/* rtw_indicate_disconnect_22b(adapter); */ /* removed@20091105 */
			_enter_critical_bh(&(pmlmepriv->scanned_queue.lock), &irqL);
			/* free old ibss network */
			/* pwlan = _rtw_find_network_22b_22b(&pmlmepriv->scanned_queue, pstadel->macaddr); */
			pwlan = _rtw_find_network_22b_22b(&pmlmepriv->scanned_queue, tgt_network->network.MacAddress);
			if (pwlan) {
				pwlan->fixed = _FALSE;
				rtw_free_network_22b_nolock(adapter, pwlan);
			...
			_rtw_memcpy_22b(pdev_network, &tgt_network->network, get_WLAN_BSSID_EX_sz(&tgt_network->network));
...
void rtw_cpwm_event_callback_22b(PADAPTER padapter, u8 *pbuf)
{
...
void rtw_wmm_event_callback_22b(PADAPTER padapter, u8 *pbuf)
{
...
/*
* rtw_join_timeout_handler_22b - Timeout/failure handler for CMD JoinBss
*/
void rtw_join_timeout_handler_22b(void *ctx)
{
	...
	if (rtw_is_drv_stopped(adapter)) {
		_rtw_up_sema_22b(&pmlmepriv->assoc_terminate);
		return;
	}
#endif
...
#ifdef CONFIG_LAYER2_ROAMING
	if (rtw_to_roam_22b(adapter) > 0) { /* join timeout caused by roaming */
		while (1) {
			rtw_dec_to_roam_22b(adapter);
			if (rtw_to_roam_22b(adapter) != 0) { /* try another */
				int do_join_r;
				RTW_INFO("%s try another roaming\n", __FUNCTION__);
				do_join_r = rtw_do_join_22b(adapter);
...
#endif
	{
		...
		rtw_cfg80211_indicate_disconnect_22b(adapter, 0, _FALSE);
#endif /* CONFIG_IOCTL_CFG80211 */
...
/*
* rtw_scan_timeout_handler_22b - Timeout/Faliure handler for CMD SiteSurvey
* @adapter: pointer to _adapter structure
*/
void rtw_scan_timeout_handler_22b(void *ctx)
{
	_adapter *adapter = (_adapter *)ctx;
	_irqL irqL;
	struct mlme_priv *pmlmepriv = &adapter->mlmepriv;
...
#if defined(CONFIG_CONCURRENT_MODE) && defined(CONFIG_IOCTL_CFG80211)
	rtw_cfg80211_indicate_scan_done_22b_for_buddy(adapter, _TRUE);
#endif
}

void rtw_mlme_reset_auto_scan_int_22b(_adapter *adapter, u8 *reason)
{
	...
	*reason = RTW_AUTO_SCAN_REASON_UNSPECIFIED;
	rtw_mi_get_ch_setting_union_22b(adapter, &u_ch, NULL, NULL);

	if (hal_chk_bw_cap_22b(adapter, BW_CAP_40M)
		&& is_client_associated_to_ap_22b(adapter) == _TRUE
		&& u_ch >= 1 && u_ch <= 14
...
void rtw_drv_scan_by_self_22b(_adapter *padapter, u8 reason)
{
	struct sitesurvey_parm parm;
	struct mlme_priv *pmlmepriv = &padapter->mlmepriv;
	int i;
	...
	if (rtw_mi_buddy_check_fwstate_22b(padapter, (_FW_UNDER_SURVEY | _FW_UNDER_LINKING | WIFI_UNDER_WPS))) {
	...
	if (reason == RTW_AUTO_SCAN_REASON_2040_BSS) {
		...
		parm.ch_num = 14;
		rtw_set_802_11_bssid_22b_list_scan(padapter, &parm);
		goto exit;
	}
	...
	rtw_set_802_11_bssid_22b_list_scan(padapter, NULL);
exit:
	return;
}
...
static void rtw_auto_scan_handler(_adapter *padapter)
{
	...
	rtw_mlme_reset_auto_scan_int_22b(padapter, &reason);
	...
	rtw_drv_scan_by_self_22b(padapter, reason);
...
static u8 is_drv_in_lps(_adapter *adapter)
{
	...
	if ((adapter_to_pwrctl(adapter)->bFwCurrentInPSMode == _TRUE)
		...
		&& (rtw_btcoex_IsBtControlLps_22b(adapter) == _FALSE)
	#endif
		)
		is_in_lps = _TRUE;
...
void rtw_iface_dynamic_check_timer_handlder_22b(_adapter *adapter)
{
	...
	if (is_drv_in_lps(adapter)) {
		...
		linked_status_chk_22b(adapter, 1);

		bEnterPS = traffic_status_watchdog_22b(adapter, 1);
		if (bEnterPS) {
			/* rtw_lps_ctrl_wk_cmd_22b(adapter, LPS_CTRL_ENTER, 1); */
			rtw_hal_dm_watchdog_22b_in_lps(adapter);
		} else {
			/* call rtw_lps_ctrl_wk_cmd_22b(padapter, LPS_CTRL_LEAVE, 1) in traffic_status_watchdog_22b() */
		}
	}
	#endif /* CONFIG_LPS_LCLK_WD_TIMER	*/
	...
	if (MLME_IS_AP(adapter)|| MLME_IS_MESH(adapter)) {
		...
		expire_timeout_chk_22b(adapter);
		#endif /* !CONFIG_ACTIVE_KEEP_ALIVE_CHECK */

		#ifdef CONFIG_BMC_TX_RATE_SELECT
		rtw_update_bmc_sta_22b_tx_rate(adapter);
	...
	if (rcu_dereference(adapter->pnetdev->rx_handler_data)
		...
		/* expire NAT2.5 entry */
		void nat25_db_expire_22b(_adapter *priv);
		nat25_db_expire_22b(adapter);

		if (adapter->pppoe_connection_in_progress > 0)
			adapter->pppoe_connection_in_progress--;
		/* due to rtw_dynamic_check_timer_handlder_22b() is called every 2 seconds */
		if (adapter->pppoe_connection_in_progress > 0)
			adapter->pppoe_connection_in_progress--;
	}
	...
	for (i = 0; i < MACID_NUM_SW_LIMIT; i++) {
		...
		if (sta && !is_broadcast_mac_addr(sta->cmn.mac_addr)) {
			...
			if (adapter->bsta_tp_dump)
				dump_sta_traffic_22b(RTW_DBGDUMP, adapter, sta);
...
void rtw_sta_traffic_info_22b(void *sel, _adapter *adapter)
{
	struct macid_ctl_t *macid_ctl = &adapter->dvobj->macid_ctl;
	struct sta_info *sta;
	int i;
	...
	for (i = 0; i < MACID_NUM_SW_LIMIT; i++) {
		sta = macid_ctl->sta[i];
		if (sta && !is_broadcast_mac_addr(sta->cmn.mac_addr))
			dump_sta_traffic_22b(sel, adapter, sta);
	}
}
...
static void collect_traffic_statistics(_adapter *padapter)
{
	...
	/*_rtw_memset_22b(&pdvobjpriv->traffic_stat, 0, sizeof(struct rtw_traffic_statistics));*/
	...
	rtw_mi_traffic_statistics_22b(padapter);
...
void rtw_dynamic_check_timer_handlder_22b(void *ctx)
{
	...
	rtw_mi_dynamic_check_timer_handlder_22b(adapter);

	if (!is_drv_in_lps(adapter))
		rtw_dynamic_chk_wk_cmd_22b(adapter);
...
#ifdef CONFIG_SET_SCAN_DENY_TIMER
inline bool rtw_is_scan_deny_22b(_adapter *adapter)
{
	struct mlme_priv *mlmepriv = &adapter->mlmepriv;
	return (ATOMIC_READ_22b(&mlmepriv->set_scan_deny) != 0) ? _TRUE : _FALSE;
}

inline void rtw_clear_scan_deny_22b(_adapter *adapter)
{
	struct mlme_priv *mlmepriv = &adapter->mlmepriv;
	ATOMIC_SET_22b(&mlmepriv->set_scan_deny, 0);
	if (0)
		RTW_INFO(FUNC_ADPT_FMT"\n", FUNC_ADPT_ARG(adapter));
}

void rtw_set_scan_deny_22b_timer_hdl_22b(void *ctx)
{
	_adapter *adapter = (_adapter *)ctx;

	rtw_clear_scan_deny_22b(adapter);
}
void rtw_set_scan_deny_22b(_adapter *adapter, u32 ms)
{
	struct mlme_priv *mlmepriv = &adapter->mlmepriv;
	if (0)
		RTW_INFO(FUNC_ADPT_FMT"\n", FUNC_ADPT_ARG(adapter));
	ATOMIC_SET_22b(&mlmepriv->set_scan_deny, 1);
	...
	, struct wlan_network **candidate, struct wlan_network *competitor)
{
	...
	if (rtw_chset_search_ch_22b(adapter_to_chset(adapter), competitor->network.Configuration.DSConfig) < 0)
		goto exit;
	...
	if (is_same_ess_22b(&competitor->network, &mlme->cur_network.network) == _FALSE)
		goto exit;

	if (rtw_is_desired_network_22b(adapter, competitor) == _FALSE)
		goto exit;
	...
	if (!is_zero_mac_addr(mlme->roam_tgt_addr)) {
		if (_rtw_memcmp_22b(mlme->roam_tgt_addr, competitor->network.MacAddress, ETH_ALEN) == _TRUE)
			goto update;
		else
			goto exit;
	}
...
int rtw_select_roaming_candidate_22b(struct mlme_priv *mlme)
{
	_irqL	irqL;
	int ret = _FAIL;
	_list	*phead;
	...
	while (!rtw_end_of_queue_search_22b(phead, mlme->pscanned)) {
	...
	/*	if parent note lost the path to root and there is no other cadidate, report disconnection	*/
		...
		if (check_fwstate(mlme, _FW_LINKED)
			...
			receive_disconnect_22b(adapter, mlme->cur_network_scanned->network.MacAddress
		...
		if (_rtw_memcmp_22b(candidate->network.MacAddress, mlme->roam_tgt_addr, ETH_ALEN) == _TRUE)
			_rtw_memset_22b(mlme->roam_tgt_addr, 0, ETH_ALEN);
	}
	    ...
	    , struct wlan_network **candidate, struct wlan_network *competitor)
{
	...
	if (mlme->assoc_by_bssid == _TRUE) {
		if (_rtw_memcmp_22b(competitor->network.MacAddress, mlme->assoc_bssid, ETH_ALEN) == _FALSE)
			goto exit;
	}
	...
	if (mlme->assoc_ssid.Ssid[0] && mlme->assoc_ssid.SsidLength) {
		if (competitor->network.Ssid.SsidLength != mlme->assoc_ssid.SsidLength
		    || _rtw_memcmp_22b(competitor->network.Ssid.Ssid, mlme->assoc_ssid.Ssid, mlme->assoc_ssid.SsidLength) == _FALSE
		   )
			goto exit;
	}
	...
	if (rtw_to_roam_22b(adapter) > 0) {
		if (rtw_get_passing_time_ms(competitor->last_scanned) >= mlme->roam_scanr_exp_ms
		    || is_same_ess_22b(&competitor->network, &mlme->cur_network.network) == _FALSE
		   )
			goto exit;
	}
#endif
...
int rtw_select_and_join_from_scanned_queue_22b(struct mlme_priv *pmlmepriv)
{
	_irqL	irqL;
	int ret;
	_list	*phead;
	...
	while (!rtw_end_of_queue_search_22b(phead, pmlmepriv->pscanned)) {
...
candidate_exist:
	...
	if (check_fwstate(pmlmepriv, _FW_LINKED) == _TRUE) {
		...
		if (is_same_network_22b(&pmlmepriv->cur_network.network, &candidate->network)) {
			RTW_INFO("%s: _FW_LINKED and is same network, it needn't join again\n", __FUNCTION__);

			rtw_indicate_connect_22b(adapter);/* rtw_indicate_connect_22b again */
...
#endif
		{
			rtw_disassoc_cmd_22b(adapter, 0, 0);
			rtw_indicate_disconnect_22b(adapter, 0, _FALSE);
			rtw_free_assoc_resources_22b(adapter, 0);
		}
	}

#ifdef CONFIG_ANTENNA_DIVERSITY
	rtw_hal_get_def_var_22b(adapter, HAL_DEF_IS_SUPPORT_ANT_DIV, &(bSupportAntDiv));
	if (_TRUE == bSupportAntDiv) {
		u8 CurrentAntenna;
		rtw_hal_get_odm_var_22b(adapter, HAL_ODM_ANTDIV_SELECT, &(CurrentAntenna), NULL);
...
#endif
	set_fwstate(pmlmepriv, _FW_UNDER_LINKING);
	ret = rtw_joinbss_cmd_22b(adapter, candidate);
...
sint rtw_set_auth_22b(_adapter *adapter, struct security_priv *psecuritypriv)
{
	struct	cmd_obj *pcmd;
	...
	_rtw_memset_22b(psetauthparm, 0, sizeof(struct setauth_parm));
	...
	_rtw_init_listhead_22b(&pcmd->list);


	res = rtw_enqueue_cmd_22b(pcmdpriv, pcmd);
...
sint rtw_set_key_22b(_adapter *adapter, struct security_priv *psecuritypriv, sint keyid, u8 set_tx, bool enqueue)
{
	u8	keylen;
	struct cmd_obj		*pcmd;
	...
	_rtw_memset_22b(psetkeyparm, 0, sizeof(struct setkey_parm));
	...
	RTW_INFO("==> rtw_set_key_22b algorithm(%x),keyid(%x),key_mask(%x)\n", psetkeyparm->algorithm, psetkeyparm->keyid, adapter->securitypriv.key_mask);

	switch (psetkeyparm->algorithm) {

	case _WEP40_:
		keylen = 5;
		_rtw_memcpy_22b(&(psetkeyparm->key[0]), &(psecuritypriv->dot11DefKey[keyid].skey[0]), keylen);
		break;
	case _WEP104_:
		keylen = 13;
		_rtw_memcpy_22b(&(psetkeyparm->key[0]), &(psecuritypriv->dot11DefKey[keyid].skey[0]), keylen);
	...
	case _TKIP_:
		keylen = 16;
		_rtw_memcpy_22b(&psetkeyparm->key, &psecuritypriv->dot118021XGrpKey[keyid], keylen);
		break;
	case _AES_:
		keylen = 16;
		_rtw_memcpy_22b(&psetkeyparm->key, &psecuritypriv->dot118021XGrpKey[keyid], keylen);
	...
	if (enqueue) {
		...
		/* _rtw_init_sema_22b(&(pcmd->cmd_sem), 0); */

		res = rtw_enqueue_cmd_22b(pcmdpriv, pcmd);
	} else {
		setkey_hdl_22b(adapter, (u8 *)psetkeyparm);
...
/* adjust IEs for rtw_joinbss_cmd_22b in WMM */
int rtw_restruct_wmm_ie_22b(_adapter *adapter, u8 *in_ie, u8 *out_ie, uint in_len, uint initial_out_len)
{
#ifdef CONFIG_WMMPS_STA
	struct mlme_priv		*pmlmepriv = &adapter->mlmepriv;
	struct qos_priv		*pqospriv = &pmlmepriv->qospriv;
...
static int SecIsInPMKIDList(_adapter *Adapter, u8 *bssid)
{
	...
	do {
		if ((psecuritypriv->PMKIDList[i].bUsed) &&
		    (_rtw_memcmp_22b(psecuritypriv->PMKIDList[i].Bssid, bssid, ETH_ALEN) == _TRUE))
			break;
		else {
			i++;
			/* continue; */
...
static int rtw_rsn_sync_pmkid(_adapter *adapter, u8 *ie, uint ie_len, int i_ent)
{
	...
	if (i_ent >= 0 && info.pmkid_cnt == 1 && _rtw_memcmp_22b(info.pmkid_list, sec->PMKIDList[i_ent].PMKID, 16)) {
	...
	if (info.gmcs)
		_rtw_memcpy_22b(gm_cs, info.gmcs, 4);
	...
	if (i_ent >= 0) {
		...
		_rtw_memcpy_22b(info.pmkid_list, sec->PMKIDList[i_ent].PMKID, 16);
	} else
		info.pmkid_cnt = 0; /* update new pmkid_cnt */

	RTW_PUT_LE16(info.pmkid_list - 2, info.pmkid_cnt);
	if (info.gmcs)
		_rtw_memcpy_22b(info.pmkid_list + 16 * info.pmkid_cnt, gm_cs, 4);
...
sint rtw_restruct_sec_ie_22b(_adapter *adapter, u8 *out_ie)
{
	...
	if (check_fwstate(pmlmepriv, WIFI_UNDER_WPS)) {
		_rtw_memcpy_22b(out_ie, psecuritypriv->wps_ie, psecuritypriv->wps_ie_len);
		ielength = psecuritypriv->wps_ie_len;

	} else if ((authmode == _WPA_IE_ID_) || (authmode == _WPA2_IE_ID_)) {
		/* copy RSN or SSN		 */
		_rtw_memcpy_22b(out_ie, psecuritypriv->supplicant_ie, psecuritypriv->supplicant_ie[1] + 2);
		...
		rtw_report_sec_ie_22b(adapter, authmode, psecuritypriv->supplicant_ie);
	}
...
void rtw_init_registrypriv_dev_network_22b(_adapter *adapter)
{
	...
	_rtw_memcpy_22b(pdev_network->MacAddress, myhwaddr, ETH_ALEN);

	_rtw_memcpy_22b(&pdev_network->Ssid, &pregistrypriv->ssid, sizeof(NDIS_802_11_SSID));
...
void rtw_update_registrypriv_dev_network_22b(_adapter *adapter)
{
	int sz = 0;
	struct registry_priv *pregistrypriv = &adapter->registrypriv;
	WLAN_BSSID_EX    *pdev_network = &pregistrypriv->dev_network;
	...
	/* rtw_set_supported_rate_22b(pdev_network->SupportedRates, pregistrypriv->wireless_mode) ; */ /* will be called in rtw_generate_ie_22b */
	sz = rtw_generate_ie_22b(pregistrypriv);
...
void rtw_get_encrypt_decrypt_from_registrypriv_22b(_adapter *adapter)
{
...
void rtw_joinbss_reset_22b(_adapter *padapter)
{
	u8	threshold;
	struct mlme_priv	*pmlmepriv = &padapter->mlmepriv;
	...
	if (phtpriv->ht_option) {
		if (padapter->registrypriv.wifi_spec == 1)
			...
			threshold = 0;
		rtw_hal_set_hwreg_22b(padapter, HW_VAR_RXDMA_AGG_PG_TH, (u8 *)(&threshold));
	} else {
		threshold = 1;
		rtw_hal_set_hwreg_22b(padapter, HW_VAR_RXDMA_AGG_PG_TH, (u8 *)(&threshold));
	}
...
#ifdef CONFIG_80211N_HT
void	rtw_ht_use_default_setting_22b(_adapter *padapter)
{
	struct mlme_priv		*pmlmepriv = &padapter->mlmepriv;
	struct ht_priv		*phtpriv = &pmlmepriv->htpriv;
	struct registry_priv	*pregistrypriv = &padapter->registrypriv;
	...
	/* LDPC support */
	rtw_hal_get_def_var_22b(padapter, HAL_DEF_RX_LDPC, (u8 *)&bHwLDPCSupport);
	CLEAR_FLAGS(phtpriv->ldpc_cap);
	if (bHwLDPCSupport) {
		if (TEST_FLAG(pregistrypriv->ldpc_cap, BIT4))
			SET_FLAG(phtpriv->ldpc_cap, LDPC_HT_ENABLE_RX);
	}
	rtw_hal_get_def_var_22b(padapter, HAL_DEF_TX_LDPC, (u8 *)&bHwLDPCSupport);
	if (bHwLDPCSupport) {
	...
	/* STBC */
	rtw_hal_get_def_var_22b(padapter, HAL_DEF_TX_STBC, (u8 *)&bHwSTBCSupport);
	CLEAR_FLAGS(phtpriv->stbc_cap);
	if (bHwSTBCSupport) {
		if (TEST_FLAG(pregistrypriv->stbc_cap, BIT5))
			SET_FLAG(phtpriv->stbc_cap, STBC_HT_ENABLE_TX);
	}
	rtw_hal_get_def_var_22b(padapter, HAL_DEF_RX_STBC, (u8 *)&bHwSTBCSupport);
	if (bHwSTBCSupport) {
...
#endif
	{
		rtw_hal_get_def_var_22b(padapter, HAL_DEF_EXPLICIT_BEAMFORMER, (u8 *)&bHwSupportBeamformer);
		rtw_hal_get_def_var_22b(padapter, HAL_DEF_EXPLICIT_BEAMFORMEE, (u8 *)&bHwSupportBeamformee);
...
void rtw_build_wmm_ie_ht_22b(_adapter *padapter, u8 *out_ie, uint *pout_len)
{
	...
	if (padapter->mlmepriv.qospriv.qos_option == 0) {
		out_len = *pout_len;
		pframe = rtw_set_ie_22b(out_ie + out_len, _VENDOR_SPECIFIC_IE_,
...
unsigned int rtw_restructure_ht_ie_22b(_adapter *padapter, u8 *in_ie, u8 *out_ie, uint in_len, uint *pout_len, u8 channel)
{
	u32 ielen, out_len;
	...
	_rtw_memset_22b(&ht_capie, 0, sizeof(struct rtw_ieee80211_ht_cap));
	...
	if (hal_chk_bw_cap_22b(padapter, BW_CAP_40M)) {
		if (channel > 14) {
	...
	if (cbw40_enable) {
		...
		if (in_ie == NULL) {
			...
			p = rtw_get_ie_22b(in_ie, WLAN_EID_HT_OPERATION, &ielen, in_len);
		...
		if (oper_bw == CHANNEL_WIDTH_40
			...
			&& !rtw_chset_is_chbw_valid_22b(adapter_to_chset(padapter), channel, oper_bw, oper_offset)
		) {
			oper_bw = CHANNEL_WIDTH_20;
			oper_offset = HAL_PRIME_CHNL_OFFSET_DONT_CARE;
			rtw_warn_on(!rtw_chset_is_chbw_valid_22b(adapter_to_chset(padapter), channel, oper_bw, oper_offset));
		}
	...
	if (TEST_FLAG(phtpriv->stbc_cap, STBC_HT_ENABLE_RX)) {
		if ((pregistrypriv->rx_stbc == 0x3) ||							/* enable for 2.4/5 GHz */
			...
			rtw_hal_get_def_var_22b(padapter, HAL_DEF_RX_STBC, (u8 *)(&rx_stbc_nss));
	...
	_rtw_memcpy_22b(ht_capie.supp_mcs_set, pmlmeext->default_supported_mcs_set, 16);
	...
	switch (rx_nss) {
	case 1:
		set_mcs_rate_by_mask_22b(ht_capie.supp_mcs_set, MCS_RATE_1R);
		break;
	case 2:
		#ifdef CONFIG_DISABLE_MCS13TO15
		if (cbw40_enable && pregistrypriv->wifi_spec != 1)
			set_mcs_rate_by_mask_22b(ht_capie.supp_mcs_set, MCS_RATE_2R_13TO15_OFF);
		else
		#endif
			set_mcs_rate_by_mask_22b(ht_capie.supp_mcs_set, MCS_RATE_2R);
		break;
	case 3:
		set_mcs_rate_by_mask_22b(ht_capie.supp_mcs_set, MCS_RATE_3R);
		break;
	case 4:
		set_mcs_rate_by_mask_22b(ht_capie.supp_mcs_set, MCS_RATE_4R);
		break;
	default:
	...
	{
		rtw_hal_get_def_var_22b(padapter, HAL_DEF_RX_PACKET_OFFSET, &rx_packet_offset);
		rtw_hal_get_def_var_22b(padapter, HAL_DEF_MAX_RECVBUF_SZ, &max_recvbuf_sz);
	...
	*/
	...
	if (padapter->driver_rx_ampdu_factor != 0xFF)
		...
		rtw_hal_get_def_var_22b(padapter, HW_VAR_MAX_RX_AMPDU_FACTOR, &max_rx_ampdu_factor);
	...
	if (padapter->driver_rx_ampdu_spacing != 0xFF)
		...
		if (padapter->securitypriv.dot11PrivacyAlgrthm == _AES_) {
			...
			*	By yiwei.sun
			*/
			rtw_hal_get_def_var_22b(padapter, HW_VAR_BEST_AMPDU_DENSITY, &best_ampdu_density);
	...
	if (TEST_FLAG(phtpriv->beamform_cap, BEAMFORMING_HT_BEAMFORMER_ENABLE)) {
		...
		rtw_hal_get_def_var_22b(padapter, HAL_DEF_BEAMFORMER_CAP, (u8 *)&rf_num);
	...
	if (TEST_FLAG(phtpriv->beamform_cap, BEAMFORMING_HT_BEAMFORMEE_ENABLE)) {
		...
		rtw_hal_get_def_var_22b(padapter, HAL_DEF_BEAMFORMEE_CAP, (u8 *)&rf_num);
#ifdef CONFIG_80211AC_VHT
	...
	pframe = rtw_set_ie_22b(out_ie + out_len, _HT_CAPABILITY_IE_,
		sizeof(struct rtw_ieee80211_ht_cap), (unsigned char *)&ht_capie, pout_len);

	phtpriv->ht_option = _TRUE;

	if (in_ie != NULL) {
		p = rtw_get_ie_22b(in_ie, _HT_ADD_INFO_IE_, &ielen, in_len);
		if (p && (ielen == sizeof(struct ieee80211_ht_addt_info))) {
			out_len = *pout_len;
			pframe = rtw_set_ie_22b(out_ie + out_len, _HT_ADD_INFO_IE_, ielen, p + 2 , pout_len);
		}
	}
...
void rtw_update_ht_cap_22b(_adapter *padapter, u8 *pie, uint ie_len, u8 channel)
{
	u8 *p, max_ampdu_sz;
	int len;
	...
	RTW_INFO("+rtw_update_ht_cap_22b()\n");
	...
	len = 0;
	p = rtw_get_ie_22b(pie + sizeof(NDIS_802_11_FIXED_IEs), _HT_CAPABILITY_IE_, &len, ie_len - sizeof(NDIS_802_11_FIXED_IEs));
	if (p && len > 0) {
		pht_capie = (struct rtw_ieee80211_ht_cap *)(p + 2);
		max_ampdu_sz = (pht_capie->ampdu_params_info & IEEE80211_HT_CAP_AMPDU_FACTOR);
		max_ampdu_sz = 1 << (max_ampdu_sz + 3); /* max_ampdu_sz (kbytes); */

		/* RTW_INFO("rtw_update_ht_cap_22b(): max_ampdu_sz=%d\n", max_ampdu_sz); */
	...
	len = 0;
	p = rtw_get_ie_22b(pie + sizeof(NDIS_802_11_FIXED_IEs), _HT_ADD_INFO_IE_, &len, ie_len - sizeof(NDIS_802_11_FIXED_IEs));
	if (p && len > 0) {
	...
	if ((cbw40_enable) &&
		...
		switch (tx_nss) {
		case 1:
			set_mcs_rate_by_mask_22b(pmlmeinfo->HT_caps.u.HT_cap_element.MCS_rate, MCS_RATE_1R);
			break;
		case 2:
			#ifdef CONFIG_DISABLE_MCS13TO15
			if (pmlmeext->cur_bwmode == CHANNEL_WIDTH_40 && pregistrypriv->wifi_spec != 1)
				set_mcs_rate_by_mask_22b(pmlmeinfo->HT_caps.u.HT_cap_element.MCS_rate, MCS_RATE_2R_13TO15_OFF);
			else
			#endif
				set_mcs_rate_by_mask_22b(pmlmeinfo->HT_caps.u.HT_cap_element.MCS_rate, MCS_RATE_2R);
			break;
		case 3:
			set_mcs_rate_by_mask_22b(pmlmeinfo->HT_caps.u.HT_cap_element.MCS_rate, MCS_RATE_3R);
			break;
		case 4:
			set_mcs_rate_by_mask_22b(pmlmeinfo->HT_caps.u.HT_cap_element.MCS_rate, MCS_RATE_4R);
			break;
		default:
...
#ifdef CONFIG_TDLS
void rtw_issue_addbareq_cmd_22b_tdls(_adapter *padapter, struct xmit_frame *pxmitframe)
{
	...
	if (pattrib->direct_link == _TRUE) {
		ptdls_sta = rtw_get_stainfo_22b(&padapter->stapriv, pattrib->dst);
		if ((ptdls_sta != NULL) && (ptdls_sta->tdls_sta_state & TDLS_LINKED_STATE)) {
			...
			if ((phtpriv->ht_option == _TRUE) && (phtpriv->ampdu_enable == _TRUE)) {
				...
				if (0 == issued) {
					...
					rtw_addbareq_cmd_22b(padapter, (u8)priority, pattrib->dst);
				}
			}
		}
	}
...
void rtw_issue_addbareq_cmd_22b(_adapter *padapter, struct xmit_frame *pxmitframe)
{
	u8 issued;
	int priority;
	struct sta_info *psta = NULL;
...
#ifdef CONFIG_TDLS
	rtw_issue_addbareq_cmd_22b_tdls(padapter, pxmitframe);
#endif /* CONFIG_TDLS */

	psta = rtw_get_stainfo_22b(&padapter->stapriv, pattrib->ra);
	...
	if ((phtpriv->ht_option == _TRUE) && (phtpriv->ampdu_enable == _TRUE)) {
		...
		if (0 == issued) {
			RTW_INFO("rtw_issue_addbareq_cmd_22b, p=%d\n", priority);
			psta->htpriv.candidate_tid_bitmap |= BIT((u8)priority);
			rtw_addbareq_cmd_22b(padapter, (u8) priority, pattrib->ra);
		}
	}

}

void rtw_append_exented_cap_22b(_adapter *padapter, u8 *out_ie, uint *pout_len)
{
	struct mlme_priv	*pmlmepriv = &padapter->mlmepriv;
	struct ht_priv		*phtpriv = &pmlmepriv->htpriv;
#ifdef CONFIG_80211AC_VHT
	...
	*/
	if (_FALSE == _rtw_memcmp_22b(cap_content, null_content, 8))
		pframe = rtw_set_ie_22b(out_ie + *pout_len, EID_EXTCapability, 8, cap_content , pout_len);
}
#endif

#ifdef CONFIG_LAYER2_ROAMING
inline void rtw_set_to_roam_22b(_adapter *adapter, u8 to_roam)
{
	if (to_roam == 0)
...
inline u8 rtw_dec_to_roam_22b(_adapter *adapter)
{
	adapter->mlmepriv.to_roam--;
	return adapter->mlmepriv.to_roam;
}

inline u8 rtw_to_roam_22b(_adapter *adapter)
{
	return adapter->mlmepriv.to_roam;
}

void rtw_roaming_22b(_adapter *padapter, struct wlan_network *tgt_network)
{
	_irqL irqL;
	struct mlme_priv	*pmlmepriv = &padapter->mlmepriv;

	_enter_critical_bh(&pmlmepriv->lock, &irqL);
	_rtw_roaming_22b_22b(padapter, tgt_network);
	_exit_critical_bh(&pmlmepriv->lock, &irqL);
}
void _rtw_roaming_22b_22b(_adapter *padapter, struct wlan_network *tgt_network)
{
	struct mlme_priv	*pmlmepriv = &padapter->mlmepriv;
	struct wlan_network *cur_network = &pmlmepriv->cur_network;
	int do_join_r;

	if (0 < rtw_to_roam_22b(padapter)) {
		RTW_INFO("roaming from %s("MAC_FMT"), length:%d\n",
			cur_network->network.Ssid.Ssid, MAC_ARG(cur_network->network.MacAddress),
			 cur_network->network.Ssid.SsidLength);
		_rtw_memcpy_22b(&pmlmepriv->assoc_ssid, &cur_network->network.Ssid, sizeof(NDIS_802_11_SSID));
		...
		while (1) {
			do_join_r = rtw_do_join_22b(padapter);
			if (_SUCCESS == do_join_r)
				break;
			else {
				RTW_INFO("roaming do_join return %d\n", do_join_r);
				rtw_dec_to_roam_22b(padapter);
...
bool rtw_adjust_chbw_22b(_adapter *adapter, u8 req_ch, u8 *req_bw, u8 *req_offset)
{
	...
	allowed_bw = hal_largest_bw_22b(adapter, allowed_bw);
...
sint rtw_linked_check_22b(_adapter *padapter)
{
	if (MLME_IS_AP(padapter) || MLME_IS_MESH(padapter)
...
u8 rtw_is_adapter_up_22b(_adapter *padapter)
{
	if (padapter == NULL)
		return _FALSE;
...
bool is_miracast_enabled_22b(_adapter *adapter)
{
	bool enabled = 0;
#ifdef CONFIG_WFD
...
bool rtw_chk_miracast_mode_22b(_adapter *adapter, u8 mode)
{
	bool ret = 0;
#ifdef CONFIG_WFD
	struct wifi_display_info *wfdinfo = &adapter->wfd_info;
...
const char *get_miracast_mode_str_22b(int mode)
{
...
inline void rtw_wfd_st_switch_22b(struct sta_info *sta, bool on)
{
#ifdef CONFIG_WFD
	if (on)
		rtw_st_ctl_register_22b(&sta->st_ctl, SESSION_TRACKER_REG_ID_WFD, &wfd_st_reg);
	else
		rtw_st_ctl_unregister_22b(&sta->st_ctl, SESSION_TRACKER_REG_ID_WFD);
#endif
}
