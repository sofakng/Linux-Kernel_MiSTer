	...
	struct sta_info *psta, u16 frame_type)
{
	...
	update_mgntframe_attrib_22b(padapter, pattr);
...
int issue_null_reply(struct rm_obj *prm)
{
	...
	pframe = rtw_set_fixed_ie_22b(pframe, 3, &prm->p.category, &pattr->pktlen);

	my_len = 0;
	if (len>5) {
		prm->p.len = len - 3 - 2;
		pframe = rtw_set_fixed_ie_22b(pframe, len - 3,
			&prm->p.e_id, &my_len);
	}
...
int rm_sitesurvey(struct rm_obj *prm)
{
	...
	_rtw_memset_22b(&parm, 0, sizeof(struct sitesurvey_parm));
	_rtw_memcpy_22b(parm.ch, pch_set,
		sizeof(struct rtw_ieee80211_channel) * MAX_OP_CHANNEL_SET_NUM);

	_rtw_memcpy_22b(&parm.ssid[0], &prm->q.opt.bcn.ssid, IW_ESSID_MAX_SIZE);
	...
	rtw_sitesurvey_cmd_22b(prm->psta->padapter, &parm);
...
static int rm_parse_ch_load_s_elem(struct rm_obj *prm, u8 *pbody, int req_len)
{
	...
	while (len) {
		...
		case ch_load_rep_info:
			...
			_rtw_memcpy_22b(&(prm->q.opt.clm.rep_cond),
	...
	u8 *pbody, int req_len)
{
	...
	while (len) {
		...
		case noise_histo_rep_info:
			...
			_rtw_memcpy_22b(&(prm->q.opt.nhm.rep_cond),
...
static int rm_parse_bcn_req_s_elem(struct rm_obj *prm, u8 *pbody, int req_len)
{
	...
	while (len && prm->q.opt.bcn.opt_id_num < BCN_REQ_OPT_MAX_NUM) {
		...
		case bcn_req_ssid:
			...
			_rtw_memcpy_22b(&(prm->q.opt.bcn.ssid.Ssid),
		...
		case bcn_req_rep_info:
			...
			_rtw_memcpy_22b(&(prm->q.opt.bcn.rep_cond),
...
static int rm_parse_meas_req(struct rm_obj *prm, u8 *pbody)
{
	...
	if (prm->q.m_type == bcn_req) {
		...
		/* BSSID */
		_rtw_memcpy_22b(&(prm->q.bssid), &pbody[p], 6);
		p+=6;
...
unsigned int rm_on_action(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if (!_rtw_memcmp_22b(adapter_mac_addr(padapter),
	...
	unsigned int *fr_len)
{
	...
	pframe = rtw_set_fixed_ie_22b(pframe, 1, &val8, &my_len);
	...
	if (prm->q.opt.bcn.rep_detail == 2) {
		pframe = rtw_set_fixed_ie_22b(pframe, pbss->IELength - 4,
	...
	pframe = rtw_set_fixed_ie_22b(pframe,
	...
	for (j = 0; j < prm->q.opt.bcn.opt_id_num; j++) {
...
#endif
			pframe = rtw_set_ie_22b(pframe, _SSID_IE_,
		...
		case bcn_req_req:
			...
			for (k=0; k<prm->q.opt.bcn.req_len; k++) {
				...
				ptr = rtw_get_ie_22b(pbss->IEs + _FIXED_IE_LENGTH_,
					eid, &len, val8);
...
#endif
				pframe = rtw_set_ie_22b(pframe, eid,
					len,ptr+2, &my_len);
			} /* for() */
			break;
		case bcn_req_ac_ch_rep:
	 ...
	 */
	val8 = my_len - 2;
	rtw_set_fixed_ie_22b(plen, 1, &val8, &j);
	...
	u8 *pframe, struct wlan_network *pnetwork, unsigned int *fr_len)
{
	...
	pframe = rtw_set_fixed_ie_22b(pframe, 1, &prm->q.op_class, &my_len);
	...
	pframe = rtw_set_fixed_ie_22b(pframe, 8, (u8 *)&val64, &my_len);

	/* Measurement Duration */
	val16 = prm->meas_end_time - prm->meas_start_time;
	val16 = cpu_to_le16(val16);
	pframe = rtw_set_fixed_ie_22b(pframe, 2, (u8 *)&val16, &my_len);
	...
	/* BSSID */
	pframe = rtw_set_fixed_ie_22b(pframe, 6, (u8 *)&pbss->MacAddress, &my_len);
	...
	pframe = rtw_set_fixed_ie_22b(pframe, 4, (u8 *)&val32, &my_len);
	 ...
	 */
	val8 = my_len - 2;
	rtw_set_fixed_ie_22b(plen, 1, &val8, &i);
	...
	u8 *pframe, struct wlan_network *pnetwork, unsigned int *fr_len)
{
	...
	my_len = 0;
	plen = pframe + 1;
	pframe = rtw_set_fixed_ie_22b(pframe, 7, &prm->p.e_id, &my_len);
...
static int retrieve_scan_result(struct rm_obj *prm)
{
	...
	while (1) {
		...
		*/
		if (rtw_chset_search_ch_22b(adapter_to_chset(padapter),
			pbss->Configuration.DSConfig) == 0)
			goto next;

		if (rtw_mlme_band_check_22b(padapter, pbss->Configuration.DSConfig)
			== _FALSE)
			goto next;

		if (rtw_validate_ssid_22b(&(pbss->Ssid)) == _FALSE)
			goto next;
		...
		for (i = 0; i < meas_ch_num; i++) {
			...
			if (is_wildcard_bssid(prm->q.bssid) == FALSE)
				if (_rtw_memcmp_22b(prm->q.bssid,
			...
			if ((prm->q.opt.bcn.ssid.SsidLength > 0) &&
				_rtw_memcmp_22b(prm->q.opt.bcn.ssid.Ssid,
				pbss->Ssid.Ssid,
			...
			len = 0;
			_rtw_memset_22b(tmp_buf, 0, MAX_XMIT_EXTBUF_SZ);
...
new_packet:
			...
			if ((MAX_XMIT_EXTBUF_SZ - (my_len+len+24+4)) > 0) {
				pbuf = rtw_set_fixed_ie_22b(pbuf,
					len, tmp_buf, &my_len);
...
int issue_beacon_22b_rep(struct rm_obj *prm)
{
	int i, my_len;
	u8 *pframe;
	_adapter *padapter = prm->psta->padapter;
...
int issue_nb_req(struct rm_obj *prm)
{
	...
	if (prm->q.pssid) {
		...
		_rtw_memcpy_22b(pie, prm->q.pssid, val8);

		pframe = rtw_set_fixed_ie_22b(pframe, val8 + 2,
			sub_ie, &pattr->pktlen);
	} else {
		...
		if (!pmlmepriv->cur_network.network.Ssid.SsidLength)
			...
			_rtw_memcpy_22b(pie, pmlmepriv->cur_network.network.Ssid.Ssid,
	...
	u8 *pframe, unsigned int *fr_len)
{
	...
	pframe = rtw_set_fixed_ie_22b(pframe, 6, bssid, &my_len);
...
int issue_radio_meas_req(struct rm_obj *prm)
{
	...
	pattr = &pmgntframe->attrib;
	pframe = build_wlan_hdr(padapter, pmgntframe, prm->psta, WIFI_ACTION);
	pframe = rtw_set_fixed_ie_22b(pframe, 3, &prm->q.category, &pattr->pktlen);

	/* repeat */
	val16 = cpu_to_le16(prm->q.rpt);
	pframe = rtw_set_fixed_ie_22b(pframe, 2,
		(unsigned char *)&(val16), &pattr->pktlen);

	my_len = 0;
	plen = pframe + 1;
	pframe = rtw_set_fixed_ie_22b(pframe, 7, &prm->q.e_id, &my_len);
...
int retrieve_radio_meas_result(struct rm_obj *prm)
{
	...
	ch = rtw_chset_search_ch_22b(adapter_to_chset(prm->psta->padapter),
		prm->q.ch_num);
...
int issue_radio_meas_rep(struct rm_obj *prm)
{
	...
	pattr = &pmgntframe->attrib;
	pframe = build_wlan_hdr(padapter, pmgntframe, psta, WIFI_ACTION);
	pframe = rtw_set_fixed_ie_22b(pframe, 3,
	...
	/* length */
	val8 = (u8)my_len-2;
	rtw_set_fixed_ie_22b(plen, 1, &val8, &i); /* use variable i to ignore it */
	...
	struct sta_info *psta, struct rtw_ieee802_11_elems *elem)
{
	if (elem->rm_en_cap) {
		...
		_rtw_memcpy_22b(psta->rm_en_cap,
...
void RM_IE_handler(_adapter *padapter, PNDIS_802_11_VARIABLE_IEs pIE)
{
	...
	_rtw_memcpy_22b(&padapter->rmpriv.rm_en_cap_assoc, pIE->data, pIE->Length);
	RTW_INFO("assoc.rm_en_cap="RM_CAP_FMT"\n", RM_CAP_ARG(pIE->data));
}
...
struct sta_info *rm_get_sta(_adapter *padapter, u16 aid, u8* pbssid)
{
	...
	for (i = 0; i < NUM_STA; i++) {
		...
		while ((rtw_end_of_queue_search_22b(phead, plist)) == _FALSE) {
			...
			if (pbssid && _rtw_memcmp_22b(psta->cmn.mac_addr,
				pbssid, 6))
				goto done;
		}
...
static void rm_dbg_show_meas(struct rm_obj *prm, char *s)
{
	...
	sprintf(pstr(s), "clock=%d, state=%s, rpt=%u/%u\n",
		(int)ATOMIC_READ_22b(&prm->pclock->counter),
...
#if (RM_SUPPORT_IWPRIV_DBG)
	if (_rtw_memcmp_22b(s, "help", 4)) {
		rm_dbg_help(padapter, s);

	} else if (_rtw_memcmp_22b(s, "list_sta", 8)) {
		rm_dbg_list_sta(padapter, s);

	} else if (_rtw_memcmp_22b(s, "list_meas", 9)) {
		rm_dbg_list_meas(padapter, s);

	} else if (_rtw_memcmp_22b(s, "add_meas", 8)) {
		rm_dbg_add_meas(padapter, s);

	} else if (_rtw_memcmp_22b(s, "del_meas", 8)) {
		rm_dbg_del_meas(padapter, s);

	} else if (_rtw_memcmp_22b(s, "activate", 8)) {
		rm_dbg_activate_meas(padapter, s);

	} else if (_rtw_memcmp_22b(s, "run_meas", 8)) {
		rm_dbg_run_meas(padapter, s);
	} else if (_rtw_memcmp_22b(s, "nb", 2)) {
