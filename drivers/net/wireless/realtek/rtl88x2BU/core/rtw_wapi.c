...
WapiGetEntryForCamWrite(_adapter *padapter, u8 *pMacAddr, u8 KID, BOOLEAN IsMsk)
{
	...
	for (i = 0; i < WAPI_CAM_ENTRY_NUM; i++) {
		if (pWapiInfo->wapiCamEntry[i].IsUsed
		    && (_rtw_memcmp_22b(pMacAddr, pWapiInfo->wapiCamEntry[i].PeerMacAddr, ETH_ALEN) == _TRUE)
	...
	if (i == WAPI_CAM_ENTRY_NUM) { /* not found */
		for (i = 0; i < WAPI_CAM_ENTRY_NUM; i++) {
			if (pWapiInfo->wapiCamEntry[i].IsUsed == 0) {
				...
				pWapiInfo->wapiCamEntry[i].type = IsMsk;
				pWapiInfo->wapiCamEntry[i].keyidx = KID;
				_rtw_memcpy_22b(pWapiInfo->wapiCamEntry[i].PeerMacAddr, pMacAddr, ETH_ALEN);
...
u8 WapiGetEntryForCamClear(_adapter *padapter, u8 *pPeerMac, u8 keyid, u8 IsMsk)
{
	...
	for (i = 0; i < WAPI_CAM_ENTRY_NUM; i++) {
		if (pWapiInfo->wapiCamEntry[i].IsUsed
		    && (_rtw_memcmp_22b(pPeerMac, pWapiInfo->wapiCamEntry[i].PeerMacAddr, ETH_ALEN) == _TRUE)
		    && pWapiInfo->wapiCamEntry[i].keyidx == keyid
		    && pWapiInfo->wapiCamEntry[i].type == IsMsk) {
			pWapiInfo->wapiCamEntry[i].IsUsed = 0;
			pWapiInfo->wapiCamEntry[i].keyidx = 2;
			_rtw_memset_22b(pWapiInfo->wapiCamEntry[i].PeerMacAddr, 0, ETH_ALEN);
...
)
{
	...
	write_cam_22b(padapter, EntryId, usConfig, pMacAddr, pKey);
...
u8 rtw_wapi_is_wai_packet(_adapter *padapter, u8 *pkt_data)
{
	...
	if (list_empty(&pWapiInfo->wapiSTAUsedList))
		...
		list_for_each_entry(pWapiSta, &pWapiInfo->wapiSTAUsedList, list) {
			if (_rtw_memcmp_22b(pTaddr, pWapiSta->PeerMacAddr, 6) == _TRUE) {
				bFind = true;
				break;
			}
		}
...
void rtw_wapi_update_info(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	_rtw_memcpy_22b((u8 *)precv_hdr->WapiSrcAddr, pTA, 6);
	pRecvPN = ptr + precv_hdr->attrib.hdrlen + 2;
	_rtw_memcpy_22b((u8 *)precv_hdr->WapiTempPN, pRecvPN, 16);
...
)
{
	...
	if (list_empty(&pWapiInfo->wapiSTAUsedList))
		...
		list_for_each_entry(pWapiSta, &pWapiInfo->wapiSTAUsedList, list) {
			if (_rtw_memcmp_22b(precv_hdr->WapiSrcAddr, pWapiSta->PeerMacAddr, ETH_ALEN) == _TRUE) {
				bFind = true;
				break;
			}
		}
	...
	if (bFind) {
		...
		if (!WapiComparePN(precv_hdr->WapiTempPN, pLastRecvPN)) {
			...
			if (IS_MCAST(precv_hdr->attrib.ra))
				_rtw_memcpy_22b(pLastRecvPN, WapiAEMultiCastPNInitialValueSrc, 16);
			else
				_rtw_memcpy_22b(pLastRecvPN, WapiAEPNInitialValueSrc, 16);
			bDrop = true;
		} else
			_rtw_memcpy_22b(pLastRecvPN, precv_hdr->WapiTempPN, 16);
	}
...
void rtw_build_probe_resp_wapi_ie(_adapter *padapter, unsigned char *pframe, struct pkt_attrib *pattrib)
{
	...
	_rtw_memcpy_22b(pframe + 2, pWapiInfo->wapiIE, WapiIELength);
...
void rtw_build_assoc_req_wapi_ie(_adapter *padapter, unsigned char *pframe, struct pkt_attrib *pattrib)
{
	...
	if (!list_empty(&(pWapiInfo->wapiBKIDStoreList))) {
		list_for_each_entry(pWapiBKID, &pWapiInfo->wapiBKIDStoreList, list) {
			bkidNum++;
			_rtw_memcpy_22b(pWapiInfo->wapiIE + WapiIELength + 2, pWapiBKID->bkid, 16);
			WapiIELength += 16;
		}
	}
	_rtw_memcpy_22b(pWapiInfo->wapiIE + WapiIELength, &bkidNum, 2);
	WapiIELength += 2;
...
void rtw_wapi_on_assoc_ok(_adapter *padapter, PNDIS_802_11_VARIABLE_IEs pIE)
{
	...
	_rtw_memcpy_22b(pWapiSta->PeerMacAddr, padapter->mlmeextpriv.mlmext_info.network.MacAddress, 6);
	_rtw_memcpy_22b(pWapiSta->lastRxMulticastPN, WapiAEMultiCastPNInitialValueSrc, 16);
	_rtw_memcpy_22b(pWapiSta->lastRxUnicastPN, WapiAEPNInitialValueSrc, 16);

	/* For chenk PN error with Qos Data after s3: add by ylb 20111114 */
	_rtw_memcpy_22b(pWapiSta->lastRxUnicastPNBEQueue, WapiAEPNInitialValueSrc, 16);
	_rtw_memcpy_22b(pWapiSta->lastRxUnicastPNBKQueue, WapiAEPNInitialValueSrc, 16);
	_rtw_memcpy_22b(pWapiSta->lastRxUnicastPNVIQueue, WapiAEPNInitialValueSrc, 16);
	_rtw_memcpy_22b(pWapiSta->lastRxUnicastPNVOQueue, WapiAEPNInitialValueSrc, 16);
...
void rtw_wapi_return_one_sta_info(_adapter *padapter, u8 *MacAddr)
{
	...
	if (check_fwstate(pmlmepriv, WIFI_STATION_STATE)) {
		while (!list_empty(&(pWapiInfo->wapiBKIDStoreList))) {
			...
			list_del_init(&pWapiBkid->list);
			_rtw_memset_22b(pWapiBkid->bkid, 0, 16);
	...
	if (list_empty(&(pWapiInfo->wapiSTAUsedList))) {
		...
		list_for_each_entry(pWapiStaInfo, &(pWapiInfo->wapiSTAUsedList), list) {
			...
			if (_rtw_memcmp_22b(pWapiStaInfo->PeerMacAddr, MacAddr, ETH_ALEN) == _TRUE) {
				pWapiStaInfo->bAuthenticateInProgress = false;
				pWapiStaInfo->bSetkeyOk = false;
				_rtw_memset_22b(pWapiStaInfo->PeerMacAddr, 0, ETH_ALEN);
				list_del_init(&pWapiStaInfo->list);
...
void rtw_wapi_clear_cam_entry_22b_22b(_adapter *padapter, u8 *pMacAddr)
{
	u8 UcIndex = 0;
	...
	if (UcIndex != 0xff) {
		...
		CAM_empty_entry_22b(padapter, UcIndex);
	}
...
void rtw_wapi_clear_all_cam_entry(_adapter *padapter)
{
	...
	invalidate_cam_all_22b(padapter); /* is this ok? */
...
void rtw_wapi_get_iv(_adapter *padapter, u8 *pRA, u8 *IV)
{
	...
	if (IS_MCAST(pRA)) {
		...
		if (list_empty(&pWapiInfo->wapiSTAUsedList)) {
			...
			_rtw_memset_22b(IV, 10, 18);
			return;
		} else {
			list_for_each_entry(pWapiSta, &pWapiInfo->wapiSTAUsedList, list) {
				WAPI_DATA(WAPI_RX, "rtw_wapi_get_iv: peermacaddr ", pWapiSta->PeerMacAddr, 6);
				if (_rtw_memcmp_22b((u8 *)pWapiSta->PeerMacAddr, pRA, 6) == _TRUE) {
					bFindMatchPeer = true;
					break;
				}
			}
			...
			if (bFindMatchPeer) {
				...
				if (pWapiSta->wapiUsk.keyId <= 1) {
					...
					_rtw_memcpy_22b(pWapiExt->PN, pWapiSta->lastTxUnicastPN, 16);
		...
		if (IS_MCAST(pRA)) {
			...
			if (!list_empty(&pWapiInfo->wapiSTAUsedList)) {
				list_for_each_entry(pWapiSta, &pWapiInfo->wapiSTAUsedList, list) {
					...
					if (_rtw_memcmp_22b(pRA, pWapiSta->PeerMacAddr, 6) == _TRUE) {
						bFindMatchPeer = true;
						break;
					}
				}
...
void rtw_wapi_set_set_encryption(_adapter *padapter, struct ieee_param *param)
{
	...
	if (param->u.crypt.set_tx == 1) {
		list_for_each_entry(pWapiSta, &pWapiInfo->wapiSTAUsedList, list) {
			if (_rtw_memcmp_22b(pWapiSta->PeerMacAddr, param->sta_addr, 6)) {
				_rtw_memcpy_22b(pWapiSta->lastTxUnicastPN, WapiASUEPNInitialValueSrc, 16);

				pWapiSta->wapiUsk.bSet = true;
				_rtw_memcpy_22b(pWapiSta->wapiUsk.dataKey, param->u.crypt.key, 16);
				_rtw_memcpy_22b(pWapiSta->wapiUsk.micKey, param->u.crypt.key + 16, 16);
		...
		list_for_each_entry(pWapiSta, &pWapiInfo->wapiSTAUsedList, list) {
			if (_rtw_memcmp_22b(pWapiSta->PeerMacAddr, get_bssid(pmlmepriv), 6)) {
				pWapiSta->wapiMsk.bSet = true;
				_rtw_memcpy_22b(pWapiSta->wapiMsk.dataKey, param->u.crypt.key, 16);
				_rtw_memcpy_22b(pWapiSta->wapiMsk.micKey, param->u.crypt.key + 16, 16);
