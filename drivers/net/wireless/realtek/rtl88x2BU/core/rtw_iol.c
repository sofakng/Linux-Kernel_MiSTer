...
#if 1
	xmit_frame = rtw_alloc_xmitframe_22b(pxmitpriv);
	if (xmit_frame == NULL) {
		RTW_INFO("%s rtw_alloc_xmitframe_22b return null\n", __FUNCTION__);
		goto exit;
	}

	xmitbuf = rtw_alloc_xmitbuf_22b(pxmitpriv);
	if (xmitbuf == NULL) {
		RTW_INFO("%s rtw_alloc_xmitbuf_22b return null\n", __FUNCTION__);
		rtw_free_xmitframe_22b(pxmitpriv, xmit_frame);
		xmit_frame = NULL;
		goto exit;
	}
...
#else
	xmit_frame = alloc_mgtxmitframe_22b(pxmitpriv);
	if (xmit_frame == NULL)
		RTW_INFO("%s alloc_mgtxmitframe_22b return null\n", __FUNCTION__);
	else {
		pattrib = &xmit_frame->attrib;
		update_mgntframe_attrib_22b(adapter, pattrib);
...
int rtw_IOL_append_cmds(struct xmit_frame *xmit_frame, u8 *IOL_cmds, u32 cmd_len)
{
	...
	_rtw_memcpy_22b(xmit_frame->buf_addr + buf_offset + pattrib->pktlen, IOL_cmds, cmd_len);
