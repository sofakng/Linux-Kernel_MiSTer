...
static struct rtw_mesh_table *rtw_mesh_table_alloc(void)
{
	...
	ATOMIC_SET_22b(&newtbl->entries,  0);
	_rtw_spinlock_22b_init_22b(&newtbl->gates_lock);
...
void rtw_mesh_path_assign_nexthop(struct rtw_mesh_path *mpath, struct sta_info *sta)
{
	...
	while (rtw_end_of_queue_search_22b(head, list) == _FALSE) {
		...
		list = get_next(list);
		_rtw_memcpy_22b(xframe->attrib.ra, sta->cmn.mac_addr, ETH_ALEN);
	}
			     ...
			     struct rtw_mesh_path *gate_mpath)
{
	...
	_rtw_memcpy_22b(attrib->ra, next_hop, ETH_ALEN);
	rtw_rcu_read_unlock();
	_rtw_memcpy_22b(attrib->mda, dst_addr, ETH_ALEN);
}
				    ...
				    bool copy)
{
	...
	while (rtw_end_of_queue_search_22b(head, list) == _FALSE) {
		...
		rtw_list_insert_tail_22b(&fskb->list, get_list_head(&gate_mpath->frame_queue));
...
bool rtw_mesh_gate_search(struct rtw_mesh_table *tbl, const u8 *addr)
{
	...
	rtw_hlist_for_each_entry_rcu(gate, node, &tbl->known_gates, gate_list) {
		if (_rtw_memcmp_22b(gate->dst, addr, ETH_ALEN) == _TRUE) {
			exist = 1;
			break;
		}
	}
				...
				const u8 *dst)
{
	...
	_rtw_memcpy_22b(new_mpath->dst, dst, ETH_ALEN);
	_rtw_memset_22b(new_mpath->rann_snd_addr, 0xFF, ETH_ALEN);
	new_mpath->is_root = false;
	new_mpath->adapter = adapter;
	new_mpath->flags = 0;
	new_mpath->gate_asked = false;
	_rtw_init_queue_22b(&new_mpath->frame_queue);
	new_mpath->frame_queue_len = 0;
	new_mpath->exp_time = rtw_get_current_time();
	_rtw_spinlock_22b_init_22b(&new_mpath->state_lock);
	rtw_init_timer_22b(&new_mpath->timer, adapter, rtw_mesh_path_timer, new_mpath);
				...
				const u8 *dst)
{
	...
	if (_rtw_memcmp_22b(dst, adapter_mac_addr(adapter), ETH_ALEN) == _TRUE)
	...
	if (ATOMIC_INC_22b_UNLESS(&adapter->mesh_info.mpaths, RTW_MESH_MAX_MPATHS) == 0)
		 ...
		 const u8 *dst, const u8 *mpp)
{
	...
	_rtw_memcpy_22b(new_mpath->mpp, mpp, ETH_ALEN);
	ret = rtw_rhashtable_lookup_insert_fast(&tbl->rhead,
					    &new_mpath->rhash,
					    rtw_mesh_rht_params);
			       ...
			       struct rtw_mesh_path *mpath)
{
	...
	ATOMIC_DEC_22b(&adapter->mesh_info.mpaths);
	ATOMIC_DEC_22b(&tbl->entries);
	_rtw_spinlock_22b_free_22b(&mpath->state_lock);
			       ...
			       const u8 *proxy)
{
	...
	while ((mpath = rtw_rhashtable_walk_next(&iter))) {
		...
		if (_rtw_memcmp_22b(mpath->mpp, proxy, ETH_ALEN) == _TRUE)
			__rtw_mesh_path_del(tbl, mpath);
	}
out:
	rtw_rhashtable_walk_stop(&iter);
