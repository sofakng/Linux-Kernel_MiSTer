...
#define rtw_phydm_func_disable_all(adapter)	\
		rtw_phydm_ability_ops_22b(adapter, HAL_PHYDM_DIS_ALL_FUNC, 0)
...
#define rtw_phydm_func_for_offchannel(adapter) \
		do { \
			...
			if (rtw_odm_adaptivity_needed_22b(adapter)) \
				rtw_phydm_ability_ops_22b(adapter, HAL_PHYDM_FUNC_SET, ODM_BB_ADAPTIVITY); \
			if (IS_ACS_ENABLE(adapter))\
				rtw_phydm_ability_ops_22b(adapter, HAL_PHYDM_FUNC_SET, ODM_BB_ENV_MONITOR); \
		} while (0)
#else
#define rtw_phydm_func_for_offchannel(adapter) \
		do { \
...
#define rtw_phydm_func_clr(adapter, ability)	\
		rtw_phydm_ability_ops_22b(adapter, HAL_PHYDM_FUNC_CLR, ability)

#define rtw_phydm_ability_backup(adapter)	\
		rtw_phydm_ability_ops_22b(adapter, HAL_PHYDM_ABILITY_BK, 0)

#define rtw_phydm_ability_restore(adapter)	\
		rtw_phydm_ability_ops_22b(adapter, HAL_PHYDM_ABILITY_RESTORE, 0)


static inline u32 rtw_phydm_ability_get(_adapter *adapter)
{
	return rtw_phydm_ability_ops_22b(adapter, HAL_PHYDM_ABILITY_GET, 0);
}
...
u8 rtw_odm_get_dfs_domain_22b(_adapter *adapter);
u8 rtw_odm_dfs_domain_unknown_22b(_adapter *adapter);
